...
create        procedure [dbo].[sp_TTSTMWAllocateRevVsPay]
	(
...
--Author: Brent Keeton
--********************************************************************
--Purpose: AllocateRevVsPay Report is intended to see all revenue and 
--taxable pay per trig segment. Revenue is being pro rated to the 
--trip segments based on the # miles
--********************************************************************
--Revision History: 
--1. Tuesday October 29,2002 Fuel Surcharge was being calculated twice
--once for fuel surcharge twice with accessorial charges 4.0 LBK 
--2. Wednesday October 30, 2002 Rev Types that are changed on invoice
--and not a order caused the report to pick up revenue for 2 different rev types
--where in fact the report should of been looking at the most recent change
--which is the invoice revtype for that order. This problem
--may not affect customers that don't change revtypes
--on a invoice. Fixed 4.0 LBK 
--3. Monday Novermber 26,2002 Fuel Surcharge was not being calculated
--correctly for Non-Invoiced Loads because of Null Values. Putting
--IsNull function around the sum fixed the problem. The Null
--Values from the first update didn't allow the 
--Non Invoiced Loads in the second update statement to be calculated
--Fixed 4.5 LBK
--4. Thursday December 12, 2002 Fixed the driver lookup by last, first name in the final select 4.6 LBK
--5. Fixed FirstBillTo Column so it picks up BillTo for Non-Invoiced Loads
--6. Added option in combining Accessorials and Fuel Surcharges 
--   into 1 column to increase performance V 4.7
--7. Moved Driver Classes restrictions against the legheader query V 4.7
--8. Added filter by paytype and query by taxable paydetails,non taxable paydetails, or all paydetails V 4.8 LBK
--9. Fixed the way consolidated loads revenue were handled only
--   for Non-Invoiced Loads(Prior to Version 5.4 only the first order
--   was being picked up--> Hence if you had more then 1 order on a trip
--   and the orders were not invoiced(only first order revenue would 
--   be picked up LBK Ver 5.4
--10. Added Currency Converting Functionality (UDF Functions)
--    Ver 5.4 LBK
--11. Added No Locks after TMW Tables
--    Ver 5.4 LBK
--12. Added X as one of the valid date type parameters when checking
--    for invalid date types being passed in
--    Ver 5.4 LBK
--13. Fixed LastPayTo field to 255 characters because some customers
--    exceeding that and getting problems running the report. LBK
...
--Set	@DateType ='D'
--SET @EarlyDate = '6/1/02'
--SET @LateDate = '6/8/02'
Declare @OnlyBranches as varchar(255)
--Declare @Nextlgh_number int
declare @NextMov int
...
BEGIN
	...
	--bcy
		AND NOT EXISTS 
			(Select M.mov_number
			        from InvoiceHeader M WITH (NOLOCK)
...
--					Sum(ISNULL(pyd_amount,0)) 
	...
	OrderStartDate= 
		ISNULL(
			...
			from orderheader o2 WITH (NOLOCK) where L.ord_hdrnumber=o2.ord_hdrnumber)
		,lgh_startdate)	,
	OrderEndDate= 
		ISNULL(
...
--			       ISNULL((SELECT sum(ISNULL( ivd_charge, 0 ) )
				...
				--<TTS!*!TMW><End><SQLVersion=2000+>                                
    		        FROM 
				Invoiceheader WITH (NOLOCK INDEX=dk_move) ,
				invoicedetail WITH (NOLOCK), 
				chargetype WITH (NOLOCK)
			WHERE 
...
-- NEW FIELDS
	...
	NumberOfOrdersOnLeg= 
		...
		From Orderheader WITH (NOLOCK) where L.ord_hdrnumber=Orderheader.ord_hdrnumber
		),
	NumberOfInvoicesOnMove=
		(Select count(distinct ivh_invoicenumber)
		From Invoiceheader WITH (NOLOCK) where L.Mov_number= Invoiceheader.mov_number
		),
...
From
	#movlist,
	legheader L WITH (NOLOCK index=uk_mov) 
where
...
Update  #LegHeaderSummary
	Set FirstBillTo=
		...
		from 
			orderheader WITH (NOLOCK)
		where 
			not exists (select * from invoiceheader WITH (NOLOCK) where invoiceheader.ord_hdrnumber = orderheader.ord_hdrnumber)
			and
...
--			IsNull(sum(ord_charge),0.00)
		...
		where
			...
			and
			not exists (select I.ord_hdrnumber from invoiceheader I WITH (NOLOCK) where I.ord_hdrnumber = orderheader.ord_hdrnumber) 
		)
	--From 
		--orderheader
	where 
...
--				IsNull(sum(ivd_charge),0.00)
			...
			WHERE 
				orderheader.mov_number =#LegHeaderSummary.mov_number
				And
				not exists (select * from invoiceheader WITH (NOLOCK) where orderheader.ord_hdrnumber = invoiceheader.ord_hdrnumber)
				AND
...
Case when lgh_carrier='UNK' Or lgh_carrier='UNKNOWN' or lgh_carrier Is Null Then
	IsNull((Select mpp_lastfirst from manpowerprofile WITH (NOLOCK) where DrvOrCarrier = manpowerprofile.mpp_id),DrvOrCarrier) 
Else	
	
	IsNull((Select car_name from carrier WITH (NOLOCK) where DrvOrCarrier = carrier.car_id),DrvOrCarrier)
End as DrvOrCarrier,
lgh_driver1,--'Drv1' ,
mov_number, --'Move#' ,
lgh_number, --'Leg#' ,
...
trl_type4, --'TRL Type4',
lgh_carrier, --'Carrier',
'cmp_id_start' = (select Company.cmp_name from Company WITH (NOLOCK) where cmp_id_start = Company.cmp_id), --'Start CMP ID' ,
lgh_startcty_nmstct,    --'Start City'   ,
'cmp_id_end' = (select Company.cmp_name from Company WITH (NOLOCK) where cmp_id_end = Company.cmp_id), --'End CMP ID' ,
...
'FirstBillTo' = (select Company.cmp_name from Company WITH (NOLOCK) where FirstBillTo = Company.cmp_id), --'First BillTo' ,
