...
SET ANSI_NULLS OFF
GO
CREATE procedure [dbo].[d_applicable_taxrates_sp] @p_billto varchar(8),@p_originstate varchar(6),@p_DestState varchar(6),@p_effdate datetime, @p_billorsettle varchar(10) = 'BILLING'
as

/**
 *
 * NAME:
 * dbo.d_applicable_taxrates_sp
 *
 * TYPE:
 * [StoredProcedure]
 *
...
*       @fake argument to match d_applicable_taxrates_enh
 *
 * REVISION HISTORY:
 ...
 * 12/5/12 PTS 65751 handle tax rate of 9.975
*      12/11/12 additional need to add fake argument to match call in mainline nvo_tax2 added last year by PTS59389
 ...
 BEGIN
   if @p_billorsettle <> 'SETTLE' and @p_billorsettle <> 'PURCHASE'
    begin
     ...
     --select tax_type,tax_rate = convert(decimal(9,4)
         --,tax_rate / 100.0000)\select tax_type,tax_rate = convert(decimal(9,4)
      select tax_type,tax_rate = convert(decimal(9,6)
      ,tax_rate / 100.000000)
      ,cht_glnum
      ...
      and @p_originstate = Case taxrate.tax_type when 3 then @p_deststate else @p_originstate end
      and tax_rate <> 0
   ...
   if @p_billorsettle = 'SETTLE'
    begin
      ...
      , 0 AS OrderOfDependency
      from taxrate
      join labelfile on labeldefinition like ('TaxType%') and abbr <> 'UNK'
...
GO
GRANT EXECUTE ON  [dbo].[d_applicable_taxrates_sp] TO [public]
GO
