...
CREATE PROCEDURE [dbo].[ResNowOV_R1_LaneByState]
...
set @StateOnly = 'Y'
		...
		BEGIN
			IF IsNull(@ItemID, '') > ''
			BEGIN
			SELECT 	(dbo.fnc_zip3(c1.cmp_zip, c1.cmp_state, c1.cmp_city, @StateOnly) + dbo.fnc_zip3(c2.cmp_zip, c2.cmp_state, c2.cmp_city, @StateOnly)) AS [Lane],
			...
			AND dbo.fnc_zip3(c1.cmp_zip, c1.cmp_state, c1.cmp_city, @StateOnly) + dbo.fnc_zip3(c2.cmp_zip, c2.cmp_state, c2.cmp_city, @StateOnly) = @ItemID
			...
			WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND dbo.fnc_zip3(c1.cmp_zip, c1.cmp_state, c1.cmp_city, @StateOnly) + dbo.fnc_zip3(c2.cmp_zip, c2.cmp_state, c2.cmp_city, @StateOnly)
				NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			ORDER BY dbo.fnc_zip3(c1.cmp_zip, c1.cmp_state, c1.cmp_city, @StateOnly) + dbo.fnc_zip3(c2.cmp_zip, c2.cmp_state, c2.cmp_city, @StateOnly)
			END
		END
		ELSE IF @Refresh = 1
		BEGIN -- Use two step process to get lane information to reduce overhead
			SELECT 		ord_number,
					[ItemID] = dbo.fnc_zip3(c1.cmp_zip, c1.cmp_state, c1.cmp_city, @StateOnly) + dbo.fnc_zip3(c2.cmp_zip, c2.cmp_state, c2.cmp_city, @StateOnly)
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_LaneByState] TO [public]
GO
