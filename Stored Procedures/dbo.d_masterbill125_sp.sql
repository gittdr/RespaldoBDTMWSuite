...
CREATE PROC [dbo].[d_masterbill125_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8),   
 ...
 * 10/17/2008 pmill customized format29 for Hilco Transport
 *
 **/
...
If UPPER(@reprintflag) = 'REPRINT'   
 ...
 ivh_billto_zip =   
  ...
  cmd_name = CASE WHEN ivd.cmd_code = 'UNKNOWN' THEN '' ELSE ivd_description END, --cmd_name = ord_description,  
  case rtrim(isNull(ivd_description,'UNKNOWN'))
    when 'UNKNOWN' then IsNull(cht_description, '')
    when '' then IsNull(cht_description, '')
 ...
 cht.cht_primary,  
  ...
  ref.ref_number,
  CASE WHEN ivh_tractor = 'UNKNOWN' THEN '' ELSE ivh_tractor End ivh_tractor,
  CASE WHEN ivh_driver = 'UNKNOWN' THEN '' ELSE ivh_driver End ivh_driver

    From  invoiceheader
  LEFT OUTER JOIN orderheader ord on ord.ord_hdrnumber = invoiceheader.ord_hdrnumber
  LEFT OUTER JOIN referencenumber ref ON (ref.ord_hdrnumber = ord.ord_hdrnumber and ref.ref_type = 'PO#'and ref.ref_sequence = 1),   
  company cmp1,   
  company cmp2,  
  company cmp3,  
  invoicedetail ivd,   
...
From  invoiceheader
 ...
 Where  ( invoiceheader.ivh_billto = @billto )    
  ...
  --AND (ord.ord_hdrnumber = invoiceheader.ord_hdrnumber)
  --And IsNull(ord_fromorder,'') = @fromorder  Hilco is not breaking on ord_fromorder
  AND ivd_charge <> 0  
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill125_sp] TO [public]
GO
