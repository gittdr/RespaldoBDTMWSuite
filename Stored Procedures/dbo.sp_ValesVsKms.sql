...
--exec sp_ValesVsKms  '2012-12-07', '2012-12-13'

CREATE PROCEDURE [dbo].[sp_ValesVsKms]  @fechaini datetime, @fechafin datetime
AS
...
INSERT Into @TTMovs_Litros 
	SELECT mov_number,  sum(pyd_quantity) , 0 , 0 , 0 , 0,'               ','               ','','', '','','', '','', ''
	FROM paydetail 
	WHERE	pyt_itemcode in ('VALECO', 'COMCOM', 'COMBUS','VALEEL','CMELEC','CVELEC','CANVAL') and 
			mov_number in (
	...
	--select * from paytype where pyt_description like '%combus%'
...
select TTL_Movimiento, TTL_VLitros as Litros, TTL_Kms as Kilometros, TTL_Unidad, TTL_Proyecto, TTL_NomProy, TTL_Flota, TTL_abbr, TTL_axles, TTL_motor, TTL_RendCargado, TTL_RendVacio
from @TTMovs_Litros
