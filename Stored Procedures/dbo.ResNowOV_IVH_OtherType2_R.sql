...
CREATE PROCEDURE [dbo].[ResNowOV_IVH_OtherType2_R]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
			SELECT 	[OtherType2Abbr] = company.cmp_othertype2
			,[OtherType2] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'OtherTypes2' and abbr = company.cmp_othertype2) 
			...
			AND company.cmp_othertype2 = @ItemID
		END
		ELSE
		BEGIN
			SELECT 	[OtherType2Abbr] = company.cmp_othertype2
			...
			FROM invoiceheader IH (NOLOCK) Join Company (NOLOCK) on IH.ivh_billto = company.cmp_id
			WHERE IH.ivh_deliverydate BETWEEN @DateStart and @DateEnd
			AND company.cmp_othertype2 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
	...
	BEGIN
		...
		,IH.ivh_deliverydate
		,[ItemID] = IsNull(Company.cmp_othertype2,'UNKNOWN')
		...
		SELECT 	[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = RNTC.[ItemID],
				[ItemDescription] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'OtherTypes2' and abbr = RNTC.[ItemID]),
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_IVH_OtherType2_R] TO [public]
GO
