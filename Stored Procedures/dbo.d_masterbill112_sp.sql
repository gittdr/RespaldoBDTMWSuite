...
CREATE PROC [dbo].[d_masterbill112_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 * BDH 04/23/08 pts 40707 created this sp from d_masterbill13_sp
 **/
...
-- 41707 requires different reference number logic.
--   UPDATE 	#masterbill_temp 
...
-- 		ivd_sequence=1
/*
select * from referencenumber where ord_hdrnumber in (3158,3159)

*/

update #masterbill_temp
set ord_firstref = (select top 1 isnull(ref_number, '') from referencenumber where ref_type = 'BL#' and ref_table = 'orderheader' and ord_hdrnumber = #masterbill_temp.ord_hdrnumber) 

update #masterbill_temp
set ord_firstref = (select top 1 isnull(ivh_ref_number, '') from invoiceheader where ivh_reftype = 'BL#' and ord_hdrnumber = #masterbill_temp.ord_hdrnumber) 
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill112_sp] TO [public]
GO
