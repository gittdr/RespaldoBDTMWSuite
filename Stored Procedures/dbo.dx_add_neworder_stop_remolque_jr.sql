...
CREATE     PROC [dbo].[dx_add_neworder_stop_remolque_jr]
	@validate char(1),
    @move_number int, 
	@stp_sequence int,
	@event varchar(6),   
	...
	@stp_reftype varchar(6),
	@stp_refnum varchar(30),
...
SELECT @remolque2 = ISNULL(@remolque2,'UNKNOWN')
    ...
    BEGIN
		...
		BEGIN
			SELECT @stp_loadstatus = LEFT(mile_typ_from_stop, 3)
			  FROM eventcodetable
			 WHERE abbr = (SELECT TOP 1 stp_event FROM stops WHERE mov_number = @move_number AND stp_sequence = @back_seq ORDER BY stp_number DESC)
  ...
  BEGIN
	...
	UPDATE stops
	   SET stp_arrivaldate = case when @arrivaldate < getdate() then stp_arrivaldate when @arrivaldate = @midnight and DATEDIFF(dd , stp_arrivaldate, @arrivaldate) > 0 
	     ...
	     , stp_schdtearliest = case when @earlydate < getdate() then stp_schdtearliest when @earlydate = @midnight and DATEDIFF(dd,stp_schdtearliest,@earlydate) > 0
	     ...
	     , stp_schdtlatest = case when @latedate < getdate() then stp_schdtlatest when @latedate = @midnight and DATEDIFF(dd,stp_schdtlatest,@latedate) > 0
	     ...
	     , stp_departuredate = case when @arrivaldate < getdate() then stp_departuredate when @arrivaldate = @midnight and DATEDIFF(dd,stp_departuredate,@arrivaldate) > 0
 ...
 SELECT @stplghmiles = 0, @stpordmiles = 0, @ord_hdrnumber = 0
  ...
  ELSE
    BEGIN
       ...
       IF (SELECT COUNT(1) FROM stops 
		    WHERE mov_number = @move_number AND stp_sequence = @stp_sequence - 1 AND cmp_id = @cmp_id AND stp_city = @stp_city) = 0
...
values ('UNKNOWN', 'UNKNOWN', 'UNKNOWN',
	@remolque1, @remolque2, @ord_hdrnumber,	@@stp_number, @arrivaldate, @earlydate,
	@latedate, @arrivaldate, 'UNK',
	'UNKNOWN', 1, @@fgt_number,
	@evt_number, @pupdrp, @event,
	'OPN' , 1, 'OPN',@@new_move_number)
...
VALUES 	( @remolque1, @ord_hdrnumber, @@stp_number, 		--1
	...
	@stp_delivery_instructions,'OPN'               --18
	)
