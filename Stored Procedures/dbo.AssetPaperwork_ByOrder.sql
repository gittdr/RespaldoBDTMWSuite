...
CREATE PROCEDURE [dbo].[AssetPaperwork_ByOrder]
...
--SELECT @ASSET_TYPE = 'DRV',  @ASSET_ID = 'ROMAL', @LEGNUMBER = 8331, @DOC_TYPE = 'ACK'
...
SELECT LegNumber, OrderNumber, DocType, DocTypeName, [Status] = CASE WHEN Received = 1 THEN 2 
											 WHEN Received = 0 AND [Required] = 1 THEN 4
											 WHEN Received = 0 AND [Required] = 0 THEN 1
											 END

FROM @DATA
ORDER BY LegNumber, OrderNumber, [Status] DESC, DocTypeName 
GO
GRANT EXECUTE ON  [dbo].[AssetPaperwork_ByOrder] TO [public]
GO
