...
CREATE PROC [dbo].[d_settlement_sheet_summary78](  
 ...
 * 
 * NAME:
 * d_settlement_sheet_summary78
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * Returns result set for settlement sheet
 *
 * RETURNS: 	1 = success
 *
 ...
 *
 * REVISION HISTORY:
 * DPETE PTS 35747 6/13/7 new for FUnks Livestock
 * 11/02/2007.01 ? PTS40116 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 4/30/08 BDH 40431.  Added group level for summary page and returning pyt_basis.
*/ 

Declare @PeriodforYTD Varchar(3)  
Declare @Periodendstartdate datetime  
declare @v_psh int,@v_psd int  

-- 40431
declare @asgn_type2 varchar(6),
	@asgn_id2 varchar(13),
	@name2 varchar(64),
	@lgh_tractor2 varchar(8),
	@pyd_payto2 varchar(12),
	@terminal2  varchar(6)  
...
CREATE TABLE #temp_pd(  
 ...
 pyd_description  varchar(75) null,   -- BDH form 30
 ...
 shipper_name  varchar(100) null,  -- from 30 BDH  
 shipper_city  varchar(18) null,  
 shipper_state  char(2) null,  
 consignee_name  varchar(100) null,    -- from 30 BDH  
...
*/  
 pyh_issuedate datetime null,  
    ...
    payperiods_startdate datetime null ,  
    co_driverflag char(1) null,  
    psd_id int null ,
...
/* customer does not like that the model format 16 prints the payperiod from date to pay period to date */  
  
select @v_psd = min (psd_id)from #temp_pd where psd_id > 0  
select @v_psh = max(psh_id) from payschedulesdetail where psd_id = @v_psd  
  
select @Periodendstartdate =  dateadd(dd,1,(select max(psd_date) from payschedulesdetail psd  
     where psh_id = @v_psh and psd.psd_date < @payperiodstart ))  
...
UPDATE #temp_pd  
   SET lgh_startdate = lh.lgh_startdate   
       ,lgh_tractor = lh.lgh_tractor  
       ,co_driverflag =   
         case @drv_yes   
           when 'DRV' then  
             case lgh_driver2 when @drv_id then 'Y' else 'N' end  
           else 'N'   
           end  
   FROM legheader lh  
   WHERE lh.lgh_number = #temp_pd.lgh_number  
   and #temp_pd.lgh_number > 0  
...
UPDATE #temp_pd  
   SET lgh_count = (SELECT COUNT(lgh_number)  
                      FROM legheader lh   
                  WHERE lh.mov_number = #temp_pd.mov_number and #temp_pd.mov_number > 0)  
  
  
--Update the temp pay details with orderheader data  
UPDATE #temp_pd  
   SET ord_enddate = oh.ord_completiondate,  
       ord_number = oh.ord_number,  
       ord_startdate = oh.ord_startdate  
  FROM #temp_pd tp,   
       orderheader oh  
 WHERE tp.ord_hdrnumber = oh.ord_hdrnumber  
 and  tp.ord_hdrnumber > 0  
...
-- BDH 4/15/08 pts 40431.  Need header values for summary page.
set @asgn_type2 = (select top 1 asgn_type from #temp_pd)
set @asgn_id2 = (select top 1 asgn_id from #temp_pd)
set @name2 = (select top 1 name from #temp_pd)
set @lgh_tractor2 = (select top 1 lgh_tractor from #temp_pd)
set @pyd_payto2 = (select top 1 pyd_payto from #temp_pd)
set @terminal2 = (select top 1 terminal from #temp_pd)
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary78] TO [public]
GO
