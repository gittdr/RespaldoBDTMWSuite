...
CREATE PROCEDURE [dbo].[get_driver_msgs_sp] (@driverID varchar(12), @startDate Datetime, @endDate Datetime)       
AS

/**
 * NAME: get_driver_msgs_sp
 *
 * TYPE:
 * StoredProcedure 
 *
 * DESCRIPTION:
 * Pulls all driver messages for the date range
 *
 * NOTE!!: THIS PROC EXISTS IN BOTH TOTALMAIL AND OPERATIONS SOURCE CONTROL. 
 *			Make sure to modify in both places.
 *
 * PARAMETERS:
 * 001 - @DriverId varchar(12)
 ...
 * 03/27/14 - PTS76140 - MIZ - Return message SN
...
WHERE tblDrivers.DispSysDriverID = @driverID AND DTSent > @startDate AND DTSent < @endDate
ORDER BY DTReceived DESC
GO
GRANT EXECUTE ON  [dbo].[get_driver_msgs_sp] TO [public]
GO
