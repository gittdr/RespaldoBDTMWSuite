...
create procedure [dbo].[pegasus_invoicedetail001_sp](@pegcontrol int)
as
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET 
 ...
 SELECT  invoiceheader.ivh_invoicenumber,   
	...
	@temp_fax   cmp_fax,
	@temp_email cmp_email
    into #invtemp_tbl
    FROM invoiceheader, invoicedetail, chargetype, commodity,pegasus_invoicelist peg
   WHERE ( invoicedetail.ivh_hdrnumber = invoiceheader.ivh_hdrnumber ) and
	 ...
	 (peg.peg_controlnumber = @pegcontrol) and
	 (peg.ivh_hdrnumber = invoiceheader.ivh_hdrnumber )

select ivh_billto,
       min( cf.email_address) temp_fax,
       min(ce.email_address)  temp_email
into   #tempemail
from #invtemp_tbl, commpanyemail cf,companyemail ce
where  cf.cmp_id      = #invtemp_tbl.ivh_billto
		AND   cf.mail_deffault = 'Y'
		and   cf.type = 'F'
		and   ce.cmp_id      = #invtemp_tbl.ivh_billto
		AND   ce.mail_deffault = 'Y'
		and   ce.type = 'E'	
      ...
      isnull(c.cmp_mailto_name,'') = '') > 0  or
		...
		set ivh_billto_name = company.cmp_name,
			...
			#invtemp_tbl.cmp_altid = company.cmp_altid,
			 cmp_fax = temp_fax,
			 cmp_email = temp_email
		from #invtemp_tbl, company,#temp_email
		where company.cmp_id = #invtemp_tbl.ivh_billto
		and   #temp_email.ivh_billto  = #invtemp_tbl.ivh_billto
...
select *
from #invtemp_tbl
order by ivh_billto,ivh_invoicehdrnumber,ivd_sequence
...
GO
GRANT EXECUTE ON  [dbo].[pegasus_invoicedetail001_sp] TO [public]
GO
