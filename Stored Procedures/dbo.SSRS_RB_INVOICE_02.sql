...
CREATE procedure [dbo].[SSRS_RB_INVOICE_02](@invoice_nbr int,@copies int,@ivs_number	int,@billdate	datetime)
as 
---[SSRS_LEE_inv] 1138,1,101,'12/3/2012'
...
if IsNull(@cmp_mailto,'') = ''
	...
	begin
		...
		select
			...
			cmp_address2,
			cmp_address3,
			UPPER(cty_name + ', ' + cty_state + ' ' + IsNull(cmp_zip,'')),
			cmp_altid,
			cmp_contact 
		from invoiceheader 
	...
	begin
		...
		select
			...
			cmp_mailto_address2,
			'',
			UPPER(cty_name + ', ' + cty_state + ' ' + IsNull(cmp_mailto_zip,'')),
			cmp_altid,
			cmp_contact			
		from invoiceheader 
			inner join company c on c.cmp_id = ivh_billto
...
select 
	...
	ISNULL(ivh.ivh_rate,'') [Invoice Rate],
	...
	(select name from labelfile where ivd.ivd_distunit = labelfile.abbr and labelfile.labeldefinition = 'DistanceUnits') as  [Invoice Detail Distance Unit],
	...
	(select name from labelfile where ivd.ivd_volunit = abbr and labelfile.labeldefinition = 'VolumeUnits') as [Invoice Detail Volume Unit],
	...
	ref.ref_number,
	la3.name AS [Company_Terms],
	dbo.[TMWSSRS_fcn_referencenumbers_CRLF](IVH.ord_hdrnumber,'stops')as 'RefNums'
into #rsInvoice
from
	invoiceheader ivh
		LEFT OUTER JOIN referencenumber ref ON (	
											   ivh.ord_hdrnumber  = ref.ref_tablekey  
		...
		LEFT JOIN labelfile la2 WITH(NOLOCK) ON (ivh.ivh_invoicestatus = la2.abbr AND la2.labeldefinition = 'InvoiceStatus')
		LEFT JOIN labelfile la3 WITH(NOLOCK) ON (bc.cmp_terms = la3.abbr AND la3.labeldefinition = 'Company'),
	 @Bill_Address BillTo
where ivh.ivh_hdrnumber = @invoice
order by [Charges Group Sort Order]
...
order by copies, [Charges Group Sort Order]
...
GO
GRANT EXECUTE ON  [dbo].[SSRS_RB_INVOICE_02] TO [public]
GO
