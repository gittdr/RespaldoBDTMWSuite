...
SET ANSI_NULLS OFF
GO
/** se agrego esta linea and pyd.asgn_id  not in (select mpp_id from driverdocument where drd_doctype = 'TDV' and drd_default = 'Y')
para excluir a los operadores que tienen tarjeta de debito atte JR
**/

CREATE procedure [dbo].[d_advance_export_por_Movimiento_sp] @id_operador varchar(13)
...
declare @adv_percentage					money,
		@temp_advance					money,
		@monto_new_anticipo				money,
		@monto_expect_advance			money,
		@monto_previous_advance			money
...
select a.lgh_number, a.asgn_type, a.asgn_id
  from TMWSUITE..assetassignment a
  left outer join TMWSUITE..paydetail pyd on pyd.asgn_type = a.asgn_type and pyd.asgn_id = pyd.asgn_id and a.lgh_number = pyd.lgh_number
...
*/
	...
	Select 'MENSAJE NO ENVIADO, El operador :',@id_operador,'No tiene ningun anticipo', 0
...
select 0 as 'status',
  ...
  from @lgh_list
*/
--toma el monto del nuevo anticipo
-- Toma los valores del calculo
select @monto_expect_advance =   expected_advance - previous_advance 
  from @lgh_list

select @monto_previous_advance =   previous_advance 
  from @lgh_list

select @monto_new_anticipo =   @monto_expect_advance - @monto_previous_advance 



-- Si el monto del nuevo anticipo es igual a Cero no envia macro...
IF @monto_new_anticipo > 0
			Select 'MENSAJE ENVIADO: El operador :',@id_operador,'Tiene un anticipo', @monto_new_anticipo

-- Envia Macro de aviso al Operador...
--INSERT Into QSP..EnviaMensajes (cuenta, unidad, macro, mensaje, detmacro)
--values (5,@id_unidad, null, 
	 --'**Proximo Deposito Monto '+ cast((ISNULL(@monto_new_anticipo,''))AS VARCHAR (10))
	--+'**, Hrs Depositos: 11 AM, 1:00, 2:00 y 4:00 PM ',	null)	
