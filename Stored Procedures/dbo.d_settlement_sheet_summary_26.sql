...
CREATE  PROC [dbo].[d_settlement_sheet_summary_26](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary_26
 *
 * TYPE:
 * [StoredProcedure]
 *
 ...
 * 1/2/2007	34197	SLM		Modify format 26 to display resource payto names
 * 1/5/2007	34197	SLM		Modify format 26 to display Payto Companyname, instead of PaytoID
 * 11/02/2007.01 ? PTS40116 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 4/7/08 BDH 41700 increased billto, consignee_name, and shipper_name.
 **/
...
CREATE TABLE #temp_pd(
	...
	pyd_payto		varchar(65) null, -- changed from 6 to 12 for PTS #5849, JET - 6/10/99.  BDH #41700 to 65.
...
-- PTS 29303 -- BL (end)	
	...
	shipper_name		varchar(100) null, --BDH PTS 41700 changed from vc(30) to vc(100)
	shipper_city		varchar(18) null,
	shipper_state		char(2) null,
	consignee_name		varchar(100) null,  --BDH PTS 41700 changed from vc(30) to vc(100)
...
-- BEGIN PTS 34197 Update the Temp pay details with the pay to company information for the resource
update #temp_pd
set pyd_payto = pt.pto_companyname
from #temp_pd tp, manpowerprofile m, payto pt
where tp.asgn_id = m.mpp_id
and   pt.pto_id = m.mpp_payto
and   tp.asgn_type = 'DRV'

update #temp_pd
set pyd_payto = pt.pto_companyname
from #temp_pd tp, tractorprofile t, payto pt
where tp.asgn_id = t.trc_number
and   pt.pto_id = t.trc_owner
and   tp.asgn_type = 'TRC'

update #temp_pd
set pyd_payto = pt.pto_companyname
from #temp_pd tp, trailerprofile l, payto pt
where tp.asgn_id = l.trl_id
and   pt.pto_id = l.trl_owner
and   tp.asgn_type = 'TRL'

update #temp_pd
set pyd_payto = pt.pto_companyname
from #temp_pd tp, carrier c, payto pt
where tp.asgn_id = c.car_id
and   pt.pto_id = c.pto_id
and   tp.asgn_type = 'CAR'
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_26] TO [public]
GO
