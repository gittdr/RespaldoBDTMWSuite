...
CREATE PROC [dbo].[d_stlmnt_sheet_summary_34](
...
UPDATE #temp_pd
   SET ord_startdate = oh.ord_startdate,
       ...
       ord_totalweight = oh.ord_totalweight,
       ord_totalweightunits = oh.ord_totalweightunits
  FROM #temp_pd tp, 
       orderheader oh
...
--Update the temp paydetails with the invoicing referencenumber for Sunset (PTS 20967)
UPDATE #temp_pd
	SET ivh_refnumber = ivh.ivh_ref_number
	FROM invoiceheader ivh, #temp_pd tp
	where tp.ord_hdrnumber = ivh.ord_hdrnumber
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_sheet_summary_34] TO [public]
GO
