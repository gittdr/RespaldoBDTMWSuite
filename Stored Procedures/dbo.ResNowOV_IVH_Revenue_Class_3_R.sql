...
CREATE PROCEDURE [dbo].[ResNowOV_IVH_Revenue_Class_3_R]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
			SELECT 	[RevType3Abbr] = IH.ivh_revtype3
			,[RevType3] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType3' and abbr = IH.ivh_revtype3) 
		...
		BEGIN
			...
			FROM invoiceheader IH (NOLOCK) 
			WHERE IH.ivh_deliverydate BETWEEN @DateStart and @DateEnd
			AND IH.ivh_revtype3 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			ORDER BY IH.ivh_revtype3
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
		...
		SELECT 	[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = RNTC.[ItemID],
				[ItemDescription] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType3' and abbr = RNTC.[ItemID]),
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_IVH_Revenue_Class_3_R] TO [public]
GO
