...
CREATE        PROC [dbo].[d_masterbill130_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 * 12/03/2008 44603 pmill adapted from invoice format 44 for Berner Trucking
 * 04/30/2009 46852 pmill change sort to be on shipper and consignee code rather than name
 *  07/06/12 DPETE 62633 mail to name is not retrieve for the bill to company
 **/
...
if UPPER(@reprintflag) = 'REPRINT' 
  BEGIN
    ...
    SELECT 	IsNull(invoiceheader.ord_hdrnumber, -1),
     ...
     ivh_shipto_name =  ISNULL(cmp2.cmp_name,''),
	 ivh_shipto_address =ISNULL(cmp2.cmp_address1,''),
	 ivh_shipto_address2 =ISNULL(cmp2.cmp_address2,''),
	 ivh_shipto_nmstct =ISNULL(SUBSTRING(cmp2.cty_nmstct,1,CHARINDEX('/',cmp2.cty_nmstct + '/') -1),''),
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	ivh_billto_zip = 
	    ...
	    ivh_consignee_nmstct =ISNULL(SUBSTRING(cmp3.cty_nmstct,1,CHARINDEX('/',cmp3.cty_nmstct + '/') -1),''),
...
--ORDER BY ivh_billto, ivh_shipper_name, ivh_consignee_name, ivh_invoicenumber, ivd_sequence
	ORDER BY ivh_billto, ivh_shipper, ivh_consignee, ivh_invoicenumber, ivd_sequence
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill130_sp] TO [public]
GO
