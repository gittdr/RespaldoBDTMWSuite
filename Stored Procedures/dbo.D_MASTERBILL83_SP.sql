...
CREATE PROC [dbo].[D_MASTERBILL83_SP] 
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	(select ord_route from orderheader where ord_hdrnumber = invoiceheader.ord_hdrnumber) ord_route 
...
into #Temp
   ...
   WHERE 
	...
	AND ((select min(stp_schdtearliest )
		from stops where stops.ord_hdrnumber = invoiceheader.ord_hdrnumber and 
		stp_sequence = (select min(stp_sequence) from stops b where b.ord_hdrnumber = invoiceheader.ord_hdrnumber))
...
GO
GRANT EXECUTE ON  [dbo].[D_MASTERBILL83_SP] TO [public]
GO
