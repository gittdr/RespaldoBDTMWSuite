...
CREATE PROC [dbo].[d_masterbill47_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 *
 * REVISION HISTORY:
 -- vjh pts23573 new proc for new format47.  Copied from mb format08, 
 -- which, oddly enough, was based on d_masterbill07_sp.sql
* 10/30/2007.01 ? PTS40029 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 2/26/10 PTS 51158 fix mail to company name
 *
 **/
DECLARE @int0  int
SELECT @int0 = 0
...
-- dpete for LOR pts4785 provide for maitlto override of billto
  ...
  ORDER BY	ivh_shipdate, ord_number, ivd_sequence

  DROP TABLE 	#masterbill_temp
GO
GRANT EXECUTE ON  [dbo].[d_masterbill47_sp] TO [public]
GO
