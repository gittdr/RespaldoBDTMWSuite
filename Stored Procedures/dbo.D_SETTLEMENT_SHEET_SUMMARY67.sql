...
CREATE PROC [dbo].[D_SETTLEMENT_SHEET_SUMMARY67](
...
DECLARE @li_pydnumber 	INT,
	@li_ordnumber	INT,
	@ls_fromorder	VARCHAR(20),
	@ls_rateunit	VARCHAR(20)
...
CREATE TABLE #temp_pd(
	...
	linehaul_wgt            float null,
	linehaul_rate           float null,
	std_priorbalance        money null,
	pyh_issuedate 		datetime null,
	ord_fromorder		int null, 
	linehaul_qty		float null,
	linehaul_unit		varchar(3) null)
...
UPDATE #temp_pd
   SET std_balance = sd.std_balance ,
       std_priorbalance = sd.std_startbalance 
...
--jyang add statement to calculate the linehaul weight and rate
UPDATE #temp_pd
--ILB PTS# 15618
--Add round function for display purposes on the settlement sheet
   SET linehaul_wgt = round(ivd_quantity,2),
       linehaul_rate = round(ivd_rate,2)
--ILB PTS# 15618
 from  invoicedetail ivd,paytype pt,invoiceheader ivh,chargetype cht
WHERE  #temp_pd.pyt_itemcode = pt.pyt_itemcode and
	pt.pyt_basis = 'LGH'and
	pt.pyt_basisunit = 'REV' and
	ivh.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	ivd.ivh_hdrnumber = ivh.ivh_hdrnumber and
	ivd.cht_itemcode = cht.cht_itemcode and
	cht.cht_basis = 'SHP' and
	cht.cht_basisunit in ('WGT', 'FLT', 'TIM') and
	cht.cht_primary = 'Y' 
...
--	LOR	PTS# 33498	Loop through invoicedetails for Flat and Hourly details
UPDATE #temp_pd
SET linehaul_qty = (SELECT	toe_tonnage
					FROM	ticket_order_entry, orderheader o
					WHERE	toe_ticket_num = #temp_pd.ord_number and
							o.ord_number = toe_ticket_num and 
							toe_master_order = ord_fromorder ),
	linehaul_unit = 'FLT'
from  invoicedetail ivd, invoiceheader ivh, chargetype cht
WHERE  ivh.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	ivd.ivh_hdrnumber = ivh.ivh_hdrnumber and
	ivd.cht_itemcode = cht.cht_itemcode and
	cht.cht_basis = 'SHP' and
	cht.cht_basisunit = 'FLT' and
	cht.cht_primary = 'Y' 

UPDATE #temp_pd
SET linehaul_qty = (SELECT	toe_hours
					FROM	ticket_order_entry, orderheader o
					WHERE	toe_ticket_num = #temp_pd.ord_number and
							o.ord_number = toe_ticket_num and 
							toe_master_order = ord_fromorder ),
...
WHERE  ivh.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	...
	ivd.cht_itemcode = cht.cht_itemcode and
	cht.cht_basis = 'SHP' and
	cht.cht_basisunit = 'TIM' and
	cht.cht_primary = 'Y' 

/*-- Loop through paydetails for Flat and Hourly details
SELECT	@li_pydnumber = MIN(pyd_number)
FROM	#temp_pd
WHERE	pyd_rateunit = 'FLT'
	OR pyd_rateunit = 'HR'

WHILE @li_pydnumber > 0
BEGIN
	SELECT	@li_ordnumber = ord_hdrnumber,
		@ls_rateunit = pyd_rateunit
	FROM	#temp_pd
	WHERE	pyd_number = @li_pydnumber

	-- Get the master order number
	SELECT	@ls_fromorder = ord_fromorder
	FROM	orderheader
	WHERE	ord_hdrnumber = @li_ordnumber

	-- If the order came from a master order, get the appropriate data
	IF @ls_fromorder IS NOT NULL AND @ls_fromorder > 0
	BEGIN
		IF LOWER(@ls_rateunit) = 'flt'
		BEGIN
			-- Set the quantity for that entry to the tonnage on the ticket
			UPDATE	#temp_pd
			SET	pyd_quantity = (SELECT	toe_tonnage
						FROM	ticket_order_entry 
						WHERE	toe_ticket_num = @li_ordnumber 
							AND toe_master_order = @ls_fromorder )
			WHERE	pyd_number = @li_pydnumber
		END
		ELSE IF LOWER(@ls_rateunit) = 'hr'
		BEGIN
			-- Set the quantity for that entry to the hours on the ticket
			UPDATE	#temp_pd
			SET	pyd_quantity = (SELECT	toe_hours
						FROM 	ticket_order_entry
						WHERE	toe_ticket_num = @li_ordnumber
							AND toe_master_order = @ls_fromorder )
			WHERE	pyd_number = @li_pydnumber
...
GO
GRANT EXECUTE ON  [dbo].[D_SETTLEMENT_SHEET_SUMMARY67] TO [public]
GO
