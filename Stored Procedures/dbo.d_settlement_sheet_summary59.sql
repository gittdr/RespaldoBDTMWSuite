...
CREATE PROC [dbo].[d_settlement_sheet_summary59] (
 ...
 * REVISION HISTORY:
   ...
   5-30-2007 PTS: 36887 - jds; adding BEELMAN driver note.
 * 11/02/2007.01 ? PTS40116 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 3/9/09 PTS46335 Very Quick change to add Tractor notes of regarding type Paperwork.
 *
 **/

-- 46335  Moved these 36887 and 46335 declarations to top:
	-- PTS 36887; <<start>> jds add note
			declare @driver_note varchar(2000)  --* must cast this as TEXT or it truncates to 255 in PB-dwobject.
			declare @max_note_count int
			declare @row_count int
			declare @comma char(1)
			set @comma = ', '
	-- PTS 36887; <<end>> jds add note
			declare @TRC_note varchar(2000)  -- 46335  Tractor notes 
			declare @generic_note varchar(2000)  -- 46335  Tractor notes 
-- 46335  end of declarations...
...
UPDATE #temp_pd
   SET ord_startdate = oh.ord_startdate,
	...
	deliverydate = oh.ord_completiondate,
	ord_fromorder = oh.ord_fromorder
  FROM #temp_pd tp, 
       orderheader oh
...
--PTS 32713 JJF 5/10/06  Use mpp_type3 for terminal
UPDATE #temp_pd
   SET terminal = m.mpp_type3
  FROM #temp_pd tp, manpowerprofile m
 WHERE m.mpp_id = tp.asgn_id 
--END PTS 32713 JJF 5/10/06  Use mpp_type3 for terminal
...
--PTS 32713 JJF 4/26/06 get 'truck earnings'
--UPDATE #temp_pd
--SET truckearnings = ivd_charge
--FROM #temp_pd tp, invoicedetail invd
--WHERE tp.ord_hdrnumber = invd.ord_hdrnumber
--	AND invd.cht_itemcode = 'TON'
--	AND tp.pyt_itemcode = 'PERCRE'

UPDATE #temp_pd
SET pyd_quantity = ivd_quantity
FROM #temp_pd tp, invoicedetail invd
WHERE tp.ord_hdrnumber = invd.ord_hdrnumber
	AND invd.ivd_type = 'SUB'
	AND tp.pyt_itemcode = 'PERCRE'
--END PTS 32713 JJF 4/26/06 get 'truck earnings'

UPDATE #temp_pd
SET ivd_rate_fsc = invd.ivd_rate
FROM #temp_pd tp, invoicedetail invd, tractorprofile trc
WHERE tp.ord_hdrnumber = invd.ord_hdrnumber
AND invd.cht_itemcode = 'FSC'
AND 	((tp.asgn_type = 'TRC' AND tp.asgn_id = trc.trc_number AND (trc.trc_type1 = 'O/O' or trc.trc_type1 = 'OOSEN')) 
	OR (tp.asgn_type = 'CAR'))
...
-- 46335 <<start>> encase this in IF condition (no need to run this if NOT Driver)
IF @drv_yes <> 'XXX'
BEGIN 	
		-- 46335 declarations moved to the top.....
			-- PTS 36887; jds add note
			--		declare @driver_note varchar(2000)  --* must cast this as TEXT or it truncates to 255 in PB-dwobject.
			--		declare @max_note_count int
			--		declare @row_count int
			--		declare @comma char(1)
		...
		select IDENTITY (INT, 1, 1 ) AS 'v_ident_count', not_number, not_type, labelfile.name, not_text
		into #temp_notes
		from notes, labelfile
		--PTS 40590 JJF/RH 20060115 
		--where last_updatedatetime between @payperiodstart and @payperiodend CHANGED TO NOTE EXPIRATION
		where not_expires between @payperiodstart and @payperiodend
		--END PTS 40590 JJF/RH 20060115 
		and ( ntb_table = 'manpowerprofile' and nre_tablekey = @drv_id ) 
		and not_type = labelfile.abbr
		and labeldefinition = 'NoteRe'
		order by not_number

		set @driver_note = ''

		IF (select count(*) FROM #temp_notes) > 0
			BEGIN
				set @row_count = 1
				set @max_note_count = (select max(v_ident_count) from #temp_notes ) 
				WHILE @row_count <= @max_note_count
					BEGIN
						-- punctuation... 
						IF @row_count = @max_note_count 
							BEGIN
								SET @comma = ' '
							END 			
						SET @driver_note = @driver_note + (select name + ': ' + not_text + @comma
														   from #temp_notes
														   WHERE v_ident_count = @row_count) 			
						SET @row_count = @row_count + 1
					END		
			END 
		-- end of PTS 36887; jds add note

		-- 46335 added generic note <<start>>
			IF LEN(@driver_note) > 0 
			SET @generic_note = @driver_note
		-- 46335 added generic note <<end>>
END -- 46335 <<end>> 

-- 46335 <<start>> Client asked for Tractor notes also (only for REGARDING type = paperwork)
IF @trc_yes <> 'XXX'
BEGIN 
		select IDENTITY (INT, 1, 1 ) AS 'v_ident_count', not_number, not_type, labelfile.name, not_text
		into #temp_notes_TRC
		from notes, labelfile		
		where not_expires between @payperiodstart and @payperiodend		
		and ( ntb_table = 'tractorprofile' and nre_tablekey = @trc_id ) 
		and not_type = labelfile.abbr
		and labeldefinition = 'NoteRe'
		and UPPER(LTRIM(RTRIM(labelfile.name))) like 'PAPERWORK'
		order by not_number

		set @TRC_note = ''

		IF (select count(*) FROM #temp_notes_TRC) > 0
			BEGIN
				set @row_count = 1
				set @max_note_count = (select max(v_ident_count) from #temp_notes_TRC ) 
				WHILE @row_count <= @max_note_count
					BEGIN
						-- punctuation... 
						IF @row_count = @max_note_count 
							BEGIN
								SET @comma = ' '
							END 			
						SET @TRC_note = @TRC_note + (select name + ': ' + not_text + @comma
														   from #temp_notes_TRC
														   WHERE v_ident_count = @row_count) 			
						SET @row_count = @row_count + 1
					END		
			END 
	IF LEN(@TRC_note) > 0 
	SET @generic_note = @TRC_note

END
-- 46335 <<end>>  Tractor notes 
...
SELECT pyd_number, 
	...
	pyd_offsetpay_number,
	pyd_credit_pay_flag,
	IsNull(total_empty_miles, 0),
	IsNull(total_loaded_miles, 0),
	IsNull(grand_total_miles, 0),
	IsNull(empty_miles, 0),
	IsNull(loaded_miles, 0),
	IsNull(total_miles, 0),
	stp_number,
	deliverydate,
	truckearnings,
	loadcount = (SELECT count(DISTINCT lgh_number) FROM #temp_pd WHERE lgh_number > 0),
	pyt_basis,
	ord_fromorder,
	ivd_rate_fsc,
	cast ( @generic_note as text ) 'driver_note' -- PTS 46335
	--cast( @driver_note as text)'driver_note' -- PTS 36887	

  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber 

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary59] TO [public]
GO
