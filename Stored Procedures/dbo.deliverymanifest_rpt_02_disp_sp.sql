...
CREATE PROC [dbo].[deliverymanifest_rpt_02_disp_sp] (@ord_number VARCHAR (12), @startdt DATETIME, @enddt DATETIME,
...
From orderheader o JOIN stops s	ON o.ord_hdrnumber = s.ord_hdrnumber 
                   ...
                   LEFT OUTER JOIN extra_info_data e ON f.fgt_consignee = e.table_key AND
                                   e.extra_id = 5 AND
                                   e.tab_id = 3 AND
                                   e.col_id = (SELECT MIN(col_id)
                                                 FROM extra_info_data
                                                WHERE extra_id = 5 AND
                                                      tab_id = 3)
...
GO
GRANT EXECUTE ON  [dbo].[deliverymanifest_rpt_02_disp_sp] TO [public]
GO
