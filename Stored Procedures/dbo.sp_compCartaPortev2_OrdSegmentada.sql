...
CREATE proc [dbo].[sp_compCartaPortev2_OrdSegmentada]  @lgh_hdrnumber varchar(20)
...
----SECCION 04 (1:1)
    ...
    'Servicio'                                                                                                           --7 Id Producto (R) --	'Viaje'
																	           +'|'+    
    isnull((select replace(cmp_street_name,'|','') from company where cmp_id = orderheader.ord_billto),'Viaje (Tarifa Fija)')     --8 Descripcion (R)    Servicio de Mercancia Correspondiente al ejercicio 2021
...
----SECCION CARTA PORTE GENERAL(1:1)
   ...
   then 0.01 when stp_lgh_mileage =  '-1' then 0.01 else convert(decimal(10,2),stp_lgh_mileage) end)   as varchar(20))                                                                                                               
	from stops
	where stops.lgh_number = @lgh_hdrnumber --new jr
	--where stops.ord_hdrnumber = orderheader.ord_hdrnumber -- original jr
	and stp_type = 'DRP'
...
replace( (STUFF(( 
   ...
   case when ( select COUNT(*) from stops
	...
	--where stp_type = 'DRP' and stops.ord_hdrnumber = orderheader.ord_hdrnumber original jr
	where stp_type = 'DRP' and stops.lgh_number = @lgh_hdrnumber --new jr
...
--SECCION MERCANCÍAS (1:1)
	...
	and stops.lgh_number = @lgh_hdrnumber)) --new jr
	as decimal(10,2))
	as varchar(20))
	...
	(select case when SUM(fgt_weight) <= 0 then 0.01 else SUM(fgt_weight)  end 
	from freightdetail where stp_number in  (select stp_number from stops where stp_type  ='PUP'  and stops.lgh_number = @lgh_hdrnumber)) 
	...
	(select stp_number from stops where stops.lgh_number = @lgh_hdrnumber 	and stp_type = 'PUP'  -- new jr
	--(select stp_number from stops where stops.ord_hdrnumber = orderheader.ord_hdrnumber 	and stp_type = 'PUP' -- original jr
...
--SECCION MERCANCÍA (1:N)
   ...
   case when isnull(fgt_weight,0) <= 0     then '0.01' 
	             when  isnull(fgt_weight,0) IS NULL  then '0.01' else ( cast(isnull(fgt_weight,0)  as varchar(20))   ) end 
...
----STUFF RECURSIVO DEDE LA TABLA DE STOPS
   ...
   case when ( select COUNT(*) from stops
	...
	--where x.stp_type = 'PUP' and x.ord_hdrnumber = orderheader.ord_hdrnumber and x.stp_sequence < y.stp_sequence   )      original jr                                                         --4 Id Origen
	where x.stp_type = 'PUP' and x.lgh_number = @lgh_hdrnumber and x.stp_sequence < y.stp_sequence   )  --new jr                                                               --4 Id Origen
	...
	-- where y.ord_hdrnumber = orderheader.ord_hdrnumber original jr
	where y.lgh_number = @lgh_hdrnumber  --new jr
...
--SECCION REMOLQUE (0:2) --REMOLQUE 2
	...
	END
     ...
     isnull(REPLACE(trl2.trl_licnum,'|',''),'')                              --3 Placa
																	        +'|'+
	 isnull(REPLACE(trl2.trl_number,'|',''),'')                              --2.5 Remolque 2 jr																	
																	        +'|'+ 																			
																		    +  '\n' )
...
--SECCION  FIGURA TRANSPORTE (0:N)
    ...
    'CP2_TIP_FIG'                                                           --1 Tipo de Registro
																	        +'|'+ 
	 isnull(REPLACE(orderheader.ord_refnum,'|',''),'')						--2 Consecutivo tipo figura transporte -- se ocupo para la referencia 
																	        +'|'+ 
