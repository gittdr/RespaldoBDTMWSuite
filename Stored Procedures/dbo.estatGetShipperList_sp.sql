...
SET ANSI_NULLS ON
GO
Create procedure [dbo].[estatGetShipperList_sp]
    @weeks integer,  -- 2048
...
begin
	insert into #tt
	select distinct ord_Shipper as custid
		...
		begin
			...
			begin
				...
				begin
					...
					insert into #ttx
					select distinct ord_Shipper as custid				--38875
						from orderheader with (index(sk_ord_bookdate))  -- 40530
						where ord_bookdate> @onemonthback 
						and
						 ord_company in (select webusercmpid from #temp2) 
...
drop table #tt
GO
GRANT EXECUTE ON  [dbo].[estatGetShipperList_sp] TO [public]
GO
