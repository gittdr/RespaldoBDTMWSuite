...
SET ANSI_NULLS ON
GO
CREATE  procedure [dbo].[sp_balancesheet] (@modo varchar(5) = NULL)
...
prueba para consultar lo insertado en la tabla pesistente
 select * from  tts_bs_dispo  order by region desc
select * from tts_bs_dispo_detail where region = 'villahermosa' and fecha = 'hoy'
...
SELECT
    ...
    Proyecto = isnull(replace((SELECT name FROM labelfile WHERE  labeldefinition = 'RevType3' and abbr = (select ord_revtype3 from orderheader where orderheader.ord_hdrnumber = legheader.ord_hdrnumber)),'BAJIO','ABIERTO'),''),
...
insert into #rawdata
   ...
   select
	...
	from legheader (nolock)
    where lgh_outstatus in ('STD','DSP')
	-- not in ('AVL','CAN','CMP' )
...
insert into tts_bs_dispo_detail
...
select 
	...
	Destino,
	Remolque,
	Proyecto = isnull(replace(Proyecto,'BAJIO','ABIERTO'), ''),
	Region = Region, 
...
insert into tts_bs_dispo 

select
O.Region,
O.Proyecto,
----al dia de hoy-------------------
outh   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = 'Hoy' and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dsph   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = 'Hoy' and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
stdh   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = 'Hoy' and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
inh    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = 'Hoy' and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
----al dia de hoy + 1 ---------------
out1   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+1'  and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dsp1   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+1'  and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
std1   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+1'  and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
in1    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+1'  and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
----al dia de hoy + 2 ---------------
out2   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+2'  and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dsp2   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+2'  and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
std2   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+2'  and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
in2    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+2'  and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
----al dia de hoy + 3 ---------------
out3   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+3'  and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dsp3   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+3'  and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
std3   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+3'  and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
in3    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+3'  and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
----al dia de hoy + 4 ---------------
out4   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+4'  and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dsp4   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+4'  and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
std4   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '+4'  and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
in4    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '+4'  and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
----al dia de hoy > 4 ---------------
outm   = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '>4'  and A.Status  = 'OUT' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
dspm   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '>4'  and A.Status  = 'DSP' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
stdm   = (select count(    *   ) from tts_bs_dispo_detail A where A.fecha = '>4'  and A.Status  = 'STD' and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
inm    = (select count(A.Fechaf) from tts_bs_dispo_detail A where A.fecha = '>4'  and A.Status  = 'IN'  and (O.Proyecto = A.Proyecto) and (O.Region = A.Region)),
ordeng = 0

from tts_bs_dispo_detail O
