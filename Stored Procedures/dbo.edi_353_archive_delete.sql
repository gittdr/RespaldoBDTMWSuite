...
CREATE PROCEDURE [dbo].[edi_353_archive_delete] @mov_number int
/**
 * 
 * NAME:
 * dbo.edi_353_archive_delete
 *
 * TYPE:
 * Stored Procedure
 *
 * DESCRIPTION:
 * This procedure archives 353 data and deletes records from the edi_353 table.
 *
 * RETURNS:
 ...
 * 02/13/2008.02 - A.Rossman - Added mov_number as input parm
 ...
 **/
 ...
 DECLARE @Counter int
  ...
  --Move the data from the edi_353 table into a temp table and then archive
  CREATE TABLE #temp_353 
  (
  	batch_seq	int 		NULL,
 ...
 SELECT @v_batchcount = COUNT(DISTINCT(batch_number)) FROM edi_353 WHERE mov_number = @mov_number
 ...
 WHILE @v_batchcount >= 1
 	BEGIN
 	SELECT @v_nextbatch = MIN(batch_number) FROM edi_353 WHERE batch_number > @v_nextbatch AND mov_number = @mov_number
 	SET @counter =0 
 	
 	     --Put the records into the temp table and then remove from the edi_353 table
 	     INSERT INTO #temp_353(doctype,batch,data_col,mov_number,archive_date,userid)
 	     SELECT
 	     		'353',
 	     		batch_number,
 	     		data_col,
	    ...
	    WHERE	batch_number = @v_nextbatch
 	    ...
 	    DELETE FROM edi_353 WHERE batch_number = @v_nextbatch AND mov_number = @mov_number
		 ...
		 SELECT 	 mov_number,
					 ...
					 'Dispatch',
					 archive_date,
					 'ACE 335 message created for move:'+ casT(@mov_number as varchar(30))+ ' Batch: ' + CAST(batch as VARCHAR(30)),
					 userid
		 FROM		 #temp_353
		 WHERE	     batch = @v_nextbatch
					AND mov_number = @mov_number
 	    ...
 	    DELETE FROM #temp_353 WHERE batch = @v_nextbatch AND mov_number = @mov_number
...
GO
GRANT EXECUTE ON  [dbo].[edi_353_archive_delete] TO [public]
GO
