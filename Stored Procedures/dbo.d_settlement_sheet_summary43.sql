...
CREATE PROC [dbo].[d_settlement_sheet_summary43](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary43
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * This procedure returns data to be used by d_stlmnt_det_report_general43
 *
 * RETURNS:
 * None
 *
 * RESULT SETS: 
 * Many columns from many pay-related tables.  Defining them is outside of the scope of
 * this Service Request
 * 072 - previous_overdrawn_amt money;
 *       sum of the most recent, consecutive, negative sums of pay,
 *       reimbursements and deductions
 *
 * PARAMETERS:
 * Many.  Defining them is outside of the scope of this Service Request
 *
 * REFERENCES:
 * Many.  Defining them is outside of the scope of this Service Request
 * 
 * REVISION HISTORY:
 * 07/25/2005.01 - Vince Herman ? Specs as defined in PTS 28952
...
Declare @PeriodforYTD		Varchar(3),
	@v_total_overdrawn	money,
	@v_test_overdrawn	money,
	@v_last_payperiod	datetime,
	@v_last_asgn_type	varchar(6),
...
CREATE TABLE #temp_pd(
	...
	pyd_refnum              varchar(20) null,
	pyh_issuedate		datetime null,
	avg_qtd_miles		money null,
	avg_qtd_weeks		int null,
	pyt_basis		varchar(6) null,
	previous_overdrawn_amt	money null,
...
SELECT pd.pyd_number,
	...
	null,
	null,
	(select pyt_basis from paytype where pd.pyt_itemcode = paytype.pyt_itemcode),
	0,
	null
...
-- 24780 JD update avgqtd
	update #temp_pd set avg_qtd_miles = (select  sum(b.pyd_quantity)
	from paytype a, paydetail b,manpowerprofile c
	where
		b.asgn_type = 'DRV' and 
		b.asgn_id = #temp_pd.asgn_id and
		b.asgn_id = c.mpp_id and 		
		a.pyt_itemcode = b.pyt_itemcode and 
		a.pyt_basisunit = 'DIS' and
		b.pyh_payperiod between mpp_90daystart and @payperiodend)

	update #temp_pd set avg_qtd_weeks = datediff (ww , mpp_90daystart , @payperiodend)
	from manpowerprofile c
	where
		#temp_pd.asgn_type = 'DRV' and
		#temp_pd.asgn_id = c.mpp_id 
...
-- vjh PTS28952 calculate previous overdrawn amount
-- walk through each asgn_id, asgn_type
select @v_last_asgn_type = min(asgn_type)
  from #temp_pd
while @v_last_asgn_type is not null begin
  select @v_last_asgn_id = min(asgn_id)
    from #temp_pd
  while @v_last_asgn_id is not null begin

    select @v_total_overdrawn = 0,
           @v_last_payperiod = '2049-12-31'
      
    select @v_last_payperiod = max(pyh_payperiod)
      from payheader
     where asgn_id = @v_last_asgn_id
       and asgn_type = @v_last_asgn_type
       and pyh_payperiod < @v_last_payperiod

    while @v_last_payperiod is not null begin
      select @v_test_overdrawn = pyh_totalcomp + pyh_totaldeduct + pyh_totalreimbrs
        from payheader
       where asgn_id = @v_last_asgn_id
         and asgn_type = @v_last_asgn_type
         and pyh_payperiod = @v_last_payperiod

      IF @v_test_overdrawn < 0
        select @v_total_overdrawn = @v_total_overdrawn + @v_test_overdrawn
      ELSE
        BREAK
    ...
    update #temp_pd
       set previous_overdrawn_amt = @v_total_overdrawn
     where asgn_id = @v_last_asgn_id
       and asgn_type = @v_last_asgn_type
  ...
  select @v_last_asgn_type = min(asgn_type)
    from #temp_pd
   where asgn_type > @v_last_asgn_type
end

	--vjh additional work on 28952 - get tractor termination date
    update #temp_pd
       set termination_date = trc_retiredate
      from tractorprofile
     where trc_number = asgn_id
       and asgn_type = 'TRC'
       and trc_retiredate < '2049-12-30'
...
/*       ph.pyh_paystatus,
	...
	avg_qtd_weeks, --70
	pyt_basis,
	round(previous_overdrawn_amt,2),
	termination_date
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary43] TO [public]
GO
