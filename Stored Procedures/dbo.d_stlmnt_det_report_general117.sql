...
CREATE PROC [dbo].[d_stlmnt_det_report_general117](
 ...
 *
 * REVISION HISTORY:
 * 1/14/2013 ? PTS65298 - TMEZE ? New format based on d_stlmnt_det_report_general107
 *
 **/
...
UPDATE #temp_pd
   SET ord_startdate = oh.ord_startdate,
	   ...
	   invoice_quantity = (select sum(ivh.ivh_quantity) from invoiceheader ivh where ivh.ord_hdrnumber = oh.ord_hdrnumber and ivh.ivh_definition != 'SUPL')
  FROM #temp_pd tp, 
       orderheader oh
...
------UPDATE #temp_pd
------   SET invoice_quantity = (select sum(ivd_quantity) from invoicedetail ivd where ord_hdrnumber = tp.ord_hdrnumber and ivd.ivd_unit in ('BBL','HRS')
------							and ord_hdrnumber <> 0)
------FROM #temp_pd tp
...
-- itemsection = 0 {it's not set} is 'Taxable Settlement Earnings'
-- section 2 is Expense Reimbursements
UPDATE #temp_pd
   SET itemsection = 2
 WHERE pyd_pretax = 'N'
       AND pyd_minus = 1

-- section 3 is Settlement Deductions
UPDATE #temp_pd
   SET itemsection = 3
 WHERE pyd_pretax = 'N'
       AND pyd_minus = -1

-- section 4 is OverDrawn
UPDATE #temp_pd
   SET itemsection = 4
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general117] TO [public]
GO
