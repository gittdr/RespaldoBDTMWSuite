...
CREATE        PROC [dbo].[d_masterbill130_subtotal_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
...
-- dpete for LOR pts4785 provide for maitlto override of billto
   ...
   WHERE	( invoiceheader.ivh_mbnumber = @mbnumber )
		AND (@shipper = invoiceheader.ivh_shipper)
		AND (@consignee = invoiceheader.ivh_consignee)
...
-- dpete for LOR pts4785 provide for maitlto override of billto
 ...
 UPDATE 	#masterbill_temp 
  ...
  SELECT cht_description, cht_primary, 
		bill_quantity = sum(bill_quantity),
	 sum(ivd_charge) 'total_charges'
  FROM		#masterbill_temp
  where ivd_charge <> 0.00
  group by cht_primary, cht_description
  order by cht_primary desc, total_charges desc


  DROP TABLE 	#masterbill_temp

GO
GRANT EXECUTE ON  [dbo].[d_masterbill130_subtotal_sp] TO [public]
GO
