...
GO
  ...
  CREATE PROCEDURE [dbo].[invoice_template96_rollin](@invoice_nbr   int,@copies int)  
AS  
  
/**
 * NAME:
 * dbo.invoice_template96_rollin
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * Data for printed Invoice
 *
 * RETURNS:
 *	Result set for Invoice Format 96
 *
 * RESULT SETS:
 * none.
 *
 * PARAMETERS:
 * 001 - @invoice_nbr, int, input;
 *       Invoiceheader number to Retrieve and Print
 * 002 - @copies, int, input;
 *       Number of Copies of the invoice to print
 *
 * REFERENCES: (None)
 *
 * REVISION HISTORY:
 * 07/20/2006 ? PTS33591 - DJM ? Created. Proc created based on invoice_template2 as new proc for Falcon 
 *	custom invoice format 96.
 *
 **/
...
SELECT ivh.ivh_invoicenumber,   
	...
	ivh_definition
  INTO #invtemp_tbl   
  FROM invoiceheader ivh inner join invoicedetail ivd on ivd.ivh_hdrnumber = ivh.ivh_hdrnumber
	 inner join chargetype cht on cht.cht_itemcode = ivd.cht_itemcode
	left outer join commodity cmd on ivd.cmd_code = cmd.cmd_code  
 WHERE /* JET removed outer join on cht_itemcode, 10/4/98 */  
...
END  
  ...
  SELECT ivd.ivh_hdrnumber,   
    ...
    INTO #invtemp_tbl2   
    FROM invoiceheader ivh inner join invoicedetail ivd ON ivh.ivh_hdrnumber = ivd.ivh_hdrnumber
		inner join chargetype cht on cht.cht_itemcode = ivd.cht_itemcode
   WHERE (IsNull(ivd.cht_rollintolh,0) = 1) AND  
        ivh.ivh_hdrnumber = @invoice_nbr 
...
UPDATE #invtemp_tbl   
 ...
 If Not Exists (Select cmp_mailto_name From company c, #invtemp_tbl t  
   ...
   And t.ivh_charge <> Case IsNull(cmp_MailtToForLinehaulFlag,'Y') When 'Y' Then 0.00 Else ivh_charge + 1.00 End )   
	...
	from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.ivh_billto   
Else   
	update #invtemp_tbl  
...
UPDATE #invtemp_tbl   
  ...
  FROM #invtemp_tbl inner join company cmp on cmp.cmp_id = #invtemp_tbl.ivh_originpoint
	inner join city on city.cty_code = #invtemp_tbl.ivh_origincity
...
UPDATE #invtemp_tbl   
  ...
  FROM #invtemp_tbl inner join company cmp on cmp.cmp_id = #invtemp_tbl.ivh_destpoint
	inner join city on city.cty_code = #invtemp_tbl.ivh_destcity  
...
UPDATE #invtemp_tbl   
 ...
 shipper_addr2 = Case ivh_hideshipperaddr when 'Y'   
  ...
  FROM #invtemp_tbl inner join company cmp on cmp.cmp_id = #invtemp_tbl.ivh_showshipper
...
UPDATE #invtemp_tbl   
 ...
 consignee_addr2 = Case ivh_hideconsignaddr when 'Y'   
  ...
  FROM #invtemp_tbl inner join company cmp on cmp.cmp_id = #invtemp_tbl.ivh_showcons
  
update #invtemp_tbl  
set terms_name = la.name  
from #invtemp_tbl inner join labelfile la on la.abbr = #invtemp_tbl.ivh_terms 
...
UPDATE #invtemp_tbl   
  ...
  FROM #invtemp_tbl inner join company cmp on cmp.cmp_id = #invtemp_tbl.cmp_id
...
-- dpete for UNKNOWN companies with cities must get city name from city table pts5319   
 update #invtemp_tbl  
  ...
  from  #invtemp_tbl inner join stops on stops.stp_number =  #invtemp_tbl.stp_number
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template96_rollin] TO [public]
GO
