...
create procedure [dbo].[invoice_template125](@invoice_nbr  	int,@copies		int)
...
09/21/2007	vjh	copy invoice_template 3 to 125 with the parallel invoice_template125_rollin
...
6/4/08 pts42860 BDH.  Converted inner join to orherheader in initial select to an outer join so we can use for misc invoices.
*/
...
SELECT invoiceheader.ivh_invoicenumber,   
  ...
  FROM invoiceheader JOIN invoicedetail ON invoiceheader.ivh_hdrnumber = invoicedetail.ivh_hdrnumber
                     ...
                     LEFT OUTER JOIN orderheader ON invoiceheader.ord_hdrnumber = orderheader.ord_hdrnumber  -- BDH 42860:  changed to outer join for misc invoices.
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template125] TO [public]
GO
