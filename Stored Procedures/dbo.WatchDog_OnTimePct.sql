...
CREATE Proc [dbo].[WatchDog_OnTimePct] 
(
	@MinThreshold float = -1,
	@MinsBack int=-20,
	@TempTableName varchar(255)='##WatchDogGlobalOnTimePct',
...
/*
if not exists (select WatchName from WatchDogItem where WatchName = 'OnTimePct')
INSERT INTO watchdogitem (WatchName, BeginDate, EndDate, SqlStatement, Operator, EmailAddress, BeginDateMinusDays, EndDatePlusDays, DateField, QueryType, ProcName, NumericOrText, MinsBackToRun, HTMLTemplateFlag, ActiveFlag, DefaultCurrency, CurrencyDateType, Description)
 VALUES ('OnTimePct','12/30/1899','12/30/1899','WatchDog_OnTimePct','','',0,0,'','','','','',1,0,'','','')
*/
...
END
	...
	SELECT Driver1 as [Driver], DriverLastName, DriverFirstName, sum(case islatevsthreshold when 'N' then 1 else 0 end) as OnTime,
		sum(case islatevsthreshold when 'Y' then 1 else 1 end) as Total,
		0 as EmploymentDays
	INTO #TempResultsStep3
	FROM #TempResultsStep2
	Group by Driver1, DriverLastName, DriverFirstName

	update #TempResultsStep3
	...
	SELECT [Driver], DriverLastName,DriverFirstName,
			...
			Total as [Total Stops],
			Ontime as [Total On Time Stops],
			EmploymentDays
	into #TempResults
...
GO
GRANT EXECUTE ON  [dbo].[WatchDog_OnTimePct] TO [public]
GO
