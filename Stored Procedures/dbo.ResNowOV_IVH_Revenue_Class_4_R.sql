...
CREATE PROCEDURE [dbo].[ResNowOV_IVH_Revenue_Class_4_R]  -- test
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
			SELECT 	[RevType4Abbr] = IH.ivh_revtype4
			,[RevType4] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType4' and abbr = IH.ivh_revtype4) 
		...
		BEGIN
			...
			FROM invoiceheader IH (NOLOCK) 
			WHERE IH.ivh_deliverydate BETWEEN @DateStart and @DateEnd
			AND IH.ivh_revtype4 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			ORDER BY IH.ivh_revtype4
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
		...
		SELECT 	[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = RNTC.[ItemID],
				[ItemDescription] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType4' and abbr = RNTC.[ItemID]),
	...
	END
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_IVH_Revenue_Class_4_R] TO [public]
GO
