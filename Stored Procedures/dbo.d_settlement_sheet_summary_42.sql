...
CREATE PROC [dbo].[d_settlement_sheet_summary_42](
...
CREATE TABLE #temp_pd(
   ...
   ord_reftype    char(20) null,
	...
	pyd_refnumtype             varchar(20) null,
...
--JLB PTS 21218  needed for format 42 
UPDATE #temp_pd
   SET pyt_basis = paytype.pyt_basis
  FROM paytype
 WHERE #temp_pd.pyt_itemcode = paytype.pyt_itemcode

UPDATE #temp_pd
   SET ord_refnum = (select ord_refnum
                       from orderheader
                      where orderheader.ord_hdrnumber = #temp_pd.ord_hdrnumber)
 WHERE pyt_basis = 'LGH'

UPDATE #temp_pd
   SET ord_reftype = (select name
                       from orderheader, labelfile lbl
                      where orderheader.ord_hdrnumber = #temp_pd.ord_hdrnumber
                        and lbl.labeldefinition = 'ReferenceNumbers'
                        and lbl.abbr = orderheader.ord_reftype)
 WHERE pyt_basis = 'LGH'
...
update #temp_pd
   set pyd_refnumtype = lbl.name
  from labelfile lbl
 where lbl.labeldefinition = 'ReferenceNumbers'
   and lbl.abbr = #temp_pd.pyd_refnumtype
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_42] TO [public]
GO
