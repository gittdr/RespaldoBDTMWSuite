...
create procedure [dbo].[invoice_template2_cryo](@invoice_nbr  	int,@copies		int)
as
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET 
 ...
 SELECT  invoiceheader.ivh_invoicenumber,   
    ...
    FROM chargetype  RIGHT OUTER JOIN  invoicedetail  ON  chargetype.cht_itemcode  = invoicedetail.cht_itemcode   
		 ...
		 invoiceheader  LEFT OUTER JOIN  referencenumber ref  ON  
					(invoiceheader.ord_hdrnumber  = ref.ref_tablekey  and ref.ref_table = 'orderheader' and ref.ref_sequence = 2)
...
-- there is no shipper city on the invoiceheader, if shipper is UNKNOWN pick up origin city
update 	#invtemp_tbl
...
-- there is no consignee city on the invoiceheader, if cons is UNKNOWN pick up dest city
update 	#invtemp_tbl
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template2_cryo] TO [public]
GO
