...
CREATE PROCEDURE [dbo].[sp_control_oper_integrated_D5_JR]   @lider varchar(12)
AS
DECLARE	
	@V_fechadom		DateTime, 
	@V_fechasab		DateTime,
...
DECLARE @TTOperadores_Control_D5 TABLE(
		...
		If Exists ( Select count(*) From  @TTOperadoresporflota )
				...
				-- Llena los totales de cada codigo.
					--Domingo Dia 1 Descansos
						Insert Into @TTOperadores_Control_D5 (codigo,OpeId,OpeNombre,Fechaini,Fechafin, FolioExt )
...
--					-- Permisos Dia 1 Inhabilitados
						--Insert Into @TTOperadores_Control_D5 (codigo,OpeId,OpeNombre,Fechaini,Fechafin )
...
select * from @TTOperadores_Control_D5
