...
--execute sp_insertaCasetasMovimiento 216880, 'NADD'
--drop procedure sp_insertaCasetas
CREATE PROCEDURE [dbo].[sp_insertaCasetasMovimiento]
...
BEGIN
	...
	Declare segmentos cursor for select lgh_number from legheader where mov_number = @ord
	open segmentos
	fetch segmentos into @lgh
	while @@FETCH_STATUS = 0
		begin
			--print 'Legheader:  '+ cast(@lgh as nvarchar(100))
				...
				begin
					...
					if @totalEfectivo <> 0
						begin
							...
							select @driver = (select lgh_driver1 from legheader where lgh_number = @lgh)
							select @pyd_prorap = (select mpp_actg_type from manpowerprofile where mpp_id = @driver)
							select @pyt_itemcode_iave = (select distinct pyt_itemcode from tollbooth  where tb_ident > 0 and tb_vendor_name = 'EFECTIVO')
							...
							pyd_lessrevenue, pyd_payrevenue, pyd_loadstate, pyd_transdate, pyd_xrefnumber, ord_hdrnumber, pyt_fee1, pyt_fee2, pyd_grossamount, pyd_adj_flag, pyd_maxquantity_used, 
							pyd_maxcharge_used, pyd_vendortopay, pyd_gst_flag 
                             ) VALUES ( @pyd_number, 0, @lgh, 0, 'DRV', @driver, 0, @pyd_prorap, 'UNKNOWN',@pyt_itemcode,substring(@casetasEfectivo,1,74), 1.0000,
