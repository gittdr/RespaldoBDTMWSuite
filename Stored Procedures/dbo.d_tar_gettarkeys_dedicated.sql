...
/*   MODIFICATION
LOR   PTS# 43821  retrieve dedicated billing rates
...
CREATE PROC [dbo].[d_tar_gettarkeys_dedicated]
   @tarnum int,
   @billdate datetime,
   @billto char(8),
   @revtype1 char(6),
   ...
   @revtype4 char(6),
   @dbs_type VARCHAR(6),
   @dbs_group VARCHAR(6)   --PTS 63450 NLOKE
as
...
create table #temp (trk_number int null,
   ...
   tar_orderstoapply int      null,    --63181
   tar_ordersremaining int    null,    --63181
   trk_dbs_group     VARCHAR(6) NULL      -- 3/4/2013 NLOKE 63450
)

Declare @temp datetime, @v_date_past datetime

Select @temp = convert(datetime,(convert(char(4), datepart(yyyy, @billdate)) + '-' +
               case when datepart(mm, @billdate) < 10 then '0' + convert(char(1), datepart(mm, @billdate) )
               else convert(char(2), datepart(mm, @billdate))
               end + '-01 00:00'), 102)

Select @v_DATE_past = dateadd(dd,-1, @temp)
...
select t.trk_number,
   ...
   t.trk_index_factor,  --PTS 38973 EMK
...
GO
GRANT EXECUTE ON  [dbo].[d_tar_gettarkeys_dedicated] TO [public]
GO
