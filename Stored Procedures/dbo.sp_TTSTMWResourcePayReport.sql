...
CREATE              Procedure [dbo].[sp_TTSTMWResourcePayReport] (@asgntype as Varchar(100),
...
SELECT Case When @showpaytoifexists = 'Y' Then
       ...
       IsNull(pyh_totalcomp,0) AS Compe, 
       IsNull(pyh_totalreimbrs,0) AS Reimb, 
       IsNull(pyh_totaldeduct,0) AS Deduct, 
       (IsNull(pyh_totalcomp,0)+IsNull(pyh_totalreimbrs,0)+IsNull(pyh_totaldeduct,0)) AS NetTotal, 
...
--<TTS!*!TMW><End><FeaturePack=Euro>
FROM   payheader 
WHERE  (pyh_payperiod between @frmdt and @tdt )
       And
...
GO
GRANT EXECUTE ON  [dbo].[sp_TTSTMWResourcePayReport] TO [public]
GO
