...
BEGIN
		insert into [dbo].[fuelpurchased](fp_id, fp_sequence, fp_cardnumber, fp_cac_id, fp_ccd_id, fp_purchcode, fp_date,
			 ...
			 select distinct 'PERC-' +Cast(p.folio as varchar(500)) as fp_id,
					1 as fp_sequence,
					 p.Tarjeta as fp_cardnumber,  
					'PERC' as fp_cac_id,
					null as fp_ccd_id, 
					'PERC' as asfp_purchcode,
					...
					'PERCDataService' as fp_enteredby,	
					...
					'PERC' as fp_chaincode,
					null as fp_servicefee 

		 from Fuel.[dbo].[purchase] p
			inner join  [dbo].[truckstops] t on p.estacion = t.ts_name
			where p.proveedor = 'PERC'
			and p.Unidad in (select trc_number from tractorprofile)
			--and p.fechatransaccion >= (Select Convert(varchar, DATEADD(day,1,max(fp.fp_date)),103) from fuelpurchased fp where fp.fp_cac_id = 'PERC')
			and folio not in(
	select folio  from Fuel.[dbo].[purchase] p
	inner join fuelpurchased fp on 'PERC-'+cast(folio as varchar) = fp.fp_id )
...
--update  Fuel.[dbo].[purchase]  set Estacion = 'SERVICIO CHACHAPA 12609       ' where estacion = 'BP OSIRIS 0M784'
GO
