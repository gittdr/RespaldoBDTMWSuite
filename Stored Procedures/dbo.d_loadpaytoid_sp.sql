...
/****** Object:  Stored Procedure dbo.d_loadpaytoid_sp    Script Date: 8/20/97 1:57:55 PM ******/
CREATE PROC [dbo].[d_loadpaytoid_sp] @comp varchar(8) , @number int AS
/**
 * 
 * NAME:
 * dbo.d_loadpaytoid_sp 
 *
 * TYPE:
 * Stored Procedure
 *
...
if exists	(	SELECT	pto_id 
	...
	SELECT  isnull(case 
		...
		isnull(cty_zip,'')				
	FROM    payto left outer join city on payto.pto_city = city.cty_code
			--PTS 64250 JJF 20120808
			--inner join RowRestrictValidAssignments_payto_fn() rsva on (payto.rowsec_rsrv_id = rsva.rowsec_rsrv_id or rsva.rowsec_rsrv_id = 0) 
			--END PTS 64250 JJF 20120808
	WHERE   pto_id like @comp + '%' 		/* 08/05/2010 MDH PTS 52432: Changed to like */
...
GO
GRANT EXECUTE ON  [dbo].[d_loadpaytoid_sp] TO [public]
GO
