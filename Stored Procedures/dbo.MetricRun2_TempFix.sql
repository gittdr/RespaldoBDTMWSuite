...
CREATE PROCEDURE [dbo].[MetricRun2_TempFix]
(
...
AS
	...
	*/
	...
	DECLARE @Extra varchar(1)
	
	DECLARE @syscolumns TABLE (colname varchar(255), colid int, xtype_name varchar(255), extra varchar(1) )
	...
	INSERT INTO @syscolumns (colname, colid, xtype_name, extra)
	SELECT t2.name, t2.colid, t3.name, CASE WHEN t3.name IN ('char', 'datetime', 'nchar', 'nvarchar', 'smalldatetime', 'varchar') THEN '''' ELSE '' END
	FROM sysobjects t1 INNER JOIN syscolumns t2 ON t1.id = t2.id
		INNER JOIN systypes t3 ON t2.xtype = t3.xtype
	WHERE t1.name = @ProcName
 -- select DISTINCT name from systypes where name LIKE '%date%' or name like '%ch%'
	...
	WHILE 1=1
		BEGIN
			...
			SELECT @Extra = Extra FROM @syscolumns WHERE ColName = @NextParameter
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						ELSE
							SET @SQL = @SQL + ', ' + @NextParameter + '=' + REPLACE(@NextParameterValue, ',', '.')
					END
				END
			END
			ELSE
...
GO
GRANT EXECUTE ON  [dbo].[MetricRun2_TempFix] TO [public]
GO
