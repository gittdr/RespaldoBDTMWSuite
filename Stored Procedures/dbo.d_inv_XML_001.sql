...
Create procedure [dbo].[d_inv_XML_001](@invoice_nbr  	int,@copies		int,@ivs_number	int)
as 
...
*	sample call
	...
	d_inv_XML_001 6084, 1, 1

*
*************/
--dummy values to allow blank select to create datawindow
--set @invoice_nbr = 6084
--set @copies = 1
--set @ivs_number = 1
--vjh PTS 54915 Add _2005 support to prevent this from blowing up SQL2000 servers
--vjh PTS 72363 Add ivh_paid_amount
...
if (select isnull(cmp_mailto_name,'')
	from invoiceheader
...
select 
	...
	ivh.ivh_invoicestatus [Invoice Header Invoice Status],
...
--	ivh.ivh_taxamount4 [Invoice Header Tax Amount 4],	--vjh using this column for balance due information
	ivh.ivh_paid_amount [Invoice Header Tax Amount 4],
	...
	ivh.ivh_billdate [Invoice Header Bill Date],
	...
	ivh.ivh_carrier [Invoice Header Carrier],
	car.car_scac [Carrier SCAC Code],
...
--	ivh.ivh_paid_amount [GP_Paid_Amount]	--vjh 72363 add column to temp table rather than in final select
	0 [GP_Paid_Amount]	--vjh 72363 column type mismatch, back to zero
...
from
	invoiceheader ivh
		...
		LEFT OUTER JOIN carrier car ON car.car_id = ivh.ivh_carrier
...
select 
	...
	#rsInvoice.*
--	IsNull((select dbo._fnc_TMW_ARHistoricalAgingBalanceDue (getdate(),#rsInvoice.[Invoice Header Invoice Number], 1)),0) as [GP_Paid_Amount]
	--0 [GP_Paid_Amount]  --vjh 72363 column is now in temp table so the previouis slect star includes the column
from #rsInvoice
...
-- cleanup
drop table #rsInvoice
drop table #rsInvoiceCopy

IF @@ERROR != 0 select @ret_value = @@ERROR
return @ret_value

GO
GRANT EXECUTE ON  [dbo].[d_inv_XML_001] TO [public]
GO
