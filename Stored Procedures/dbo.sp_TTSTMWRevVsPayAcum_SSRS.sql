...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[sp_TTSTMWRevVsPayAcum_SSRS]
	(
...
DECLARE LegHeaderAcumulado_SSRS TABLE(
...
Update #OrdCompDTFixAcum
	Set ord_completiondate =ivh_deliverydate
	from
	invoiceheader (NOLOCK)
	where invoiceheader.ord_hdrnumber = #OrdCompDTFixAcum.ord_hdrnumber
--bcy
--select count(*)  as 'count from #OrdCompDTFixAcum' from #OrdCompDTFixAcum
--=========================================================================
Insert into #movlistAcum
SELECT 
...
--bcy
--select * from #movlistAcum
IF @DebugOnYN='Y'			SELECT ' =Y SELECT * from #movlistAcum'
IF @DebugOnYN			='Y'  SELECT * from #movlistAcum
...
END
--bcy
--select count(*)  as 'count from #movlistAcum ' from #movlistAcum
IF @DebugOnYN='Y' select count(*) CNTMOVES from #movlistAcum
select 
	L.ord_hdrnumber ,
	L.lgh_tractor,
...
--					Sum(ISNULL(pyd_amount,0)) 
	...
	LoadedMlsMove=ISNULL((SELECT sum( isnull( stp_lgh_mileage , 0 ) )
			 ...
			 WHERE #movlistAcum.mov_number = s1.mov_number
...
--			       ISNULL((SELECT sum(ISNULL( ivd_charge, 0 ) )
				...
				chargetype (NOLOCK)
			WHERE 
				Invoiceheader.mov_number = #movlistAcum.mov_number
	...
	FirstBillDate=
		ISNULL(
			...
			where
				Invoiceheader.mov_number = #movlistAcum.mov_number
				and
				L.mov_number= #movlistAcum.mov_number
			)
		,'1/1/50'),
	LastBillDate=
		ISNULL(
...
into #LegHeaderSummaryAcum
From
	#movlistAcum,
	legheader L (index=uk_mov) 
where
	#movlistAcum.mov_number=l.mov_number
...
-- order by #movlistAcum.mov_number, lgh_startdate
--Update the BillTo For Non Invoiced Loads
Update  #LegHeaderSummaryAcum
	Set FirstBillTo=
		IsNull((Select 
		...
		where 
			...
			and
			orderheader.ord_hdrnumber=#LegHeaderSummaryAcum.ord_hdrnumber
			
			
		),FirstBillTo)
--Where FirstBillTo Is Null or LTrim(RTrim(FirstBillTo)) = ''
IF @DebugOnYN='Y' select count(*) CNTLegHeaderSummary from #LegHeaderSummaryAcum
IF @DebugOnYN='Y' select mov_number, TotLHChargeForMove from #LegHeaderSummaryAcum
--====================================================================================
...
Update  #LegHeaderSummaryAcum
	Set MinOrderHdrnumber=
		...
		from 
			...
			legheader (NOLOCK)
		where 
			legheader.lgh_number=#LegHeaderSummaryAcum.lgh_number
			and
...
--			IsNull(sum(ord_charge),0.00)
		...
		where
			--orderheader.ord_hdrnumber=MinOrderHdrnumber
                        orderheader.mov_number=#LegHeaderSummaryAcum.mov_number
			and
	...
	--From 
		--orderheader
	where 
		--orderheader.ord_hdrnumber=#LegHeaderSummaryAcum.MinOrderHdrnumber
...
--				IsNull(sum(ivd_charge),0.00)
			...
			FROM 
				...
				chargetype (NOLOCK)
			WHERE 
				Invoiceheader.mov_number =#LegHeaderSummaryAcum.mov_number
				And
				invoiceheader.ivh_invoicestatus <> 'CAN' 
				AND
				Invoiceheader.ivh_hdrnumber= invoicedetail.ivh_hdrnumber
...
Update #LegHeaderSummaryAcum
	...
	1 / 	(Select 
			count(l2.lgh_number) 
		From 
			#LegHeaderSummaryAcum l2
		where
			l2.lgh_number=#LegHeaderSummaryAcum.lgh_number
		)
	where TotMlsMove=0 and PercentLghMlsOfMoveMls=0
--====			
Update  #LegHeaderSummaryAcum
...
Update  #LegHeaderSummaryAcum
	Set MinCommodityCode=
		...
		From
			Stops (NOLOCK)
		where
			stops.lgh_number=#LegHeaderSummaryAcum.lgh_number
		)
	where #LegHeaderSummaryAcum.lgh_number>0
...
Update  #LegHeaderSummaryAcum
	Set MinCommodityClass=
		...
		From
			Commodity (NOLOCK)
		where
			#LegHeaderSummaryAcum.MinCommodityCode=commodity.cmd_code
		)
	where #LegHeaderSummaryAcum.lgh_number>0
...
Update #LegHeaderSummaryAcum
	Set CompletionDateOnly =
		...
		from
			--Orderheader --DELVFIX
			#OrdCompDTFixAcum ----DELVFIX
			
		where
			#LegHeaderSummaryAcum.MinOrderHdrnumber =
			--Orderheader.ord_hdrnumber ----DELVFIX
			#OrdCompDTFixAcum.ord_hdrnumber ----DELVFIX
		)
	where
		MinOrderHdrnumber>0
	...
	Set LastPayTo=
		ISNull( 
			...
			from
				paydetail (NOLOCK)
			where
				paydetail.lgh_number=#LegHeaderSummaryAcum.lgh_number
				and
				pyd_pretax ='Y'
			)
		,0)
...
SET NOCOUNT OFF 
-- FINAL SELECT
IF @DebugOnYN			='Y' select count(distinct(mov_number)) from #LegHeaderSummaryAcum 
IF @DebugOnYN			='Y'  select mov_number,ord_hdrnumber, lgh_number  from #LegHeaderSummaryAcum  order by mov_number, lgh_number  
/* jr
...
mpp_teamleader,
CountofLoads  
from #LegHeaderSummaryAcum 
order by mov_number, lgh_startdate
jr */
-- Limpia la tabla de la inf
delete LegHeaderAcumulado_SSRS;

Insert LegHeaderAcumulado_SSRS (ord_hdrnumber,
...
--SET NOCOUNT ON
ENDE:
Drop table #movlistAcum
Drop table #LegHeaderSummaryAcum
Drop table #OrdCompDTFixAcum
--SET NOCOUNT OFF
