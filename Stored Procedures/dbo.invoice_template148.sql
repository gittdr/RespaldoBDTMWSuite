...
CREATE PROC [dbo].[invoice_template148](@p_invoice_nbr int,@p_copies int)
as

/*
 * 
 * NAME:invoice_template148
 * dbo.
 *
 * TYPE:
 * StoredProcedure
 ...
 * 10/20/2008 pmill custom from invoice format62 for Cardinal Metal
 **/
...
declare	@temp_name   varchar(30) ,
	...
	@ret_value  int,
		...
		@GST_ivd_description varchar(30),
		@PST_ivd_description varchar(30),
		@QST_ivd_description varchar(30),
        @ref_type varchar(20),
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET 
 ...
 SELECT  invoiceheader.ivh_invoicenumber,   
	...
	IsNull(ivh_charge,0.0) ivh_charge,
        ...
        @GST_ivd_description gst_ivd_description,
        @PST_ivd_description pst_ivd_description,
		@QST_ivd_description qst_ivd_description,
        @ref_type ref_type1,
...
Select @GST_ivd_description = ivd_description
	from invoicedetail ivd
	where  ivd.ivh_hdrnumber = @p_invoice_nbr 
     and  ivd.cht_itemcode = 'GST'

Select @PST_ivd_description = ivd_description
	from invoicedetail ivd
	where ivd.ivh_hdrnumber = @p_invoice_nbr
	and ivd.cht_itemcode = 'PST'

Select @QST_ivd_description = ivd_description
	from invoicedetail ivd
	where  ivd.ivh_hdrnumber = @p_invoice_nbr 
     and  ivd.cht_itemcode IN ('QST', 'TAX3')
...
Update #invtemp_tbl
	set gst_ivd_description = @GST_ivd_description,
		pst_ivd_description = @PST_ivd_description,
		qst_ivd_description = @QST_ivd_description
...
Update #invtemp_tbl
  ...
  from invoicedetail ivd
 where ivd.ivh_hdrnumber = @p_invoice_nbr and
       ivd.cht_itemcode IN ('QST','TAX3')

Update #invtemp_tbl
	Set pst_total = ivd.ivd_charge,
		 pst_rate = ivd.ivd_rate
		from invoicedetail ivd
		where ivd.ivh_hdrnumber = @p_invoice_nbr 
		and ivd.cht_itemcode = 'PST'
...
Select @TVQ_PST = cht_description
	From #invtemp_tbl
	Where upper(cht_itemcode) = 'PST'

Update #invtemp_tbl
   Set qst_desc = @TVQ_QST 

Update #invtemp_tbl
	Set pst_desc = @TVQ_PST
...
begin
	...
	IsNull(ivh_charge,0) ivh_charge,
        ...
        isnull(pst_total,0)pst_total,
        isnull(gst_total,0)gst_total,
        isnull(qst_rate,0)qst_rate,
		isnull(pst_rate,0)pst_rate,
...
--	 @counter,
	...
	ivh_showcons,
        ...
        isnull(gst_total,0)gst_total,
        isnull(qst_rate,0)qst_rate,
        isnull(pst_rate,0)qst_rate,
        isnull(gst_rate,0)pst_rate,
 	gst_number,
	qst_number,
		gst_ivd_description,
		pst_ivd_description,
	...
	ivh_billto_addr3,
        ...
        notes
from #invtemp_tbl
where cht_itemcode NOT IN ('QST','TAX3','GST')
--where cht_itemcode NOT IN ('GST', 'PST')
and ivd_charge > 0 or (ivd_type = 'DRP' and cmd_code <> 'UNKNOWN' and ivh_consignee = cmp_id)

/* SET RET VALUE TO @@ERROR IF ONE HAS OCCURED. */
IF @@ERROR != 0 select @ret_value = @@ERROR 
return @ret_value
GO
GRANT EXECUTE ON  [dbo].[invoice_template148] TO [public]
GO
