...
CREATE PROCEDURE [dbo].[invoice_template156_rollin] (@invoice_nbr  	int,@copies int)
AS
/*
*
* 
* NAME:invoice_template156_rollin
* dbo.invoice_template156_rollin
*
* TYPE:
* StoredProcedure
*
...
FROM invoiceheader ivh join invoicedetail ivd on ivd.ivh_hdrnumber = ivh.ivh_hdrnumber 
	LEFT OUTER JOIN orderheader ord on ord.ord_hdrnumber = ivh.ord_hdrnumber  AND ivh.ord_hdrnumber <> 0  --44798
...
SET shipper_name = cmp.cmp_name, 
	...
	then ''
	else cmp.cmp_address3
	end,
    --shipper_nmctst = substring(cmp.cty_nmstct,1, (charindex('/', cmp.cty_nmstct))) + ' ' + cmp.cmp_zip 
	shipper_nmctst = ISNULL(SUBSTRING(cmp.cty_nmstct,1,
				CASE
						WHEN CHARINDEX('/',cmp.cty_nmstct)- 1 < 0 THEN 0
						ELSE CHARINDEX('/',cmp.cty_nmstct) -1
				END),'') + ' ' + cmp.cmp_zip
...
SET consignee_name = cmp.cmp_name, 
	...
	consignee_nmctst = ISNULL(SUBSTRING(cmp.cty_nmstct,1,
				CASE
						WHEN CHARINDEX('/',cmp.cty_nmstct)- 1 < 0 THEN 0
						ELSE CHARINDEX('/',cmp.cty_nmstct) -1
				END),'') + ' ' + cmp.cmp_zip
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template156_rollin] TO [public]
GO
