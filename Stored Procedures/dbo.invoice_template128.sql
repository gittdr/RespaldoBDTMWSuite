...
create procedure [dbo].[invoice_template128](@invoice_nbr int, @copies int)
as  
/*
*
* 
* NAME:invoice_template128
* dbo.invoice_template128 
*
* TYPE:
* StoredProcedure
*
...
* 
* REVISION HISTORY:
* 1/7/8 created by BPISK PTS 38641
*/
...
SELECT invoiceheader.ivh_invoicenumber,     
    ...
    case when invoicedetail.ivd_type = 'DRP' then invoiceheader.ivh_rate else invoicedetail.ivd_rate end ivd_rate,     
    case when invoicedetail.ivd_type = 'DRP' then invoiceheader.ivh_charge else invoicedetail.ivd_charge end ivd_charge,     
    ...
    (select top 1 ref_number from referencenumber where ref_table = 'orderheader'  and ref_tablekey > 0  and ref_tablekey = invoiceheader.ord_hdrnumber  and ref_type in ('PO#','PO') order by ref_sequence) as customer_order_po,
    (select top 1 ref_number from referencenumber where ref_table = 'orderheader'  and ref_tablekey > 0  and ref_tablekey = invoiceheader.ord_hdrnumber  and ref_type in ('BL#','BOL') order by ref_sequence) as customer_bol
into #invtemp_tbl
...
where invoiceheader.ivh_hdrnumber = @invoice_nbr
  and (invoicedetail.ivd_type = 'DRP'
      or chargetype.cht_basis = 'ACC')
  and chargetype.cht_itemcode <> 'MIN'


update #invtemp_tbl
   set cmd_code = (select top 1 cmd_code from #invtemp_tbl where ivd_type = 'DRP')
where #invtemp_tbl.cht_basis = 'ACC'
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template128] TO [public]
GO
