...
/*
  This proc is used recursively to parse a long string with intervening CR/LF entries into 
  individual lines by returning the next line (up to the next CR/LF or end of string) and 
...
CREATE PROC [dbo].[parse_multiline] 
	@multilinetext varchar(250) output, 
	@singlelinetext varchar(250) output
AS
DECLARE @pos int
	
	SELECT @pos = isnull(charindex(char(13),@multilinetext),0)
	...
	IF LEN(@multilinetext) > 0
		...
		return 0
GO
GRANT EXECUTE ON  [dbo].[parse_multiline] TO [public]
GO
