...
CREATE PROCEDURE [dbo].[INVOICE_TEMPLATE76_ROLLIN](@invoice_nbr  	int,@copies int)
AS
...
8/14/07 DPETE PTS 38905 rollinto LH not working when MIN charge is applied
...
SELECT ivh.ivh_invoicenumber, 
 ...
 WHERE (IsNull(ivd.cht_rollintolh,0) = 0) AND 
...
-- PTS 32482 -- BL (start)
--								AND chargetype.cht_basisunit in ('VOL', 'FLT')
								AND chargetype.cht_basisunit in ('VOL', 'FLT', 'WGT')
-- PTS 32482 -- BL (end)
...
IF (SELECT COUNT(*) FROM #invtemp_tbl2) > 0	
	...
	SET #invtemp_tbl.ivd_rate = #invtemp_tbl.ivd_rate + #invtemp_tbl2.rate,
        --DPH PTS 27671 - Do not want rate displayed on Invoice if rolling in other charges (PTS 29883 reversed this change)
	--SET #invtemp_tbl.ivd_rate = 0, 
...
-- PTS 32482 -- BL (start)
--		AND chargetype.cht_basisunit in ('VOL', 'FLT')
		--38905 AND chargetype.cht_basisunit in ('VOL', 'FLT', 'WGT')
        AND chargetype.cht_basisunit in ('VOL', 'FLT', 'WGT','MIN')
-- PTS 32482 -- BL (end)
        --38905  AND	chargetype.cht_primary = 'Y'
		AND	((chargetype.cht_primary = 'Y' and #invtemp_tbl.ivd_charge <> 0)or #invtemp_tbl.cht_itemcode = 'MIN')
ELSE
	begin
...
SET shipper_name = cmp.cmp_name, 
       ...
       -- 38905 shipper_nmctst = substring(cmp.cty_nmstct,1, (charindex('/', cmp.cty_nmstct))) + ' ' + cmp.cmp_zip 
...
SET consignee_name = cmp.cmp_name, 
	...
	consignee_addr2 = Case ivh_hideconsignaddr when 'Y' 
       ...
       -- 38905 consignee_nmctst = substring(cmp.cty_nmstct,1, (charindex('/', cmp.cty_nmstct))) + ' ' + cmp.cmp_zip 
...
update #invtemp_tbl
-- 38905 set 	stop_nmctst = substring(city.cty_nmstct,1, (charindex('/', city.cty_nmstct)))+ ' ' +city.cty_zip 
...
GO
GRANT EXECUTE ON  [dbo].[INVOICE_TEMPLATE76_ROLLIN] TO [public]
GO
