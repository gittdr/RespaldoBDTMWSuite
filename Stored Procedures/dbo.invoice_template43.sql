...
create procedure [dbo].[invoice_template43]
		(@invoice_nbr	int
		,@copies  		int)  
as  
...
-----------	---------------	-------	-------	---------------------------------------------------
05/05/2003	Vern Jewett		18082	(none)	Original, copied from invoice_template2.  
...
declare @temp_name   		varchar(30)
		...
		,@li_ord_hdrnumber	int
		,@li_ref_seq		int
		,@ls_ref_number		varchar(100)
...
select	 invoiceheader.ivh_invoicenumber,     
		 ...
		 case invoiceheader.ivh_currency
			when 'Z-US$' then
				'US'
				 ...
				 convert(varchar(8), left(ltrim(rtrim(isnull(invoiceheader.ivh_currency, ''))), 2)) 
	 	 	end ivh_currency,
         ...
         convert(varchar(200), invoiceheader.ivh_ref_number) ivh_ref_number,     
...
--Display up to 2 reference #'s for the invoice, 1st reference..
select	@li_ord_hdrnumber = ord_hdrnumber
  from	invoiceheader
  where	ivh_hdrnumber = @invoice_nbr

select	@li_ref_seq = min(ref_sequence)
  from	referencenumber
  where	ref_table = 'orderheader'
	and	ref_tablekey = @li_ord_hdrnumber

select	@ls_ref_number = isnull(ref_type, '') + ' ' + isnull(ref_number, '')
  from	referencenumber
  where	ref_table = 'orderheader'
	and	ref_tablekey = @li_ord_hdrnumber
	and	ref_sequence = @li_ref_seq
...
select	@ls_ref_number = @ls_ref_number + ', ' + isnull(ref_type, '') + ' ' + 
							isnull(ref_number, '')
  from	referencenumber
  where	ref_table = 'orderheader'
	and	ref_tablekey = @li_ord_hdrnumber
...
--Clean up..
if @ls_ref_number is null
	select @ls_ref_number = ''
select	@ls_ref_number = ltrim(rtrim(@ls_ref_number))

if @ls_ref_number <> ','
		and @ls_ref_number <> ''
	update	#invtemp_tbl
	  set	ivh_ref_number = @ls_ref_number


--If the invoiceheader doesn't have a recognized currency, go to the Bill-To Company..
update	#invtemp_tbl
  set	ivh_currency = 
			case c.cmp_currency
				when 'Z-US$' then
					...
					left(ltrim(rtrim(isnull(c.cmp_currency, ''))), 2)
			 	end
  from	#invtemp_tbl it
		,company c
  where	it.ivh_currency <> 'US'
	and	it.ivh_currency <> 'CA'
	and	it.ivh_currency <> 'CN'
	and	c.cmp_id = it.ivh_billto

update	#invtemp_tbl
  set	ivh_currency = 'CA'
  where	ivh_currency = 'CN'

--If neither Canadian nor US, display it as 'UNK'..
update	#invtemp_tbl
  set	ivh_currency = 'UNK'
  where	ivh_currency not in ('CA', 'US')

--Return in report-friendly format..
update	#invtemp_tbl
  set	ivh_currency = 'Canadian'
  where	ivh_currency = 'CA'
...
return @ret_value  
GO
GRANT EXECUTE ON  [dbo].[invoice_template43] TO [public]
GO
