...
CREATE PROC [dbo].[d_masterbill10_sp] (@reprintflag varchar(10),@mbnumber int, @billto varchar(8),@shipper varchar(8),@consignee varchar(8), 
 ...
 *
 * REVISION HISTORY:
-- dpete pts 6691 change ivd_count and ivd_volume on temp tableto float
-- converted from d_mb_format03 JTEUB PTS 8801/8812
-- jteub pts 	8/25/00
-- dpete 10/10/01 clean up where clause , eliminate unused joins and joins to retrieve unused data
-- This proc assumed that there is volume or quantity recorded (removed).
-- dpete 12/18/01 added billdate to arguments to match format 12 (neither use the date) 
 * 10/30/2007.01 ? PTS40029 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 2/18/09 DPETE PTS 45694 bill date is nto printing what is on the print window for RTP masteerbills
 **/
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	ivd.ivd_sequence, 
        ivd.ivd_refnum 
    FROM invoiceheader left outer join orderheader oh on invoiceheader.ord_hdrnumber = oh.ord_hdrnumber, --pts40029 outer join conversion
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	@billdate, --ivh_billdate      billdate,
	'',
	ISNULL(cmp1.cmp_mailto_name,'') cmp_mailto_name,
	IsNull(ivd.ivd_wgt, 0),
	IsNull(ivd.ivd_wgtunit, ''),
	...
	company cmp1,   -- never used city cty1, city cty2,
	 invoicedetail ivd,   chargetype cht   -- commodity cmd, stops
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill10_sp] TO [public]
GO
