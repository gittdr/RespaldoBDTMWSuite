...
-- creates procedure
CREATE PROCEDURE [dbo].[d_available_trips_ltsl_sp]
...
Declare @ttbl1 Table (
	...
	ord_totalweight			decimal(12,2)	NULL,
	...
	lgh_act_weight			decimal(12,4)	NULL,
	lgh_est_weight			decimal(12,4)	NULL,
	lgh_tot_weight			decimal(12,4)	NULL,
	...
	origin_cmp_lat			decimal(14,6)	NULL, --PTS92864
...
--Remove spaces between parameter values separated by commas
SELECT @p_revType1 = REPLACE(@p_revType1,' ','')
SELECT @p_revType2 = REPLACE(@p_revType2,' ','') 
SELECT @p_revType3 = REPLACE(@p_revType3,' ','') 
SELECT @p_revType4 = REPLACE(@p_revType4,' ','') 
SELECT @p_trltype1 = REPLACE(@p_trltype1,' ','') 
SELECT @p_bookedRevtype = REPLACE(@p_bookedRevtype,' ','') 
SELECT @p_bookedBy = REPLACE(@p_bookedBy,' ','')
SELECT @p_billTo = REPLACE(@p_billTo,' ','') 
SELECT @p_route = REPLACE(@p_route,' ','') 
SELECT @p_teamLeader = REPLACE(@p_teamLeader,' ','') 
SELECT @p_company = REPLACE(@p_company,' ','') 
SELECT @p_oCmpIds = REPLACE(@p_oCmpIds,' ','') 
SELECT @p_oReg1 = REPLACE(@p_oReg1,' ','') 
SELECT @p_oReg2 = REPLACE(@p_oReg2,' ','') 
SELECT @p_oReg3 = REPLACE(@p_oReg3,' ','') 
SELECT @p_oReg4 = REPLACE(@p_oReg4,' ','') 
SELECT @p_dCmpIDs = REPLACE(@p_dCmpIDs,' ','') 
SELECT @p_dReg1 = REPLACE(@p_dReg1,' ','') 
SELECT @p_dReg2 = REPLACE(@p_dReg2,' ','') 
SELECT @p_dReg3 = REPLACE(@p_dReg3,' ','') 
SELECT @p_dReg4 = REPLACE(@p_dReg4,' ','') 
SELECT @p_tmStatus = REPLACE(@p_tmStatus,' ','') 
SELECT @p_lghType1 = REPLACE(@p_lghType1,' ','') 
SELECT @p_lghType2 = REPLACE(@p_lghType2,' ','') 
SELECT @p_HzdCmdClasses = REPLACE(@p_HzdCmdClasses,' ','') 
SELECT @p_orderedby = REPLACE(@p_orderedby,' ','') 
SELECT @p_status = REPLACE(@p_status,' ','')
SELECT @p_oStates = REPLACE(@p_oStates,' ','')
SELECT @p_dStates = REPLACE(@p_dStates,' ','')


-- wrap all the following variable with ,..., in order to make sure the CHARINDEX function returns an exact match
SELECT @p_revType1 = ',' + ISNULL(@p_revType1,'') + ',' 
SELECT @p_revType2 = ',' + ISNULL(@p_revType2,'')  + ',' 
SELECT @p_revType3 = ',' + ISNULL(@p_revType3,'')  + ',' 
SELECT @p_revType4 = ',' + ISNULL(@p_revType4,'')  + ',' 
SELECT @p_trltype1 = ',' + ISNULL(@p_trltype1,'')  + ',' 
SELECT @p_bookedRevtype = ',' + ISNULL(@p_bookedRevtype,'')  + ',' 
SELECT @p_bookedBy = ',' + ISNULL(@p_bookedBy,'') + ',' 
SELECT @p_billTo = ',' + ISNULL(@p_billTo,'')  + ',' 
SELECT @p_route = ',' + ISNULL(@p_route,'')  + ',' 
SELECT @p_teamLeader = ',' + ISNULL(@p_teamLeader,'')  + ',' 
SELECT @p_company = ',' + ISNULL(@p_company,'')  + ',' 
SELECT @p_oCmpIds = ',' + ISNULL(@p_oCmpIds,'')  + ',' 
SELECT @p_oReg1 = ',' + ISNULL(@p_oReg1,'')  + ',' 
SELECT @p_oReg2 = ',' + ISNULL(@p_oReg2,'')  + ',' 
SELECT @p_oReg3 = ',' + ISNULL(@p_oReg3,'')  + ',' 
SELECT @p_oReg4 = ',' + ISNULL(@p_oReg4,'')  + ',' 
SELECT @p_dCmpIDs = ',' + ISNULL(@p_dCmpIDs,'')  + ',' 
SELECT @p_dReg1 = ',' + ISNULL(@p_dReg1,'')  + ',' 
SELECT @p_dReg2 = ',' + ISNULL(@p_dReg2,'')  + ',' 
SELECT @p_dReg3 = ',' + ISNULL(@p_dReg3,'')  + ',' 
SELECT @p_dReg4 = ',' + ISNULL(@p_dReg4,'')  + ',' 
SELECT @p_tmStatus = ',' + ISNULL(@p_tmStatus,'')  + ',' 
SELECT @p_lghType1 = ',' + ISNULL(@p_lghType1,'')  + ',' 
SELECT @p_lghType2 = ',' + ISNULL(@p_lghType2,'')  + ',' 
SELECT @p_HzdCmdClasses = ',' + ISNULL(@p_HzdCmdClasses,'')  + ',' 
SELECT @p_orderedby = ',' + ISNULL(@p_orderedby,'') + ','  
SELECT @p_status = ',' + ISNULL(@p_status,'') + ',' 
SELECT @p_oStates = ',' + ISNULL(@p_oStates,'') + ',' 
SELECT @p_dStates = ',' + ISNULL(@p_dStates,'') + ',' 


DECLARE @pseudolegheaders TABLE
	(leghdr int NOT NULL, ordhdr int NOT NULL)  --FMM PTS 35327 changes leghdr from NOT NULL to NULL
...
SET @sql = @sql + N'AND (CHARINDEX('','' + mpp1.mpp_teamleader + '','',''' + @p_teamleader + ''') > 0  OR oh.ord_driver1 = ''UNKNOWN'') ' 
	...
	If @v_displaydecisionsonly = 'Y' or @v_displaypending990s = 'Y'
	BEGIN
		set @sql = @sql + N'AND ('

		If @v_displaydecisionsonly = 'Y'
			BEGIN
			set @sql = @sql + N'ISNULL(edi.esc_useractionrequired,''N'') = ''Y'' ' 
...
SELECT lgh_number = p.leghdr, 
       ...
       ord_totalweight = convert(decimal(12, 4), oh.ord_totalweight), 
       ...
       act_weight = convert(decimal(12, 4), lh.lgh_act_weight),
       est_weight = convert(decimal(12, 4), lh.lgh_est_weight),
       tot_weight = convert(decimal(12, 4), oh.ord_totalweight), 
       ...
       fgt_length = (SELECT convert(decimal(8, 2), MAX(fgt_length)) FROM freightdetail  with (nolock), stops  with (nolock) WHERE freightdetail.stp_number = stops.stp_number AND stops.ord_hdrnumber = p.ordhdr), 
       fgt_width = (SELECT convert(decimal(8, 2), MAX(fgt_width)) FROM freightdetail  with (nolock), stops  with (nolock) WHERE freightdetail.stp_number = stops.stp_number AND stops.ord_hdrnumber = p.ordhdr),
       fgt_height = (SELECT convert(decimal(8, 2), MAX(fgt_height)) FROM freightdetail  with (nolock), stops  with (nolock) WHERE freightdetail.stp_number = stops.stp_number AND stops.ord_hdrnumber = p.ordhdr),
       o_zip = ocomp.cmp_zip, 
       d_zip = dcomp.cmp_zip, 
       ...
       convert(decimal(14, 6), ROUND(ISNULL(ocomp.cmp_latseconds, 0.000)/3600.000, 6)) o_cmp_lat, 
       convert(decimal(14, 6), ROUND(ISNULL(ocomp.cmp_longseconds, 0.000)/3600.000, 6)) o_cmp_long, 
       o_cty_lat = convert(decimal(14, 6), ROUND(ISNULL(octy.cty_latitude, 0.000), 6)), 
       o_cty_long = convert(decimal(14, 6), ROUND(ISNULL(octy.cty_longitude, 0.000), 6)), 
       route = oh.ord_route, 
...
Select @ud_column1 = Upper(ISNULL(gi_string1,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_COLUMNS'
Select @ud_column2 = Upper(ISNULL(gi_string2,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_COLUMNS'
Select @ud_column3 = Upper(ISNULL(gi_string3,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_COLUMNS'
Select @ud_column4 = Upper(ISNULL(gi_string4,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_COLUMNS'

IF @ud_column1 = 'Y'
BEGIN
		Select @procname = Upper(ISNULL(gi_string1,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_FUNCTIONS'
...
IF @ud_column2 = 'Y'
BEGIN
		Select @procname = Upper(ISNULL(gi_string2,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_FUNCTIONS'
		If @procname not in ('','N')
		BEGIN 
...
IF @ud_column3 = 'Y'
BEGIN
		Select @procname = Upper(ISNULL(gi_string3,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_FUNCTIONS'
		If @procname not in ('','N')
		BEGIN 
...
IF @ud_column4 = 'Y'
BEGIN
		Select @procname = Upper(ISNULL(gi_string4,'N')) from generalinfo where gi_name = 'UD_STOP_LEG_FUNCTIONS'
		If @procname not in ('','N')
		BEGIN 
...
GO
GRANT EXECUTE ON  [dbo].[d_available_trips_ltsl_sp] TO [public]
GO
