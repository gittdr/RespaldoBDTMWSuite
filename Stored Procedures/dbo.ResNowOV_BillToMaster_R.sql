...
CREATE PROCEDURE [dbo].[ResNowOV_BillToMaster_R]
...
*/
	...
	BEGIN
		...
		ORDER BY SUM(IsNull(dbo.fnc_TMWRN_Revenue('Order',Null,Null,Null,ord_hdrnumber,Null,Null,Null,Null,'','','','','','',''),0)) DESC
		END
		ELSE
		BEGIN
		SELECT c1.cmp_mastercompany AS [Master ID],
	...
	BEGIN
		...
		SELECT 	ord_hdrnumber,
				...
				,Rev = dbo.fnc_TMWRN_Revenue('Order',Null,Null,Null,ord_hdrnumber,Null,Null,Null,Null,'','','','','','','')
			INTO #temp1
	...
	END
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_BillToMaster_R] TO [public]
GO
