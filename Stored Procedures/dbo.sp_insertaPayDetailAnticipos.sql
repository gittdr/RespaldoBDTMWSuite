...
CREATE PROCEDURE [dbo].[sp_insertaPayDetailAnticipos] 
(
@lgh_number int,
@asgn_type varchar(6),
@asgn_id varchar(13),
...
select @pyt_itemcode = (select gi_string2 from generalinfo where gi_name = 'BankExportConfigSettings')

--Obtener mensaje 
select @mensaje = (select 'Anticipo para segmento de viaje de ' + lgh_startcty_nmstct + ' a ' + lgh_endcty_nmstct from legheader where lgh_number =@lgh_number) 

select @pyt_rateunit = (SELECT  pyt_rateunit FROM paytype WHERE pyt_itemcode = (select gi_string2 from generalinfo where gi_name = 'BankExportConfigSettings'))

select @pyt_unit = (SELECT  pyt_unit FROM paytype WHERE pyt_itemcode = (select gi_string2 from generalinfo where gi_name = 'BankExportConfigSettings'))

select @pyt_minus = (SELECT  pyt_pretax FROM paytype WHERE pyt_itemcode = (select gi_string2 from generalinfo where gi_name = 'BankExportConfigSettings'))
...
--Obtener la orden
select @orden = (select max(ord_hdrnumber) from stops where lgh_number =@lgh_number)

--Obtener el movimiento
select @movimiento = (select mov_number from legheader where lgh_number =@lgh_number )
