...
/****** Object:  Stored Procedure dbo.d_settlement_sheet_summary    Script Date: 6/24/98 10:17:22 AM ******/
CREATE PROC [dbo].[d_settlement_sheet_summary](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary
 *
 * TYPE:
 * Stored Procedure
 *
...
UPDATE #temp_pd
   SET crd_cardnumber = (SELECT crd_cardnumber 
                           FROM payheader 
                          WHERE pyh_pyhnumber = #temp_pd.pyh_number)
...
--jyang pts13004 check @PeriodforYTD to decide how the ytd be calculated
...
UPDATE #ytdbal
   SET ytdcomp = ytdcomp + (SELECT SUM(ROUND(ISNULL(tp.pyd_amount, 0), 2))
                              FROM #temp_pd tp
				   ...
				   AND pyh_number = 0)
UPDATE #ytdbal
   SET ytddeduct = ytddeduct + (SELECT SUM(ROUND(ISNULL(tp.pyd_amount, 0), 2)) 
                                  FROM #temp_pd tp
                                 WHERE tp.asgn_id = #ytdbal.asgn_id
                                       AND tp.asgn_type = #ytdbal.asgn_type
                                       AND tp.pyd_pretax = 'N'
				       ...
				       AND pyh_number = 0)
UPDATE #ytdbal
   SET ytdreimbrs = ytdreimbrs + (SELECT SUM(ROUND(ISNULL(tp.pyd_amount, 0), 2))
                                    FROM #temp_pd tp
                                   WHERE tp.asgn_id = #ytdbal.asgn_id
                                         AND tp.asgn_type = #ytdbal.asgn_type
                                         AND tp.pyd_pretax = 'N'
...
FROM #temp_pd
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary] TO [public]
GO
