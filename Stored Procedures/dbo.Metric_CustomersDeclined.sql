...
Create PROCEDURE [dbo].[Metric_CustomersDeclined] 
	(
		--Standard Parameters
		...
		@UseMetricParms INT, 
		@ShowDetail INT,
		@MetricCode VARCHAR(255)='CustomersDeclined',
		...
		@PercentBelowAverageToFlagAsDeclined DECIMAL(2,2) = .2 ,
		@DaysInPeriod INT = 7,
...
/* --debug
		...
		declare @PercentBelowAverageToFlagAsDeclined DECIMAL(2,2) 
		set @PercentBelowAverageToFlagAsDeclined = .2 
		declare @DaysInPeriod INT 
		set @DaysInPeriod = 7
		declare @NumberPeriodsInHistory INT 
		set @NumberPeriodsInHistory = 1
...
*/
	...
	/*	<METRIC-INSERT-SQL>
		...
		EXEC MetricInitializeItem
			...
			@nPlusDeltaIsGood = 0,
			@nCumulative = 0,
			@sCaption = 'Customers Declined',
			@sCaptionFull = 'Number of customers whos business declined by a percentage below a period average.',
			@sProcedureName = 'Metric_CustomersDeclined',
			@sCachedDetailYN = '',
	...
	*/
	...
	INSERT INTO #CustomerSum 
		SELECT 	ivh_billto, 
			...
			ivh_deliverydate, 
			ord_hdrnumber, /* PTS 38007 was set to 0 */
			0,
			'I',
			SUM(ivh_totalmiles)
		...
		WHERE ivh_deliverydate >= @FindAverageDateStart AND ivh_deliverydate < @DateEnd
			...
			AND ord_hdrnumber > 0
		group by ivh_billto, ivh_deliverydate, ord_hdrnumber /* PTS 38007 needed to add ord_hdrnumber to GroupBy */
...
--drop table #temp1
	...
	CREATE TABLE #Results	(	
				...
				AvgOrders INT,
				TotalOrders INT,
				ActivityPercLessThanNorm INT,
				ActivityPercLessThanNormOrder INT,
	...
	SELECT BillTo, CONVERT(INT, AvgMiles), CONVERT(INT, TotalMiles), 
		...
		ActivityPercLessThanNorm = (SELECT COUNT(*)
			FROM #CustomerSum2 
			WHERE #temp1.BillTo = #CustomerSum2.BillTo
			AND	PeriodNumber < @RequirePeriodTrend 
			AND	#CustomerSum2.TotalMiles < (#temp1.AvgMiles * (1 - @PercentBelowAverageToFlagAsDeclined)) 
				),
		ActivityPercLessThanNormOrder = (SELECT COUNT(*)
			FROM #CustomerSum2 
			WHERE #temp1.BillTo = #CustomerSum2.BillTo
			AND	PeriodNumber < @RequirePeriodTrend 
			AND	#CustomerSum2.CountOfOrders < (#temp1.AvgOrders * (1 - @PercentBelowAverageToFlagAsDeclined)) 
				),
	...
	IF @Mode = 'Miles'
		SELECT @ThisCount = (SELECT COUNT(*) FROM #Results WHERE ActivityPercLessThanNorm = @RequirePeriodTrend)
	ELSE
		SELECT @ThisCount = (SELECT COUNT(*) FROM #Results WHERE ActivityPercLessThanNormOrder = @RequirePeriodTrend)
	...
	BEGIN
		IF @Mode = 'Miles'
			...
			Where ActivityPercLessThanNorm = @RequirePeriodTrend
		ELSE
			SELECT 	cmp_name , 
				AvgOrders ,
				TotalOrders ,
			...
			FROM #Results  join company on cmp_id = billto
			Where ActivityPercLessThanNormOrder = @RequirePeriodTrend
	END
...
GO
GRANT EXECUTE ON  [dbo].[Metric_CustomersDeclined] TO [public]
GO
