...
CREATE PROC [dbo].[d_stlmnt_det_report_general103](
 ...
 *
 * REVISION HISTORY:
 * 4-20-2009 PTS 46949 JSWINDELL - proc created.  (new proc from original: d_settlement_sheet_summary_new)
 * 8-11-2009 PTS 48614 JSwindell - Client Wants Segment END DATE instead of arrival date. 
 *
 **/
SET NOCOUNT ON
-- jyang pts13004
...
-- PTS 29303 -- BL (end)	
	...
	--ord_number		char(12) null,				-- PTS 48614  truncation errors
	ord_number			varchar(20) null,
	ref_number		varchar(30) null,
	stp_arrivaldate		datetime null,
	--shipper_name		varchar(30) null,			-- PTS 48614  truncation errors
	shipper_name		varchar(100) null,
	shipper_city		varchar(18) null,
	shipper_state		char(2) null,
	--consignee_name		varchar(30) null,		-- PTS 48614  truncation errors
	...
	--otherid			varchar(8) null,		-- PTS 48614  truncation errors
	...
	lgh_enddate			datetime null,			--PTS48614	
	ls_expense_ordnumber   varchar(20) null		--PTS48614	addtl changes
 )
...
SELECT pd.pyd_number,
	...
	null, -- pyt_basis
	null,	-- lgh_enddate					-- pts48614
	null	-- ls_expense_ordnumber   	    --PTS48614	addtl changes
...
-- pts 48614 <<start>>
UPDATE #temp_pd
   SET lgh_enddate = (SELECT lgh_enddate 
                          FROM legheader lh
                         WHERE lh.lgh_number = #temp_pd.lgh_number)
-- pts 48614 <<end>>
...
UPDATE #temp_pd
   SET ord_number = LTRIM(RTRIM(ord_number)) + '/S'			-- PTS 48614
...
-- PTS 46949 COMMENT OUT SELECTION BY MIN... USERS WANT MAX.
--UPDATE #temp_pd
...
-- PTS 46949 USERS WANT MAX.
UPDATE #temp_pd
   SET stp_mfh_sequence = (SELECT MAX(stp_mfh_sequence)
                             FROM stops st
                            WHERE st.mov_number = #temp_pd.mov_number 
                                  AND stp_event IN ('DRL', 'LUL', 'DUL', 'PUL')) 
...
--PTS48614	addtl changes <<start>>
update #temp_pd
set ls_expense_ordnumber = (convert(varchar(20), ord_number))
where itemsection <> 0

update #temp_pd
set ls_expense_ordnumber = (convert(varchar(20), ord_hdrnumber))
where ( ls_expense_ordnumber = '' OR ls_expense_ordnumber IS NULL ) 
AND itemsection <> 0
...
SELECT pyd_number, 
	...
	terminal,			--35
	type1,
	round(tp.pyh_totalcomp, 2) as 'ytd_comp',
	round(tp.pyh_totaldeduct, 2) as 'ytd_deduct',
	round(tp.pyh_totalreimbrs, 2) as 'ytd_reimbrs',
	ph.crd_cardnumber 'crd_cardnumber',		--40
	lgh_startdate,
	std_balance,
	itemsection,
	ord_startdate,
	LTRIM(RTRIM(CAST (ord_number as char(12)))) 'ord_number',	--45   -- PTS 48614
	ref_number,
	stp_arrivaldate,
	cast(shipper_name as varchar(30)) 'shipper_name',		-- PTS 48614
	shipper_city,
	shipper_state,				--50
	cast(consignee_name as varchar(30)) 'consignee_name',	-- PTS 48614
	consignee_city,
	consignee_state,
	cmd_name,
	pyd_billedweight,			--55 -- as ivh_quantity??? 
	...
	userlabelname,					--60
	label_name,
	cast(otherid as varchar(8)) 'otherid',					-- PTS 48614  
	pyt_fee1,
	pyt_fee2,
	ph.pyh_issuedate,			-- PTS 46949	--65	--(was already part of temp table - send it to dwo.)
	---convert(datetime, (convert(varchar(8), lgh_enddate, 112)), 101	 ) 'lgh_enddate'  -- 48614 NEW REQUEST (sorting)
	lgh_enddate,
	LTRIM(RTRIM(CAST (ls_expense_ordnumber as char(12)))) 'ls_expense_ordnumber'

  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber  --PTS48614	addtl changes



GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general103] TO [public]
GO
