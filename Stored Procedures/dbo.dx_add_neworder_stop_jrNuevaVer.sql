...
CREATE     PROC [dbo].[dx_add_neworder_stop_jrNuevaVer]
	@validate char(1),
		@move_number int, 
	@stp_sequence int,
	@event varchar(6),   
...
@cmp_name varchar(30),
@stp_state varchar(2), 
@stp_address1 varchar(40), 
@stp_address2 varchar(40),
@stp_zip varchar(9), 
@stplghmiles int, 
@stpordmiles int, 
@ord_hdrnumber int, 
@cmd_gravity float, @retcode int,
@from_stop varchar(8), 
@to_stop varchar(8),
 @stp_loadstatus varchar(3),
@back_seq int,
@midnight datetime, 
@datediff int		--20081209 AR
...
SELECT @dolly = ISNULL(@dolly,'UNKNOWN')
SELECT @carrier = ISNULL(@carrier,'UNKNOWN')
IF @carrier = '' 
...
--jr	declare @Driver VARCHAR(8), @Driver2 VARCHAR(8), @Tractor varchar(8), @TrailerID varchar(13), @PupID varchar(13), @Carrier varchar(8), @status varchar(3)
	declare @Driver VARCHAR(8), @Driver2 VARCHAR(8), @Tractor varchar(8), @TrailerID varchar(13), @PupID varchar(13),  @status varchar(3)
...
INSERT INTO event 
	...
	--7.5 validar que tipo de lgh_type1 poner de acuerdo al numero de evt_trailer1 y 2
	evt_lghtype1,evt_dolly,
...
values (@Driver, @Driver2, @Tractor,
	...
	@status , 1, @status,
	CASE WHEN @remolque1 <> 'UNKNOWN' AND @remolque2 <>'UNKNOWN' THEN 'FULL' ELSE 'SEN' END,
	@dolly,
	@@new_move_number)
