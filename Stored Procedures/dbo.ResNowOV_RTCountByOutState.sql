...
CREATE PROCEDURE [dbo].[ResNowOV_RTCountByOutState]
(
		...
		BEGIN
			...
			INSERT @RNTrial_Cache
				SELECT 	0, -- field not critical, record id for debugging purposes
						...
						[ItemID] = State,
						/* COUNT IS HOW THE VALUE OF THE SLICE IS MEASURED. HERE IT IS Sum of Total Trip count */
						[Count] = TotalTrips
...
--				WHERE 	RTEndDate BETWEEN @DateStart AND @DateEnd
		...
		END
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_RTCountByOutState] TO [public]
GO
