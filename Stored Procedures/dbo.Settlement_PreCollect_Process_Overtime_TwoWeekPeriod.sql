...
GO
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| BEGIN CREATE procedure for 'Settlement_PreCollect_Process_Overtime_TwoWeekPeriod' for .NET Overtime Pay calculations                                        	                 |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[Settlement_PreCollect_Process_Overtime_TwoWeekPeriod](@pl_pyhnumber INT, @ps_asgn_type VARCHAR(6), @ps_asgn_id VARCHAR(13), @pdt_payperiod DATETIME, @psd_id INT)
AS
...
/*	Determine the last date on which this driver was paid. This will be used later when
	...
	IF this is the first pay period ever, start counting FROM 2 weeks before. */
...
SELECT @last_week = DATEADD(dd, -14, @pdt_payperiod)

IF @ldt_lastpayperiod <> @last_week
	SELECT @ldt_lastpayperiod = @last_week

SELECT @ldt_lastpayperiod_7 = DATEADD(dd, -7, @pdt_payperiod)

/*	Get weekly time limits */
SELECT 	@ldec_period_max = ISNULL(mpp_periodguarenteedhours, 40.0),
		@ls_payto = mpp_payto
FROM 	manpowerprofile 
WHERE 	mpp_id = @ps_asgn_id

/*	IF asset record didn't have a weekly OT limit, generate an error message. */
...
BEGIN	
	...
	/*	Get the total quantity of existing time-based paydetails for the 1st week of this payperiod
	AND store it in the variable @ldec_period_new. 
	NOTE: this section of the code assumes a 2 week pay period with OT calculated for each week separately */
	SELECT @ii = 1
	WHILE @ii < 3
	BEGIN
		IF @ii = 1
		BEGIN
			SELECT @ldt_from = @ldt_lastpayperiod
			SELECT @ldt_to = @ldt_lastpayperiod_7
		END
		ELSE
		BEGIN
			SELECT @ldt_from = @ldt_lastpayperiod_7
			SELECT @ldt_to = @pdt_payperiod
		END

		/*	Get the total quantity of existing time-based non OT paydetails for this payperiod	*/
		...
		WHERE 	a.lgh_number = b.lgh_number 
				...
				AND b.lgh_enddate between @ldt_from AND @ldt_to
		...
		WHERE 	a.asgn_type = @ps_asgn_type 
				...
				AND a.pyd_transdate between @ldt_from AND @ldt_to
...
GO
GRANT EXECUTE ON  [dbo].[Settlement_PreCollect_Process_Overtime_TwoWeekPeriod] TO [public]
GO
