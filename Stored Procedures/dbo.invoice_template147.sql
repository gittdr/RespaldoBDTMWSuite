...
GO
  ...
  create procedure [dbo].[invoice_template147](@invoice_nbr  int,@copies  int)  
as  
/* PROCEDURE RETURNS 0 - IF NO DATA WAS FOUND  
 1 - IF SUCCESFULLY EXECUTED  
 @@ERROR - db GLOBAL VARIABLE VALUE IF AN ERROR OCCURS  
  
10/06/2008 - PTS439699 Adapted from invoice format 58 for Trinity Logistics Group, Inc.
*/  
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET   
 ...
 SELECT invoiceheader.ivh_invoicenumber,     
	...
	invoiceheader.ivh_order_cmd_code,
  	commodity2.cmd_name ord_cmd_name
    	into #invtemp_tbl  
    	FROM chargetype  RIGHT OUTER JOIN  invoicedetail  ON  (chargetype.cht_itemcode  = invoicedetail.cht_itemcode and chargetype.cht_basis in ('ACC','SHP')) 
			...
			invoiceheader LEFT OUTER JOIN commodity commodity2 on invoiceheader.ivh_order_cmd_code = commodity2.cmd_code
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template147] TO [public]
GO
