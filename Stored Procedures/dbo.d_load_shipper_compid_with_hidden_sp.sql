...
create proc [dbo].[d_load_shipper_compid_with_hidden_sp] 
	@comp 		varchar(8) 
	,@number	int 
as
/* Revision History:
...
--	if exists(SELECT cmp_name FROM company WHERE cmp_id LIKE @comp + '%')
	if exists(SELECT cmp_name 
				...
				WHERE cmp_id LIKE @comp + '%' 
						AND (cmp_shipper = 'Y' OR cmp_hiddenid = 'Y')
						--PTS 53255 JJF 20101130
						AND	(	(@rowsecurity <> 'Y')
...
set rowcount 0 
GO
GRANT EXECUTE ON  [dbo].[d_load_shipper_compid_with_hidden_sp] TO [public]
GO
