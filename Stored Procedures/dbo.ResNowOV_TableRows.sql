...
CREATE PROCEDURE [dbo].[ResNowOV_TableRows]
(
...
AS 
	...
	BEGIN /* START DETAIL SECTION */ 
		...
		BEGIN
			...
			SELECT name, id, crdate FROM sysobjects t1 (READPAST) WHERE type = 'u' AND NOT EXISTS(SELECT RecNum FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode AND  ItemId = t1.name)
	...
	--		INSERT INTO #t (name, rows, reserved, data, index_size, unused)	EXEC sp_spaceused @name
		...
		SELECT 	t2.id, -- field not critical, record id for debugging purposes
				...
				t1.rows  /* COUNT IS HOW THE VALUE OF THE SLICE IS MEASURED. HERE IT IS a count of SPACE. */
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_TableRows] TO [public]
GO
