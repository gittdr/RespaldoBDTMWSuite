...
DPETE PTS 16764 correct YTD amounts using d_settlment_sheet_summary fix as a model
09/01/2008 PTS 43639 JSwindell - fix fee1 and fee2 & truncation error.
09/15/2008 PTS 43639 JSwindell - clarification from Glenn B. Change to this proc.

*/
/****** Object:  Stored Procedure dbo.d_settlement_sheet_summary_02n   Script Date: 3/2/99 ******/
CREATE PROC [dbo].[d_settlement_sheet_summary_02n](
...
--PTS 43639 set nocount on for performance improvement
set nocount on 
...
CREATE TABLE #temp_pd(
	...
	--ord_number		char(10) null,	    -- PTS 43639 eliminate truncation error
	ord_number			varchar(20) null,	-- PTS 43639 eliminate truncation error	
...
-- PTS 43639 eliminate truncation error	
...
-- PTS 43639 use paydetail **ONLY** data for fee1 and fee2 9-15-2008
...
-- PTS 43639 use paydetail data for fee1 and fee2 << 1 of 2 >>
UPDATE #temp_pd
SET pyt_fee1 = pd.pyt_fee1   
FROM #temp_pd tp, paydetail pd
WHERE tp.pyd_number = pd.pyd_number
--and pd.pyt_fee1 > 0	-- removed: PTS 43639 use paydetail **ONLY** data for fee1 and fee2 9-15-2008

-- PTS 43639 use paydetail data for fee1 and fee2 << 2 of 2 >>
UPDATE #temp_pd
SET pyt_fee2 = pd.pyt_fee2
FROM #temp_pd tp, paydetail pd
WHERE tp.pyd_number = pd.pyd_number
--and pd.pyt_fee2 > 0  -- removed: PTS 43639 use paydetail **ONLY** data for fee1 and fee2 9-15-2008
...
SET stp_mfh_sequence = (SELECT MIN(st.stp_mfh_sequence)
	FROM stops st
	WHERE st.ord_hdrnumber > 0 and tp.ord_hdrnumber > 0 --JD Added this clause to stop joins on zero ord_hdrnumbers 35949
	  AND st.ord_hdrnumber = tp.ord_hdrnumber
...
-- PTS 15251 - DJM - Was not computing Loaded miles correctly.
-- Determine the total stp_lgh_miles for this order
UPDATE #temp_pd
SET order_stp_lgh_miles = (SELECT SUM(ISNULL(st.stp_lgh_mileage,0))
			FROM stops st
			WHERE st.ord_hdrnumber > 0 and tp.ord_hdrnumber > 0 --JD Added this clause to stop joins on zero ord_hdrnumbers 35949
				and st.ord_hdrnumber = tp.ord_hdrnumber
				and st.stp_loadstatus = 'LD'
				and st.lgh_number = tp.lgh_number)
FROM #temp_pd tp
...
SELECT pyd_number, 
	...
	itemsection,
	ord_startdate,	
	--PTS 43639 eliminate truncation error	but still return 10 char to dwo.
	--ord_number,
	substring(ord_number,1,10) as 'ord_number',
	ref_number,
	stp_arrivaldate,
	shipper_name,
	shipper_city,
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_02n] TO [public]
GO
