...
GO
/********
exec sp_pilgrims_cmpfreights_ruta 'FR21-00028-20181206', 'PI15765-QA76000-APY15765'
...
CREATE proc [dbo].[sp_pilgrims_cmpfreights_ruta] ( @ruta varchar(200),@token varchar(254))
...
EstadoDestino,
Operador,
(select sum(isnull(stp_ord_toll_cost,0)) from stops where stops.ord_hdrnumber  = kms.orden) as Casetas,(select cht_itemcode as IdCargoSec,ivd_description as Descripcion,  ivd_unit as Unidad, cast(round(cast(ivd_quantity as float),2) as varchar(20)) as Cantidad, round(ivd_rate,2) as Tarifa,ivd_charge as Monto
...
where inv.ivh_billto =  'PILGRIMS' 
 ...
 and (isnull((select max(ref_number) from referencenumber ref where  ref.ref_table = 'orderheader' and  ref.ref_type = 'SID' and ref.ref_tablekey = ord.ord_hdrnumber),'Fuera SDS'))  = @ruta
