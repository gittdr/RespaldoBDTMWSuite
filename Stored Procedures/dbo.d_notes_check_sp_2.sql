...
CREATE PROCEDURE [dbo].[d_notes_check_sp_2] (	@mode		int,
					@mov_number	int,
					@ord_hdrnumber 	int,
...
/* JG PTS38031 fix performance in "Convert (char (12), TASK.TASK_LINK_ENTITY_SYS_VALUE) =@key" */
...
if @ord_hdrnumber=0 and exists (select * from orderheader 
		  ...
		  SELECT	top 1 @bitmap_id = nir_output_bitmap_id, @processing_order = nir_processing_order
		   ...
		   WHERE	IsNull (n.not_urgent, '') = IsNull (nr.nir_not_urgent, '')
			...
			AND     n.ntb_table = 'TASK'
  			...
  			AND 	TASK.TASK_LINK_ENTITY_SYS_VALUE = convert(int, @ord_number)
			and	getdate() <= IsNull(DATEADD(day, 4, n.not_expires), getdate())
		  order by nr.nir_processing_order
                END
...
/*PTS 24849 CGK 10/6/2004*/
  ...
  END
   ...
   --ILB 15728 03/31/03
 --Only check Third party notes for Ord Entry (Mode 1) 
   IF (@mode = 1  or @mode = 3)
      IF @third_party = ''
	BEGIN
	   SELECT	@urgent_code = MIN(IsNull(not_urgent, 'N'))
   ...
   If @cmd_code = ''
	BEGIN
	   ...
	   WHERE  n.ntb_table = 'commodity' AND
		  getdate() <= IsNull(DATEADD(day, @grace, n.not_expires), getdate()) AND
		  n.nre_tablekey IN (SELECT	cmd_code
				     FROM 	legheader
				     WHERE	mov_number = @mov_number)
...
End
  ...
  RETURN @ord_notes
GO
GRANT EXECUTE ON  [dbo].[d_notes_check_sp_2] TO [public]
GO
