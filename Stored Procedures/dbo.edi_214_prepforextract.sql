...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[edi_214_prepforextract] 
/*
 * 
 * NAME:
...
DECLARE @EDIout   TABLE
 (data_col varchar(200),   
  ...
  e214_source varchar(128) null,
  e214_user varchar(128) null) 
...
INSERT INTO @EDI (data_col, doc_id, trp_id, identity_col, e214_source, e214_user)
SELECT	data_col,
	doc_id,
	trp_id,
	identity_col,
	 	...
	 	SELECT @batch=CONVERT(varchar(6),@batchnbr)  -- for output   
	  	     ...
	  	     SELECT @scac = substring(data_col,4,4) FROM @edi WHERE LEFT(data_col,1) = '1' and trp_id = @nexttrpid
		     	...
		     	-- Insert detail records
		     	    INSERT INTO @Ediout (data_col,doc_id,cmp_id,e214_source,e214_user)
		     	    SELECT 	data_col,doc_id,@v_cmp_id,e214_source,e214_user
		     	    FROM   @EDI
		     	    WHERE   trp_id = @nexttrpid
	   			 ORDER BY  trp_id, doc_id, identity_col
...
--Final Select
SELECT data_col, doc_id, control_number,cmp_id, e214_source, e214_user FROM @EDIout
order by eo_identity_col 	 
     

GO
GRANT EXECUTE ON  [dbo].[edi_214_prepforextract] TO [public]
GO
