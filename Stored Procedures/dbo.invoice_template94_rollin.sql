...
CREATE PROCEDURE [dbo].[invoice_template94_rollin](@invoice_nbr   int,@copies int)  
AS  

/*
 * 
 * NAME:invoice_template94_rollin
 * dbo.
 *
 * TYPE:
 * StoredProcedure
...
SELECT ivh.ivh_invoicenumber,   
 ...
 @v_varchar50 cmp_contact,
  ...
  INTO #invtemp_tbl        
  FROM invoicedetail as ivd JOIN invoiceheader as ivh ON ( ivd.ivh_hdrnumber = ivh.ivh_hdrnumber ) 
...
/*
 FROM invoiceheader ivh, invoicedetail ivd, chargetype cht, commodity cmd   
 WHERE (ivd.ivh_hdrnumber = ivh.ivh_hdrnumber) AND   
       (cht.cht_itemcode = ivd.cht_itemcode) AND  
--JET removed outer join on cht_itemcode, 10/4/98   
...
UPDATE #invtemp_tbl   
 ...
 shipper_addr2 = Case ivh_hideshipperaddr when 'Y'   
   ...
   shipper_country = cmp.cmp_country
...
UPDATE #invtemp_tbl   
 ...
 consignee_addr2 = Case ivh_hideconsignaddr when 'Y'   
   ...
   consignee_country = cmp.cmp_country
  FROM #invtemp_tbl, company cmp   
...
RETURN @v_ret_value 
GO
GRANT EXECUTE ON  [dbo].[invoice_template94_rollin] TO [public]
GO
