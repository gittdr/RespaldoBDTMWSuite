...
CREATE PROC [dbo].[d_masterbill27a_sp] (@reprintflag varchar(10),@mbnumber int, @billto varchar(8),@shipper varchar(8),@consignee varchar(8),     
...
/**
 * DESCRIPTION:
    Created PTS15743 10/7/02 as a copy of d_mastetbill26a_sp with a change to the description  
    to show the ivd_description where it is set and the the cht_description where the ivd_description  
    is missing.  Customer First Fleet uses the tariff item description for linehaul to override  
    the description on the charge type.  That field goes to the ivd_description when the   
    order is rated in the Invoicing application.    
 *
 * PARAMETERS:
 *
 * RETURNS:
...
--DPETE Modified for First Fleet Build Only PTS15742    
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill27a_sp] TO [public]
GO
