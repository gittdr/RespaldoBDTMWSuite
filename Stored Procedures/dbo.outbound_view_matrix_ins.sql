...
CREATE PROCEDURE [dbo].[outbound_view_matrix_ins]
...
SELECT
	...
	Region =  isnull((select rgh_name from regionheader where rgh_id = (select cty_region1 from city c where c.cty_code = a.d_city  )   ),'UNKNOWN'),
    Tractor = a.evt_tractor,
	Operador = replace(replace(a.evt_driver1_id ,'UNKNOWN', cast(ord_hdrnumber as varchar)),' ','BLANCO'),
	Status = 'IN',
