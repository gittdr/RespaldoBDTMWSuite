...
CREATE PROC [dbo].[d_settlement_sheet_summary_30](
...
BEGIN
    IF @lgh_type1 = 'UNK' --20161 JD added check for lgh_type1
	BEGIN
	...
	END
	ELSE
	BEGIN	-- PTS# 20161 05/02/05 JD added lgh_type1 restrictions.Get the driver pay header and detail numbers for held pay		
		IF @drv_yes != 'XXX'			
			INSERT INTO #temp_pay
			SELECT pyd_number,
				pyh_number,
				...
				pyd_status,
				@drv_type1
			FROM paydetail,legheader
			WHERE paydetail.lgh_number = legheader.lgh_number 
				AND legheader.lgh_type1 = @lgh_type1
				AND	asgn_type = 'DRV'
				AND asgn_id = @drv_id
				AND pyh_number = 0 
				AND pyd_status = 'HLD'
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_30] TO [public]
GO
