...
*/
CREATE PROC [dbo].[d_stlmnt_sheet_summary_greatrig](
...
update #temp_pd
set    mpp_name = isnull(mpp_firstname,'') + ' ' + isnull(mpp_middlename,'') + ' ' + isnull(mpp_lastname,'')
from   manpowerprofile
where  mpp_id = (case asgn_type when 'drv' then asgn_id when 'trc' then trc_drv end)

update #temp_pd
set     ivd_qty = (select sum(invoicedetail.ivd_quantity)
from   invoicedetail
where  invoicedetail.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	ivd_type = 'DRP' and
	invoicedetail.cht_itemcode = 'PCS')

update #temp_pd
set    ivd_rate = (select max(invoicedetail.ivd_rate)
from   invoicedetail
where  invoicedetail.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	ivd_type = 'DRP' and
	invoicedetail.cht_itemcode = 'PCS')
...
update #temp_pd
set    ivd_charge = (select sum(invoicedetail.ivd_charge)
from   invoicedetail
where  invoicedetail.ord_hdrnumber = #temp_pd.ord_hdrnumber and
	ivd_type = 'DRP' and
	invoicedetail.cht_itemcode = 'PCS')
...
/*update #temp_pd
set    ivd_qty = (select sum(fgt_quantity) 
		  from  freightdetail,stops
		  where #temp_pd.pyt_itemcode = 'TSR' and
			#temp_pd.ord_hdrnumber = stops.ord_hdrnumber and
			stp_type = 'DRP' and 
			stops.stp_number = freightdetail.stp_number and
			freightdetail.cmd_code <> 'UNKNOWN' and 
			freightdetail.fgt_unit = 'PCS' and
			freightdetail.cht_basisunit = 'CNT')

update #temp_pd
set    ivd_rate = (select max(fgt_rate)
		   from  freightdetail,stops
		   where #temp_pd.pyt_itemcode = 'TSR' and
			#temp_pd.ord_hdrnumber = stops.ord_hdrnumber and
			stp_type = 'DRP' and 
...
update #temp_pd
set    ivd_charge = (select sum(fgt_charge)
		     from  freightdetail,stops
		     where #temp_pd.pyt_itemcode = 'TSR' and
			#temp_pd.ord_hdrnumber = stops.ord_hdrnumber and
			stp_type = 'DRP' and 
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_sheet_summary_greatrig] TO [public]
GO
