...
CREATE Proc [dbo].[WatchDog_ValesDieselElect] 
(
...
insert into @Mostrar
	 ...
	 select 
            ...
            trc_id as Tractor,
		    '=concatenar(' + substring((select tca_id from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id),0,11)+','+ substring((select tca_id from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id),11,18)  + ')' as 'NumeroTarjeta',
		...
		and 
		--condicion de que solo tome la flota 15 qu es WALMART------------------------
		/*****************************************************************************/
		trc_id in (select trc_number from tractorprofile where trc_fleet = 15)
		/*****************************************************************************/

	   group by trc_id

--agregamos la informacion extra de la lista de los movimientos--------------------------------
update @Mostrar set NumeroViaje = 'Movs ' + STUFF((select distinct top (6)  ' ' + cast(mov_number as varchar) from fuelticket 
...
---agregamos la inforomacion extra de la lista los vales--------------------------------------------
	update @Mostrar set Observaciones= 'Vales ' +  STUFF((select top (6) ' ' + cast(ftk_ticket_number as varchar) from fuelticket 
                                            where tractor  = trc_id  and ftk_printed_by = 'VELEC' 
                                            and ftk_ticket_number  not in (select numVale from fuelticketelect where numvale is not null)
		                                     and ftk_canceled_on is null
											 and trc_id = (select tca_tractor		from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id)
...
*/
  ...
  ****************************************************************************************************************************************************************************************************************************/	
  -- se modifica la siguiente linea para el mensaje
  ...
  --'Prox.Dep.Edenred: $'+  cast(CONVERT(DECIMAL(10,2),(sum(ftk_cost * ftk_liters))) as varchar)  + ' por: ' +   cast(sum(ftk_liters) as varchar) + ' Lts'
  ...
  --          5,
  --          trc_id,
  --          'Prox.Dep.Edenred: $'+ ' por: ' +   cast(sum(ftk_liters) as varchar) + ' Lts'
  ...
  ****************************************************************************************************************************************************************************************************************************/
  ...
  --insertamos el mensaje en la tabla tblmessage
	   ...
	   select 
		...
		Folder =362,
		-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		Contents =     ('Prox.Dep.Edenred por: ' +   cast(sum(ftk_liters) as varchar) + ' Lts'
        + ' Mov: ' + cast(mov_number as varchar) + ', saldo expira: '+ replace(CONVERT(VARCHAR(10), dateadd(dd,8,max(ftk_created_on)), 105),'-','/')),
		Fromname = 'Admin',
		[Subject] =    ('Prox.Dep.Edenred por: ' +   cast(sum(ftk_liters) as varchar) + ' Lts'
        + ' Mov: ' + cast(mov_number as varchar) + ', saldo expira: '+ replace(CONVERT(VARCHAR(10), dateadd(dd,8,max(ftk_created_on)), 105),'-','/')),
		DeliverTo = trc_id,
		HistDrv = NULL,
		HistDrv2 = NULL,
...
---D)           INSERTAR EN TABLA DE AUDITORIA FUELTICKETELECT-------------------------------------------------------------------------------------------------------------------------------------
	...
	***/
	  ...
	  insert into fuelticketelect
		 select 
		   isnull((select tca_id from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id),'NA') as numtarjeta ,
		   trc_id as Tractor,
...
BEGIN
	 ...
	 select 
			...
			isnull((select tca_id from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id),'NA')  as 'NumeroTarjeta',
			trc_id as Tractor,
        ...
        and ftk_ticket_number  not in (select numVale from fuelticketelect where Tipo = 'Cancelado' and numvale is not null)
        and ftk_recycled <> 'Y'
		and trc_id = (select tca_tractor		from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id)
	   group by trc_id, mov_number
	  ...
	  insert into fuelticketelect
	    ...
	    --datediff(dd,ftk_created_on,getdate())= 0 and
		and ftk_canceled_on is not null
        and ftk_ticket_number  not in (select numVale from fuelticketelect where tipo = 'Cancelado' and numvale is not null)  and ftk_recycled <> 'Y'
		and trc_id = (select tca_tractor		from tractoraccesories where tca_type = 'TDE' and tca_tractor = trc_id)
