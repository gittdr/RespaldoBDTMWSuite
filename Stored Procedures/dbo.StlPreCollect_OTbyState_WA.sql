...
CREATE procedure [dbo].[StlPreCollect_OTbyState_WA](	@pl_pyhnumber int, 
 ...
 * NAME:
 * PTS 71874
 * dbo.StlPreCollect_OTbyState_WA
 * CUSTOM PROC
 * Returns @ps_ReturnVal = 1 success OR -1 error
 * CUSTOM WASHINGTON OverTime Rules
...
DECLARE @typesMustMatch	float
 ...
 Declare @mpp_day15_rt_min				float	-- this where Constant_WeeklyHrs stored in table.
			Select @mpp_day15_rt_min = 	  IsNull(mpp_day15_rt_min,0) 	from	manpowerprofile_CA_OT_rules 
												...
												AND		mpp_OT_state = 'WA'	-- we are IN the Washington proc
...
While @loopcnt <= @rowcnt 
 ...
 --=================================  Compute OverTime =========================================   
	...
	-- 03/07/2014 (remove next 2 selects) 
		...
		----------------------------------------------------------------------  update OT pay
		 ...
		 select @newRegularOT = IsNull(DailyHrsOT,0)  FROM #TmpPayWeek	
			  ...
			  begin
				if @newRegularOT > 0
					begin
						Print 'Debug ON: (Washington OT) WAOTRules Proc:  SUMMARY New Insert'  + space(1) +
	...
	select @rowcnt = count(TmpXref_ID) 	from #tmp_XrefPydDow where 	pyd_Reg_updated <> pyd_quantity	and pyt_basisunit <> 'SUMMRY'	-- if we need updates
	 ...
	 -- debug.start
		  ...
		  begin
			if @rowcnt > 0
				begin
					Print 'Debug ON: (Washington OT) WAOTRules Proc:  SUMMARY UPDATE EXISTING BasePay Hours'  + space(1) +
						'Number of PayDetail records to Update:' + space(1) + Convert(varchar(10),@rowcnt )
				end
			else
				begin
					Print 'Debug ON: (Washington OT) WAOTRules Proc:  SUMMARY NO updates needed for existing BasePay hours. '  						
				end
			-- end if	
		  end
	-- debug.end			
...
GO
GRANT EXECUTE ON  [dbo].[StlPreCollect_OTbyState_WA] TO [public]
GO
