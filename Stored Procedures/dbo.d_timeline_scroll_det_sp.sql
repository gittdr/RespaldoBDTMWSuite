...
CREATE PROC [dbo].[d_timeline_scroll_det_sp] 
@name varchar(32),
@Expires datetime,
@Effective datetime,
@supplier varchar(8),
 ...
 *	@Expires datetime,	Required ---- No longer used. 29912
 ...
 * 08/24/2005 - MRH Add dock and stopat. Remove multiple route functionallity.
 * 09/22/2005 - MRH removed Expires, changed Effective to be between effective and expires.
 * LOR	PTS# 29932	search for UNK branch
 **/
...
declare @BeginDate datetime -- calculated date used in pom
...
--	@Routes_out routes, 
	@PickupTime PickupTime, @Arrival Arrival, @BeginDate BeginDate
into #Temp
from timeline_header 
where convert(datetime, @Effective) >= convert(datetime, tlh_effective)
      and @Effective < dateadd(dd, 1, tlh_expires) and
--MRH 4/17/08 @Effective between tlh_effective AND tlh_expires and
--	@Effective <= tlh_effective and
...
drop table #temp
GO
GRANT EXECUTE ON  [dbo].[d_timeline_scroll_det_sp] TO [public]
GO
