...
CREATE PROCEDURE [dbo].[Metric_HoursPerDriver] 
	(
		--Standard Parameters
		...
		@UseMetricParms int, 
		@ShowDetail int,
		@MetricCode varchar(255)= 'HoursPerDriver',		
...
AS
	...
	EXEC MetricInitializeItem
		...
		@nPlusDeltaIsGood = 1,
		@nCumulative = 0,
		@sCaption = 'Travelled Hours Per Driver',
		@sCaptionFull = 'Travelled Hours Per Driver',
		@sProcedureName = 'Metric_HoursPerDriver',
		@sCachedDetailYN = '',
	...
	*/
	...
	Select	l.ord_hdrnumber as [Order Number],
			...
			cast((cast(DateDiff(mi,lgh_startdate,lgh_enddate) as float)/60.00) as decimal(12,2)) as [Leg Duration],
	...
	where	(
		  		...
		  		(@DateType = 'End' And lgh_enddate >= @DateStart AND lgh_enddate < @DateEnd)
					OR
		  		(@DateType = 'Book' and ord_bookdate >= @DateStart AND ord_bookdate < @DateEnd)
					OR
		  		(@DateType = 'Ship' and ord_startdate >= @DateStart AND ord_startdate < @DateEnd)
		   			OR
		  		(@DateType = 'Delivery' and ord_completiondate >= @DateStart AND ord_completiondate < @DateEnd)	
		 	)
	...
	Else
		Set @ThisCount = (	Select sum(on_duty_hrs) 
	...
	--Set @ThisCount = (Select sum(isnull([Leg Duration],0.0)) from #Legheader)
	Set @ThisTotal = (Select count(distinct driverid) from #Legheader)
...
GO
GRANT EXECUTE ON  [dbo].[Metric_HoursPerDriver] TO [public]
GO
