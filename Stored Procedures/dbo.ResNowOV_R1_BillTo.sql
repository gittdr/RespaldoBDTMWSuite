...
CREATE PROCEDURE [dbo].[ResNowOV_R1_BillTo]
...
*/
	...
	BEGIN
		...
		WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND ord_BillTo NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			ORDER BY ord_BillTo 
		END
	...
	BEGIN
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = ord_BillTo, 
				[ItemDescription] = (SELECT cmp_name + ' - ' + cty_nmstct FROM company (NOLOCK) where cmp_id = ord_BillTo),
				[Count] = COUNT(*),
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_BillTo] TO [public]
GO
