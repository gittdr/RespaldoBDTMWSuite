...
CREATE PROCEDURE [dbo].[outbound_view_brokerplan_cmp](
...
-- MTC 66176 major overall to dynamic SQL for performance reasons. Index addition also.
...
select @SQL = @SQL + N' from legheader l With (NoLock)   inner join orderheader o With (NoLock) on l.mov_number = o.mov_number '  
...
select @SQL = @SQL + N'and ord_status = ''CMP'' AND l.lgh_carrier <> ''' + @newcarrier + ''' '
...
GO
GRANT EXECUTE ON  [dbo].[outbound_view_brokerplan_cmp] TO [public]
GO
