...
---Sp utilizado para generar el reporte de Bardahl

--execute sp_datosBardahl  '2013-01-01 00:00:00.000', '2013-10-04 00:00:00.000','Todos'
--select * from invoiceheader where ord_hdrnumber = 222714
--select * from orderheader where ord_hdrnumber = 236451


--drop procedure sp_datosBardahl
CREATE PROCEDURE [dbo].[sp_datosBardahl]
...
ord_tractor,ord_bookdate,ord_origincity,ord_destcity,ord_totalweight from orderheader  where  orderheader.ord_status != 'CAN' and orderheader.ord_status = 'CMP' and ord_billto = 'BARDAHL' 
...
open factura2
		...
		begin
			...
			while @@FETCH_STATUS = 0
				...
				--select @Casetas = @caseta1+@caseta2+@caseta3+@caseta4+@caseta5+@casetaReposicionamiento
				 ...
				 insert into tblBardahl(invoice,invstatus,contador,referencia,orden,mov_number,unidad,operador,caja,origen,rep1,rep2,
  ...
  if @Statusinv = 'Todos'
    BEGIN
     select * from tblBardahl where invstatus in ('HLD','HLA')
    END
ELSE IF @Statusinv = 'NFAC'
   BEGIN
	select * from tblBardahl where invstatus in ('NFAC')
  END
ELSE
	select * from tblBardahl where invstatus in (@Statusinv)
