...
CREATE PROCEDURE [dbo].[outbound_view_brokerplan_available](
...
select @SQL = @SQL + N'isnull(recordlock.locked_by, ''Not Locked'') mov_lockedby, isnull(recordlock.session_date,''19500101'') mov_lockedon, '
...
select @SQL = @SQL + N' from legheader_active l With (NoLock) left outer join orderheader o With (NoLock) on l.ord_hdrnumber = o.ord_hdrnumber '  
--These next 2 lines look wrong, but they are correct. The ord_hdrnumber field in recordlock is actual mov_numbers
select @SQL = @SQL + N'left outer join recordlock (nolock) on (l.mov_number = recordlock.ord_hdrnumber and '
select @SQL = @SQL + N'recordlock.session_date = (SELECT MAX(rm.session_date) FROM recordlock rm (nolock) WHERE rm.ord_hdrnumber = l.mov_number)) '
...
select @SQL = @SQL + N'and (charindex(lgh_outstatus, ''' + @lgh_oustatuslist + ''') > 0 OR l.lgh_carrier = ''' + @newcarrier + ''') '
...
GO
GRANT EXECUTE ON  [dbo].[outbound_view_brokerplan_available] TO [public]
GO
