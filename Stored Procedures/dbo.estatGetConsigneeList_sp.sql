...
SET ANSI_NULLS ON
GO
Create procedure [dbo].[estatGetConsigneeList_sp]
	@weeks integer,  -- 2048
	@login varchar(132),   
	@limitcust char(16),
	@format char(16) -- 2048 
...
begin
	insert into #tt
	select distinct ord_Consignee as custid
...
begin
	...
	begin
		...
		begin
			if @limitcust = 'orderby' --if customer must be the order-by 
			begin
				insert into #tt
				select distinct ord_consignee as custid
				from orderheader with (index(sk_ord_bookdate)) 
				...
				begin
				if @limitcust = 'any'  -- If customer can be any
				begin
                 insert into #tt
					select distinct ord_consignee as custid
						from orderheader with (index(sk_ord_bookdate))  
...
drop table #tt
GO
GRANT EXECUTE ON  [dbo].[estatGetConsigneeList_sp] TO [public]
GO
