...
create procedure [dbo].[invoice_template94](@invoice_nbr   int,@copies  int)  
as  

/*
 * 
 * NAME:invoice_template94
 * dbo.
 *
 * TYPE:
 * StoredProcedure
 ...
 *       invoice header number selected for printing
 ...
 * 06/29/2001 - PTS10870 - Vern Jewett  vmj1 - not returning copy # correctly.  
 ...
 * 06/06/2006 - PTS32846 - Imari Bremer - Create new invoice format for VTL Transport
 **/
...
-- PTS 30050 -- BL (end)  
 ...
 --pts 36350
 @v_ivh_invnumber varchar(12),
 @v_totalmiles float,
 @v_ivh_currency varchar(6),
 @v_ivh_billto varchar(8), 
 @v_ivh_billto_name varchar(100),
 @v_ivh_billto_addr varchar(100),
 @v_ivh_billto_addr2 varchar(100),
 @v_ivh_billto_nmctst varchar(36),
 @v_billto_country varchar(50),
 @v_cmp_altid varchar(25),
 @v_ivd_countunit varchar(6)    
 --36350 end 
...
invoiceheader.ivh_trailer,     
 ...
 -- invoicedetail.ivd_taxable3,     
  ...
  @v_temp_nmstct ivd_billto_nmctst,  
		...
		--pts 36350
		 case chargetype.cht_basis when 'ACC' then invoicedetail.ivd_sequence + 2000
			WHEN 'TAX' THEN invoicedetail.ivd_sequence + 4000
			ELSE invoicedetail.ivd_sequence + 1000 
		 end ivd_sequence,
		--36350 end      
...
--pts 36350
select distinct @v_ivh_invnumber = ivh_invoicenumber,@v_totalmiles = ivh_totalmiles,
@v_ivh_currency = ivh_currency,@v_ivh_billto = ivh_billto, @v_ivh_billto_name = ivh_billto_name,
@v_ivh_billto_addr = ivh_billto_addr,@v_ivh_billto_addr2 = ivh_billto_addr2,
@v_ivh_billto_nmctst = ivh_billto_nmctst,@v_billto_country = billto_country,@v_cmp_altid = cmp_altid,
@v_ivd_countunit = ivd_countunit  
from #invtemp_tbl 

insert into #invtemp_tbl (ivh_invoicenumber, ivh_hdrnumber, ivh_billto, ivh_billto_name , ivh_billto_addr, ivh_billto_addr2, ivh_billto_nmctst, 
ivh_totalmiles, ivh_currency, ivd_sequence, ivd_countunit,cht_basis, ivh_charge, copies, cht_description, cmp_altid, billto_country) 
values (@v_ivh_invnumber, @invoice_nbr, @v_ivh_billto,@v_ivh_billto_name,@v_ivh_billto_addr,@v_ivh_billto_addr2,@v_ivh_billto_nmctst,
@v_totalmiles, @v_ivh_currency, 3000, @v_ivd_countunit,'PRETAX', 0, 1, '', @v_cmp_altid, @v_billto_country)
--36350 end 
...
return @v_ret_value  
GO
GRANT EXECUTE ON  [dbo].[invoice_template94] TO [public]
GO
