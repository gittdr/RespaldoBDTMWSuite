...
create procedure [dbo].[d_settlement_sheet_summary47_sp](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary47_sp
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * 10/13/2005   - PTS29129 - Imari Bremer - New format created for Vedder Group 
 **/

-- jyang pts13004
Declare @v_PeriodforYTD Varchar(3), @v_FSCPayTypes varchar(60), @v_fuelsurcharge money,
        @v_netrevenue money, @v_searchpos int, @v_fsctype varchar(10), @v_start int,
        @v_length int, @v_amount money
...
--PTS# 29129 ILB 10/11/2005
SELECT @v_FSCPayTypes = isnull(gi_string1,'') 
FROM generalinfo
...
DECLARE @p_gi_string1 varchar(60)

SELECT @p_gi_string1 = gi_string1 
FROM generalinfo
...
SELECT pd.pyd_number,
	...
	pd.pyd_sequence,
	pd.pyd_rate,
	ROUND(pd.pyd_amount, 2)pyd_amount,
	pd.pyd_payrevenue,
	pd.mov_number,
	pd.lgh_number,
	pd.ord_hdrnumber,
...
WHILE @v_SearchPos > 0 
   BEGIN     		
	SELECT @v_SearchPos = CharIndex(',',@v_FSCPayTypes,@v_SearchPos)        
	IF @v_searchpos = 0 
	   BEGIN
                
		SELECT @v_length = LEN(@v_FSCPayTypes) - (@v_start - 1)
		SELECT @v_fsctype = upper(SubString(@v_FSCPayTypes,@v_start,@v_length)) 
		
                select @v_amount = sum(isnull(pyd_amount,0))
                  from #temp_pd
                 where pyt_itemcode = @v_fsctype and
                       pyd_status <> 'HLD'
		
		SELECT @v_fuelsurcharge = isnull(@v_fuelsurcharge,0) + isnull(@v_amount,0)		
		BREAK
	   END 
	ELSE
	   BEGIN
		
		SELECT @v_length = @v_searchpos - @v_start
        	SELECT @v_fsctype = upper(SubString(@v_FSCPayTypes,@v_start,@v_length)) 
	   ...
	   END	
        ...
        SELECT @v_searchpos = @v_searchpos + 1 
        SELECT @v_start = @v_searchpos 
 	SELECT @v_amount = 0       
        
   END

UPDATE #temp_pd
   SET fuelsurcharge_total = @v_fuelsurcharge

----PTS# 32910 ILB 05/04/2006
SET @v_amount = 0
SELECT @v_amount = sum(pyd_amount)
  FROM #temp_pd
--SELECT DISTINCT @v_amount = pyh_totalcomp
--  FROM #temp_pd
--PTS# 32910 ILB 05/04/2006

UPDATE #temp_pd
   SET netrevenue_total = @v_amount - @v_fuelsurcharge
--PTS# 29129 ILB 10/11/2005
...
SELECT pyd_number, 
  ...
  --WHERE tp.pyh_number *= ph.pyh_pyhnumber
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary47_sp] TO [public]
GO
