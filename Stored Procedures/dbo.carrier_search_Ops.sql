...
CREATE PROC [dbo].[carrier_search_Ops]
...
IF @destination > 0 AND @dradius > 0
BEGIN
      ...
      INSERT INTO #originlanes
         SELECT DISTINCT laneid
          ...
          WHERE cll.isorigin = 1 AND
              ((cll.type = 2 AND
                cll.citycode = @origin) OR
               (cll.type = 5 AND
                cll.stateabbr IN (SELECT city.cty_state
                                    FROM city
                                   WHERE city.cty_code = @origin)) OR
               (cll.type = 3 AND
         ...
         INSERT INTO #lanes
            SELECT DISTINCT cll.laneid
             ...
             WHERE cll.isorigin = 2 AND
                 ((cll.type = 2 AND
                   cll.citycode = @destination) OR
                  (cll.type = 5 AND
                   cll.stateabbr IN (SELECT city.cty_state
                                       FROM city
                                      WHERE city.cty_code = @destination)) OR
                  (cll.type = 3 AND
             ...
             WHERE cll.isorigin = 2 AND
                 ((cll.type = 2 AND
                  ...
                  (cll.type = 5 AND
                   cll.stateabbr IN (SELECT city.cty_state
                                       FROM city
                                      WHERE city.cty_code = @destination)))
      END
      ELSE
      BEGIN
         INSERT INTO #lanes
...
IF @ll_lanescount > 0 AND @destination = 0 AND @ll_dstates = 0
BEGIN
   INSERT INTO #lanes
      SELECT laneid
        FROM #originlanes
...
IF @origin = 0 AND @ll_ostates = 0 AND @destination = 0 AND @ll_dstates = 0
BEGIN
   INSERT INTO #lanes
      SELECT DISTINCT laneid
...
BEGIN
   UPDATE #temp1
      SET orig_domicile = 'Y'
    WHERE ((@oradius = 0 AND #temp1.cty_code = @origin) OR
   ...
   INSERT INTO #temp2 (trk_carrier, car_name, car_address1, car_address2,
      ...
      SELECT carrier.car_id,
       ...
       WHERE ((@oradius = 0 AND carrier.cty_code = @origin) OR
...
BEGIN
   UPDATE #temp1
      SET dest_domicile = 'Y'
    WHERE ((@dradius = 0 AND #temp1.cty_code = @destination) OR
   ...
   INSERT INTO #temp2 (trk_carrier, car_name, car_address1, car_address2,
      ...
      SELECT carrier.car_id,
        ...
        FROM carrier WITH (NOLOCK) JOIN city WITH (NOLOCK) ON carrier.cty_code = city.cty_code
                                   LEFT OUTER JOIN carrierhistory WITH (NOLOCK) ON carrier.car_id = carrierhistory.crh_carrier
       WHERE ((@dradius = 0 AND carrier.cty_code = @destination) OR
...
update #temp1 set exp_priority1 =   
	(select count(0) from expiration with (nolock)  
...
ELSE
	...
	car_contact = CASE WHEN ISNULL(car_contact,'') = '' THEN (SELECT Top 1 contact_name FROM companyemail WHERE cmp_id =#temp1.trk_carrier  and ce_defaultcontact = 'Y') ELSE car_contact END
...
GO
GRANT EXECUTE ON  [dbo].[carrier_search_Ops] TO [public]
GO
