...
CREATE Proc [dbo].[WatchDog_OrderFollowUp] 
(
	@MinThreshold float = 1,
	@MinsBack int=-20,
	@TempTableName varchar(255)='##WatchDogGlobalOrderFollowUp',
	@WatchName varchar(255)='OrderFollowUp',
...
Select 	ord_number as [Order #],
		last_updatedate as [Last Updated],
		...
		ord_company as [Ordered By ID],
       	cmporderedby.cmp_name as [Ordered By]
into   	#TempResults
From   	orderheader (NOLOCK) Left Join company  cmpshipper (NOLOCK)  On ord_shipper = cmpshipper.cmp_id
		Left Join company cmpconsignee (NOLOCK)  On ord_destpoint = cmpconsignee.cmp_id		
		Left Join company cmpbillto (NOLOCK) On ord_billto = cmpbillto.cmp_id
		Left Join company cmporderedby (NOLOCK) on ord_company = cmporderedby.cmp_id
Where (@RevType1 =',,' or CHARINDEX(',' + ord_revtype1 + ',', @RevType1) >0)
    ...
    And DateDiff(mi,last_updatedate,GetDate())>= @MinThreshold
	AND DateDiff(mi,last_updatedate,getdate())<= -@MinsBack
