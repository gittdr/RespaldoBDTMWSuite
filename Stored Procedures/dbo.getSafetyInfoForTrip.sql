...
Create Proc [dbo].[getSafetyInfoForTrip] @ord varchar(12), @lgh int,@mov int
As
/**
 * DESCRIPTION:
 *
 ...
 * REVISION HISTORY:
	...
	PTS 19362 add on correct spelling of this procs name (take out extra for)
 * 11/30/2007.01 - PTS40464 - JGUO - convert old style outer join syntax to ansi outer join syntax.
 * 1/26/10 PTS50647 return branch and bill to information
 *
 **/

Declare @cmdcode varchar(8),@cmdname varchar(50),@shipper varchar(8),@ordnbr int,@shippername varchar(100),@hazmat tinyint
declare @ordbookedrevtype1 varchar(12),@lghbookedrevtype1 varchar(12),@billto varchar(8),@billtoname varchar(100)
declare  @consignee varchar(8), @consigneename varchar(100),@revtype1 varchar(6),@revtype2 varchar(6),@revtype3 varchar(6),@revtype4 varchar(6)
   ...
   If Rtrim(@ord) = ''
    BEGIN      --*****************
     ...
     If @ordNbr > 0 
        ...
        Select @cmdcode = orderheader.cmd_code,@shipper = ord_shipper ,@billto = ord_billto,
        @consignee = ord_consignee,@ordbookedrevtype1 = ord_booked_revtype1, @revtype1 = ord_revtype1,
        @revtype2 = ord_revtype2, @revtype3 = ord_revtype3, @revtype4 = ord_revtype4
        From orderheader Where ord_hdrnumber = @ordnbr
     ...
     Else -- no order on leg
       BEGIN           --===================
        ...
        Select @consignee = 'UNKNOWN'
       END             --===================
    END       --********************
   Else
    BEGIN  -- retrieve for passed order number
      Select @cmdcode = orderheader.cmd_code,@shipper = ord_shipper ,@billto = ord_billto,
        @consignee = ord_consignee,@ordbookedrevtype1 = ord_booked_revtype1, @revtype1 = ord_revtype1,
        @revtype2 = ord_revtype2, @revtype3 = ord_revtype3, @revtype4 = ord_revtype4
        From orderheader Where ord_number = @ord
     
    END   -- retrieve for passed order number
 END --------------------------
Else  /* find the leg_number */
 BEGIN   --.................................
  If Rtrim(@ord) > ''

    Select @lgh = lgh_number,@cmdcode = orderheader.cmd_code,@shipper = ord_shipper ,@billto = ord_billto,
    @consignee = ord_consignee,@ordbookedrevtype1 = ord_booked_revtype1 , @revtype1 = ord_revtype1,
    @revtype2 = ord_revtype2, @revtype3 = ord_revtype3, @revtype4 = ord_revtype4
...
Select @billtoname = cmp_name from company where cmp_id = @billto
Select @consigneename = cmp_name from company where cmp_id = @consignee
if @lgh > 0
  select @lghbookedrevtype1 = lgh_booked_revtype1 from legheader where lgh_number = @lgh
...
Select evt_Driver1,
  ...
  trl2Ownername = (Select IsNull(pto_Fname+' ','')+IsNull(pto_lname,'')+'~r'
      +IsNull(pto_address1+'~r','')+IsNull(cty_name+', ','')+Isnull(cty_state+'~r','')+
      IsNull(pto_phone1,'')
      From city RIGHT OUTER JOIN payto ON city.cty_code = pto_city Where payto.pto_ID = trl2.trl_owner and trl2.trl_Owner <> 'UNKNOWN'), 
  ...
  @ordbookedrevtype1 ord_booked_revtype1,
  @lghbookedrevtype1 lgh_booked_revtype1,
  @billto ord_billto,
...
GO
GRANT EXECUTE ON  [dbo].[getSafetyInfoForTrip] TO [public]
GO
