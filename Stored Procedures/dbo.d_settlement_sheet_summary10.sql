...
CREATE PROC [dbo].[d_settlement_sheet_summary10](
...
CREATE TABLE #temp_pd(
	...
	lgh_tractor		varchar(13) null,  --vjh pts8041
...
--       lgh_number = lh.lgh_number,
   ...
   lgh_tractor = (SELECT lgh_tractor 
                          FROM legheader lh
                         WHERE lh.lgh_number = #temp_pd.lgh_number)
...
SELECT pyd_number,		-- 1
	...
	terminal,		-- 35
	type1,			-- 36
	round(tp.pyh_totalcomp, 2),		-- 37
	round(tp.pyh_totaldeduct, 2),		-- 38
	round(tp.pyh_totalreimbrs, 2),		-- 39
	...
	end_city,		-- 67
	end_state, 		-- 68
	lgh_tractor		--vjh pts8041 69
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber 

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary10] TO [public]
GO
