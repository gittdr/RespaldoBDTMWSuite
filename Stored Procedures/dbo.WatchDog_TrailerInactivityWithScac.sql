...
create PROC [dbo].[WatchDog_TrailerInactivityWithScac]
	(
		...
		@ExcludeTrailerLocationCompanyID VARCHAR(255)='',
...
AS
	...
	****************************************************************/
	...
	SET @ExcludeTrailerLocationCompanyID = ',' + ISNULL(@ExcludeTrailerLocationCompanyID,'') + ','
	...
	*****************************************************************************/
		...
		'Trailer Start Event' = (
				...
				WHERE stops.lgh_number = legheader.lgh_number 
					AND stp_arrivaldate = (
						...
						FROM stops (NOLOCK)
						WHERE stops.lgh_number = legheader.lgh_number 
							AND stops.trl_id = trl_ilt_scac  +','+trl_number --trl_number 
					)
			),
		'Trailer End Event' = (
				SELECT TOP 1 stp_event 
					...
					AND stp_arrivaldate = (
						...
						WHERE stops.lgh_number = legheader.lgh_number 
							And
							  ...
							  evt_sequence = 1
							  And
							  (evt_trailer1 = trl_ilt_scac  +','+trl_number  Or evt_trailer2 = trl_ilt_scac  +','+trl_number ) 
							  and stp_status = 'DNE'
					)
			),
		...
		trl_ilt_scac  ,
			...
			trl_status,
	      	'MaxAsgnNumber'= (
								...
								WHERE trl_ilt_scac  +','+trl_number =asgn_id
									AND asgn_type = 'TRL'
									AND asgn_enddate = (
	...
	ORDER BY CASE WHEN @OrderByMode='TrlFleet' THEN trl_fleet 
				  WHEN @OrderByMode='TrlType1' THEN  trl_type1
				  WHEN @OrderByMode='TrlType2' THEN  trl_type2  
				  WHEN @OrderByMode='TrlType3' THEN  trl_type3
				  WHEN @OrderByMode='TrlType4' THEN  trl_type4  
			 ELSE '' 
			 END desc, DaysInactive DESC
		
	--ORDER BY trl_fleet,DaysInactive DESC

	IF @OnlyTrailerLocationCompanyID<>',,'
		DELETE from #TempResults WHERE (CHARINDEX(',' + IsNull([Trailer Destination Company ID],'') + ',', @OnlyTrailerLocationCompanyID) =0)

	IF @ExcludeTrailerLocationCompanyID<>',,'
		DELETE from #TempResults WHERE (CHARINDEX(',' + IsNull([Trailer Destination Company ID],'') + ',', @ExcludeTrailerLocationCompanyID) >0)
...
GO
GRANT EXECUTE ON  [dbo].[WatchDog_TrailerInactivityWithScac] TO [public]
GO
