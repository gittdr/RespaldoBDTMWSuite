...
---Sp utilizado para generar el reporte de SIGMA

--execute sp_datosSigma  '2013-01-01 00:00:00.000', '2013-10-04 00:00:00.000','Todos'
--select * from invoiceheader where ord_hdrnumber = 222714
--select * from orderheader where ord_hdrnumber = 236261
--select * from tblSigma


--drop procedure sp_datosBardahl
CREATE PROCEDURE [dbo].[sp_datosSigma]
...
--Verifica segmentos de la orden
	Declare factura2 cursor for select ord_bookdate,ord_refnum,ivh_invoicenumber,ivh_invoicestatus,ord_hdrnumber,mov_number,mpp_lastname + ' ' + mpp_firstname as operador,ord_tractor,ord_trailer,cty_name,ord_totalweight
 from (
select * from (select ord_refnum,ivh_invoicenumber,ivh_invoicestatus,orderheader.ord_hdrnumber,orderheader.mov_number,ord_driver1,ord_trailer,ord_tractor,ord_bookdate,ord_origincity,ord_destcity,ord_totalweight 
from orderheader left outer join invoiceheader on invoiceheader.ord_hdrnumber = orderheader. ord_hdrnumber   where  ord_billto = 'SIGMAALI'  and (ord_startdate between @fechaIni and @fechaFin)
and orderheader.ord_hdrnumber in (select invoiceheader.ord_hdrnumber from invoiceheader p where p.ivh_invoicenumber not like 'S%') and ivh_billto = 'SIGMAALI'    
...
ord_tractor,ord_bookdate,ord_origincity,ord_destcity,ord_totalweight from orderheader  where  orderheader.ord_status != 'CAN'  and orderheader.ord_status = 'CMP' and ord_billto = 'SIGMAALI' 
...
open factura2
	fetch factura2 into @fecha,@referencia,@invoice,@invstatus,@orden,@mov_number,@operador,@unidad,@caja,@origen,@toneladas
	while @@FETCH_STATUS = 0
		begin
			...
			while @@FETCH_STATUS = 0
				...
				Declare conceptos cursor for select cht_itemcode,ivd_charge from invoicedetail where ord_hdrnumber = @orden and not cht_itemcode in ('MODESC')
				open conceptos
				...
				begin
					...
					if @cargo = 'TOLL'
						...
						begin
							select @iva = @totalCargo
						end
                  else if @cargo = 'PST'
						begin
							select @retencion = @totalCargo
						end
				...
				--select @Casetas = @caseta1+@caseta2+@caseta3+@caseta4+@caseta5+@casetaReposicionamiento
				 ...
				 insert into tblSigma(fecha,invoice,invstatus,contador,referencia,orden,mov_number,unidad,operador,caja,origen,rep1,rep2,
					rep3,rep4,rep5,reposicionamiento,toneladas,km1,km2,km3,km4,km5,kmRep,kmsTotales,casetas1,casetas2,casetas3,casetas4,casetas5,casetaRep,Casetas,maniobras1,maniobras2,maniobras3,maniobras4,maniobras5,Maniobras,fuel,iva,retencion)
					values (datename( dw, @fecha )+','+convert(varchar, @fecha, 106),@invoice,@invstatus,@contador,@referencia,@orden,@mov_number,@unidad,@operador,@caja,@origen,@rep1,@rep2,
					@rep3,@rep4,@rep5,@reposicionamiento,@toneladas,@km1,@km2,@km3,@km4,@km5,@kmRep,@kmsTotales,@caseta1,@caseta2,@caseta3,@caseta4,@caseta5,@casetaReposicionamiento,@Casetas,@maniobras1,@maniobras2,@maniobras3,@maniobras4,@maniobras5,@Maniobras,@fuel,@iva,@retencion)
			...
			FETCH NEXT FROM factura2 INTO  @fecha,@referencia,@invoice,@invstatus,@orden,@mov_number,@operador,@unidad,@caja,@origen,@toneladas
  ...
  if @Statusinv = 'Todos'
    BEGIN
     select * from tblsigma where invstatus in ('HLD','HLA')
    END
ELSE IF @Statusinv = 'NFAC'
   BEGIN
	select * from tblsigma where invstatus in ('NFAC')
  END
ELSE
	select * from tblsigma where invstatus in (@Statusinv)
