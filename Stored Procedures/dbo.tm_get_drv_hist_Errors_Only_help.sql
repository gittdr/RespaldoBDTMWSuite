...
CREATE PROCEDURE [dbo].[tm_get_drv_hist_Errors_Only_help]	
						@DriverSN INT,
						@MaxMessages INT
...
SET NOCOUNT ON 
	...
	SELECT @UNKNOWN_DispatchGroupSN = SN FROM tblDispatchGroup WHERE Name = ' UNKNOWN'
	...
	INSERT INTO @TH2 (SN,
		...
		FROM tblHistory (NOLOCK) 
			...
			JOIN dbo.tblMsgProperties (NOLOCK) on MSG.SN = tblMsgProperties.MsgSN 
			INNER JOIN tblDrivers (NOLOCK) ON tblDrivers.SN = DriverSN --For Dispatch Group
		WHERE tblHistory.DriverSN = @DriverSN AND tblMsgProperties.PropSN = 6 
...
GO
GRANT EXECUTE ON  [dbo].[tm_get_drv_hist_Errors_Only_help] TO [public]
GO
