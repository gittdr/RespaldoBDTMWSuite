...
CREATE  proc [dbo].[dx_create_order_from_stops_nuevaVersionJr]  
	@validate char(1) OUTPUT,
    @mov_number int,  
	...
	@carrier varchar(8) OUTPUT,
	@dispatch_status varchar(6) OUTPUT,
...
/*******************************************************************************************************************  
  ...
  -----------  ---------------  ----------   ----------------------------------------
  09/14/2016   David Wilks      100026       ord_carrier should not be blank but rather UNKNOWN
********************************************************************************************************************/
...
DECLARE @ls_ord_customer varchar(8), @ls_ord_orderby varchar(8), @ls_ord_billto varchar(8), @ls_ord_revtype1 varchar(6),
	...
	@ls_ord_priority varchar(6), @ls_edictkey varchar(6), @ls_ord_subcompany varchar(8), 
...
END
 ...
 /* assign an order header number (key) must be unique */
  ...
  /* If I cannot find at least 2 stops with this move number return error */
    ...
    IF (@mov_number = 0) -- script returns mov_number 0 when only 1 stop created
        BEGIN
          IF @validate != 'N'
            BEGIN
             SELECT @err_ret = -2
 ...
 INSERT INTO orderheader 
    ...
    ord_revenue_pay, ord_booked_revtype1, ord_carrier, iva_req)
VALUES ( @orderby_cmpid, @@ord_number, ISNULL(@ls_ord_customer,'UNKNOWN'), 	        	--1
	...
	CASE @ord_rateby WHEN 'T' THEN ISNULL(@ord_charge,0) ELSE null END, @ord_revtype1, @carrier,'N')
