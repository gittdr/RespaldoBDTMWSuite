...
CREATE PROCEDURE [dbo].[MetricUpdateSummariesAlternate] 
(
...
END
			...
			-- BEGIN
				...
				IF @WeekOneIsFirstFullWeekYN = 'N'
				BEGIN
					SELECT @PlainWeek = DATEPART(week, @DateCur)
				...
				BEGIN
					UPDATE @MetricDetailUseForUpdates SET ThisWTD = @Result, PlainWeek = @PlainWeek, PlainYearWeek = @PlainYearWeek, PlainDayOfWeek = DATEPART(dw, @DateCur),
				...
				ELSE
				BEGIN
					INSERT INTO @MetricDetailUseForUpdates (SPID, MetricCode, PlainDate, ThisWTD, PlainDayOfWeek, PlainWeek, PlainMonth, PlainQuarter, PlainYear, PlainYearWeek)
					SELECT @@SPID, @MetricCode, @DateCur, @Result, DATEPART(dw, @DateCur), @PlainWeek, DATEPART(m, @DateCur), DATEPART(q, @DateCur), DATEPART(yyyy, @DateCur), @PlainYearWeek
				END
				*/
...
GO
GRANT EXECUTE ON  [dbo].[MetricUpdateSummariesAlternate] TO [public]
GO
