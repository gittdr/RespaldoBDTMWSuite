...
CREATE PROC [dbo].[d_settlement_sheet_summary44] (
 ...
 * 
 * NAME:
 * D_SETTLEMENT_SHEET_SUMMARY44
 *
 * TYPE:
 * Stored Procedure
 *
 * DESCRIPTION:
 * This StoredProcedure supplies arguements to datawindow d_stlmnt_det_report_general44
 *
 * RETURNS:
 * None
 *
 * RESULT SETS: 
 * Select statement from #temp_pd
 *
 * PARAMETERS:
 ...
 *	 This parameter represents the Driver Id
 * 008 - @trc_id varchar(8);
 *	 This parameter represents the Tractor Id
 * 009 - @trl_id varchar(13);
 *	 This parameter represents the Trailer Id
 ...
 *	
 * 019 - @pyhnumber int;
 *	 This parameter holds the Unique Payment Identifier 
 * 020 - @relcol varchar(3);
 *	 
 * 021 - @relncol varchar(3);
 *	 
 * 022 - @workperiodstart datetime;
 *	 This parameter designates the Beginning of the work period to be settled
 ...
 * 
 * REVISION HISTORY:
 * 08/18/2005.01 ? PTS27987 - DMEEK ? Copied from SP d_settlement_sheet_summary15, 
 * modifed for new settlement datawindow d_stlmnt_det_report_general44
 *
 **/
...
SELECT pd.pyd_number,
	...
	pd.pyd_payto,
	pd.pyt_itemcode,
        --CASE paydetail_pyt_itemcode 'Route Pay' then ('(' + pd.lgh_startpoint + ')' + city.cty_name + ',' + city.cty_state + ',' + city.cty_zip,pd.pyd_description),
	Description =  CASE  pyt_itemcode
           WHEN 'TSM' THEN '(' + pd.lgh_startpoint + ')' + city.cty_name + ',' + city.cty_state + ',' + city.cty_zip
	   WHEN 'FSC' THEN '(' + pd.lgh_startpoint + ')' + city.cty_name + ',' + city.cty_state + ',' + city.cty_zip
           Else pd.pyd_description
           END ,	
	pd.pyr_ratecode,
	pd.pyd_quantity,
	pd.pyd_rateunit, 
	...
	left outer join city on pd.lgh_startcity = city.cty_code
...
UPDATE #temp_pd
   SET ord_startdate = oh.ord_startdate,
       ...
       ord_fromorder = rtrim(oh.ord_fromorder)
  FROM #temp_pd tp, 
       orderheader oh
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary44] TO [public]
GO
