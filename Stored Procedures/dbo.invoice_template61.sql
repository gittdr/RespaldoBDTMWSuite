...
CREATE procedure [dbo].[invoice_template61](@invoice_nbr int,@copies  int)  
as  
...
--PTS# 25401 ILB 11/17/2004
--Get the quantity excluding the miles
SELECT @BILLING_QTY = SUM(tmp.ivd_quantity)
  FROM #invtemp_tbl tmp,chargetype chg
 WHERE tmp.cht_itemcode = chg.cht_itemcode and
       tmp.cht_itemcode <> 'LHD' and 
       tmp.ivd_type in('PUP','DRP') and
       chg.cht_basisunit <> 'DIS'

--Get the quantity for miles only
SELECT @MILES = SUM(ivd_quantity)
  FROM #invtemp_tbl tmp, chargetype chg
 WHERE tmp.cht_itemcode = chg.cht_itemcode and
       tmp.cht_itemcode = 'LHD' and 
       tmp.cht_basis = 'SHP' and
       chg.cht_basisunit = 'DIS'

--If the billing quantity = the miles then the quantity should be set to 1 and
--the rate should be set to the total miles * the rate for the mileage record
IF @BILLING_QTY = @MILES
   Begin  
     --Loop through the invoicedetail and find all of the mileage based records
     Set @MinIvd = 0
     WHILE (SELECT COUNT(*) 
	      FROM #invtemp_tbl tmp, chargetype chg 
	     WHERE ivd_number > @MinIvd and
                   tmp.cht_itemcode = chg.cht_itemcode and
       		   --tmp.cht_itemcode = 'LHD' and 
       		   tmp.cht_basis IN('SHP', 'ACC') and
       		   chg.cht_basisunit = 'DIS') > 0
	BEGIN
	  
	  --Get the minimum mileage based stop
	  SELECT @MinIvd = (SELECT MIN(ivd_number) 
                              FROM #invtemp_tbl tmp, chargetype chg
                             WHERE tmp.ivd_number > @MinIvd  and
				   tmp.cht_itemcode = chg.cht_itemcode and
       				   --tmp.cht_itemcode = 'LHD' and 
       				   tmp.cht_basis IN('SHP', 'ACC') and
       		   		   chg.cht_basisunit = 'DIS') 
	            
	  --Get the rate for miles only
  	  SELECT @RATE = IVD_RATE
    	    FROM #invtemp_tbl tmp, chargetype chg
   	   WHERE tmp.ivd_number = @MinIvd and
		 tmp.cht_itemcode = chg.cht_itemcode and
       		 --tmp.cht_itemcode = 'LHD' and 
...
--PTS# 27265 ILB 03/14/2005
--Determine if the line haul has a Flat rate applied
Select @rateunit = ivd_rateunit
  from #invtemp_tbl
 where upper(cht_itemcode) = 'LHF'

--IF the line haul has a Flat rate applied then set the detail rows quantity to 1
IF upper(@rateunit) = 'FLT'
	BEGIN
		UPDATE #invtemp_tbl
	           SET ivd_quantity = 1
                 WHERE upper(cht_basis) NOT IN ('ACC','SHP')
	END
--END PTS# 27265 ILB 03/14/2005
...
return @ret_value 
GO
GRANT EXECUTE ON  [dbo].[invoice_template61] TO [public]
GO
