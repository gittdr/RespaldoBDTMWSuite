...
CREATE PROC [dbo].[d_masterbill02_sp] 
 ...
 * 9/8/9 PTS48808 if there are multiple BL# ref nubmers with the same ref nubmer vallue, the charges
 *    are nto cleared on the duplicates.
 *
 **/
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	billto_zip = 
     ...
     ref_sequence = isnull(ref.ref_sequence,1),
     blrefsequence = isnull((select min(ref_sequence) 
...
--	AND (ref.ref_type ='BL#' )
   Order by ivh_invoicenumber,ivh_totalcharge 
...
-- yet still show the rest of the info
    ...
    FROM 
	invoiceheader 
    LEFT OUTER JOIN referencenumber ref ON (ref.ref_tablekey = invoiceheader.ord_hdrnumber and ref.ref_table = 'orderheader' and ref.ref_type ='BL#') 
	left outer join company cmp1 on  invoiceheader.ivh_billto = cmp1.cmp_id
...
Update #Temp
	Set ivh_totalcharge=0
	from #temp
	where blrefsequence > 0
    and blrefsequence <> ref_sequence
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill02_sp] TO [public]
GO
