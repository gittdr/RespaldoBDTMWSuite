...
CREATE Proc [dbo].[WatchDog_ValesDieselSodexobup] 
(
...
insert into @Mostrar
	 ...
	 select 
			...
			CONVERT(DECIMAL(10,2),( 14.63 * sum(ftk_liters)))  as Importe
          from fuelticket
		where 
...
/***************************************************************************************************************************************************************************************************************************
  ...
  ****************************************************************************************************************************************************************************************************************************/
  ...
  --insertamos el mensaje en la tabla tblmessage
       ...
       FromTrcSN	,MaxDelayMins	,BaseSN	,McuId	,Export, envnavman	)
	   ...
	   select 
		...
		Mculd = NULL,
		Export = NULL,
		envnavman = 8  --- (8 = admin a tractor preparado)
 ...
 --cambiamos la bandera de envio navmana a 88 para indicar que ya fue procesado (88 = admin a tractor navman procesado)--------------------------------------------------------------------
  update tmwsuite.dbo.tblMessages set envnavman = 88  where envnavman = 8
