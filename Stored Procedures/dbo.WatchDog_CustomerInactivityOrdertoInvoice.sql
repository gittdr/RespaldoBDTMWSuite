...
CREATE PROC [dbo].[WatchDog_CustomerInactivityOrdertoInvoice]  
 (  
  @MinThreshold float = 3,  
  @MinsBack int=-20,  
  @TempTableName VARCHAR(255) = '##WatchDogGlobalCustomerInactivityOrdertoInvoice',  
  @WatchName VARCHAR(255)='CustomerInactivityOrdertoInvoice',  
...
/*  
Procedure Name:    CustomerInactivityOrdertoInvoice 
...
/*  
if not exists (select WatchName from WatchDogItem where WatchName = 'CustomerInactivityOrdertoInvoice')  
INSERT INTO watchdogitem (WatchName, BeginDate, EndDate, SqlStatement, Operator, EmailAddress, BeginDateMinusDays, EndDatePlusDays, DateField, QueryType, ProcName, NumericOrText, MinsBackToRun, HTMLTemplateFlag, ActiveFlag, DefaultCurrency, CurrencyDateType, Description)  
 VALUES ('CustomerInactivityOrdertoInvoice','12/30/1899','12/30/1899','WatchDog_CustomerInactivityOrdertoInvoice','','',0,0,'','','','','',1,0,'','','')  
*/  
...
/************************************************************************************  
 ...
 active   
************************************************************************************/  
SELECT ord_company AS [Company ID],  
  ...
  AS [Last Date Taken]  
INTO   #TempCompanies  
FROM   orderheader o (NOLOCK)join company c (NOLOCK) on o.ord_company = c.cmp_id   
...
GO
GRANT EXECUTE ON  [dbo].[WatchDog_CustomerInactivityOrdertoInvoice] TO [public]
GO
