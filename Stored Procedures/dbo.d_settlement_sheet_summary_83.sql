...
CREATE PROC [dbo].[d_settlement_sheet_summary_83](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary_83
 *
 * TYPE:
 * Stored Procedure
 *
 * DESCRIPTION:
 * copy of d_settlement_sheet_summary_new before it was
 * changed on PTS 39323 with currency added
 *
 * RETURNS:
 * See selection list.
 * 
 ...
 *
 * REVISION HISTORY:
 * 09/17/2007.01 PTS39323 - Vince Herman - copy from _new
...
CREATE TABLE #temp_pd(
	...
	currency_format		varchar(6)	null)
...
--vjh 39323
update #temp_pd 
	set currency_format = case 
		when mpp_payto = 'UNKNOWN' then mpp_currency
		else pto_currency
	end
	from manpowerprofile
	join payto on  pto_id = mpp_payto
	where asgn_type = 'DRV'
	and manpowerprofile.mpp_id = #temp_pd.asgn_id

update #temp_pd 
	set currency_format = pto_currency
	from payto 
	where pto_id = pyd_payto
	and asgn_type <> 'DRV'
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_83] TO [public]
GO
