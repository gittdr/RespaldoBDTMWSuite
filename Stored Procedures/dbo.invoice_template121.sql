...
create  procedure [dbo].[invoice_template121](@invoice_nbr int,@copies int)
as
/**
 * 
 * NAME:
 ...
 *
 * REVISION HISTORY:
 * 16/18/07 DPETE PTS 37966  modified from invoice_template63
 * 10/04/07 PTS 39514 EMK Retrieve first PUP for detail lines.
 * 12/20/07 PTS 40551 EMK Don't retrieve firt PUP for invoices that don't apply to LH
 * 09/10/08 PTS 43721 PMill - Retrieve all PUPs with freight details; add event code and name
 * 09/16/09 PTS 49058 SGB - Correct Sequence for DRP stops so that it lower than all accessorials
 * 03/24/2010 PTS 51469 SGB - Change mailto address to use cmp_mailto_zip instead of cmp_mailto 
*/
...
create table #invtemp_tbl (
	...
	,eventcode varchar(6) null  --43721
	,eventname varchar(50) null --43721
)
 ...
 SELECT  invoiceheader.ivh_invoicenumber,     
    ...
    ,isnull((select 'Y' from labelfile 
		where labeldefinition = 'distanceUnits' 
			and labelfile.abbr = ivd_unit
			and labelfile.abbr <> 'UNK'),'N')
    ,isnull(cht_primary,'N') cht_primary
    ,', '+isnull(scity.cty_state,'')+'   '+ isnull(scmp.cmp_zip,'')
    ,', '+isnull(ccity.cty_state,'')+'   '+ isnull(ccmp.cmp_zip,'')
    ,' '
	, event.evt_eventcode   --43721
	, eventcodetable.name  --43721
    FROM invoiceheader
	    join invoicedetail on invoiceheader.ivh_hdrnumber = invoicedetail.ivh_hdrnumber
		left outer join event on event.stp_number = invoicedetail.stp_number   --43721
		left outer join eventcodetable on eventcodetable.abbr = event.evt_eventcode  --43721
	    ...
	    left outer join labelfile on ivh_terms = labelfile.abbr and labeldefinition = 'CreditTerms'
   WHERE 
...
INSERT INTO #invtemp_tbl
 select
	...
	,fgt.fgt_description  --43721 ''  -- tmp.ivd_description    
	,tmp.cht_itemcode  
	,0 --tmp.ivd_quantity     
	,tmp.ivd_rate     
	,tmp.ivd_charge  
	...
	,tmp.ivd_glnum   
	,stp.stp_type -- 43721  tmp.ivd_type  
	,tmp.ivd_rateunit   
	,tmp.ivd_billto      
	,tmp.ivd_billto_name 
	,tmp.ivd_billto_addr  
	...
	,0  -- tmp.ivd_sequence     
	,tmp.ivd_refnum    
	,fgt.cmd_code  -- 43721 tmp.cmd_code     
	...
	,tmp.ivd_distance     
	,tmp.ivd_distunit   
	,fgt.fgt_weight -- 43721stp_weight  --tmp.ivd_wgt     
	,fgt.fgt_weightunit -- 43721 stp_weightunit -- tmp.ivd_wgtunit   
	,fgt.fgt_count -- 43721 stp.stp_count  -- tmp.ivd_count     
	,fgt.fgt_countunit -- 43721 stp.stp_countunit --tmp.ivd_countunit 
	,evt.evt_number  -- tmp.evt_number  
	,tmp.ivd_reftype     
	,fgt.fgt_volume -- 43721 stp.stp_volume  --tmp.ivd_volume    
	,fgt.fgt_volumeunit -- 43721 stp.stp_volumeunit --tmp.ivd_volunit     
	,stp.cmp_id -- 43721 tmp.ivd_orig_cmpid   
	,tmp.ivd_payrevenue 
	...
	,tmp.cht_basis 
	,tmp.cht_description 
	,fgt.fgt_description -- 43721 tmp.cmd_name 
	,tmp.cmp_altid  
	,tmp.ivh_showshipper 
	,tmp.ivh_showcons 
	,tmp.terms_name 
	,tmp.ivh_billto_addr3 
	,'' -- 43721 tmp.ivd_remark 
	,tmp.cmp_contact 
	,tmp.shipper_addr3 
	,tmp.CONSIGNEE_ADDR3 
	,tmp.ivh_charge
	...
	,tmp.billtostatezip
	, evt.evt_eventcode -- 43721
	, ect.name -- 43721
	from stops stp 
		join invoiceheader ivh on stp.ord_hdrnumber = ivh.ord_hdrnumber
		join event evt on evt.stp_number = stp.stp_number
		join eventcodetable ect on ect.abbr = evt.evt_eventcode -- 43721
		join freightdetail fgt on fgt.stp_number = evt.stp_number -- 43721 
	...
	where 
		...
		and ivh.ivh_applyto_definition = 'LH' --40551 EMK Only add PUP when printing LH invoices
...
Else   
	...
	set ivh_billto_name = isnull( company.cmp_mailto_name,''),  
		...
		ivh_billto_nmctst =   isnull(city.cty_name ,''), 
		billtostatezip =  ', '+isnull(city.cty_state,'')+'   '+ isnull(cmp_mailto_zip,'')   ,  -- PTS 51469 SGB 03/24/2010 change cmp_zip to cmp_mailto_zip
...
--43721 fix substring to get rid of '/' on city name
...
--43721 pmill We are now adding all PUPs with freight details
--   Need to make sure the sequence is adjusted for the added PUP rows.
  BEGIN
	update #invtemp_tbl
	set ivd_sequence = stp_sequence
	from stops
	where #invtemp_tbl.stp_number = stops.stp_number

/* --09/16/09 PTS 49058 SGB BEGIN Use the highest delivery line to determine the sequence for the rest of the details
	select @pupcount = count(*) from #invtemp_tbl 
						where ivd_type = 'PUP'
						and stp_number not in (select stp_number from #invtemp_tbl where ivd_type = 'DRP')
						*/
						
	select @pupcount = max(ivd_sequence) from #invtemp_tbl 
	where ivd_type IN ('DRP','PUP')
	--09/16/09 PTS 49058 SGB END 

	update #invtemp_tbl
	set ivd_sequence = ivd_sequence + @pupcount
 ...
 select @counter = @counter + 1  
  insert into #invtemp_tbl 
   select
	...
	from #invtemp_tbl  
	where copies = 1 
	AND COALESCE(eventcode,'') in ('LUL','LLD','CTR','')  --43721/45686    
 end   
...
select
	...
	,eventname -- 43721
from  #invtemp_tbl
	WHERE COALESCE(eventcode,'') in ('LUL','LLD','CTR','')  --43721/45686     

return @ret_value  
GO
GRANT EXECUTE ON  [dbo].[invoice_template121] TO [public]
GO
