...
CREATE PROC [dbo].[d_stlmnt_sheet_summary_53](
...
CREATE TABLE #temp_pd(
	...
	pyd_prorap				varchar(6) null, 
		...
		pyd_payto				varchar(12) null, ----- PTS 41646 JSwindell paydetail col is now varchar 12 
	pyt_itemcode			varchar(6) null, 
		--pyd_description			varchar(30) null, 
		pyd_description			varchar(75) null, ----- PTS 41646 JSwindell paydetail col is now varchar 75 
	...
	ord_startdate			datetime null,
		...
		ord_number				varchar(16) null,	----- PTS 41646 JSwindell Increase col to char 12 
	...
	pyd_refnumtype			varchar(6) null)	---- PTS 41646 JSwindell - add column to proc
...
SELECT pd.pyd_number,
	...
	NULL,
	0,
	ISNULL(tariffheaderstl.tar_reduction, 0),
	pyd_refnumtype								---- PTS 41646 JSwindell - add column to proc
   FROM paydetail pd  LEFT OUTER JOIN  tariffheaderstl  ON  pd.tar_tarriffnumber  = tariffheaderstl.tar_number ,
		#temp_pay tp
...
SET	address_lastfirst	= manpowerprofile.mpp_lastfirst,
	...
	mpp_misc4		= manpowerprofile.mpp_misc4
...
UPDATE #temp_pd
   SET ivh_quantity = ih.ivh_quantity
  FROM invoiceheader ih
 WHERE #temp_pd.ord_hdrnumber = ih.ord_hdrnumber AND
       #temp_pd.ord_hdrnumber > 0

--PTS32704 MBR 04/20/06
UPDATE #temp_pd
   SET ivh_quantity = id.ivd_quantity
  FROM invoicedetail id
 WHERE #temp_pd.ord_hdrnumber = id.ord_hdrnumber AND
       #temp_pd.ord_hdrnumber > 0 AND
       id.cht_itemcode = 'MIN' AND
       id.ivd_quantity > 0

UPDATE #temp_pd
   SET ivh_rate = ivd_rate
  FROM invoicedetail id
 WHERE #temp_pd.ord_hdrnumber = id.ord_hdrnumber AND
       id.ivd_type = 'SUB' AND
       #temp_pd.ord_hdrnumber > 0
    
--Calculate total loaded miles for all orders
SET @loaded_miles = 0
SET @min_ord = 0
WHILE 1=1
BEGIN
   SELECT @min_ord = MIN(DISTINCT ord_hdrnumber)
     FROM #temp_pd
    WHERE ord_hdrnumber > @min_ord AND
          ord_hdrnumber > 0
    
   IF @min_ord IS NULL
      BREAK
   
   SELECT @loaded = ISNULL(SUM(stp_lgh_mileage), 0)
     FROM stops
    WHERE ord_hdrnumber = @min_ord AND
          stp_loadstatus = 'LD'
   SET @loaded_miles = @loaded_miles + @loaded
   
END
UPDATE #temp_pd
   SET loaded_miles = @loaded_miles
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_sheet_summary_53] TO [public]
GO
