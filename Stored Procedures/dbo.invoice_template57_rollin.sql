...
GO
  ...
  CREATE PROCEDURE [dbo].[invoice_template57_rollin](@invoice_nbr   int,@copies int)  
AS  
...
03/26/03 DPETE PTS16739 add cmp_contact to return set  
11/27/07 PTS40188 jguo outer join conversion
*/  
...
SELECT ivh.ivh_invoicenumber,   
 ...
 --PTS# 23342 ILB 08/23/2004
 @varchar8 ivh_carrier,     
 --ivh_carrier,
 --END PTS# 23342 ILB 08/20/2004
 ...
 @varchar50 cmp_contact,
  ...
  INTO #invtemp_tbl   
  FROM invoicedetail ivd  LEFT OUTER JOIN  commodity cmd  ON  ivd.cmd_code  = cmd.cmd_code ,  --pts40188 outer join conversion
	 invoiceheader ivh,
	 chargetype cht    
...
--PTS# 23342 ILB 08/23/2004
SELECT @max_leg = MAX(LGH_NUMBER)
  FROM STOPS STP , #invtemp_tbl tmp_tbl 
 WHERE tmp_tbl.IVH_HDRNUMBER = @invoice_nbr AND
       STP.ORD_HDRNUMBER     = tmp_tbl.ORD_HDRNUMBER AND 
       STP.STP_NUMBER        = tmp_tbl.STP_NUMBER AND
       tmp_tbl.IVD_TYPE      = 'DRP'

UPDATE #invtemp_tbl
   SET ivh_driver  = assetasgn.asgn_id
  FROM assetassignment assetasgn
 WHERE lgh_number = @max_leg and
       assetasgn.asgn_type = 'DRV'

UPDATE #invtemp_tbl
   SET ivh_carrier  = assetasgn.asgn_id
  FROM assetassignment assetasgn
 WHERE lgh_number = @max_leg and
       assetasgn.asgn_type = 'CAR'
       
--END PTS# 23342 ILB 08/23/2004	
...
RETURN @ret_value 
GO
GRANT EXECUTE ON  [dbo].[invoice_template57_rollin] TO [public]
GO
