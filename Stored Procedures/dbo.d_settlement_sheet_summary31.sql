...
CREATE  PROC [dbo].[d_settlement_sheet_summary31](
 ...
 * 05/11/2015    - PTS89403 - vjh - details with zero payheader duplicated in ytd mileage totals
 *
 **/
...
CREATE TABLE #temp_pd(
	...
	pyh_issuedate datetime null,
        ytd_mileage_ld		float null,
	ytd_mileage_mt		float null)
...
CREATE TABLE #ytdbal (asgn_type	varchar (6) not null,
	...
	pyh_payperiod		datetime null,
	pyh_issuedate		datetime null,
	ytd_miles_ld		float null,
 	ytd_miles_mt		float null)

--Insert into the temp YTD balances table the assets from the temp pay details table
INSERT INTO #ytdbal
     SELECT DISTINCT asgn_type, asgn_id, 0, 0, 0, pyh_payperiod, pyh_issuedate, 0,0
       FROM #temp_pd
   ...
   SET ytdcomp = (SELECT SUM(ROUND(ISNULL(ph.pyh_totalcomp, 0), 2))
    ...
    ytdreimbrs = (SELECT SUM(ROUND(ISNULL(ph.pyh_totalreimbrs, 0), 2))
                  ...
                  WHERE ph.asgn_id = #ytdbal.asgn_id
                        ...
                        AND ph.pyh_payperiod < @payperiodend
                        AND ph.pyh_paystatus <> 'HLD') ,
    ytd_miles_ld = (SELECT SUM(ROUND(ISNULL(pd.pyd_quantity, 0), 2))
                   FROM paydetail pd
                  WHERE pd.asgn_id = #ytdbal.asgn_id
                        AND pd.asgn_type = #ytdbal.asgn_type
                        AND pd.pyh_payperiod >= '01/01/' + datename(yy, @payperiodend)
                        AND pd.pyh_payperiod < @payperiodend
						--AND pyh_number <> 0	--PTS89403 details with zero payheader number picked up in update below
                        AND pd.pyd_status <> 'HLD'
			AND pd.pyd_unit = 'MIL'
			AND pd.pyd_loadstate = 'LD') ,
    ytd_miles_mt = (SELECT SUM(ROUND(ISNULL(pd.pyd_quantity, 0), 2))
                   FROM paydetail pd
                  WHERE pd.asgn_id = #ytdbal.asgn_id
                        AND pd.asgn_type = #ytdbal.asgn_type
                        AND pd.pyh_payperiod >= '01/01/' + datename(yy, @payperiodend)
                        ...
                        AND pd.pyd_status <> 'HLD'
			AND pd.pyd_unit = 'MIL'
			AND pd.pyd_loadstate in ('UNLD','MT')) 
END 
ELSE 
BEGIN
	UPDATE #ytdbal
	SET	ytdcomp = ISNULL((SELECT SUM(ROUND(ph.pyh_totalcomp, 2))
		...
		ytdreimbrs = ISNULL((SELECT SUM(ROUND(ph.pyh_totalreimbrs, 2))
   		 ...
   		 ytd_miles_ld = (SELECT SUM(ROUND(ISNULL(pd.pyd_quantity, 0), 2))
				FROM paydetail pd
				WHERE pd.asgn_id = yb.asgn_id
						AND pd.asgn_type = yb.asgn_type
						AND pd.pyh_payperiod >= '01/01/' + datename(yy, isnull(yb.pyh_issuedate,yb.pyh_payperiod))
						AND pd.pyh_payperiod <= isnull(yb.pyh_issuedate,yb.pyh_payperiod)
						AND pyh_number <> 0	--PTS89403 details with zero payheader number picked up in update below
						AND pd.pyd_status <> 'HLD'
						AND pd.pyd_unit = 'MIL'
						AND pd.pyd_loadstate = 'LD') ,
...
UPDATE #ytdbal SET ytd_miles_ld = isnull(ytd_miles_ld,0), ytd_miles_mt = isnull(ytd_miles_mt,0)
...
UPDATE #ytdbal
   SET ytd_miles_ld = ytd_miles_ld + ISNULL((SELECT SUM(ROUND(tp.pyd_quantity, 2))
                                    FROM #temp_pd tp
                                   WHERE tp.asgn_id = yb.asgn_id
                                         AND tp.asgn_type = yb.asgn_type
                                         AND tp.pyd_status <> 'HLD'
					 AND pyh_number = 0 
					 AND tp.pyd_unit = 'MIL'
					 AND tp.pyd_loadstate = 'LD') , 0)
   FROM  #ytdbal yb

UPDATE #ytdbal
   SET ytd_miles_mt = ytd_miles_mt + ISNULL((SELECT SUM(ROUND(tp.pyd_quantity, 2))
                                    FROM #temp_pd tp
                                   WHERE tp.asgn_id = yb.asgn_id
                                         AND tp.asgn_type = yb.asgn_type
                                         AND tp.pyd_status <> 'HLD'
					 AND pyh_number = 0 
					 AND tp.pyd_unit = 'MIL'
					 AND tp.pyd_loadstate in ('UNLD', 'MT')) , 0)
   FROM  #ytdbal yb
...
SET pyh_totalcomp = yb.ytdcomp,
       ...
       ytd_mileage_ld = yb.ytd_miles_ld,
       ytd_mileage_mt = yb.ytd_miles_mt
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary31] TO [public]
GO
