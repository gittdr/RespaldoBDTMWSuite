...
CREATE  proc [dbo].[dx_create_order_from_stops_Rem_yunidad_Jr]  
	@validate char(1),
    @mov_number int,  
	...
	@lgh_extra_gps varchar(20),
	@trl_type3	varchar(6),
	@master_orden varchar(12),
	@ruta	varchar(30),
...
SELECT @operador  = ISNULL(@operador,'UNKNOWN')
SELECT @unidad	  = ISNULL(@unidad,'UNKNOWN')
SELECT @remolque1 = ISNULL(@remolque1,'UNKNOWN')
SELECT @remolque2 = ISNULL(@remolque2,'UNKNOWN')
Select @trl_type3 = Isnull(@trl_type3,'UNK')
Select @ruta	  = ISNULL(@ruta,'UNKNOWN')
...
--			 evt_trailer1	= @remolque1,	evt_trailer2 = @remolque2, 
			 evt_driver1	= @operador,	evt_tractor	 = @Unidad
	       WHERE stp_number = @next_stpnumber
      ELSE
	      UPDATE event
		 SET ord_hdrnumber = @ord_hdrnumber,
 ...
 INSERT INTO orderheader 
	...
	ord_unit, trl_type1, ord_trl_type3, ord_driver1, 			--18
    ...
    ord_editradingpartner, ord_origin_zip, ord_dest_zip, --31
    ord_revenue_pay, ord_booked_revtype1,ord_fromorder, ord_route)
VALUES ( @orderby_cmpid, @@ord_number, ISNULL(@ls_ord_customer,'UNKNOWN'), 	        	--1
	...
	@ord_billing_unit, @trailer_type, @trl_type3, @operador, 			--18
	'UNKNOWN', @unidad, @remolque1, 			--19
	...
	CASE @ord_rateby WHEN 'T' THEN ISNULL(@ord_charge,0) ELSE null END, @ord_revtype1,@master_orden,@ruta)
  ...
  IF @carrier <> 'UNKNOWN'
	BEGIN
	  ...
	  WHILE @evt_number is not null
		...
		END
	  UPDATE legheader
	  SET lgh_carrier = @carrier, lgh_outstatus = @dispatch_status, 
	  lgh_type1 = @lgh_type1, lgh_primary_trailer = @remolque1, lgh_primary_pup = @remolque2,
	  lgh_extrainfo1 = @lgh_extra_gps
	  WHERE mov_number = @mov_number
	  SELECT @retcode = @@error
	  IF @retcode<>0
    	    BEGIN
