...
SET ANSI_NULLS OFF
GO
create proc [dbo].[d_stlmnt_stops_svcustom_sp] (@stringparm varchar(13),
			@numberparm int,
...
--	LOR	PTS# 59051
  ...
  --WHERE ( stops.stp_city = city.cty_code ) and 
		...
		--( legheader.lgh_number = @lgh_number) and 
...
FROM 	stops
  ...
  WHERE legheader.lgh_number = @lgh_number and event.evt_sequence = 1 )
--	LOR
else
Insert into #stopstempmov (
				stp_event ,
...
--		( event.evt_sequence = 1 )) )
	FROM 	stops
		...
		left join legheader on stops.lgh_number = legheader.lgh_number
		left join eventcodetable on stops.stp_event = eventcodetable.abbr
	WHERE stops.mov_number = @numberparm  AND event.evt_sequence = 1 ) 
--	LOR
...
BEGIN
	...
	set		#stopstempmov.ivd_distance = (select sum(invoicedetail.ivd_distance)
											from 	invoicedetail
											where 	#stopstempmov.stp_number = invoicedetail.stp_number and --pts40186 jguo removed the left outer join from the the corelated query
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_stops_svcustom_sp] TO [public]
GO
