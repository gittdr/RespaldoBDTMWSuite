...
CREATE PROCEDURE [dbo].[MetricReportCard] 
(
...
AS
	SET NOCOUNT ON
	-- Example: MetricReportCard 'revpermile', 'week', 5
	...
	IF (@TimeFrame = 'year' OR @TimeFrame = 'yy' OR @TimeFrame = 'yyyy') SELECT @TimeFrame = 'year', @FirstDate = dbo.fnc_Metric_DateAdd('Alt01', 'yyyy', -@TimeUnits, @Date)
	ELSE IF (@TimeFrame = 'quarter' OR @TimeFrame = 'qq' OR @TimeFrame = 'q') SELECT @TimeFrame = 'quarter', @FirstDate = dbo.fnc_Metric_DateAdd('Alt01', 'quarter', -@TimeUnits, @Date)
	ELSE IF (@TimeFrame = 'month' OR @TimeFrame = 'mm' OR @TimeFrame = 'm') SELECT @TimeFrame = 'month', @FirstDate = dbo.fnc_Metric_DateAdd('Alt01', 'month', -@TimeUnits, @Date)
	ELSE IF (@TimeFrame = 'week' OR @TimeFrame = 'wk' OR @TimeFrame = 'ww') SELECT @TimeFrame = 'week', @FirstDate = DATEADD(week, -@TimeUnits, @Date)
	ELSE IF (@TimeFrame = 'day' OR @TimeFrame = 'dd' OR @TimeFrame = 'd') SELECT @TimeFrame = 'day', @FirstDate = DATEADD(day, -@TimeUnits, @Date) 
	...
	BEGIN
		...
		BEGIN
			SELECT @FirstDateInPeriod = DATEADD(day, 1-DATEPART(dw, @DateCur), @DateCur)
			
-- SELECT DATEADD(day, 6, DATEADD(day, 1-DATEPART(dw, '20110424'), '20110424'))
			SELECT @LastDateInPeriod = DATEADD(day, 6, DATEADD(day, 1-DATEPART(dw, @FirstDateInPeriod), @FirstDateInPeriod))
			IF (@LastDateInPeriod > @Date) SET @LastDateInPeriod = @Date
-- select Date = @Date, DateCur = @DateCur, FirstDateInPeriod = @FirstDateInPeriod, LastDateInPeriod = @LastDateInPeriod
		...
		BEGIN
			...
			BEGIN
				IF @Style = ''
					SELECT @MetricValue = ThisMTD, @ThisAve = ThisMTD FROM MetricDetail (NOLOCK) WHERE metriccode = @metriccode AND PlainDate = @LastDateInPeriod 
				ELSE
				BEGIN
			...
			BEGIN
				IF @Style = ''
					SELECT @MetricValue = ThisQTD, @ThisAve = ThisQTD FROM MetricDetail (NOLOCK) WHERE metriccode = @metriccode AND PlainDate = @LastDateInPeriod 
				ELSE
				BEGIN
			...
			BEGIN
				IF @Style = ''
					SELECT @MetricValue = ThisYTD, @ThisAve = ThisYTD FROM MetricDetail (NOLOCK) WHERE metriccode = @metriccode AND PlainDate = @LastDateInPeriod 
				ELSE
				BEGIN
...
-- select @MetricValue, @LastDateInPeriod
		...
		ELSE
		BEGIN
			IF (@TimeFrame = 'year') SELECT @DateCur = dbo.fnc_Metric_DateAdd('Alt01', 'yyyy', -1, @DateCur)
			ELSE IF (@TimeFrame = 'quarter') SELECT @DateCur = dbo.fnc_Metric_DateAdd('Alt01', 'q', -1, @DateCur)
			ELSE IF (@TimeFrame = 'month') SELECT @DateCur = dbo.fnc_Metric_DateAdd('Alt01', 'm', -1, @DateCur)
...
GO
GRANT EXECUTE ON  [dbo].[MetricReportCard] TO [public]
GO
