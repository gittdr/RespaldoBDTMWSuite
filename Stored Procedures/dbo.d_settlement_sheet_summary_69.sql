...
CREATE  PROC [dbo].[d_settlement_sheet_summary_69](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary_69
 *
 * TYPE:
 * [StoredProcedure]
 *
 ...
 * 12/27/2006	34748	SLM		Customized settlementsheet format 69 based upon format 5
 * 11/02/2007.01 ? PTS40116 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 12/8/2011	58495	TMEZE	Added two new columns to get the mileage summary for non mileage based paydetails
 *								when pyt_basis = lgh and pyd_unit <> mil
**/
...
-- PTS 29515 -- BL (end)
	pyt_basisunit		varchar(6) null) --PTS 34738
...
-- PTS 34738 (begin) 
update #temp_pd
set pyt_basisunit = p.pyt_basisunit
from #temp_pd tp, paytype p
where tp.pyt_itemcode = p.pyt_itemcode
-- PTS 34738 (end) 
...
-- PTS 29515 -- BL (end)
-- PTS 58495.s
	IsNull((select sum(stpl.stp_lgh_mileage) from stops stpl where stpl.lgh_number = tp.lgh_number and stpl.stp_loadstatus = 'LD' and
	tp.lgh_number > 0 and tp.pyd_unit <> 'MIL' and tp.pyt_basis = 'LGH'),0) loaded_mileage,  
	IsNull((select sum(stpe.stp_lgh_mileage) from stops stpe where stpe.lgh_number = tp.lgh_number and stpe.stp_loadstatus <> 'LD' and
	tp.lgh_number > 0 and tp.pyd_unit <> 'MIL' and tp.pyt_basis = 'LGH'),0) empty_mileage  
-- PTS 58495.e
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_69] TO [public]
GO
