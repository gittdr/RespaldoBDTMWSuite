...
--  exec sp_datosfactura_montos_JR

CREATE PROCEDURE [dbo].[sp_datosfactura_montos_JR_PORMB]
...
DECLARE @TTmontosfactura TABLE(
		...
		fact_IVA		float Null,
		fact_Ret		float Null,
		fact_IVA_det	float Null,
		fact_Ret_det	float Null,
		IVA_calc		float Null,
		Ret_calc		float Null)
...
from invoiceheader where ivh_mbnumber = 26895
...
Select fact_orden,fact_invoice, fact_total,fact_IVA,fact_IVA_det,IVA_calc,fact_Ret,fact_Ret_det,Ret_calc from @TTmontosfactura
