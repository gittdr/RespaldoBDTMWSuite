...
create procedure [dbo].[invoice_template31]
		(@invoice_nbr	int
		,@copies		int)
as
...
Revision History:
	...
	-----------	---------------	-------	-------	-----------------------------------------
	06/19/2003	Vern Jewett		18758	(none)	Original, copied from invoice_template11.
...
declare	@temp_name   		varchar(30)
		...
		,@counter    		int
		,@ret_value			int	
		,@ls_cht_itemcode_1	varchar(6)
		,@ls_cht_itemcode_2	varchar(6)
		,@ls_cht_itemcode_3	varchar(6)
		,@ls_cht_itemcode_4	varchar(6)
...
select	@ls_cht_itemcode_1 = gi_string1
		,@ls_cht_itemcode_2 = gi_string2
		,@ls_cht_itemcode_3 = gi_string3
		,@ls_cht_itemcode_4 = gi_string4
  from	generalinfo
  where	gi_name = 'FuelSurchargeChargeTypes'

update	#invtemp_tbl
  set	subtotal_section = 2
  where	cht_itemcode in (@ls_cht_itemcode_1, @ls_cht_itemcode_2, @ls_cht_itemcode_3, 
							@ls_cht_itemcode_4)
...
BEGIN
 select @next_seq = min(ref_sequence)
  ...
  where ref_type = 'BL#' AND
	...
	ref_tablekey = @ord_hdr AND
	ref_sequence > @last_seq
...
-- PTS 20247 -- BL (end)
 ...
 Select @minref = ref_number 
  ...
  where ref_type = 'BL#' AND
	ref_table = 'orderheader' AND
	ref_tablekey = @ord_hdr AND
	ref_sequence = @next_seq

 if @i > 5 BREAK
 
 SELECT @sql = 'update #invtemp_tbl set ord_bol'+ convert(varchar, @i) +' = ''' + @minref + ''''
...
return @ret_value
GO
GRANT EXECUTE ON  [dbo].[invoice_template31] TO [public]
GO
