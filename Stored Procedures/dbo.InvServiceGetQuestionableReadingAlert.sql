...
SET ANSI_NULLS ON
GO
create proc [dbo].[InvServiceGetQuestionableReadingAlert] @cmp_revtype1 varchar(6),
...
select cmp_id, forecast_bucket, cmd_code, SafeFill, [ShutDown], PriorReadingDate, PriorReading, 
  ...
  abs((PriorReading + LastReadingDeliveries - LastReading) - LastReadingEstSales) as [Difference],  
  RunOutDate, Status
  from #readings
  where Questionable = 'Y'
order by runoutdate

drop table #readings
drop table #companies
GO
GRANT EXECUTE ON  [dbo].[InvServiceGetQuestionableReadingAlert] TO [public]
GO
