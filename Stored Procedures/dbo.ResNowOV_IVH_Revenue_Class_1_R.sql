...
CREATE PROCEDURE [dbo].[ResNowOV_IVH_Revenue_Class_1_R]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
			SELECT 	[RevType1Abbr] = IH.ivh_revtype1
			,[RevType1] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType1' and abbr = IH.ivh_revtype1) 
		...
		BEGIN
			...
			FROM invoiceheader IH (NOLOCK) 
			WHERE IH.ivh_deliverydate BETWEEN @DateStart and @DateEnd
			AND IH.ivh_revtype1 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			ORDER BY IH.ivh_revtype1 
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_IVH_Revenue_Class_1_R] TO [public]
GO
