...
SET ANSI_NULLS ON
GO
--Drop Proc sp_TTSTMWAllocateRevVsPay_SSRS

CREATE procedure [dbo].[sp_TTSTMWAllocateRevVsPay_GRAL]
	(
...
trc_fleet,
flota = (select [name] from  vista_flotas where amestring  =  trc_fleet) , --'TRC Fleet',
mpp_status = (select mpp_status from manpowerprofile where mpp_id = lgh_driver1),
...
--delete LegHeaderSummary_SSRS;

Insert into bi.dbo.legdriver 
...
lgh_startdate,
lgh_Enddate,
day(CompletionDateOnly),
year(CompletionDateOnly),
DATENAME(month,CompletionDateOnly),
WeekOfTheYear,
mpp_type1,
mpp_type2,
mpp_type3,
lgh_class4,
lgh_tractor,
'Orden:'+cast(ord_hdrnumber as varchar) +'|Cliente:'+FirstBillTo + '|Operador:'+ lgh_driver1+'Lider:'+ cast(mpp_teamleader as varchar)  as drvtr,
(select mpp_status from manpowerprofile where mpp_id = lgh_driver1),
TotLHChargeForMove,
AllocatedLHChargeForLgh,
TotMlsLegHeader,
AllChargesMove,
...
lgh_tractor,
trc_fleet,
flota = (select [name] from  vista_flotas where amestring  =  trc_fleet),
FirstBillTo,
RevPerMileAllCharges
from #LegHeaderSummary
where DATENAME(month,CompletionDateOnly) is not null
