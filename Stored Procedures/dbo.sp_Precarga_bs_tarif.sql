...
CREATE proc [dbo].[sp_Precarga_bs_tarif] (@modo varchar(5) = NULL)
...
(select cty_nmstct from company where cmp_id = ord_consignee) as CiudadDestino,
 ...
 '' as ErrorNoteType
  from orderheader o 
 where  
isnull(tar_number,0)  <= 0 and ord_status = 'CMP'
--and ord_completiondate >= '2020-01-01' 
and o.ord_billto NOT IN( 'TDRQUERE','SAE') and ord_status = 'CMP' 
and ord_completiondate < CONVERT(varchar, getdate(), 101)   and ord_invoicestatus = 'AVL'
--and ord_hdrnumber in (select ord_hdrnumber from orderheader with(Nolock) where isnull(tar_number,0)  = 0 and ord_completiondate >= '2020-01-01' and ord_status = 'CMP' and ord_completiondate < CONVERT(varchar, getdate(), 101)   and ord_invoicestatus = 'AVL')
and ord_hdrnumber not in (select ord_hdrnumber from invoiceheader where ivh_invoicestatus <> 'XFR' and ord_hdrnumber in  (select ord_hdrnumber from orderheader with(Nolock) where isnull(tar_number,0)  = 0 and ord_completiondate >= '2020-01-01' and ord_status = 'CMP' and ord_completiondate < CONVERT(varchar, getdate(), 101)   and ord_invoicestatus = 'AVL') )
