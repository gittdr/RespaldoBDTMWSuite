...
Create Proc [dbo].[d_spillreport01_sp] @srpid int 
As
/**
 * DESCRIPTION:
 *
 ...
 *
 * REVISION HISTORY:
	SR 17782 DPETE created 12/22/03
...
--  opdcount = (Select count(*) From propertydamage Where propertydamage.srp_id = @srpid),
  ...
  spllawenfdeptcitystatezip = IsNull(cty_name+', ','')+IsNull(spl_lawenfdeptState,'')+IsNull('    '+spl_LawEnfDeptZip,''),
  ...
  From 
    spill LEFT OUTER JOIN city ON  city.cty_code = spill.spl_LawEnfDeptCity
   ,tractorprofile
   , trailerprofile tr1
...
GO
GRANT EXECUTE ON  [dbo].[d_spillreport01_sp] TO [public]
GO
