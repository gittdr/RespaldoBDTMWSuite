...
--drop proc pod_format04_REC_JR
-- Exec pod_format04_REC_JR 604367 , 686028

CREATE PROCEDURE [dbo].[pod_format04_LAZARO_JR] (@ord_hdrnumber 	Varchar(10), @lgh		INTEGER) 
AS
DECLARE @temp_name   	VARCHAR(30) ,  
		...
		@ls_carrier	VARCHAR(8),
		@ls_tractor	VARCHAR(10),
		@stp_numberdiv Int,
		@V_cmp_recoleccion VARCHAR(30) ,
		@V_cty_recoleccion VARCHAR(40) 
...
--	ord_bookedby		VARCHAR(20) NULL,
	...
	nombre_carrier		VARCHAR(50) NULL,
	...
	cmp_recoleccion		VARCHAR(30) Null,
	cty_recoleccion		VARCHAR(40) Null,
	chassis1		VARCHAR(13) Null,
	chassis2		VARCHAR(13) Null
)
...
UPDATE #temp 
   SET driver = lgh_driver1, 
	   ...
	   chassis1 = lgh_chassis,
	   chassis2 = lgh_chassis2
  FROM legheader 
 WHERE lgh_number = @lgh


---- Aqui revisa si la unidad es nula, si es asi toma los datos del carrier
select  @ls_carrier = lgh_carrier, @ls_tractor =lgh_tractor
from legheader 
where lgh_number = @lgh

IF  @ls_carrier <> 'UNKNOWN'  and @ls_tractor ='UNKNOWN'
	BEGIN
		UPDATE #temp 
		   SET driver_lastname = lgh_driver_name, 
			   tractor = lgh_carrier_truck, 
			   trailer = lgh_trailernumber
...
WHERE #temp.ord_billto in( 'SCHENKER','MEASIA','CHROBINS','WERNER', 'JANEL','CARYSERV',
'UNIVRSAL','VERQRO','DMARTIN','NESTLE','KATNAT','TECMEX','SAE')

---- cuando se trata de un permisionario el que se llevara la carga
--IF  @ls_carrier <> 'UNKNOWN' 
--	begin
--		UPDATE #temp
--		Set billto_credit_terms = '', rate = 0.00, 
--		charge = 0.00, ord_accessorial_chrg = 0.00, toll_charge = 0.00, montoenletra = '',
...
-- se valida que tenga en los stops el evento DIV

If Exists ( Select count(*) from  stops where stp_event = 'DIV' and ord_hdrnumber = @ord_hdrnumber )

	BEGIN
		select @stp_numberdiv	=	min(stp_number)
		from  stops 
		where stp_event = 'DIV' and lgh_number = @lgh 

		select @V_cmp_recoleccion	=	cmp_name, 
				@V_cty_recoleccion	=	stp_address
		from  stops 
		where stp_number = @stp_numberdiv

		UPDATE #temp 
		   SET	cmp_recoleccion = @V_cmp_recoleccion,
				cty_recoleccion = @V_cty_recoleccion
		  FROM orderheader 
		 WHERE #temp.ord_number = @ord_hdrnumber
