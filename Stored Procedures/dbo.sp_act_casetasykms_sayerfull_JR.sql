...
IF @v_movimiento > 0
	...
	begin tran
				...
				select @V_li_i = 1
				select @v_kms_total = 0
				select @v_casetas_total = 0
				...
				BEGIN
					...
					Begin
						...
						execute dbo.miles_between_CasetasJR   @type = 15, @o_cmp = @v_cmp_orig_ant, @d_cmp = @v_cmp_dest, @o_cty = @V_cty_orig_ant, @d_cty = @V_cty_dest, @o_zip = '0', @d_zip = '0', @haztype = 0,@casetas = @v_casetas output, @kilometros = @v_kms output
						...
						select @v_kms_total= @v_kms_total + @v_kms
						select @v_casetas_total	= @v_casetas_total + @v_casetas
					end

					SET @V_li_i = @V_li_i + 1
				END
			update orderheader set ord_totalmiles = @v_kms_total, ord_toll_cost = @v_casetas_total  where ord_hdrnumber = @P_NumOrden
				commit tran
	End -- movimiento > 0
Return 0
END
