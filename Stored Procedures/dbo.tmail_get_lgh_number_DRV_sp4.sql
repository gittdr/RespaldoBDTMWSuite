...
CREATE PROCEDURE [dbo].[tmail_get_lgh_number_DRV_sp4]
...
Fuel Mode makes two modifications: first the last open shift will be found for the driver.
Only legs on that shift are eligible.  Second, when determining the status of each of those 
legs, the last stop will be disregarded if it is an EBT or EMT.
*/
...
-- If no Driver is supplied, raise error and exit
IF ISNULL(@DriverID, '') = ''
  BEGIN
	SET @sT_1 = 'TMWERR: {Load AssignDrv} No Driver Specified.'
	RAISERROR (@sT_1,16,-1)
	RETURN 1
  END
...
IF ISNULL(@move, '') > ''
	...
	INSERT #tmp1 SELECT lgh_number, lgh_outstatus, lgh_tm_status, lgh_startdate, dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode)
				 ...
				 WHERE mov_number = CONVERT(int, @move)
				   AND (lgh_Driver1 = @DriverID OR lgh_Driver2 = @DriverID)
...
ELSE
	...
	IF ISNULL(@order_number, '') > ''
		...
		-- Insert all legs for this Driver and ord_hdrnumber into the temp table
		INSERT #tmp1 SELECT DISTINCT l.lgh_number, l.lgh_outstatus, l.lgh_tm_status, l.lgh_startdate, dbo.tmail_get_eff_outstatus(l.lgh_number, @FuelMode)
					 ...
					 WHERE l.mov_number = s.mov_Number
				  	   ...
				  	   AND (l.lgh_Driver1 = @DriverID OR l.lgh_Driver2 = @DriverID)
	...
	ELSE
	  BEGIN	-- No order number
		INSERT INTO @Candidates (lgh) 
			...
			WHERE asgn_type = 'DRV' and asgn_id = @DriverID and asgn_status = 'STD'
		...
		INSERT INTO @Candidates (lgh) 
			...
			from Assetassignment 
			WHERE asgn_type = 'DRV' and asgn_id = @DriverID and asgn_status = 'CMP'
				   AND asgn_enddate > @FuelStartDate
				   AND asgn_date < @FuelEndDate
			ORDER BY asgn_date desc
		...
		IF (@iReturnAllLegs = 0) OR (@AllowStarted <> 0 AND @iReturnAllLegs <> 0)
			INSERT #tmp1 
				...
				WHERE (lgh_driver1 = @DriverID OR lgh_driver2 = @DriverID) AND dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode)  = 'STD' and 
				  ISNULL(@TMStatus, ''''+ISNULL(lgh_tm_status, '')+'''') LIKE '%'''+ISNULL(lgh_tm_status, '')+'''%'
				   AND lgh_enddate > @FuelStartDate
				   AND lgh_startdate < @FuelEndDate
				  ORDER BY lgh_startdate

		-- Get earliest or all DSP
		INSERT #tmp1 
			SELECT TOP 1 lgh_number, lgh_outstatus, lgh_tm_status, lgh_startdate, dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode) eff_Status
			FROM legheader (NOLOCK)
			WHERE (lgh_driver1 = @DriverID OR lgh_driver2 = @DriverID) AND lgh_outstatus = 'DSP' and 
			  ISNULL(@TMStatus, ''''+ISNULL(lgh_tm_status, '')+'''') LIKE '%'''+ISNULL(lgh_tm_status, '')+'''%'
				   AND lgh_enddate > @FuelStartDate
				   AND lgh_startdate < @FuelEndDate
			  ORDER BY lgh_startdate
		...
		IF (@iReturnAllLegs = 0) OR (@NoPlanned = 0 AND @iReturnAllLegs <> 0)
			INSERT #tmp1 
				SELECT TOP 1 lgh_number, lgh_outstatus, lgh_tm_status, lgh_startdate, dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode) eff_Status
				FROM legheader (NOLOCK)
				WHERE (lgh_driver1 = @DriverID OR lgh_driver2 = @DriverID) AND lgh_outstatus = 'PLN' and 
				  ISNULL(@TMStatus, ''''+ISNULL(lgh_tm_status, '')+'''') LIKE '%'''+ISNULL(lgh_tm_status, '')+'''%'
				   AND lgh_enddate > @FuelStartDate
				   AND lgh_startdate < @FuelEndDate
				  ORDER BY lgh_startdate
		...
		IF (@iReturnAllLegs = 0) OR (@NoCompleted = 0 AND @iReturnAllLegs <> 0)
			INSERT #tmp1 
				SELECT TOP 1 lgh_number, lgh_outstatus, lgh_tm_status, lgh_startdate, dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode) eff_Status
				FROM legheader (NOLOCK) INNER JOIN @Candidates c ON lgh_number = lgh
				WHERE (lgh_driver1 = @DriverID OR lgh_driver2 = @DriverID) AND dbo.tmail_get_eff_outstatus(lgh_number, @FuelMode)  like 'CMP%' and 
				  ISNULL(@TMStatus, ''''+ISNULL(lgh_tm_status, '')+'''') LIKE '%'''+ISNULL(lgh_tm_status, '')+'''%'
				   AND lgh_enddate > @FuelStartDate
				   AND lgh_startdate < @FuelEndDate
...
GO
GRANT EXECUTE ON  [dbo].[tmail_get_lgh_number_DRV_sp4] TO [public]
GO
