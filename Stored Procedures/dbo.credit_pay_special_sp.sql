...
CREATE PROC [dbo].[credit_pay_special_sp](@ivh_hdrnumber int, @ivh_definition varchar(6), @user_id varchar(20), @ivh_credit_hdrnumber int, @ord_hdrnumber int)
...
If @ivh_definition = 'SUPL' OR
	(select count(*) from invoicedetail i, chargetype c where i.ivh_hdrnumber = @ivh_hdrnumber and c.cht_itemcode = i.cht_itemcode and cht_basis = 'SHP') <= 0
	...
	select pyd_number,
		...
		pyd_ivh_hdrnumber 
	from paydetail p
	where pyd_ivh_hdrnumber = @ivh_hdrnumber
Else
	insert into #pay_temp
	select pyd_number,
		pyh_number,
		...
		pyd_ivh_hdrnumber 
	from paydetail p
	where pyd_ivh_hdrnumber = @ivh_hdrnumber or (ord_hdrnumber = @ord_hdrnumber and IsNull(pyd_ivh_hdrnumber, 0) = 0)
...
select pyd_number,
	...
	pyd_offsetpay_number,
	'Y',
	@ivh_credit_hdrnumber
from #pay_temp

GO
GRANT EXECUTE ON  [dbo].[credit_pay_special_sp] TO [public]
GO
