...
CREATE PROC [dbo].[d_settlement_sheet_summary_18](
...
UPDATE #temp_pd
SET    bal_damage = IsNull((SELECT SUM(ROUND(pd.pyd_amount, 2))
			  FROM paydetail pd, payheader ph
			  WHERE pd.asgn_id = #temp_pd.asgn_id
				AND pd.asgn_type = #temp_pd.asgn_type
				AND pd.pyd_status <> 'HLD'
				and pd.pyt_itemcode = 'DAMAGE'
				and ph.pyh_pyhnumber = pd.pyh_number
				AND ph.pyh_paystatus <> 'HLD'
				and ph.asgn_id = pd.asgn_id
				and ph.asgn_type = pd.asgn_type
...
SELECT pyd_number, 
	...
	ivd_number, 
	pyd_prorap, 
	case pyd_payto when 'UNKNOWN' then isnull(pyh_payto,'UNKNOWN') else pyd_payto end,
	pyt_itemcode,
	pyd_description,
	pyr_ratecode, 
	pyd_quantity, 
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_18] TO [public]
GO
