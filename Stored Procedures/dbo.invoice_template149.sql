...
CREATE  procedure [dbo].[invoice_template149](@invoice_nbr  int,@copies int)  
as  
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET   
 ...
 SELECT  invoiceheader.ivh_invoicenumber,     
	...
	@refnums stop_refnums,
	@refnums freight_refnums,
	(case evt_eventcode
	when 'LLD' then 'PICKUP'
	when 'LUL' then 'DELIVERY'
	else evt_eventcode
	end )evt_eventcode ,
    ...
    FROM chargetype  RIGHT OUTER JOIN  invoicedetail  ON  chargetype.cht_itemcode  = invoicedetail.cht_itemcode   
			...
			left outer join event on invoicedetail.evt_number = event.evt_number
...
SELECT freightdetail.stp_number,freightdetail.cmd_code,fgt_description,fgt_weight,fgt_quantity,
       fgt_volume, fgt_count,STP_weightunit, (case evt_eventcode
	when 'LLD' then 'PICKUP'
	when 'LUL' then 'DELIVERY'
	else evt_eventcode
	end )evt_eventcode 
...
JOIN EVENT ON (EVENT.STP_NUMBER = STOPS.STP_NUMBER AND EVENT.evt_sequence = 1)
-- WHERE stp_number = @stp_number
WHERE stp_type = 'PUP' and STOPS.ord_hdrnumber = @ord_hdrnumber
/*
 WHERE freightdetail.stp_number IN (select distinct(stp_number) 
                        from stops
                       where stp_type = 'PUP' and
                             ord_hdrnumber = @ord_hdrnumber)
  */                   

ORDER BY STP_sequence desc
...
FETCH NEXT FROM fgt_cursor INTO @stp_no,@cmd_code,@fgt_desc,@fgt_wgt,
                                @fgt_quantity,@fgt_vol,@fgt_cnt,@fgt_wgtunit, @eventcode
...
select @stp_no = min (stp_number) from #invtemp_tbl where isnull (stp_number,0) > 0
...
begin
   ...
   begin
	SET @tmp = ''
        SELECT @tmp =  ISNULL(ref_type, '') + ':' + ISNULL(ref_number, '') + ' '
          FROM referencenumber 
         WHERE ref_tablekey = @stp_no and ref_table = 'stops' and ref_sequence = @i
        
        if len(@tmp) > 1
           set @refnums = @refnums + @tmp
        set @i = @i + 1
   end
	update #invtemp_tbl set stop_refnums = @refnums where @stp_no = stp_number

   select @stp_no = min (stp_number) from #invtemp_tbl where stp_number > @stp_no
end


select @fgt_no = min (fgt_number) from #invtemp_tbl where isnull (fgt_number,0) > 0 AND evt_eventcode = 'DELIVERY'
...
begin
   ...
   begin
	SET @tmp = ''
        SELECT @tmp =  ISNULL(ref_type, '') + ':' + ISNULL(ref_number, '') + ' '
          FROM referencenumber 
         WHERE ref_tablekey = @fgt_no and ref_table = 'freightdetail' and ref_sequence = @i
        
        if len(@tmp) > 1
           set @refnums = @refnums + @tmp
        set @i = @i + 1
   end
	update #invtemp_tbl set freight_refnums = @refnums where @fgt_no = fgt_number
   select @fgt_no = min (fgt_number) from #invtemp_tbl where fgt_number > @fgt_no AND evt_eventcode = 'DELIVERY'
end
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template149] TO [public]
GO
