...
CREATE PROCEDURE [dbo].[ResNowOV_R1_Revenue_Class_1]
...
*/
	...
	BEGIN
		...
		SELECT 	ord_RevType1 AS [RevClass Abbr], 
			(SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType1' and abbr = ord_RevType1) AS [Revenue Class],
	...
	BEGIN
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = ord_RevType1, 
				[ItemDescription] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType1' and abbr = ord_RevType1),
				[Count] = COUNT(*),
			...
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			GROUP BY ord_RevType1 
			ORDER BY COUNT(*) DESC
		SET ROWCOUNT 0
	END

GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_Revenue_Class_1] TO [public]
GO
