...
SET ANSI_NULLS ON
GO
create procedure [dbo].[SSRS_AllocateRevVsPay]
	(
		@DateType char(1) = 'D',		-- B=Bill, 
		...
		@tractorid			varchar (max),
		@carrierid			varchar (max),
		@teamleaderid			varchar (max),
...
--Author: Brian O'Sickey
--********************************************************************
--Purpose: AllocateRevVsPay replacement of original Trip Segment 
--Analysis from Management Reports
...
exec SSRS_AllocateRevVsPay
		...
		@OnlyShipperID 	='',
		@OnlyConsigneeID ='',
		@InvoiceStatusList='.All',
		@IncludeDeadHeadTripsYN ='N',
		
		@IncludeNonInvoicedLoadsYN ='Y',
...
set @LateDate = dateadd(ss,86399,@LateDate)
...
if @InvoiceStatusList='.All'
BEGIN
	set @InvoiceStatusList=',,'
END
ELSE
BEGIN
...
--			       ISNULL((SELECT sum(ISNULL( ivd_charge, 0 ) )
				...
				--<TTS!*!TMW><Begin><SQLVersion=2000+>
				ISNULL((SELECT convert(money,sum(IsNull(dbo.TMWSSRS_fnc_convertcharge(ivd_charge,ivh_currency,'Revenue',ivd_number,ivh_currencydate,ivh_shipdate,ivh_deliverydate,ivh_billdate,ivh_revenue_date,ivh_xferdate,default,ivh_printdate,default,default,default),0)))
				--<TTS!*!TMW><End><SQLVersion=2000+>                                
    		        FROM 
				Invoiceheader  
...
-- NEW FIELDS
	...
	NumberOfSplitsOnMove= 
		...
		From legheader l2  where L2.Mov_number=L.Mov_number
		),
	NumberOfOrdersOnLeg= 
		(Select count(distinct ord_hdrnumber) 
		From Orderheader  where L.ord_hdrnumber=Orderheader.ord_hdrnumber
		),
	NumberOfInvoicesOnMove=
		(Select count(distinct ivh_invoicenumber)
		From Invoiceheader  where L.Mov_number= Invoiceheader.mov_number
		),
...
Update  #LegHeaderSummary
	Set FirstBillTo=
		...
		from 
			orderheader 
		where 
			not exists (select * from invoiceheader  where invoiceheader.ord_hdrnumber = orderheader.ord_hdrnumber)
			and
...
--			IsNull(sum(ord_charge),0.00)
			...
			--<TTS!*!TMW><Begin><SQLVersion=2000+>
			IsNull(convert(money,sum(IsNull(dbo.TMWSSRS_fnc_convertcharge(ord_charge,ord_currency,'Revenue',ord_hdrnumber,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0.00))),0.00)
			--<TTS!*!TMW><End><SQLVersion=2000+> 		
		From orderheader 
		where
			...
			and
			not exists (select I.ord_hdrnumber from invoiceheader I  where I.ord_hdrnumber = orderheader.ord_hdrnumber) 
		)
	--From 
		--orderheader
...
--				IsNull(sum(ivd_charge),0.00)
				...
				--<TTS!*!TMW><Begin><SQLVersion=2000+>
				IsNull(convert(money,sum(IsNull(dbo.TMWSSRS_fnc_convertcharge(ivd_charge,ivh_currency,'Revenue',ivd_number,ivh_currencydate,ivh_shipdate,ivh_deliverydate,ivh_billdate,ivh_revenue_date,ivh_xferdate,default,ivh_printdate,default,default,default),0.00))),0.00)
				--<TTS!*!TMW><End><SQLVersion=2000+>  			
			FROM 
				Invoiceheader ,
				invoicedetail , 
...
--				IsNull(sum(ivd_charge),0.00)
				...
				--<TTS!*!TMW><Begin><SQLVersion=2000+>
				IsNull(convert(money,sum(IsNull(dbo.TMWSSRS_fnc_convertcharge(ivd_charge,ord_currency,'Revenue',ivd_number,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0.00))),0.00)
				--<TTS!*!TMW><End><SQLVersion=2000+> 
...
GO
GRANT EXECUTE ON  [dbo].[SSRS_AllocateRevVsPay] TO [public]
GO
