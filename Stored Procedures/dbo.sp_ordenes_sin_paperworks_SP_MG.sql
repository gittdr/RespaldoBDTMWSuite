...
--exec sp_ordenes_sin_paperworks_SP_MG @Client = 'DHLEXPRE'

CREATE  PROCEDURE [dbo].[sp_ordenes_sin_paperworks_SP_MG] @Client nvarchar(MAX)
AS
DECLARE	
	@V_orden		Integer,
	@V_billto		varchar(8),
...
BEGIN --1 Principal
	...
	From orderheader where ord_bookdate > '2015-01-01' and ord_status not in ('MAS','CAN') and ord_billto = @Client
	--From orderheader where ord_status = 'CMP' and ord_invoicestatus = 'AVL' and year(ord_startdate) >= 2014 and ord_billto in (@Client)
	...
	-- Se declara un curso para ir leyendo la tabla de paso
		...
		select @V_pworden	=	count(distinct(abbr)) FROM BillDoctypes, paperwork where  cmp_id = @V_billto and abbr = bdt_doctype and ord_hdrnumber = @V_orden  AND IsNull(bdt_inv_required,'Y') = 'Y' and lgh_number >0
