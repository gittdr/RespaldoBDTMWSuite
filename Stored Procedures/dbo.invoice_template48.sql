...
create procedure [dbo].[invoice_template48](@invoice_nbr  	int,  @copies		int)
as
 ...
 * 04/03/2009 PTS46549 pmill conversion issue for rolled_amount
 * 04/23/2009 PTS46549 pmill change to float so that rolled_amount it not rounded
 * 09/16/2009 PTS 49045 SGB Do not get stops For Misc Invoices 
 * 05/21/2010 PTS 52160 SGB Use GI setting InvoiceFormat48DefaultRefto get the default ref to find the header ref number
 * 03/21/2011 PTS 53735 TMEZE Modify Origin/Destination to use the zip code from the company file versus the city file.
 *
 **/
...
-- PTS 29461 -- BL (end)
	@ref_type varchar(6),
	@ref_number varchar(30) -- PTS 52160 SGB
...
select @ret_value = 1
--PTS 52160 SGB
select @ref_type = gi_string1 
from generalinfo
where gi_name = 'InvoiceFormat48DefaultRef'
Select @ref_number = NULL
--PTS 52160 SGB
...
--bcy	
	...
	--  convert(money,0.00) rolled_amount,  --46549 pmill issue with conversion
	convert(float,0.00) rolled_amount,
	null num_fgt_det,
...
select 	@ord = (select MAX(ord_hdrnumber) from	#invtemp_tbl)

if @ord > 0 
BEGIN
	select 	@cont = ref_number
	from	referencenumber 
	where	ref_type = 'CONT1'
		and ref_tablekey = @ord 
		and ref_table = 'orderheader'
	
	select 	@cont = @cont + '  ' + ref_number
	from	referencenumber 
	where	ref_type = 'CONT2'
		and ref_tablekey = @ord 
		and ref_table = 'orderheader'

	If len(@cont) > 0 
	BEGIN
		select 	@cont = '   Container(s): '+ rtrim(@cont)
	END
END	
...
set num_fgt_det = (select count(distinct(fgt_description))
		   ...
		   where s.ord_hdrnumber = tmp.ord_hdrnumber  
		     ...
		     and s.stp_type = 'DRP'
		     and s.ord_hdrnumber <> 0) -- 09/16/09 SGB PTS 49045 Do not get stops For Misc Invoices 

IF (SELECT COUNT(*) FROM #invtemp_tbl2) > 0
UPDATE #invtemp_tbl 
   SET #invtemp_tbl.ivd_charge = #invtemp_tbl.ivd_charge + #invtemp_tbl2.charge,
	rolled_amount = #invtemp_tbl2.charge
...
If (Select isnull(ivh_reftype,'UNK') from invoiceheader where ivh_hdrnumber = @invoice_nbr ) = @ref_type
	BEGIN
		-- If the invoiceheader has the correct type use the ref number from the header
		Select @ref_number = max(ivh_ref_number)
		from #invtemp_tbl
	...
	ELSE
	BEGIN
	-- Check to see if there is a reference number for the default type
		Select @ref_number = max(ref_number)
			from	referencenumber 
			where	ref_type = @ref_type
				and ref_tablekey = @ord 
				and ref_table = 'orderheader'
			
			If 	@ref_number is NULL
			BEGIN
			-- If not one by order check by invoiceheader 
				Select @ref_number = max(ref_number)
				from	referencenumber 
				where	ref_type = @ref_type
					and ref_tablekey = @invoice_nbr 
					and ref_table = 'invoiceheader'
			END	
			
			If @ref_number is not NULL
			-- Only need to run this if was not on header
				BEGIN
					UPDATE #invtemp_tbl
					SET ivh_ref_number = @ref_number
				END	
	END 
...
-- PTS 29440 -- BL (start)
--         ivh_remark,   
	   ivh_remark = isNull(ivh_remark,'') + '  ' + @cont,
-- PTS 29440 -- BL (end)
         ivh_driver,   
         ivh_tractor,   
         ivh_trailer,   
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template48] TO [public]
GO
