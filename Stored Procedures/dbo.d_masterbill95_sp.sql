...
CREATE        PROC [dbo].[d_masterbill95_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
...
/*
* NAME:
 * dbo.d_masterbill95_sp
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * This procedure was a copy of masterbill format 44 
 * It was created for Transwood  on PTS 34920
 *
 *
 * REVISION HISTORY:
 * 09/17/2007 PTS 39442 S. Glenn Behra Added and and ivh_rateby = 'T' so roll into only occurs for rate by total
 * PTS 47675 SGB Added logic so that miscellanious invoices would be included in MB
 *	changed hard join to Orderheader to LEFT OUTER JOIN also added check on ref numbers to include ord_hdrnumber <> 0
 * Rearranged order of tables in FROM clause
 * PTS 47760 SGB added logic to allow for UNKNOWN  cty_nmstct for the UNKNOWN company for MISC invoices
 * 
 *
 **/
...
CREATE TABLE #masterbill_temp (		ord_hdrnumber int,
		...
		ref_bl varchar(38) NULL, 
		ref_po varchar(38) NULL)
...
if UPPER(@reprintflag) = 'REPRINT' 
  BEGIN
	...
	-- dpete for LOR pts4785 provide for mailto override of billto
	 ivh_shipto_address = 
	    CASE
	...
	and (c3.cht_primary = 'Y' or c3.cht_rollintolh = 1) ) else ivd_charge End,  
		...
		refbl.ref_type + ': ' + refbl.ref_number,
		refpo.ref_type + ': ' + refpo.ref_number

--PTS 47675 changed hard join to Orderheader to LEFT OUTER JOIN also added check on ref numbers to include ord_hdrnumber <> 0
-- Rearranged order of tables in FROM clause
    FROM 	invoiceheader 
				LEFT OUTER JOIN referencenumber refbl ON  refbl.ord_hdrnumber = invoiceheader.ord_hdrnumber  and refbl.ref_type = 'BL#' and refbl.ord_hdrnumber <> 0
						 AND refbl.ref_sequence = (SELECT MIN(ref_sequence)
							FROM referencenumber r
							WHERE r.ord_hdrnumber = invoiceheader.ord_hdrnumber
							AND r.ref_type = 'BL#'
							and refbl.ord_hdrnumber <> 0)
				LEFT OUTER JOIN referencenumber refpo ON refpo.ord_hdrnumber = invoiceheader.ord_hdrnumber and refpo.ref_type = 'PO#' and refbl.ord_hdrnumber <> 0
						AND refpo.ref_sequence = (SELECT MIN(ref_sequence)
							FROM referencenumber r
							WHERE r.ord_hdrnumber = invoiceheader.ord_hdrnumber
							AND r.ref_type = 'PO#'
							and refbl.ord_hdrnumber <> 0)
   ...
   WHERE		 ( invoiceheader.ivh_mbnumber = @mbnumber )
		...
		-- PTS 47675 allow invoiceheader = 0 for misc invoices
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill95_sp] TO [public]
GO
