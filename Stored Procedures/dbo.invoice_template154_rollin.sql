...
GO
  ...
  create procedure [dbo].[invoice_template154_rollin](@invoice_nbr   int,@copies  int)  
as  
--Derived from invoice_template2
 ...
 SELECT  invoiceheader.ivh_invoicenumber,     
  ...
  @temp_nmstct dest_nmctst,  
		...
		--invoiceheader.ivh_remark,     
          CASE LEFT(invoiceheader.ivh_remark, 1)
			WHEN '"' THEN 
				CASE RIGHT(invoiceheader.ivh_remark, 1)
					WHEN '"' THEN 
						SUBSTRING(invoiceheader.ivh_remark, 2, LEN(invoiceheader.ivh_remark) - 2)
					ELSE
						invoiceheader.ivh_remark
				END
			ELSE
...
@varchar30 as ref_number_special,
@varchar30 as ref_number_bol,
@temp_addr billto_extrainfo_tab1_col1,
@temp_addr billto_extrainfo_tab1_col2,
@tempdatetime	detention_appointmentdate,
@tempdatetime	detention_arrivaldate,
@tempdatetime	detention_departuredate,
isnull(invoicedetail.ivd_remark, '') as ivd_remark,
'N' currentinvoice
 ...
 invoiceheader.ivh_hdrnumber = @invoice_nbr  
  and     (IsNUll(invoicedetail.cht_rollintolh,0) = 0) 
...
update #invtemp_tbl  
set ref_number_special = ref_number
FROM #invtemp_tbl
	INNER JOIN referencenumber as ref_special ON (ref_special.ref_tablekey = #invtemp_tbl.ord_hdrnumber AND ref_special.ref_table = 'orderheader' AND ref_special.ref_type = 'SID')

update #invtemp_tbl  
set ref_number_bol = ref_number
FROM #invtemp_tbl
	INNER JOIN referencenumber as ref_bol ON (ref_bol.ref_tablekey = #invtemp_tbl.ord_hdrnumber AND ref_bol.ref_table = 'orderheader' AND ref_bol.ref_type = 'BL#')

update #invtemp_tbl  
set	billto_extrainfo_tab1_col1 = eid.COL_DATA
FROM	EXTRA_INFO_DATA eid
WHERE	eid.EXTRA_ID = 5
		AND eid.tab_id = 1
		AND eid.COL_ROW = 1
		AND eid.COL_ID = 1
		AND eid.TABLE_KEY = #invtemp_tbl.ivh_billto


update #invtemp_tbl  
set	billto_extrainfo_tab1_col2 = eid.COL_DATA
FROM	EXTRA_INFO_DATA eid
WHERE	eid.EXTRA_ID = 5
		AND eid.tab_id = 1
		AND eid.COL_ROW = 1
...
SET ROWCOUNT 1
update #invtemp_tbl  
SET	detention_appointmentdate = stp.stp_schdtlatest,
	detention_arrivaldate= stp.stp_arrivaldate, 
	detention_departuredate = stp.stp_departuredate,
	stop_nmctst = substring(cmp.cty_nmstct,1, (charindex('/', cmp.cty_nmstct + '/')) - 1)
FROM	stops stp 
		INNER JOIN #invtemp_tbl ivd ON (stp.ord_hdrnumber = ivd.ord_hdrnumber and stp.cmp_id = ivd.cmp_id)
		LEFT OUTER JOIN company cmp on stp.cmp_id = cmp.cmp_id
WHERE	ivd.cht_itemcode = 'DETPRO'
		and ivd.cmp_id <> 'UNKNOWN'
SET ROWCOUNT 0

--force fuel to bottom
update	#invtemp_tbl  
SET		ivd_sequence = 999
WHERE	cht_itemcode like 'FSC%'

--PTS 52025 JJF 20100506
/*
UPDATE	#invtemp_tbl  
SET		currentinvoice = 'Y'
WHERE	NOT EXISTS(	SELECT	*
					FROM	invoiceheader ivh
					WHERE	ivh.ord_hdrnumber = #invtemp_tbl.ord_hdrnumber
							AND #invtemp_tbl.ivh_invoicenumber < ivh.ivh_invoicenumber 
							AND ivh.ivh_definition NOT IN ('SUPL')
				)
		OR EXISTS(	SELECT *
					FROM	invoiceheader ivh
					WHERE	ivh.ivh_hdrnumber = #invtemp_tbl.ivh_hdrnumber
							AND ivh.ivh_definition IN ('SUPL')
				)
*/
--PTS 52025 JJF 20100506	
...
while @counter <>  @copies  
 ...
 ref_number_special,
 ref_number_bol,
	billto_extrainfo_tab1_col1,
	billto_extrainfo_tab1_col2,
	detention_appointmentdate,
	detention_arrivaldate,
	detention_departuredate,
	ivd_remark
...
--from #invtemp_tbl  
  ...
  dest_nmctst,  
         ...
         ivd_number,     
         stp_number,     
         isnull(ivd_description, '') as ivd_description,     
         cht_itemcode,     
         ivd_quantity,     
         ivd_rate,     
         ivd_charge,     
  ...
  stop_addr,  
  stop_addr2,  
  isnull(stop_nmctst, '') as stop_nmctst,  
         ivd_distance,     
         ivd_distunit,     
         ivd_wgt,     
         ivd_wgtunit,     
...
--  @counter,  
  ...
  terms_name,
	...
	ivd_remark,
	currentinvoice,
	0 c_total_charge_forallinvoices
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template154_rollin] TO [public]
GO
