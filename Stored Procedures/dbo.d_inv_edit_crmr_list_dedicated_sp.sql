...
CREATE PROCEDURE [dbo].[d_inv_edit_crmr_list_dedicated_sp] (	
@dbh_id				int
)
AS
...
/*
* Change Log
* PTS 57682 Adding columns added by PTS 53500 and 57140
*/
...
(SELECT SUM(ISNULL(ivd_charge, 0))
		  ...
		  WHERE invoicedetail.ivh_hdrnumber = C.ivh_hdrnumber
		   AND ISNULL(invoicedetail.ivd_allocated_ivd_number, 0) = 0) ivh_invoicecharge,
(SELECT SUM(ISNULL(ivd_charge, 0))
		   FROM invoicedetail
		  WHERE invoicedetail.ivh_hdrnumber = C.ivh_hdrnumber
		   AND ISNULL(invoicedetail.ivd_allocated_ivd_number, 0) > 0) ivh_allocated,
		 C.ivh_billto, 
		 C.ivh_shipper, 
		 C.ivh_consignee,
		 C.ivh_deliverydate,
		...
		0	,
		C.ivh_definition,
		C.ivh_billdate,		-- 57682 RE:PTS 53500 SGB
		'N'		-- 57682 RE:PTS 57140 NQIAO	
  FROM invoiceheader C 
  WHERE C.ivh_cmrbill_link in (select ivh_hdrnumber from #invoiceheaders)
	and  isnull(C.dbh_id,0) <> @dbh_id
...
notes_count,
ivh_definition,
retrieval_date, -- 57682 RE:PTS 53500 SGB 
allocate_flag	-- 57682 RE:PTS 57140 NQIAO
from #results         
        
       
       
GO
GRANT EXECUTE ON  [dbo].[d_inv_edit_crmr_list_dedicated_sp] TO [public]
GO
