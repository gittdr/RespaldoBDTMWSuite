...
create procedure [dbo].[invoice_template96](@invoice_nbr   int,@copies  int)  
as  
/**
 * NAME:
 * dbo.invoice_template96
 *
 * TYPE:
 * StoredProcedure
 *
...
FROM invoiceheader inner join invoicedetail on invoicedetail.ivh_hdrnumber = invoiceheader.ivh_hdrnumber
	left join chargetype on invoicedetail.cht_itemcode = chargetype.cht_itemcode 
	left join commodity  on invoicedetail.cmd_code = commodity.cmd_code
...
from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.ivh_originpoint
...
from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.ivh_destpoint 
...
from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.ivh_showshipper  
...
from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.ivh_showcons 
...
from #invtemp_tbl inner join company on company.cmp_id = #invtemp_tbl.cmp_id  
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template96] TO [public]
GO
