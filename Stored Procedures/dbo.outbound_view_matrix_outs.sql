...
CREATE PROCEDURE [dbo].[outbound_view_matrix_outs]
...
SELECT
	...
	Cliente = a.ord_billto,	
    Origen =  (a.o_cmpname),
	Destino = (a.d_cmpname),
	Remolque = isnull(lgh_primary_trailer,''),
    Proyecto = replace((SELECT name FROM labelfile WHERE  labeldefinition = 'RevType3' and abbr = (select ord_revtype3 from orderheader where orderheader.ord_hdrnumber = (A.ord_hdrnumber))),'BAJIO','ABIERTO'),
	---Se basa en la region de ORIGEN--------------------------------------------------------------------------------------------------------------------
	Region =  isnull((select rgh_name from regionheader where rgh_id = (select cty_region1 from city c where c.cty_code = a.o_city  )   ),'UNKNOWN'),
    Tractor = a.evt_tractor,
	Operador = replace(replace(a.evt_driver1_id ,'UNKNOWN', cast(lgh_number as varchar)),' ','BLANCO'),
	Status = 'OUT',
	LghStatus = lgh_outstatus,
	---Se basa en la fecha de inicio de leg---------------------------------------------------------------------------------------------------------------
	Fecha= a.lgh_startdate,
	---Se basa en la ciudad de ORIGEN---------------------------------------------------------------------------------------------------------------------
	Ciudad = a.o_ctyname
   
	
FROM @ttbl1 a
where (select ord_status from orderheader where orderheader.ord_hdrnumber = (A.ord_hdrnumber)) <> 'CAN'
