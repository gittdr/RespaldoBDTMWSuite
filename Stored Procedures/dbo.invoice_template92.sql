...
--invoice_template92 5941  ,1

Create procedure [dbo].[invoice_template92](@p_invoice_nbr  int,@p_copies	int)
as

/**
 * 
 * NAME:
 * dbo.invoice_template92
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * 12/5/2002    - PTS16314 - DPETE       -use GI settings to control terms and linehaul restricitons on mail to
 * 04/12/2006   - PTS32463 - ILB        - New invoice format for Truck Load services
 **/
...
declare	@v_temp_name   varchar(30),
	...
	@v_ret_value   int,
        ...
        @v_ordhdrnumber int,
        @v_min_sequence int,
        @v_refnumber   varchar(30),
        @v_airbill_count int,
        @v_oceanbill_count int,
	@v_companyloc varchar(200),
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET 
 ...
 SELECT  invoiceheader.ivh_invoicenumber,   
	...
	@v_temp_altid cmp_altid, 
         CONVERT(FLOAT, NULL) ord_length, 
         CONVERT(FLOAT, NULL) ord_width, 
         CONVERT(FLOAT, NULL) ord_height, 
         CONVERT(VARCHAR(255), '') serial_numbers,
	ivh_hideshipperaddr,
	ivh_hideconsignaddr,
	...
	IsNull(ivh_charge,0.0) ivh_charge,
        @v_ref_number ref_deppt,
        @v_ref_number ref_despt,
        @v_ref_number ref_awb_obl_1,
        @v_ref_number ref_awb_obl_2, 
        @v_reftype    reftype_awb_obl_1,
        @v_reftype    reftype_awb_obl_2,
        @v_companyloc  company_loc ,
        @v_companyname company_name
    into #invtemp_tbl
   ...
   WHERE --(invoicedetail.ivh_hdrnumber = invoiceheader.ivh_hdrnumber ) and
	...
	-- ( invoiceheader.ivh_hdrnumber between @p_invoice_no_lo and @p_invoice_no_hi) AND
  -- 	 ( @p_invoice_status  in ('ALL', invoiceheader.ivh_invoicestatus)) and
	-- ( @p_revtype1 in('UNK', invoiceheader.ivh_revtype1)) and
	-- ( @p_revtype2 in('UNK', invoiceheader.ivh_revtype2)) and  			
   --      ( @p_revtype3 in('UNK', invoiceheader.ivh_revtype3)) and  
 --        ( @p_revtype4 in('UNK', invoiceheader.ivh_revtype4)) and
	-- ( @p_billto in ('UNKNOWN',invoiceheader.ivh_billto)) and
	-- ( @p_shipper in ('UNKNOWN', invoiceheader.ivh_shipper)) and
	-- ( @p_consignee in ('UNKNOWN',invoiceheader.ivh_consignee)) and
	-- (invoiceheader.ivh_shipdate between @p_shipdate1 and @p_shipdate2 ) and
    --     (invoiceheader.ivh_deliverydate between @p_deldate1 and @p_deldate2) and
	-- ((invoiceheader.ivh_billdate between @p_billdate1 and @p_billdate2) or
...
UPDATE #invtemp_tbl
 ...
 WHERE ivs_invoicedatawindow = 'd_inv_format92'

UPDATE #invtemp_tbl
   SET company_name = ivs_logocompanyname
  FROM invoiceselection
 WHERE ivs_invoicedatawindow = 'd_inv_format92'
...
update #invtemp_tbl 
   set ord_width = (select isnull(fgt_width,0) 
		      from freightdetail
		     where freightdetail.stp_number = (select max(stops.stp_number) 
					                from stops , #invtemp_tbl inv
				                       where stops.ord_hdrnumber = inv.ord_hdrnumber and
                                                             stops.cmd_code =  inv.cmd_code and
                                                             stops.ord_hdrnumber <> 0) and			 
			  freightdetail.fgt_sequence = (select max(fgt_sequence)
                                    			  from freightdetail
				   			 where freightdetail.stp_number =(select max(stops.stp_number) 
								     			    from stops , #invtemp_tbl inv
				  				    			   where stops.ord_hdrnumber = inv.ord_hdrnumber and         
       ...
       ord_length = (select MAX(isnull(fgt_length,0))
		      from freightdetail
	    	     where freightdetail.stp_number = (select max(stops.stp_number) 
			                                 from stops , #invtemp_tbl inv
			                                where stops.ord_hdrnumber = inv.ord_hdrnumber and
       ...
       ord_height = (select MAX(isnull(fgt_height,0))
		      from freightdetail
		     where freightdetail.stp_number = (select max(stops.stp_number) 
			                                 from stops , #invtemp_tbl inv
			                                where stops.ord_hdrnumber = inv.ord_hdrnumber and
...
--ILB PTS# 32463 04/13/2006
--Deppt ref type
Select @v_min_sequence = 0
Select @v_refnumber = ''

select @v_ordhdrnumber = min(ord_hdrnumber)
  from #invtemp_TBL

select @v_min_sequence = min(ref_sequence)
  from referencenumber,#invtemp_tbl
 where referencenumber.ref_tablekey = @v_ordhdrnumber and
       referencenumber.ref_table = 'orderheader' and
       referencenumber.ref_type = 'deppt'

select @v_refnumber = ref_number
  from referencenumber
 where ref_sequence = @v_min_sequence and
       referencenumber.ref_table = 'orderheader' and
       referencenumber.ref_type = 'deppt'

Update #invtemp_tbl
   set ref_deppt = @v_refnumber
...
select @v_min_sequence = min(ref_sequence)
 ...
 where referencenumber.ref_tablekey = @v_ordhdrnumber and
       referencenumber.ref_table = 'orderheader' and
       referencenumber.ref_type = 'despt'
...
Update #invtemp_tbl
   set ref_despt = @v_refnumber


--MAWB, HAWB REF TYPE
Select @v_airbill_count = count(*)
  from referencenumber
 where referencenumber.ref_table = 'orderheader' and
       referencenumber.ref_type IN ('MAWB','HAWB') and
       referencenumber.ref_tablekey = @v_ordhdrnumber

IF @v_airbill_count > 0
  BEGIN
	Select @v_min_sequence = 0
	Select @v_refnumber = ''
        Select @v_reftype = ''
	...
	Update #invtemp_tbl
           set ref_awb_obl_1 = @v_refnumber,
               reftype_awb_obl_1 = @v_reftype
	...
	Update #invtemp_tbl
           set ref_awb_obl_2 = @v_refnumber,
               reftype_awb_obl_2 = @v_reftype
  END

--CNTR,OBL REF TYPE
Select @v_oceanbill_count = count(*)
  from referencenumber
 where referencenumber.ref_table = 'orderheader' and
       referencenumber.ref_type IN ('CNTR','OBL')  and
       referencenumber.ref_tablekey = @v_ordhdrnumber 

IF @v_oceanbill_count > 0
  BEGIN
	Select @v_min_sequence = 0
	Select @v_refnumber = ''
	Select @v_reftype = ''
...
select ivh_invoicenumber, 
	 ...
	 --vmj1+	@p_counter is constant for all rows!
	 copies,
--	 @p_counter,
	 --vmj1-
	...
	ivh_showcons,
        ivh_charge,
        ISNULL(ref_deppt, '') ref_deppt,
        ISNULL(ref_despt,'') ref_despt,
        ISNULL(ref_awb_obl_1,'') ref_awb_obl_1,
        ISNULL(ref_awb_obl_2,'')ref_awb_obl_2,
        ISNULL(reftype_awb_obl_1,'')reftype_awb_obl_1,
        ISNULL(reftype_awb_obl_2,'')reftype_awb_obl_2,
        company_loc,
	company_name
from #invtemp_tbl

/* SET RET VALUE TO @p_@p_ERROR IF ONE HAS OCCURED. */
IF @@ERROR != 0 select @v_ret_value = @@ERROR 
return @v_ret_value
GO
GRANT EXECUTE ON  [dbo].[invoice_template92] TO [public]
GO
