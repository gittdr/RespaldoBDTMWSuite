...
CREATE PROC [dbo].[d_masterbill04_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	ivh_consignee_zip =  ISNULL(cmp3.cmp_zip,''),
		...
		@copy,
		ivd.cmp_id cmp_id
	FROM  invoicedetail ivd  LEFT OUTER JOIN  stops stp  ON  ivd.stp_number  = stp.stp_number   LEFT OUTER JOIN  commodity cmd  ON  ivd.cmd_code  = cmd.cmd_code ,
		 invoiceheader,
		 company cmp1,
		 company cmp2,
		 company cmp3,
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill04_sp] TO [public]
GO
