...
CREATE PROC [dbo].[d_masterbill133_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8),     
 ...
 *
 * REVISION HISTORY:
 * 12/17/08 PTS 45265 copy of d_masterbill20_sp for new format 133 
 *
 **/
...
-- month runs from 7 am on the 1st thru 6:59 AM on the first of the subsequent month    
 ...
 Insert Into #tax_temp    
  Select        sum(isnull(ivd_charge,0)) tax_amount,IsNull(ivd_description,cht_description) cht_description    
  From          #masterbill_tempx    
  Where         cht_basis = 'tax'or cht_itemcode = 'QST'    
  group by      IsNull(ivd_description,cht_description)  
  order by      IsNull(ivd_description,cht_description)    
...
--Get the appropriate GST number for the company associated with the orders
 ...
 Drop Table #tax_temp 
GO
GRANT EXECUTE ON  [dbo].[d_masterbill133_sp] TO [public]
GO
