...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[tmail_IssueGeocodeRequest2Alt]
	@TMFormId int, 
...
SET NOCOUNT ON
	...
	IF (@v_cmp_id = 'UNKNOWN')
		BEGIN
			SELECT @v_city_code = stp_city, @v_state = stp_state, @v_zip = stp_zipcode, @v_address = stp_address, @v_GeoCodeRequested = stp_GeoCodeRequested FROM stops WHERE stp_number = @v_stp_number
			IF ISNULL(@v_address,'') = '' 
				BEGIN		-- Need to go against city table
					SELECT @v_address = '', @v_city = cty_name, @v_state = ISNULL(cty_state, 'UN'), @v_zip = cty_zip, @v_GeoCodeRequested = cty_GeoCodeRequested FROM city WHERE cty_code = @v_city_code
	...
	ELSE  -- Look at company table
		BEGIN
			SELECT @v_address = cmp_address1, @v_city_code = cmp_city, @v_zip = cmp_zip, @v_state = cmp_state, @v_GeoCodeRequested = cmp_GeoCodeRequested FROM company WHERE cmp_id = @v_cmp_id
			IF ISNULL(@v_address,'') = ''		-- Validate that we have a good address
				BEGIN
					SELECT @v_city_code = stp_city, @v_state = stp_state, @v_zip = stp_zipcode, @v_address = stp_address, @v_GeoCodeRequested = stp_GeoCodeRequested FROM stops WHERE stp_number = @v_stp_number
					IF ISNULL(@v_address,'') = '' 
						BEGIN		-- Need to go against city table
							SELECT @v_address = '', @v_city = cty_name, @v_state = cty_state, @v_zip = cty_zip, @v_GeoCodeRequested = cty_GeoCodeRequested FROM city WHERE cty_code = @v_city_code
							IF (ISNULL(@v_GeoCodeRequested,'') = '')
								SET @v_IssueRequest = 3				-- Set cty_GeoCodeRequested field on city table
						END
					ELSE		-- Need to get city name from code
			...
			ELSE	-- Found all info on Company Table
				BEGIN
					SELECT @v_city = cty_name FROM city WHERE cty_code = @v_city_code			-- Get City name from cty_code
...
IF (@v_IssueRequest > 0)
	...
	END
GO
GRANT EXECUTE ON  [dbo].[tmail_IssueGeocodeRequest2Alt] TO [public]
GO
