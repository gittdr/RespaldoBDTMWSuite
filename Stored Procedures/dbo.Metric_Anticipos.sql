...
CREATE  PROCEDURE [dbo].[Metric_Anticipos]
	(
...
-- cost
		,SelectedPay = IsNull(dbo.fnc_TMWRN_Anticipos('Segment',default,default,L.mov_number,default,L.lgh_number,@IncludePayTypeList,@ExcludePayTypeList,@PreTaxYN,default),0.00)
--- miles
		...
		begin
			...
			Insert into @LegList
				(
					ord_hdrnumber,OrderNumber,MoveNumber,LegNumber,OrderedBy,BillTo,BillToName,Shipper,ShipperName,ShipperLocation,ord_origincity
					,ShipDate,Consignee,ConsigneeName,ConsigneeLocation,ord_destcity,DeliveryDate,MoveStartDate,LegStartDate,LegEndDate,MoveEndDate
					,DriverID,Tractor,Trailer,RevType1,RevType2,RevType3,RevType4,SelectedRevenue,SelectedPay,TravelMiles,LoadedMiles,EmptyMiles
...
*/
	...
	If @ShowDetail = 1 and @Numerator = 'Cost'
		BEGIN
			SELECT KAM=RevType1
...
Else If @ShowDetail = 2 and @Numerator = 'Cost'
		BEGIN
...
Else If @ShowDetail = 3 and @Numerator = 'Cost'
		BEGIN
...
Else If @ShowDetail = 4 and @Numerator = 'Cost'
		BEGIN
...
Else If @ShowDetail = 5 and @Numerator = 'Cost'
		BEGIN
			SELECT DriverID
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
...
Else If @ShowDetail =6 and @Numerator = 'Cost'
		BEGIN
			SELECT Tractor
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
...
Else If @ShowDetail = 8 and @Numerator = 'Cost'
		BEGIN
			SELECT Lider
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
...
Else If @ShowDetail = 9 and @Numerator = 'Cost'
		BEGIN
...
Else If @ShowDetail = 10 and @Numerator = 'Cost'
		BEGIN
			SELECT Region
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
...
Else If @ShowDetail = 11 and @Numerator = 'Cost'
		BEGIN
			SELECT BillToName
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
...
Else If @ShowDetail = 12 and @Numerator = 'Cost'
		BEGIN
			SELECT Flota
			,'$' + dbo.fnc_TMWRN_FormatNumbers(Sum(SelectedPay),0) as Costo
			,dbo.fnc_TMWRN_FormatNumbers(Sum(LoadCount),2) as CuentaCargas
