...
-- =============================================
CREATE PROCEDURE [dbo].[sp_ConvoyCorreoClientePatron] (@accion int)
...
BEGIN
	...
	select ISNULL((select TOP 1 ref_number from referencenumber where REF_TYPE = 'BL#' AND ord_hdrnumber in (select ord_hdrnumber from orderheader oh where lg.ord_hdrnumber = ord_hdrnumber)),lg.ord_hdrnumber)
			...
			where oh.ord_billto = 'WERGLOBA'  and oh.ord_status in ('STD') and lgh_outstatus <> ('CMP')  AND lgh_carrier = 'UNKNOWN' and tp.trc_licnum is not null 
...
cast(((select ckc_longseconds from checkcall where ckc_number =( select max(ckc_number) from checkcall where ckc_tractor = lg.lgh_tractor))/3600.00)*-1 as varchar) +'">Ir a la ubicacion </a></p>'
			...
			,@subject = 'Viaje Tequila Patrón: ' +ISNULL((select TOP 1 ref_number from referencenumber where REF_TYPE = 'BL#' AND ord_hdrnumber in (select ord_hdrnumber from orderheader oh where lg.ord_hdrnumber = ord_hdrnumber)),lg.ord_hdrnumber)
			from legheader lg
			inner join orderheader oh on oh.ord_hdrnumber = lg.ord_hdrnumber
			inner join tractorprofile tp on tp.trc_number = lg.lgh_tractor
			where oh.ord_billto = 'WERGLOBA'  and oh.ord_status in ('STD') and lgh_outstatus <> ('CMP')  AND lgh_carrier = 'UNKNOWN' and tp.trc_licnum is not null and (DATEADD(second, DATEDIFF(second, GETDATE(), GETUTCDATE()), (select ckc_date from checkcall where ckc_number =( select max(ckc_number) from checkcall where ckc_tractor = lg.lgh_tractor)))) is not null
			...
			--and oh.ord_completiondate >= cast(GETDATE()-14 as date)
						...
						EXEC msdb.dbo.sp_send_dbmail  
							...
							@recipients = 'jferrer@tdr.com.mx;monitoreo@tequilapatron.com;jguzman@tequilapatron.com;amariscal@tequilapatron.com;etejeda@werner.com;evperez@werner.com;vmendiburu@werner.com;sacnorte@tdr.com.mx',  
							@body = @Body,
							@body_format='HTML',
							@subject = @subject ; 
