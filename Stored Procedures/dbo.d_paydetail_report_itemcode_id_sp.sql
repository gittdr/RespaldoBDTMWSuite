...
CREATE   PROC [dbo].[d_paydetail_report_itemcode_id_sp] 
...
/**
 * DESCRIPTION:
 *   Created to get driver itemcode_id paydetail for settlement detail report  
...
CREATE TABLE #itemcode_id_paydetail_temp  (
...
-- Get paydetail info
INSERT INTO #itemcode_id_paydetail_temp
...
-- PTS 32226 -- BL (end)   (31363)
	...
	and paydetail.pyh_number not in 
		...
		from #itemcode_id_paydetail_temp)
-- PTS 25416 -- BL (end)
...
--PTS 48237 - DJM
	if @resourcetypeonleg = 'N'
		Begin
			...
			IF @driver_type1 <> 'UNK'
				delete #itemcode_id_paydetail_temp from manpowerprofile tp where asgn_type = 'DRV' and asgn_id = tp.mpp_id and mpp_type1 <> @driver_type1

			IF @driver_type2 <> 'UNK'
				delete #itemcode_id_paydetail_temp from manpowerprofile tp where asgn_type = 'DRV' and asgn_id = tp.mpp_id and mpp_type2 <> @driver_type2

			IF @driver_type3 <> 'UNK'
				delete #itemcode_id_paydetail_temp from manpowerprofile tp where asgn_type = 'DRV' and asgn_id = tp.mpp_id and mpp_type3 <> @driver_type3
			
			IF @driver_type4 <> 'UNK'
				delete #itemcode_id_paydetail_temp from manpowerprofile tp where asgn_type = 'DRV' and asgn_id = tp.mpp_id and mpp_type4 <> @driver_type4
		end
	else
		Begin
			IF @driver_type1 <> 'UNK'
				delete #itemcode_id_paydetail_temp from legheader l where asgn_type = 'DRV' and l.lgh_number = #itemcode_id_paydetail_temp.lgh_number and isNull(#itemcode_id_paydetail_temp.lgh_number,0) > 0 and l.mpp_type1 <> @driver_type1

			IF @driver_type2 <> 'UNK'
				delete #itemcode_id_paydetail_temp from legheader l where asgn_type = 'DRV' and l.lgh_number = #itemcode_id_paydetail_temp.lgh_number and isNull(#itemcode_id_paydetail_temp.lgh_number,0) > 0 and l.mpp_type2 <> @driver_type2

			IF @driver_type3 <> 'UNK'
				delete #itemcode_id_paydetail_temp from legheader l where asgn_type = 'DRV' and l.lgh_number = #itemcode_id_paydetail_temp.lgh_number and isNull(#itemcode_id_paydetail_temp.lgh_number,0) > 0 and l.mpp_type3 <> @driver_type3
			
			IF @driver_type4 <> 'UNK'
				delete #itemcode_id_paydetail_temp from legheader l where asgn_type = 'DRV' and l.lgh_number = #itemcode_id_paydetail_temp.lgh_number and isNull(#itemcode_id_paydetail_temp.lgh_number,0) > 0 and l.mpp_type4 <> @driver_type4
...
update 	#itemcode_id_paydetail_temp
set 	ivh_billdate = (SELECT 	max(ivh_billdate)
						from 	invoiceheader
						where	#itemcode_id_paydetail_temp.ord_hdrnumber = invoiceheader.ord_hdrnumber)
where	#itemcode_id_paydetail_temp.ord_hdrnumber > 0

update 	#itemcode_id_paydetail_temp
set		ivh_invoicenumber = (select max(ivh_invoicenumber) 
							from 	invoiceheader 
							where 	ivh_billdate = #itemcode_id_paydetail_temp.ivh_billdate)
where 	#itemcode_id_paydetail_temp.ord_hdrnumber > 0
-- PTS 31363 -- BL (end)
...
Begin
	...
	Delete from #itemcode_id_paydetail_temp  
...
if @sch_date1 > convert(datetime, '1950-01-01 00:00') OR 
	...
	Delete from #itemcode_id_paydetail_temp 
	where #itemcode_id_paydetail_temp.ord_hdrnumber > 0 and 
		#itemcode_id_paydetail_temp.ord_hdrnumber in (select ord_hdrnumber 
						from stops
...
IF @mpp_terminal <> 'UNK'
	delete #itemcode_id_paydetail_temp from manpowerprofile mp where asgn_type = 'DRV' and asgn_id = mp.mpp_id and mp.mpp_terminal <> @mpp_terminal
...
-- PTS 35274 11/2008 JSwindell RECODE<<start>>
	...
	IF isNull(@revtype1,'UNK') <> 'UNK'
		Begin
			If @excl_revtype1 = 'Y'
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype1,'UNK') = @revtype1
			Else
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype1,'UNK') <> @revtype1
		End
	IF isNull(@revtype2,'UNK') <> 'UNK'
		Begin
			If @excl_revtype2 = 'Y'
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype2,'UNK') = @revtype2
			Else
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype2,'UNK') <> @revtype2
		End
	IF isNull(@revtype3,'UNK') <> 'UNK'
		Begin
			If @excl_revtype3 = 'Y'
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype3,'UNK') = @revtype3
			Else
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype3,'UNK') <> @revtype3
		End
	IF isNull(@revtype4,'UNK') <> 'UNK'
		Begin
			If @excl_revtype4 = 'Y'
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype4,'UNK') = @revtype4
			Else
				DELETE FROM #itemcode_id_paydetail_temp WHERE isNull(#itemcode_id_paydetail_temp.ord_revtype4,'UNK') <> @revtype4
		End
	--	LOR
...
SELECT * from #itemcode_id_paydetail_temp  
DROP TABLE #itemcode_id_paydetail_temp  
  
GO
GRANT EXECUTE ON  [dbo].[d_paydetail_report_itemcode_id_sp] TO [public]
GO
