...
Example call 
exec TMI_imagingrecord_30DayHist

Stored proc created to get the 30 day history of orders. The proc will populate
ImageOrderList with all ord_hdrnumber values from the orderheader.db where the 
ord_bookedDate is 30 days or less prior to today and the value does not currently 
exist in ImageOrderList

DPETE 1/22/03 TMI requests records with assets only
*/

CREATE Procedure [dbo].[TMI_imagingrecord_30DayHist] 
As

Declare @ord_hdrnumber int

 --Create a cursor to loop though all ord_hdrnumber(s) on the orderheader.db.
 --to determine which orders have a book date 30 days or less prior to today. If
 --the book date is 30 days or less than today and the record does not exists in 
 --ImageOrderList then insert the new record.  
  
 --Determine if the vendor is TMI
IF (SELECT UPPER(gi_string1)
      FROM generalinfo
     WHERE gi_name = 'ImagingVendor')='TMI'
BEGIN
  ...
  SELECT ord_hdrnumber
   ...
   WHERE ord_hdrnumber <> 0 and
        (select DATEDIFF(day, ord_bookdate, getdate())) <= 30 
        and ord_status in ('DSP','PLN','STD','CMP','ICO')
...
GO
GRANT EXECUTE ON  [dbo].[TMI_imagingrecord_30DayHist] TO [public]
GO
