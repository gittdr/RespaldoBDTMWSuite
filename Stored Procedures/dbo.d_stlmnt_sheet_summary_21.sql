...
CREATE PROC [dbo].[d_stlmnt_sheet_summary_21](
...
-----------	---------------	-------	-------	-----------------------------------------------------
06/21/2002	Vern Jewett		14623	(none)	Original, copied from d_stlmnt_sheet_summary_titus_n.
1/8/3       DPETE          16764 Fix ytd totals using model
12/4/06     DPETE           35360 YTD amounts are doubled
...
SET ytdcomp = ISNULL((SELECT SUM(ROUND(ph.pyh_totalcomp, 2))
		...
		WHERE ph.asgn_id = yb.asgn_id
	  		...
	  		AND ph.pyh_payperiod >= '01/01/' + datename(yy, @payperiodstart)
	  		AND ph.pyh_payperiod < @payperiodstart
...
/* LOR PTS# 6790
UPDATE #temp_pd
SET 	bal_gl2472 = IsNull((SELECT SUM(ROUND(pd.pyd_amount, 2)) 
			FROM paydetail pd, payheader ph
			WHERE pd.asgn_id = #temp_pd.asgn_id
				AND pd.asgn_type = #temp_pd.asgn_type
				AND pd.pyd_status <> 'HLD'
				and pd.pyd_glnum like '2472%'
				and ph.pyh_pyhnumber = pd.pyh_number
				AND ph.pyh_paystatus <> 'HLD'
				and ph.asgn_id = pd.asgn_id
				and ph.asgn_type = pd.asgn_type), 0)


UPDATE #temp_pd
SET 	bal_gl2470 = IsNull((SELECT SUM(ROUND(pd.pyd_amount, 2)) 
			FROM paydetail pd, payheader ph
			WHERE pd.asgn_id = #temp_pd.asgn_id
				AND pd.asgn_type = #temp_pd.asgn_type
				AND pd.pyd_status <> 'HLD'
				and pd.pyd_glnum like '2470%'
				and ph.pyh_pyhnumber = pd.pyh_number
				AND ph.pyh_paystatus <> 'HLD'
				and ph.asgn_id = pd.asgn_id
				and ph.asgn_type = pd.asgn_type), 0)	*/
...
--Set Type2 labels & values; start with Carriers..
update	#temp_pd
  set	type2_label = 
			case ltrim(rtrim(l.userlabelname))
			  when	'UNKNOWN' then l.labeldefinition
			  else	isnull(l.userlabelname, isnull(l.labeldefinition, 'Carrier Type 2'))
			end
		,type2 = isnull(l.name, c.car_type2)
  from	#temp_pd t
		,labelfile l RIGHT OUTER JOIN carrier c ON (l.abbr = c.car_type2 and l.labeldefinition = 'CarType2')
  where	t.asgn_type = 'CAR'
	and	c.car_id = t.asgn_id
	--and	l.labeldefinition = 'CarType2'
	--and	l.abbr =* c.car_type2
...
update	#temp_pd
  set	type2_label = 
			case ltrim(rtrim(l.userlabelname))
			  when	'UNKNOWN' then l.labeldefinition
			  else	isnull(l.userlabelname, isnull(l.labeldefinition, 'Driver Type 2'))
			end
		,type2 = isnull(l.name, mpp.mpp_type2)
  from	#temp_pd t
		,labelfile l RIGHT OUTER JOIN manpowerprofile mpp ON (l.abbr = mpp.mpp_type2 and l.labeldefinition = 'DrvType2')
  where	t.asgn_type = 'DRV'
	and	mpp.mpp_id = t.asgn_id
	--and	l.labeldefinition = 'DrvType2'
	--and	l.abbr =* mpp.mpp_type2
...
update	#temp_pd
  set	type2_label = 
			case ltrim(rtrim(l.userlabelname))
			  when	'UNKNOWN' then l.labeldefinition
			  else	isnull(l.userlabelname, isnull(l.labeldefinition, 'Third Party Type 2'))
			end
		,type2 = isnull(l.name, tpr.tpr_thirdpartytype2)
  from	#temp_pd t
		,labelfile l RIGHT OUTER JOIN thirdpartyprofile tpr ON (l.abbr = tpr.tpr_thirdpartytype2 and l.labeldefinition = 'TprType2')
  where	t.asgn_type = 'TPR'
	and	tpr.tpr_id = t.asgn_id
	--and	l.labeldefinition = 'TprType2'
	--and	l.abbr =* tpr.tpr_thirdpartytype2
...
update	#temp_pd
  set	type2_label = 
			case ltrim(rtrim(l.userlabelname))
			  when	'UNKNOWN' then l.labeldefinition
			  else	isnull(l.userlabelname, isnull(l.labeldefinition, 'Tractor Type 2'))
			end
		,type2 = isnull(l.name, tp.trc_type2)
  from	#temp_pd t
		,labelfile l RIGHT OUTER JOIN tractorprofile tp ON (l.abbr = tp.trc_type2 and l.labeldefinition = 'TrcType2')
  where	t.asgn_type = 'TRC'
	and	tp.trc_number = t.asgn_id
--	and	l.labeldefinition = 'TrcType2'
--	and	l.abbr =* tp.trc_type2
...
update	#temp_pd
  set	type2_label = 
			case ltrim(rtrim(l.userlabelname))
			  when	'UNKNOWN' then l.labeldefinition
			  else	isnull(l.userlabelname, isnull(l.labeldefinition, 'Trailer Type 2'))
			end
		,type2 = isnull(l.name, trl.trl_type2)
  from	#temp_pd t
		,labelfile l RIGHT OUTER JOIN trailerprofile trl ON (l.abbr = trl.trl_type2 and l.labeldefinition = 'TrlType2')
  where	t.asgn_type = 'TRC'
	and	trl.trl_number = t.asgn_id
--	and	l.labeldefinition = 'TrlType2'
--	and	l.abbr =* trl.trl_type2
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_sheet_summary_21] TO [public]
GO
