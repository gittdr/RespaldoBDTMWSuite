...
CREATE PROCEDURE [dbo].[ResNowOV_R1_DestinationZip3byOrderInfo]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN	SELECT 
				dbo.fnc_zip3(ord_dest_zip, ord_deststate, ord_destcity, @StateOnly) AS [Zip3Destination],
				ord_Destpoint AS [Destination Code], 
				c1.cmp_name + ' - ' + c1.cty_nmstct AS [Destination],
			...
			FROM orderheader (NOLOCK) JOIN company c1 (NOLOCK) on c1.cmp_id = ord_Destpoint
			WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
				AND ord_status in ('AVL','PLN','DSP','STD','CMP')
				AND dbo.fnc_zip3(ord_dest_zip, ord_deststate, ord_destcity, @StateOnly) = @ItemID
		...
		BEGIN
			...
			WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
				AND ord_status in ('AVL','PLN','DSP','STD','CMP')
				AND dbo.fnc_zip3(ord_dest_zip, ord_deststate, ord_destcity, @StateOnly) NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
				AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			ORDER BY dbo.fnc_zip3(ord_dest_zip, ord_deststate, ord_destcity, @StateOnly) 
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
		SELECT 		ord_number,
				[ItemID] = dbo.fnc_zip3(ord_dest_zip, ord_deststate, ord_destcity, @StateOnly)
		INTO #RNTrial_Cache_DestinationZip3 
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[DateEnd] = @DateEnd,
				[ItemCategory] = @Mode,
				[ItemID] = #RNTrial_Cache_DestinationZip3.[ItemID],
				[ItemDescription] = IsNull((SELECT TOP 1 State + '-' FROM ResNowZip3Translation (NOLOCK) WHERE PID = #RNTrial_Cache_DestinationZip3.[ItemID]), '') + #RNTrial_Cache_DestinationZip3.[ItemID],
				[Count] = COUNT(*),
		...
		FROM #RNTrial_Cache_DestinationZip3 (NOLOCK) 
		GROUP BY #RNTrial_Cache_DestinationZip3.[ItemID]
			ORDER BY COUNT(*) DESC		
		SET ROWCOUNT 0
	END

GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_DestinationZip3byOrderInfo] TO [public]
GO
