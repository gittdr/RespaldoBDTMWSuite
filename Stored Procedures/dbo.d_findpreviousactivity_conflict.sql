...
CREATE PROCEDURE [dbo].[d_findpreviousactivity_conflict] @asgnid varchar (13), @asgntype varchar (6), @asgndate datetime, @mov integer, @ico_mov_list varchar (2000)
AS
SET NOCOUNT ON
SELECT lh.mov_number, lh.lgh_number, aa.asgn_date, aa.asgn_enddate, 
 ...
 WHERE lh.lgh_number = aa.lgh_number 
	...
	AND charindex(',' + convert(varchar(15), aa.mov_number) + ',', @ico_mov_list, 1) = 0
GO
GRANT EXECUTE ON  [dbo].[d_findpreviousactivity_conflict] TO [public]
GO
