...
create proc [dbo].[d_load_consignee_compname_sp] 
...
/* Revision History:
   ...
   PTS17782     DPETE           Add fields to make it match other company DDDWs
	PTS38461 	EMK				Adde column address3
	PTS 52827 Copied d_load_consignee_compID_sp and changed to lookup by name
*/
	...
	if @daysout = 999
		...
		SELECT	cmp_name ,
		  ...
		  WHERE cmp_name LIKE @comp + '%'
			...
			--END PTS 53255 JJF 20101130
			--AND (EXISTS(select * FROM @tbl_restrictedbyuser rsva WHERE company.rowsec_rsrv_id = rsva.rowsec_rsrv_id or rsva.rowsec_rsrv_id = 0))
		  ORDER BY cmp_name 
else 
	SELECT	cmp_name ,
		cmp_id , 
...
set rowcount 0 
GO
GRANT EXECUTE ON  [dbo].[d_load_consignee_compname_sp] TO [public]
GO
