...
CREATE PROC [dbo].[d_settlement_sheet_summary_113](
 ...
 *
 * REVISION HISTORY:
 * 07/16/2010 PTS 53008	07/16/2010	New format copied from d_settlement_sheet_summary_new and added two new columns
 *								    and new address functionality as found in format 25.  Also includes third party pay.
 *
 **/
...
-- PTS 53008 start
	...
	TotalEmptyMiles	int null,
	TotalLoadedMiles	int null,
	min_pyd_number	int	null,
	miles_rec_cnt	int null,
	HoldEmptyMiles	int null,
	HoldLoadedMiles	int null)
-- PTS 53008 end
...
/* PTS 53008 -  Add address for asset		*/
...
/* PTS 53008 -  Add address for asset End	*/
...
-- PTS 29515 -- BL (end)
-- PTS 53008 start
         (select isnull(sum(stp_lgh_mileage),0) from stops
          where stops.lgh_number = tp.lgh_number
          and tp.lgh_number > 0
      ...
      address_zip,
     (select isnull(sum(stp_lgh_mileage),0) from stops
      where stops.lgh_number in (select lgh_number from #temp_pd
     ...
     (select isnull(sum(stp_lgh_mileage),0) from stops
      ...
      and stops.stp_loadstatus = 'LD' )
     as 'TotalLoadedMiles',
	IsNull((select min(pdm.pyd_number) from paydetail pdm where pdm.asgn_number = tp.asgn_number and pdm.asgn_number <> 0 ),0) as 'min_pyd_number',
	IsNull((select count(*) from paydetail pdc where pdc.asgn_number = tp.asgn_number and pdc.asgn_number <> 0 and pdc.pyd_unit = 'MIL' and pdc.pyd_loadstate in ('UNLD', 'MT', 'LD') and tp.pyt_basis in ('LGH','ANC','INV')),0) as 'miles_rec_cnt',
         (select isnull(sum(stp_lgh_mileage),0) from stops
          where stops.lgh_number = tp.lgh_number
          and tp.lgh_number > 0
          and stops.stp_loadstatus <> 'LD')
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_113] TO [public]
GO
