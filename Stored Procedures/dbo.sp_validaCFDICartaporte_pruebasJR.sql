...
CREATE proc [dbo].[sp_validaCFDICartaporte_pruebasJR] @lgh_hdrnumber varchar(20)
as
...
select replace(Mensaje,'^','') as Mensaje, case when Mensaje like '%^%' then 'Error' else 'OK' end as Validacion from (
		...
		select 
		  ...
		  case when 
			  (select count(Folio) from VISTA_Carta_Porte where LegNum = @num_factura) >=1 then '<br> <br> ****Ya existe un CFDI Complemento Carta Porte Generado 
																								   para el número de factura '+ '<b style='+''''+'color:black;'+''''+'>' + @num_factura+ ' </b> *****'  + '<br>'+  
																								   '<br>' + 
...
--				cast(isnull(replace(dolly.trl_axles,0,2),2) as varchar(3)) + 'R' +
		...
		where ivh_invoicenumber = @num_factura ) as q
