...
create proc [dbo].[TMT_GetCompanyForUnit]
@unit varchar(12)
as
/**
*
* NAME:
* dbo.TMT_GetCompanyForUnit
*
* TYPE:
* StoredProcedure
*
...
*
* RETURNS:
* Transman company for a given unit.
* RESULT SETS:
* none.
*
* PARAMETERS:
...
SET @SQL= 'SELECT CUSTOMERID FROM ' + @tmtserver + '.' + @tmtdb + '.dbo.VIEW_UNITS WHERE UNITID = ''' + @UNIT + ''''
EXEC sp_executesql @sql
END
ELSE
BEGIN
select @tmtdb = '[' + gi_string2 + ']' from generalinfo where gi_name = 'Shoplink'
SET @SQL= 'SELECT CUSTOMERID FROM ' + @tmtdb + '.dbo.VIEW_UNITS WHERE UNITID = ''' + @UNIT + ''''
EXEC sp_executesql @sql
END
SET ANSI_NULLS OFF
set ANSI_WARNINGS OFF
GO
GRANT EXECUTE ON  [dbo].[TMT_GetCompanyForUnit] TO [public]
GO
