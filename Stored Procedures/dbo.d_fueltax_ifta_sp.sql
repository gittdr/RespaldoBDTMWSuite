...
/****** Object:  Stored Procedure dbo.d_fueltax_ifta_sp    Script Date: 6/1/99 11:54:11 AM ******/
create PROC [dbo].[d_fueltax_ifta_sp]
@BegDate			datetime,
@EndDate			datetime,
@Owner			varchar (12),
@TrcType1		varchar (6),
...
/*	Next, tally the FUELPURCHASED table */
INSERT INTO #fueltaxtemp (ftr_state, ftr_total, ftr_free, ftr_toll, ftr_quantity,
ftr_owner, ftr_type1, ftr_type2, ftr_type3, ftr_type4, ftr_company, ftr_division,
ftr_fleet, ftr_terminal, ftr_uom)
...
0,								0,				ROUND((sum(f.ftr_total)/@tot_miles),4),
sum(f.ftr_total),		
...
/*	If IFTA <> Y or N, make it N */
UPDATE #fueltaxreport SET ftr_ifta	 = 'N' WHERE ftr_ifta is null

/*	Read Kilometer-Liter info into variables for calculations */
SELECT @tot_miles = sum(ftr_total), @tot_fuel = sum(ftr_quantity) 
FROM #fueltaxreport

SELECT @avg_mpg = @tot_miles / @tot_fuel

/*	Compute AvgMPG consumption */
UPDATE #fueltaxreport SET 	ftr_consumed 	= ROUND((ftr_total / @avg_mpg), 2)
WHERE ftr_method = "Avg MPG" OR ftr_method IS NULL

/*	Compute FixedMPG consumption */
UPDATE #fueltaxreport SET 	ftr_consumed 	= ROUND((ftr_total / (ftr_mpg * 1.609)), 2)
WHERE ftr_method = "Fixed MPG"

/*	Compute InState consumption */
UPDATE #fueltaxreport SET 	ftr_consumed	= ROUND((ftr_instate * @tot_fuel), 2)
WHERE ftr_method = "InSt Ratio"
...
/*	Stupid, but easier to code for now */
...
GO
GRANT EXECUTE ON  [dbo].[d_fueltax_ifta_sp] TO [public]
GO
