...
CREATE PROC [dbo].[d_masterbill126_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 *
-- 10/20/2008 pmill from mb17a for Hilco Transport
-- 01/12/09 change to display the commodity name from the order ehader for the line haul charges (this is a rate by total invoice)
 **/
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	@copy,
	cht.cht_primary,
    case cht.cht_primary + ivh_rateby
     when 'YT' then isnull(ord_description,cht_description)
     else 
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	@copy,
	cht.cht_primary,
-- want commoidty name on line haul lines (assume one cmd per order for rate by total)
	case cht.cht_primary + ivh_rateby
     when 'YT' then isnull(ord_description,cht_description)
     else 
	   case rtrim(isnull(ivd_description,'UNKNOWN'))
...
ORDER by ivh_invoicenumber, cht_primary desc, cht_description
DROP TABLE #masterbill_temp

GO
GRANT EXECUTE ON  [dbo].[d_masterbill126_sp] TO [public]
GO
