...
CREATE PROC [dbo].[Timeline_holidays_unload_sp]
	...
	@DrpDate datetime output,	-- First date
	@UnloadDate datetime output,	-- Last date
 ...
 * 
 * NAME:
 * dbo.Timeline_holidays_unload_sp
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 *
 * This proc differs from Timeline_holidays_sp in that if the resulting date is holiday or weekend
 * 	it will roll forward past the holiday instead of back as you would do with the pickup.
 *
 * RETURNS: 
 *	Date
 * RESULT SETS: 
...
------ Force an error if the trip ends on a holiday or weekend so that we can move forward from that point.
SELECT @PrimarySegment = 'Y'
------
...
select @FirstCharTime = substring(convert(char, @DrpDate, 108), 1, 8)
select @DrpDate = cast(@chardate as DATETIME)

-- Get the unload date part
select @chardate = substring(convert(char, @UnloadDate, 101), 1, 10)
-- Get the time part and save it for the return
select @LastCharTime = substring(convert(char, @UnloadDate, 108), 1, 8)
select @UnloadDate = cast(@chardate as DATETIME)

select @ReturnPupDate = @DrpDate
select @ReturnDrpDate = @UnloadDate

select @LoopDate = @DrpDate
select @LoopEnd = @UnloadDate
select @DateChange = 1

if @debug = 0 print '@UnloadDate: ' + convert(varchar(30), @UnloadDate)
if @debug = 0 print '@LoopEnd: ' + convert(varchar(30), @LoopEnd)


-- Loop through each date in the range
while 1=1 -- Exit test at the end of the loop.
begin
	if @debug = 0 print '@LoopDate: ' + convert(varchar(30), @LoopDate)
	...
	if @holidays > 0
	begin
		if @debug = 0 print 'Weekend or holiday found'
		if @Exception > 0
	...
	-- Loop control
	if @LoopDate >= @LoopEnd and @holidays = 0
		BREAK
...
-- Return the dates
select @DrpDate = @ReturnPupDate
select @UnloadDate = @ReturnDrpDate

if @debug = 0 print 'No holiday exit'

GOTO THEEXIT

---------------- Move the date forward past the weekend or holiday ----------
MOVEFORWARD:
if @debug = 0 print 'Move Forward'

select @DrpDate = @ReturnPupDate
select @UnloadDate = @ReturnDrpDate
select @LoopDate = @UnloadDate
select @LoopEnd = @UnloadDate
select @DateChange = 1
select @loopcount = 0

if @debug = 0 print '@DrpDate: ' + convert(varchar(30), @DrpDate)
if @debug = 0 print '@UnloadDate: ' + convert(varchar(30), @UnloadDate)
if @debug = 0 print '@LoopDate: ' + convert(varchar(30), @LoopDate)
if @debug = 0 print '@LoopEnd: ' + convert(varchar(30), @LoopEnd)
...
begin
	...
	-- If @holidays > 0 then it is a non delivery date, shift the date.
	if @holidays > 0
		if @LoopDate = @ReturnDrpDate
...
GO
GRANT EXECUTE ON  [dbo].[Timeline_holidays_unload_sp] TO [public]
GO
