...
CREATE PROCEDURE [dbo].[tmail_rpt_tractor_hist2]	@StartDate datetime,
					@EndDate datetime,
					@Truck varchar (8),
					@SysTZHrs int,
					@SysTZMin int,
...
**  (it uses SQL Server function calls)                   **
***********************************************************/

/* 05/12/99 MZ: Single Tractor history Report. 
 * 10/22/01 MZ: Translated report headers  
   11/26/01 MZ: Created new version 2 
   01/02/02 MZ  Fixed sort order, was ignoring AM/PM in sort */
...
ORDER BY checkcall.ckc_date 

GO
GRANT EXECUTE ON  [dbo].[tmail_rpt_tractor_hist2] TO [public]
GO
