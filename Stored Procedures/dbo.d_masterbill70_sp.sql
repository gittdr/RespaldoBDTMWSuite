...
CREATE PROC [dbo].[d_masterbill70_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8),   
 ...
 *
 * REVISION HISTORY:
 * 6/3/05 Tony Leonardi - based on invoice_template2
 *	1/2/07 PTS 28118 DPETE check in for Tony (consol to 34920)
 *  2/20/07 PTS 34920 (28118 Consolidated) EMK - Changed reference numbers to give both BL# and PO  
 **/ 
...
If UPPER(@reprintflag) = 'REPRINT'   
 ...
 cht.cht_primary,  
  ...
  IsNull(ord.ord_fromorder ,'') ,
	...
	--PTS 34920 (28118) EMK -- Give the minimum reference number, not limited by ref_sequence
	...
	-- 
	ref_number = isnull((select min(ref_number) from referencenumber where referencenumber.ord_hdrnumber = invoiceheader.ord_hdrnumber and ref_type = 'PO#'),''),
	bl_number = isnull((select min(ref_number) from referencenumber where referencenumber.ord_hdrnumber = invoiceheader.ord_hdrnumber and ref_type = 'BL#'),''),
	po_number = isnull((select min(ref_number) from referencenumber where referencenumber.ord_hdrnumber = invoiceheader.ord_hdrnumber and ref_type = 'PO'),'')
	--PTS 34920 (28118) EMK 
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill70_sp] TO [public]
GO
