...
create procedure [dbo].[FALCON_V001_SP] (
...
*/
Begin
	declare @ldec_fal_ratio money,@ldec_fal_segmiles money,@ldec_fal_totmiles money
	declare @ll_fal_mov int
	select @ldec_fal_ratio = 1
	select @ll_fal_mov = mov_number from legheader where lgh_number = @pl_lgh_number
	select @ldec_fal_totmiles = sum(stp_lgh_mileage) from stops where mov_number = @ll_fal_mov
	select @ldec_fal_segmiles = sum(stp_lgh_mileage) from stops where lgh_number = 	@pl_lgh_number
	select @ldec_fal_totmiles = IsNull(@ldec_fal_totmiles,0)
	select @ldec_fal_segmiles = IsNull(@ldec_fal_segmiles,0)
	If @ldec_fal_totmiles > 0 and @ldec_fal_segmiles > 0
		select @ldec_fal_ratio = @ldec_fal_segmiles/@ldec_fal_totmiles
	else
		select @ldec_fal_ratio = 1
	...
	begin
		...
		select @pdec_calcrevenue = @pdec_calcrevenue * @ldec_fal_ratio
	end
End
/* 
* END FALCON
...
GO
GRANT EXECUTE ON  [dbo].[FALCON_V001_SP] TO [public]
GO
