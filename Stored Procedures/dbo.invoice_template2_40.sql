...
create procedure [dbo].[invoice_template2_40](@invoice_nbr  	int,@copies		int)
as
...
--	begin
  ...
  If Not Exists (Select cmp_mailto_name From company c, #invtemp_tbl t
		...
		set ivh_billto_name = company.cmp_name,
			--ivh_billto_nmctst = Case charindex('/', company.cty_nmstct) When 0 Then company.cty_nmstct Else substring(company.cty_nmstct,1, (charindex('/', company.cty_nmstct) - 1))+ ' ' + IsNull(company.cmp_zip,'') END,
	...
	Else	
		...
		set ivh_billto_name = company.cmp_mailto_name,
			 ...
			 --ivh_billto_nmctst = Case CHARINDEX('/',company.mailto_cty_nmstct) When 0 Then IsNULL(company.cmp_mailto_zip,'') Else  substring(company.mailto_cty_nmstct,1, (charindex('/', company.mailto_cty_nmstct) - 1))+ ' ' + IsNull(company.cmp_mailto_zip,'') END,
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template2_40] TO [public]
GO
