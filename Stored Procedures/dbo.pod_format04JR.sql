...
--Exec pod_format04JR 724035,816874
--select * from orderheader where ord_hdrnumber = 171620
--drop proc pod_format04JR
-- Exec pod_format04JR  306278,347975
-- Exec pod_format04JR 493301, 562662

--Exec pod_format04JR 318155, 360563
CREATE PROCEDURE [dbo].[pod_format04JR] (@ord_hdrnumber 	Varchar(10), @lgh		INTEGER) 
AS
...
Print 'aqui inicia el primer substring'
...
UPDATE #temp 
   SET ord_number = orderheader.ord_number,
       ...
       ord_enddate = orderheader.ord_dest_latestdate,
...
Print 'aqui inicia el segundo substring'
...
UPDATE #temp 
   SET billto_name = company.cmp_name,
       ...
       billto_nmstct = CASE cty_nmstct 
                          ...
                          ELSE replace(cty_nmstct,'/MEX','') + '  ' + ISNULL(cmp_zip, ' ')
                       END,
 ...
 Print 'aqui inicia el tercer substring'
...
UPDATE #temp 
 ...
 Print 'aqui inicia el cuarto substring'
...
UPDATE #temp 
   SET consignee_name = company.cmp_name,
       ...
       consignee_nmstct =  CASE cty_nmstct 
                          ...
                          END
						  ,
       consignee_contact = substring(company.cmp_contact,0,20),
...
UPDATE #temp
   SET load_comment = CAST(stops.stp_comment AS VARCHAR(60))
  FROM stops
...
UPDATE #temp
   SET unload_comment = CAST(stops.stp_comment AS VARCHAR(60))
  FROM stops
 WHERE stops.ord_hdrnumber = @ord_hdrnumber AND
...
'UNIVRSAL','VERQRO','DMARTIN','NESTLE','KATNAT','TECMEX','SAE','KATNAT','R.H.SHIP','LESCHACO')
...
GO
GRANT EXECUTE ON  [dbo].[pod_format04JR] TO [public]
GO
