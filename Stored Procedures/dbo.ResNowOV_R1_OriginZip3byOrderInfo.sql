...
CREATE PROCEDURE [dbo].[ResNowOV_R1_OriginZip3byOrderInfo]
...
*/
	...
	BEGIN
		...
		BEGIN	
			SELECT 
				dbo.fnc_zip3(ord_origin_zip, ord_originstate, ord_origincity, @StateOnly) AS [Zip3Origin],
			...
			AND dbo.fnc_zip3(ord_origin_zip, ord_originstate, ord_origincity, @StateOnly) = @ItemID
		...
		BEGIN
			...
			WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND dbo.fnc_zip3(ord_origin_zip, ord_originstate, ord_origincity, @StateOnly) NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			ORDER BY dbo.fnc_zip3(ord_origin_zip, ord_originstate, ord_origincity, @StateOnly) 
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
		SELECT 		ord_number,
				[ItemID] = dbo.fnc_zip3(ord_origin_zip, ord_originstate, ord_origincity, @StateOnly)
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_OriginZip3byOrderInfo] TO [public]
GO
