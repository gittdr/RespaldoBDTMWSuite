...
create procedure [dbo].[d_external_inbound_view] (
...
select @city_latlong = LEFT (gi_string1, 1) FROM generalinfo where gi_name = 'citylatlongunits'
...
begin
	...
	BEGIN
		...
		END
		IF @city_latlong = 'S'
			SELECT @ete_originlat = @ete_originlat / 3600.0, @ete_originlong = @ete_originlong / 3600.0 
	END
...
INSERT INTO #results (ete_id, ete_source, ete_sourcerefnumber, ete_origlocation, 
          ...
          (SELECT ete_id, ete_source, ete_sourcerefnumber, ete_origlocation, 
			...
			--END PTS 50742 JJF 20100216
              ...
              --AND ete_availabledate BETWEEN @ete_availabledatefrom AND @ete_availabledateto 
              -- PTS 56037 - DJM - modified the date range logic
              AND ((ete_availabledate BETWEEN @ete_availabledatefrom AND @ete_availabledateto) OR (ete_expirationdate BETWEEN @ete_availabledatefrom AND @ete_availabledateto)) 
...
IF (SELECT ISNULL(gi_string1, 'N') FROM generalinfo WHERE gi_name = 'IncludeActiveTripCarriers') = 'Y'
	INSERT INTO #results (ete_id, ete_postingage, ete_postingdate, ete_availabledate, 
                          ...
                          ete_truckid, ete_driver_name, ete_driver_phone, 
   ...
   SELECT DISTINCT 
          ...
          convert(varchar(50), lgh_carrier_truck), convert(varchar(255), lgh_driver_name), convert(varchar(25), lgh_driver_phone), 
...
GO
GRANT EXECUTE ON  [dbo].[d_external_inbound_view] TO [public]
GO
