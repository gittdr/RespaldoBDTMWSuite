...
CREATE PROC [dbo].[d_stlmnt_det_report_general105](
 ...
 * 5-12-2009  PTS 47251 Proc Created ( BASED on format=general: d_settlement_sheet_summary_new)
 * 5-12-2009  PTS 47251	Add orderheader BOL number to settelement sheet.
 * 6-8-09     PTS 47251 FIX Ref#'s not pulling consistently.
 **/
...
-- PTS 29303 -- BL (end)	
	...
	bol_number      varchar(200) null,		-- PTS 47251
	pos1			int null,				-- PTS 47251
	ls_freightdetail_bol	varchar(200) null,	-- PTS 47251 FIX
	CC_IDENTITY				INT IDENTITY		-- PTS 47251 FIX
)
...
SELECT pd.pyd_number,
	...
	null, -- pyt_basis   -- PTS 29515 -- BL (start)
	null, -- bol_number  -- PTS 47251
	null,  -- pos1		 -- PTS 47251	
	null -- ls_freightdetail_bol  -- PTS 47251 FIX
...
-- PTS 47251	Add orderheader BOL number to settelement sheet <<start>>
-- show more.
--UPDATE #temp_pd 
--SET bol_number = (SELECT MIN(ref_number)
--				   FROM referencenumber
--				   WHERE ref_table = 'orderheader'
--				   AND ref_type = 'BL#'
--				   AND ref_tablekey = #temp_pd.ord_hdrnumber)
--where itemsection = 0
-- PTS 47251	Add orderheader BOL number to settelement sheet <<END>>

--select distinct 
--referencenumber.ref_tablekey, referencenumber.ref_number
--into #temp_bol
--FROM referencenumber, #temp_pd
--WHERE ref_table = 'orderheader'
--AND ref_type = 'BL#'
--AND ref_tablekey = #temp_pd.ord_hdrnumber
--and #temp_pd.ord_hdrnumber is not null
--and #temp_pd.ord_hdrnumber > 0 
----and itemsection = 0
--
--select ref_tablekey, ref_number, IDENTITY (INT, 1, 1 ) AS 'v_ident_count'
--into #temp_bol2
--from #temp_bol
--order by ref_tablekey, ref_number
--
--select ref_tablekey, ref_number, v_ident_count, cast('' as varchar(200))'workref' 
--into #temp_bol3
--from #temp_bol2
--
--drop table #temp_bol
--drop table #temp_bol2
--
-----select '#temp_bol3 ' , * from #temp_bol3
--
--declare @ref_ord_nbr int
--declare @row_count int
--declare @totalrows int
--declare @ref varchar(30)
--declare @work_ref varchar(200)
--declare @max_count int
--set @row_count = 1
--set @totalrows = (select count(*) from #temp_bol3)
--
--
--IF (select count(*) FROM #temp_bol3) > 0
--BEGIN
...
--		--set @work_ref = 'BOL#: '
--		set @work_ref = ''
--		set @ref_ord_nbr = (select ref_tablekey from #temp_bol3 where @row_count = v_ident_count)	
--		set @max_count =   (select max(v_ident_count) from #temp_bol3 where ref_tablekey = @ref_ord_nbr) 
--
--		while @row_count <= @max_count
--		BEGIN
--			set @ref  = (select ref_number   from #temp_bol3 where @row_count = v_ident_count)
--			IF @row_count < @max_count
--				BEGIN
--					set @work_ref  =  LTRIM(@work_ref) + @ref + ', '
--				END
--			IF @row_count = @max_count
--				BEGIN
--					set @work_ref  =  LTRIM(@work_ref) + @ref 
...
--
--		update #temp_bol3 
--		set workref = @work_ref
--		where ref_tablekey = @ref_ord_nbr
--
--		set @row_count = @row_count + 1
--	END	
--END 
--
--
----select * from #temp_bol3
--
--UPDATE #temp_pd 
--SET bol_number = (select min(workref) from #temp_bol3 where #temp_pd.ord_hdrnumber = #temp_bol3.ref_tablekey) 
--where itemsection = 0

--select 'debug ', itemsection, pos1, ord_hdrnumber,  bol_number from #temp_pd 

--========================================================== Start of BL# REFERENCE NUMBERS section 
-- 6-8-09     PTS 47251 FIX Ref#'s not pulling consistently <<start>>
...
create table #temp_BL_refnums (lsrowcnt int identity not null primary key clustered, ord_hdrnumber int, REF_NUMBER 
...
While @BLloopCnt <= @Maxlsrowcnt
	BEGIN
		SET @work_ord_hdrnumber = (select ord_hdrnumber from #temp_distinct_ord_hdrnumber where 
...
LTRIM(RTRIM(SUBSTRING(@work_string, 1, LEN(@work_string)-1)))
							end

						IF @work_string IS not NULL
							begin			
								set @work_CC_IDENTITY	= 

(select min(CC_IDENTITY) from  #temp_pd  where ord_hdrnumber = @work_ord_hdrnumber) 	
								UPDATE  #temp_pd
								SET ls_freightdetail_bol = 

LTRIM(RTRIM(@work_string)) 								
								WHERE CC_IDENTITY = 
...
@next_ord_hdrnumber
				...
				IF @next_ord_hdrnumber = @work_ord_hdrnumber
					Begin
						Set @work_string = @work_string + (select REF_NUMBER 

from #temp_BL_refnums where lsrowcnt = @BLloopCnt ) + ', '
						Set @BLloopCnt = @BLloopCnt + 1
						set @next_ord_hdrnumber = (select ord_hdrnumber from 

#temp_BL_refnums where lsrowcnt = @BLloopCnt) 				
					End	
...
-- 6-8-09     PTS 47251 FIX Ref#'s not pulling consistently <<end>>
...
SELECT pyd_number, 
	...
	pyd_refnumtype,
	pyd_refnum,
	pyt_basis,		-- PTS 29515 -- BL (start)
	--bol_number	-- PTS 47251	
	ls_freightdetail_bol as 'bol_number'
	-----------pos1			-- PTS 47251	
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general105] TO [public]
GO
