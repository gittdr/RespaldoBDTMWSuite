...
/** se agrego esta linea and pyd.asgn_id  not in (select mpp_id from driverdocument where drd_doctype = 'TDDE' and drd_default = 'Y')
...
--Drop proc d_advance_export_queue_sp_PLN
-- exec d_advance_export_queue_sp_PLN_PM
CREATE procedure [dbo].[d_advance_export_queue_sp_PLN_PM]
as 
...
insert into @lgh_list (lgh_number, asgn_type, asgn_id, a.mov_number, pyd.ord_hdrnumber, a.asgn_date, Ope_principal)
select a.lgh_number, a.asgn_type, a.asgn_id, a.mov_number, pyd.ord_hdrnumber, a.asgn_date, a.asgn_controlling
 ...
 where a.asgn_status in ('PLN')
--and asgn_controlling = 'Y'
   and a.actg_type in ('A','P')
   and a.asgn_date < getdate()+3
 ...
 group by a.lgh_number, a.asgn_type, a.asgn_id, a.mov_number, pyd.ord_hdrnumber, a.asgn_date, a.asgn_controlling
...
select 0 as 'status',
       ...
       ord_hdrnumber,	   
	   mov_number,
	   Substring((select CONVERT(char(19), asgn_date,120)),1,16) as asgn_date
