...
-- PTS #:		95224
-- =============================================
CREATE PROCEDURE [dbo].[sp_CalculateAllocationByCount]
( 
...
BEGIN
	...
	INSERT INTO @Numerators SELECT OrderId, LegId, SUM(Count) AS Value FROM @AllocationInputs WHERE Type = 'Numerator' GROUP BY LegId, OrderId;
	INSERT INTO @Denominators SELECT OrderId, LegId, SUM(Count) AS Value FROM @AllocationInputs WHERE Type = 'Denominator' GROUP BY LegId, OrderId;
...
END
GO
GRANT EXECUTE ON  [dbo].[sp_CalculateAllocationByCount] TO [public]
GO
