...
CREATE PROC [dbo].[d_settlement_sheet_summary_38](
...
-- jyang pts13004
Declare @PeriodforYTD Varchar(3),@trip_pay money, @lost_pay money,@misc_pay money,
        @total_pay money,@trip_rev float,@lost_rev float,@total_rev float,
        @total_loads float, @pay_amount money, @pay_basis varchar(6),
        @pay_quantity float, @mov int, @mov_prior int
...
CREATE TABLE #temp_pd(
	...
	pyt_basis               varchar(6) null,
        trip_stlmnt_pay         money null,
        lost_time_pay           money null,
        misc_pay                money null,
        total_gross_pay         money null,
        trip_stlmnt_rev         float null,
        lost_time_rev           float null,
        total_rev               float null,
        total_loads             int null )
...
--PTS# 21155 ILB 03/04/2004
--Update pay type basis via paytype itemcode
UPDATE #temp_pd
   SET #temp_pd.pyt_basis = paytype.pyt_basis
  FROM paytype, #temp_pd
...
select @total_pay = 0
select @total_rev = 0
select @total_loads = 0
select @mov_prior = 0

--Create a cursor based on the select statement below
DECLARE revpay_cursor CURSOR FOR  

  SELECT pyd_amount, pyd_quantity, mov_number, pyt_basis
    FROM #temp_pd 
ORDER BY mov_number
...
--Execute the initial fetch of the first parameters
FETCH NEXT FROM revpay_cursor INTO @pay_amount, @pay_quantity, @mov, @pay_basis

--If the fetch is succesful continue to loop
WHILE @@fetch_status = 0  
 BEGIN  
  
  --Trip Segment charges
  IF @pay_basis = 'LGH'     
     BEGIN
       select @trip_pay = @trip_pay + @pay_amount
       select @trip_rev = @trip_rev + @pay_quantity            
     END
  --Invoice line Item charges
  IF @pay_basis = 'INV'    
     BEGIN
       select @lost_pay = @lost_pay + @pay_amount
       select @lost_rev = @lost_rev + @pay_quantity       
     END
  --Ancillary charges
  IF @pay_basis = 'ANC'         
     BEGIN
       select @misc_pay = @misc_pay + @pay_amount       
     END

  --Load count
  IF @mov <> @mov_prior        
     Begin
       select @total_loads = @total_loads + 1      
     End
     select @mov_prior = @mov
...
DEALLOCATE revpay_cursor

--Sum of total pay and total revenue
select @total_pay = @total_pay + (@trip_pay + @lost_pay + @misc_pay)
select @total_rev = @total_rev + (@trip_rev + @lost_rev)
 
--Update pay and revenue 
UPDATE #temp_pd
   SET 
--Pay
       trip_stlmnt_pay = @trip_pay,
       lost_time_pay   = @lost_pay,
       misc_pay        = @misc_pay,
       total_gross_pay = @total_pay,
--Loads
         total_loads   = @total_loads,
--Revenue
       trip_stlmnt_rev = @trip_rev,
       lost_time_rev   = @lost_rev,
       total_rev       = @total_rev
--PTS# 21155 ILB 03/04/2004
...
SELECT pyd_number, 
	...
	end_city,
	end_state, 
        --PTS# 21155 ILB 03/02/2004
        --Columns not available on the datawindow
        --ph.pyh_paystatus,
	--ref_number_tds,
	--pyd_offsetpay_number,
	--pyd_credit_pay_flag,
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_38] TO [public]
GO
