...
CREATE PROCEDURE [dbo].[MetricAutoAddGeneric]
AS
	SET NOCOUNT ON
	...
	SELECT MetricCode = CASE WHEN NOT EXISTS(SELECT sn FROM metricitem WHERE metriccode = RIGHT(t1.name, LEN(t1.name) - 7) ) 
			...
			ProcedureName = name,
			RefreshHistoryYN = ''
		FROM sysobjects t1 WHERE t1.type = 'p' AND t1.name like 'metric%' AND substring(t1.name, 7,1) = '_'
...
/* END: AutoAddByTMW */
GO
GRANT EXECUTE ON  [dbo].[MetricAutoAddGeneric] TO [public]
GO
