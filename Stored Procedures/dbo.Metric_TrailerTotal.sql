...
CREATE  PROCEDURE [dbo].[Metric_TrailerTotal]
	(
...
-- DETAILOPTIONS=1:Remolque,2:Tipo,3:SubTipo,4:Tamaño,5:Flota,6:División
	...
	Declare @NumeratorList Table (lgh_Trailer varchar(12), fleet varchar(20), div varchar(12), tipo varchar (20), subtipo varchar (20),tamano varchar (30))
	Declare @DenominatorList Table (lgh_Trailer varchar(12))



----------NUMERADOR CUENTA TRAILERS TOTAL-----------------------------------------------------------------------------------------------------------------------------
			...
			from dbo.fnc_TMWRN_TrailerCount3 
					(
						'TOTAL',@NumeratorOnlyTrlType1List,@NumeratorOnlyTrlType2List
...
update @NumeratorList set subtipo = case when tipo = 'DOLLY' then 'DOLLY'  else ( select max(name) from labelfile where labeldefinition = 'TrlType1' and abbr = (select max(trl_type1) from trailerprofile where trailerprofile.trl_number = lgh_trailer)) end
update @NumeratorList set tamano = case when tipo = 'DOLLY' then 'DOLLY' else  (select max( trl_type2) from trailerprofile where trailerprofile.trl_number = lgh_trailer) end
...
--DETALLE A NIVEL DE REMOLQUE
	...
	If @ShowDetail = 1
		BEGIN
            ...
            ,Tamano = replace(isnull(tamano,'Sin Tamaño'),'UNKNOWN','N.A')
            ,Flota = replace(isnull(fleet,'Sin Flota'),'UNKNOWN','N.A')
            ,Division = replace(isnull(div,'Sin División'),'UNKNOWN','N.A')
            ,ID = (select max(trl_id) from trailerprofile where trl_number = lgh_Trailer)
			From @NumeratorList
            order by Remolque
...
--DETALLE A NIVEL DE TIPO
       ...
       If @ShowDetail = 2
		BEGIN
			Select 
            Tipo = replace(isnull(tipo,'Sin Tipo'),'UNKNOWN','N.A')
            ,Remolque = count(lgh_trailer)
            From @NumeratorList
            group by tipo
            order by count(lgh_trailer) DESC

		END


--DETALLE A NIVEL DE SUBTIPO
