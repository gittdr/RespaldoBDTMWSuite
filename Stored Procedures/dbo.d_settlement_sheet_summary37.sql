...
CREATE  PROC [dbo].[d_settlement_sheet_summary37] (
...
CREATE TABLE #temp_pd(
	...
	mpp_otherid		varchar(9) null,
	pyt_basis		varchar(6) null)  --Added 01/23/07 MRK (hotfix)
...
SELECT pd.pyd_number,
	...
	mpp_otherid,
	null		--Added 01/23/07 MRK (hotfix)
...
*/
--Added 1/23/07 MRK hotfix
UPDATE #temp_pd
SET pyt_basis = pt.pyt_basis
FROM #temp_pd tp join paytype pt on (tp.pyt_itemcode = pt.pyt_itemcode)
...
SELECT pyd_number, 
	...
	mpp_senioritydate,
	mpp_otherid,
	pyt_basis --Added 01/23/07 MRK (hotfix)
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary37] TO [public]
GO
