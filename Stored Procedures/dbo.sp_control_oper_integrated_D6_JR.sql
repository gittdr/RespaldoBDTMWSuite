...
CREATE PROCEDURE [dbo].[sp_control_oper_integrated_D6_JR]   @lider varchar(12)
AS
DECLARE	
	@V_fechadom		DateTime, 
	@V_fechasab		DateTime,
...
DECLARE @TTOperadores_Control_D6 TABLE(
		...
		If Exists ( Select count(*) From  @TTOperadoresporflota )
				...
				-- Llena los totales de cada codigo.
					--Domingo Dia 1 Descansos
						Insert Into @TTOperadores_Control_D6 (codigo,OpeId,OpeNombre,Fechaini,Fechafin, FolioExt )
...
--						Insert Into @TTOperadores_Control_D6 (codigo,OpeId,OpeNombre,Fechaini,Fechafin )
...
select * from @TTOperadores_Control_D6
