...
CREATE PROCEDURE [dbo].[ResNowOV_R1_Destination]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
		SELECT 	ord_destpoint AS [Destination Code], 
		...
		WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND ord_destpoint NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			ORDER BY ord_destpoint 
		END
	...
	BEGIN
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[DateEnd] = @DateEnd,
				[ItemCategory] = @Mode,
				[ItemID] = ord_destpoint, 
				[ItemDescription] = (SELECT cmp_name + ' - ' + cty_nmstct FROM company (NOLOCK) where cmp_id = ord_destpoint),
				[Count] = COUNT(*),
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_Destination] TO [public]
GO
