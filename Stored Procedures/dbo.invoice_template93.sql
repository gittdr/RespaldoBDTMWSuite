...
GO
  ...
  create procedure [dbo].[invoice_template93](@p_invoice_nbr   int,@p_copies  int)  
as  
/**
 * 
 * NAME:
 * dbo.invoice_template93
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * based on the invoice number selected in the interface.  This also removes
 * trailing and leading alpha characters from the invoice number.
 * RETURNS:
 ...
 * 05/11/2006.01 - PTS32739 - PRB - Added ability to remove alpha char from beginning and end
...
from  #invtemp_tbl INNER JOIN stops
      ON stops.stp_number =  #invtemp_tbl.stp_number
...
while @v_counter <>  @p_copies  
 ...
 select @v_counter = @v_counter + 1  
  ...
  SELECT   
     --PRB added for PTS32739 -- Need to remove all alpha characters from front and back.
     --Basically return the order number.
     ivh_invoicenumber = CASE convert( int , 
         case
           ...
           -- 0 means that it is already all numbers.
           when patindex('%[0-9]%' , ivh_invoicenumber) = 0
                then 0
	   -- Get the first non-numeric char. this assumes
           -- the format '###AA'
           -- where A represents non-numeric digit
           when patindex('%[0-9]%' , ivh_invoicenumber) = 1
                then 1
	   --This represents a supplemental invoice of S####
	   when patindex('%[0-9]%', ivh_invoicenumber) > 1
		then 2
           -- this is a completely numeric value.
           else 123
         end )
	WHEN 0 THEN ivh_invoicenumber
	WHEN 1 THEN substring( ivh_invoicenumber , 1 , patindex('%[^0-9]%' , ivh_invoicenumber) - 1)
	WHEN 2 THEN substring(ivh_invoicenumber ,  patindex('%[0-9]%' , ivh_invoicenumber), LEN(ivh_invoicenumber))       
...
--select *  
--from #invtemp_tbl  
/* --PTS 39121 EMK Not working for invoices without letters
  SELECT   
     --PRB added for PTS32739 -- Need to remove all alpha characters from front and back.
     --Basically return the order number.
     ivh_invoicenumber = CASE convert( int , 
	...
	END,     
*/
	SELECT 	ivh_invoicenumber =  substring(ivh_invoicenumber,patindex('%[^0-9][0-9]%', ivh_invoicenumber)+1,
											CASE when patindex('%[0-9][^0-9]%', ivh_invoicenumber) > 0 then patindex('%[0-9][^0-9]%', ivh_invoicenumber)
											else len(ivh_invoicenumber)
											end - patindex('%[^0-9][0-9]%', ivh_invoicenumber)),
--PTS 39121
	ivh_hdrnumber,   
	ivh_billto,   
  	ivh_billto_name ,  
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template93] TO [public]
GO
