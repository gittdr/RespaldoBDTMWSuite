...
CREATE PROC [dbo].[d_masterbill119_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 * BDH 6/6/08 PTS 42429:  Created this from d_masterbill32_sp.
 *
 **/
DECLARE @int0  int
SELECT @int0 = 0
...
---- PTS 29278 -- BL (start)
--			#masterbill_temp.sort_field = ref_number
---- PTS 29278 -- BL (end)
...
update #masterbill_temp
set ord_firstref = ih.ivh_ref_number
from invoiceheader ih
where #masterbill_temp.ord_hdrnumber = ih.ord_hdrnumber 
and ih.ivh_reftype = 'TICKET'
  ...
  UPDATE 	#masterbill_temp 
  SET		#masterbill_temp.ivh_totalvolume = i.ivh_totalvolume
  FROM		#masterbill_temp, invoiceheader i 
  WHERE		#masterbill_temp.ivh_hdrnumber = i.ivh_hdrnumber and
		ivd_sequence=1
...
-- PTS 29278 -- BL (start)
  UPDATE 	#masterbill_temp 
  SET		#masterbill_temp.sort_field = (select max(sort_field) + '&&&&&'
						from #masterbill_temp b
						where b.ord_hdrnumber = #masterbill_temp.ord_hdrnumber)
  FROM		#masterbill_temp
  WHERE		#masterbill_temp.sort_field is NULL
  OR		rtrim(#masterbill_temp.sort_field) = ''
-- PTS 29278 -- BL (end)

update #masterbill_temp
	set ord_remark = oh.ord_remark
	from orderheader oh
	where #masterbill_temp.ord_hdrnumber = oh.ord_hdrnumber 

  SELECT * 
  FROM		#masterbill_temp
-- PTS 29278 -- BL (start)
--  ORDER BY	ord_hdrnumber, ivd_sequence
  ORDER BY	sort_field, ord_hdrnumber, ivd_sequence
-- PTS 29278 -- BL (end)

  DROP TABLE 	#masterbill_temp
GO
GRANT EXECUTE ON  [dbo].[d_masterbill119_sp] TO [public]
GO
