...
CREATE PROC [dbo].[AssetsAutoMatch_FilterRequirements_sp]
(
	@ord_hdrnumber	int
)
 ...
 * 
 * NAME:
 * dbo.AssetsAutoMatch_FilterRequirements_sp
 *
 * TYPE:
 * Stored Proc
 *
 * DESCRIPTION:
 * 
 * Adjusts existing temp table #matching_assets, created by caller, and removes assets that cannot be assigned to passed in order
 *
 * PTS 46342 JJF 20090820
 *
 **/
...
BEGIN
	...
	SELECT	@lgh_number = lgh.lgh_number,
			@mov_number = oh.mov_number
	FROM	orderheader oh
			inner join legheader lgh on oh.mov_number = lgh.mov_number
	...
	WHILE ISNULL(@current_item, 0) > 0 BEGIN
			...
			SELECT	@car_id = ma.car_id
			FROM	#matching_assets ma
			WHERE	ma.ma_id = @Current_item
							...
							IF @lrq_not = 'N' BEGIN  -- Should/Must NOT have
								IF @lrq_equip_type = 'DRV' BEGIN
									...
									WHERE	@car_id in (SELECT DISTINCT drq.drq_driver 
											...
											and #matching_assets.ma_id = @current_item
								END
			...
			IF EXISTS(	SELECT	*
				...
				DELETE #matching_assets
				WHERE ma_id = @current_item
			END

		SELECT @current_item = MIN(ma.ma_id)
		FROM #matching_assets ma
		WHERE ma.ma_id > @current_item
...
END
GO
GRANT EXECUTE ON  [dbo].[AssetsAutoMatch_FilterRequirements_sp] TO [public]
GO
