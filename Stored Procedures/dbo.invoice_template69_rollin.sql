...
CREATE procedure [dbo].[invoice_template69_rollin](@invoice_nbr   int,@copies  int)  
as 
/****** DOES NOT SUPPORT ROLL IN WORKS LIKE MAIN PROC *******/
...
set consignee_name = company.cmp_name,  
 ...
 consignee_nmctst =substring(city.cty_nmstct,1, (charindex('/',city.cty_nmstct)))+ ' ' + ISNULL(city.cty_zip,'')   
from #invtemp_tbl, company, city  
where company.cmp_id = #invtemp_tbl.ivh_destpoint  
 and city.cty_code =  #invtemp_tbl.ivh_destcity    
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template69_rollin] TO [public]
GO
