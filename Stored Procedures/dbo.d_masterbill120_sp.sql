...
CREATE PROC [dbo].[d_masterbill120_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
...
-- yet still show the rest of the info
   ...
   WHERE 
	...
	update #temp
	set refnum_1 = ref.ref_number
	from referencenumber ref, invoiceheader ivh
	where #temp.ivh_invoicenumber = ivh.ivh_invoicenumber
		and ivh.ord_hdrnumber = ref.ord_hdrnumber 
		and ref.ref_sequence = 1


	update #temp
	set refnum_2 = ref.ref_number
	from referencenumber ref, invoiceheader ivh
	where #temp.ivh_invoicenumber = ivh.ivh_invoicenumber
		and ivh.ord_hdrnumber = ref.ord_hdrnumber 
...
IF UPPER(@reprintflag) <> 'REPRINT' 
  BEGIN
   ...
   WHERE (invoiceheader.ivh_billto = @billto )  
	...
	set refnum_1 = ref.ref_number
	from referencenumber ref, invoiceheader ivh
	where #temp2.ivh_invoicenumber = ivh.ivh_invoicenumber
		and ivh.ord_hdrnumber = ref.ord_hdrnumber 
		and ref.ref_sequence = 1
...
END
GO
GRANT EXECUTE ON  [dbo].[d_masterbill120_sp] TO [public]
GO
