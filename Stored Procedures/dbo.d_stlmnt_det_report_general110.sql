...
CREATE  PROC [dbo].[d_stlmnt_det_report_general110](
 ...
 *
 * REVISION HISTORY:
 * 9/25/2009 PTS 48318 Custom Report Created.
 * 10-7-2009 PTS 48318 adjustments for descr + amounts when orderheader <> a number > 0 
 *
 **/
...
-- PTS 29303 -- BL (end)	
	...
	pyt_basis		varchar(6) null,   -- PTS 29515 -- BL (start)	
	ls_duplicate	char(1)	   -- PTS 48318
)
...
UPDATE #temp_pd
   SET ord_number = CAST( LTRIM(RTRIM(ord_number)) + '/S' as char(12))
...
--JD #11490 09/24/01
UPDATE #temp_pd
SET    shipper_city = cast(ct.cty_name as char(30)),
...
UPDATE #temp_pd
SET    consignee_city = cast(ct.cty_name as char(18)),
...
-- PTS 48318 <<start of modifications for 48318>>
-- PTS 48318 itemsection 0: descr needs to be order start/end city-state.
update #temp_pd
set pyd_description = shipper_city  + ', ' + shipper_state + ' to ' + consignee_city  + ', ' + consignee_state
where itemsection = 0
and ( ( shipper_city is not null  and shipper_city <> '' )			-- 48318 10-7-09 fix 1
and  ( consignee_city is not null and consignee_city <> '' ) )		-- 48318 10-7-09 fix 1

-- 48318 10-7-09 fix 2 <<start>>
update #temp_pd
set pyd_description = (select pyt_description from paytype where pyt_itemcode = #temp_pd.pyt_itemcode)
where itemsection = 3
and pyd_description is NULL or pyd_description = ''
-- 48318 10-7-09 fix 2 <<end>>

-- PTS 48318 itemsection 0: sum of the pyd_amounts
select ord_hdrnumber, sum(pyd_amount)as 'sum_pyd_amount'
into #temp_sum_earnings 
from #temp_pd 
where itemsection = 0					
group by ord_hdrnumber  

update #temp_pd
set #temp_pd.pyd_amount = (select sum_pyd_amount from #temp_sum_earnings where  #temp_pd.ord_hdrnumber = #temp_sum_earnings.ord_hdrnumber)
where itemsection = 0	
AND (ord_hdrnumber <> 0)			-- 48318 10-7-09 fix 3
AND ord_hdrnumber is not null		-- 48318 10-7-09 fix 3

-- PTS 48318 itemsection 0: remove duplicate rows
select ord_hdrnumber, MIN(pyd_number) as 'min_pyd_number' 
into #temp_min_pyd_number
from #temp_pd 
where itemsection = 0	
AND ord_hdrnumber > 0				-- 48318 10-7-09 fix 4
AND ord_hdrnumber is not null		-- 48318 10-7-09 fix 4						
group by ord_hdrnumber  

update #temp_pd
set #temp_pd.ls_duplicate = 'Y' 
where itemsection = 0	

-- 48318 10-7-09 fix 5 <<start>>
update #temp_pd
set #temp_pd.ls_duplicate = 'N' 
where itemsection = 0	
and ord_hdrnumber = 0
or ord_hdrnumber is null 
-- 48318 10-7-09 fix 5 <<end>>

update #temp_pd
set #temp_pd.ls_duplicate = 'N' 
where itemsection = 0	
and pyd_number = (select min_pyd_number from #temp_min_pyd_number where  #temp_pd.ord_hdrnumber = #temp_min_pyd_number.ord_hdrnumber)

delete from #temp_pd
where #temp_pd.ls_duplicate = 'Y' 
and  itemsection = 0	
-- PTS 48318 <<end of modifications for 48318>>
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general110] TO [public]
GO
