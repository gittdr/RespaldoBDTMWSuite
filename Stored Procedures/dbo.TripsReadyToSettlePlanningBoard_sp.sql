...
CREATE PROCEDURE [dbo].[TripsReadyToSettlePlanningBoard_sp](@drvyes                VARCHAR(3),
                                                    @trcyes                VARCHAR(3),
                                                    @caryes                VARCHAR(3),
                                                    @loenddate             DATETIME,
                                                    @hienddate             DATETIME,
 ...
 *
 * NAME:
 * dbo.TripsReadyToSettlePlanningBoard_sp
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * 023 - trip_description varchar(5000) null,
 * 024 - lgh_type1   varchar(6) null,
 * 025 - lgh_type1_name  varchar(8) null, 
 * 026 - lgh_type2   varchar(6) null,
 * 027 - lgh_type2_name  varchar(8) null,
 * 028 - lgh_type3   varchar(6) null,
 * 029 - lgh_type3_name  varchar(8) null,
 * 030 - lgh_type4   varchar(6) null,
 * 031 - lgh_type4_name  varchar(8) null,
 * 032 - ivh_billdate datetime Null,
 * 033 - ivh_invoicenumber varchar(12) Null,
 * 034 - lgh_booked_revtyep1 varchar(12)
 * 035 - ivh_billto
 * 036 - asgn_controlling
 * 037 - shiftdate datatime
 * 038 - lgh_shiftnumber
 * 039 - asgn_payto - The pay to associated to the asset
 *
 * PARAMETERS:
 ...
 * 072 - @pay_sch_mode           VARCHAR(6) - Pay ScheduleMode either Regular or AutClose Trip Pay
 * 073 - @lgh_type2 varchar(6),  -- PTS 104814 - GT - Support filter criteria for LegType2,LegType3, LegType4
 * 074 - @lgh_type3 varchar(6),  -- PTS 104814 - GT - Support filter criteria for LegType2,LegType3, LegType4
 * 075 - @lgh_type4 varchar(6),  -- PTS 104814 - GT - Support filter criteria for LegType2,LegType3, LegType4
 *
 * REVISION HISTORY:
 ...
 * 2014/10/16 | PTS 83249 | vjh	  - fix use of accounting type
 * 2014/10/20 | PTS 81134 | AVANE - change PayTo filters to strictly filter to "settle by PayTo" functionality (pto_stlByPayTo)
 * 2015/03/16 | PTS 87815 | APAND - Consider Pay ScheduleMode filter "pay Schedule Mode" functionality (pay_sch_mode)
 * 2015/11/17 | PTS 96292 | GTEJW - Skip legs for queue processing where lgh_Ratemode/ord_ratemode is '3PLINV'
 * 2016/30/16 | PTS 103309 | EHAMM - Implemented additional filtering to remove non-invoiced returns from the result set when STLMUSTINV = 'Y'.
 * 2016/08/02 | PTS 104706 | AVANE - Refactor query introduced by PTS 96292 for poor performance on multiple joins to stop table
 * 2016/08/19 | PTS 104851 | AV - Provide filtering of Thirdpartyassignment ThirdPartyType1-4 values
 * 2016/08/30 | PTS 105064 | AV - DBA standards fixes
 * 2016/10/14 | PTS 104814 | GTEJW - Support filter criteria for LegType2,LegType3, LegType4
 ...
 **/
     ...
     (lgh_number          INT NOT NULL,
      ...
      trip_description    VARCHAR(5000) NULL,
      lgh_type1           VARCHAR(6) NULL,
      lgh_type1_name      VARCHAR(8) NULL,
      ...
      lgh_type2_name      VARCHAR(8) NULL,
      lgh_type3           VARCHAR(6) NULL,
      lgh_type3_name      VARCHAR(8) NULL,
      lgh_type4           VARCHAR(6) NULL,
      lgh_type4_name      VARCHAR(8) NULL,
    ...
    , gi_date1
     ...
     IF @drvyes <> 'XXX'
         INSERT INTO @tmp
                ...
                FROM manpowerprofile
                     ...
                     LEFT OUTER JOIN ManpowerProfile_moreSettings ON ManpowerProfile_moreSettings.resource_id = manpowerprofile.mpp_id
                WHERE(@driver = mpp_id
                      OR @driver = 'UNKNOWN')
                     --      AND ((@acct_typ = 'X' AND mpp_actg_type IN('A', 'P')) OR (@acct_typ = mpp_actg_type) ) --NSUITE-200028 | vjh - actg type from profile is irrevelant
                     AND (@company = 'UNK'
                     ...
                     AND ((@pay_sch_mode = 'REG'
                           AND COALESCE(ManpowerProfile_moreSettings.AutoCloseStatus, 'DIS') = 'DIS')
                          OR (@pay_sch_mode = 'ACTP'
                              AND COALESCE(ManpowerProfile_moreSettings.AutoCloseStatus, 'DIS') IN('XRF', 'CLS'))) --- APAND when Regular mode look for Disabled settings, when AutoClose mode Look for Transferred or Closed Settings only	
         ...
         INSERT INTO @tmp1
                ...
                FROM tractorprofile
                     JOIN RowRestrictValidAssignments_tractorprofile_fn() rsva ON tractorprofile.rowsec_rsrv_id = rsva.rowsec_rsrv_id
                                                                                  OR rsva.rowsec_rsrv_id = 0
                     LEFT OUTER JOIN tractorprofile_moreSettings ON tractorprofile_moreSettings.resource_id = tractorprofile.trc_number
                WHERE(@tractor = trc_number
                      OR @tractor = 'UNKNOWN')
                     --      AND ((@acct_typ = 'X' AND trc_actg_type IN('A', 'P')) OR (@acct_typ = trc_actg_type) ) --NSUITE-200028 | vjh - actg type from profile is irrevelant
                     AND (@company = 'UNK'
                     ...
                     AND ((@pay_sch_mode = 'REG'
                           AND COALESCE(tractorprofile_moreSettings.AutoCloseStatus, 'DIS') = 'DIS')
                          OR (@pay_sch_mode = 'ACTP'
                              AND COALESCE(tractorprofile_moreSettings.AutoCloseStatus, 'DIS') IN('XRF', 'CLS'))) --- APAND when Regular mode look for Disabled settings, when AutoClose mode Look for Transferred or Closed Settings only	
         ...
         INSERT INTO @tmp2
                ...
                FROM carrier
                     JOIN RowRestrictValidAssignments_carrier_fn() rsva ON carrier.rowsec_rsrv_id = rsva.rowsec_rsrv_id
                                                                           OR rsva.rowsec_rsrv_id = 0
                     LEFT OUTER JOIN carrier_moreSettings ON carrier_moreSettings.resource_id = carrier.car_id
                WHERE(@carrier = car_id
                      OR @carrier = 'UNKNOWN')
                     --AND ((@acct_typ = 'X' AND car_actg_type IN('A', 'P')) OR (@acct_typ = car_actg_type)) --NSUITE-200028 | vjh - actg type from profile is irrevelant
                     AND (@cartyp1 = 'UNK'
                     ...
                     AND ((@pay_sch_mode = 'REG'
                           AND COALESCE(carrier_moreSettings.AutoCloseStatus, 'DIS') = 'DIS')
                          OR (@pay_sch_mode = 'ACTP'
                              AND COALESCE(carrier_moreSettings.AutoCloseStatus, 'DIS') IN('XRF', 'CLS'))) --- APAND when Regular mode look for Disabled settings, when AutoClose mode Look for Transferred or Closed Settings only
     ...
     IF @trlyes <> 'XXX'
         INSERT INTO @tmp3
                ...
                WHERE(@trailer = trl_id
                      OR @trailer = 'UNKNOWN')
                     --     AND ( (@acct_typ = 'X' AND trl_actg_type IN('A', 'P')) OR (@acct_typ = trl_actg_type) ) --NSUITE-200028 | vjh - actg type from profile is irrevelant
                     AND (@company = 'UNK'
     ...
     --PTS 96292 Skip legs for queue processing where lgh_Ratemode/ord_ratemode is '3PLINV'
     DELETE t
     FROM #temp_rtn t
     WHERE EXISTS
     (
         SELECT 1
         FROM legheader lh
         WHERE lh.lgh_number = t.lgh_number
               AND lh.lgh_ratemode = '3PLINV'
     )

     -- Attempt to find an associated order number which has order rate mode as '3PLINV' 
     DELETE t
     FROM #temp_rtn t
     WHERE EXISTS
     (
         ...
         WHERE s.mov_number = t.mov_number
               AND o.ord_ratemode = '3PLINV'
     )
     ...
     -- PTS 104814 - GT - Support filter criteria for LegType2,LegType3, LegType4
     SELECT @lgh_type2 = COALESCE(@lgh_type2, 'UNK')
     IF @lgh_type2 <> 'UNK'
        AND @lgh_type2 <> ''
         DELETE FROM #temp_rtn
         WHERE lgh_type2 <> @lgh_type2

     SELECT @lgh_type3 = COALESCE(@lgh_type3, 'UNK')
     IF @lgh_type3 <> 'UNK'
        AND @lgh_type3 <> ''
         DELETE FROM #temp_rtn
         WHERE lgh_type3 <> @lgh_type3

     SELECT @lgh_type4 = COALESCE(@lgh_type4, 'UNK')
     IF @lgh_type4 <> 'UNK'
        AND @lgh_type4 <> ''
         DELETE FROM #temp_rtn
         WHERE lgh_type4 <> @lgh_type4
...
/* Insert a record into #temp_pwk for every Legheader/Orderheader combination.  Gets all the Orderheaders on
   a Leg so Paperwork can be tracked for every Order.    */
     ...
     -- PTS 103309 - Remove results with no associated invoice numbers if stlmustinv is 'Y'.
     IF @stlmustinv = 'Y'
         BEGIN
             DELETE FROM #temp_rtn1
     ...
     -- PTS 52192 <<end>>
     ----------------------------------------------------------------------------------------------------------
     --PTS 82128 - remove trips from queue for orders related to Agent Pay
     DELETE FROM #temp_rtn1
     WHERE #temp_rtn1.ord_hdrnumber IN
     (
         SELECT tpa.ord_hdrnumber
         FROM Thirdpartyassignment tpa
...
GO
GRANT EXECUTE ON  [dbo].[TripsReadyToSettlePlanningBoard_sp] TO [public]
GO
