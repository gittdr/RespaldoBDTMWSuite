...
CREATE PROC [dbo].[credit_pay_pi_paydetail_list_sp]	(	@ivh_hdrnumber int, 
													@user_id varchar(20), 
...
select pyd_number,
	...
	pyd_credit_pay_flag,
	pyd_ivh_hdrnumber,
	isNull(pyd_min_period, pyh_payperiod) pyd_min_period
from paydetail 
where pyd_ivh_hdrnumber = @ivh_hdrnumber
	and pyt_itemcode in (select pyt_itemcode from paytype
							where pyt_basisunit = 'REV' or pyt_basis = 'INV')
	And pyd_number In ( Select Distinct pyd_number from @PayDetailList ) 
...
select @li_count = count(*) from #pay_temp
...
GO
GRANT EXECUTE ON  [dbo].[credit_pay_pi_paydetail_list_sp] TO [public]
GO
