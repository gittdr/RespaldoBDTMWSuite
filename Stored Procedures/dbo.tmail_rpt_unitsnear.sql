...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[tmail_rpt_unitsnear] @dispgroup int
 ...
 * 08/30/01 DAG: Coneverting state to length 6 for international
	...
	IF (@dispgroup <> -1)	-- Does user want all trucks or only a specific dispatch group?
	  BEGIN
		IF EXISTS (SELECT truckname 
		  ...
		  END
	  END
	ELSE  -- Pull all tractors, not just a specific dispatch group
  	  BEGIN
		SELECT @WorkDate = MAX(ckc_date)
		FROM checkcall (NOLOCK)
		WHERE ckc_tractor = @WorkTractor
...
GO
GRANT EXECUTE ON  [dbo].[tmail_rpt_unitsnear] TO [public]
GO
