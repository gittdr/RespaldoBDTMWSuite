...
CREATE PROC [dbo].[d_masterbill111_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 * 01/02/2008 BDH PTS 40096.  Created this (d_masterbill111_sp) from D_MASTERBILL07_SP for Earl Martin.  Above comments are from D_MASTERBILL07_SP.
 **/
...
-- dpete for LOR pts4785 provide for maitlto override of billto
	...
	ivh_billto_zip = 
		...
		@copy,
		-- ref.ref_number
		(select top 1 ref_number from referencenumber where ref_table = 'orderheader' and ref_tablekey = invoiceheader.ord_hdrnumber and invoiceheader.ord_hdrnumber > 0 and ref_type in ('PO#','REF#' )) ref_number,  -- 40096 BDH
		ivd.cmp_id cmp_id,
		cmp2.cmp_name,
		invoiceheader.ivh_remark
    --pts40029 jg outer join conversion
    FROM 	invoiceheader, /*left outer join referencenumber ref on (invoiceheader.ord_hdrnumber = ref.ref_tablekey and ref.ref_table = 'orderheader' and ref.ref_sequence = 2), */  -- 40096 BDH
		company cmp1,
		company cmp2, 
		city cty1, 
		city cty2, 
...
GO
GRANT EXECUTE ON  [dbo].[d_masterbill111_sp] TO [public]
GO
