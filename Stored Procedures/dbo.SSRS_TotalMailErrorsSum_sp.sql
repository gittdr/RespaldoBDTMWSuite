...
create Procedure [dbo].[SSRS_TotalMailErrorsSum_sp]
(
@DateStart datetime,
@DateEnd datetime
)
/*
SSRS_TotalMailErrorsSum_sp
10/22/2014

Example call

exec [SSRS_TotalMailErrorsSum_sp] @DateStart = '10/1/2014',@DateEnd='10/15/2014'
Change Log:
...
select [ErrorShort],count([ErrorShort]) as Errors,
CONVERT(float,0) as [Percent of Total Errors]
into #temp2
from #temp1
WHERE [ErrorCount]=1
GROUP BY [ErrorShort]
ORDER BY Errors desc,[ErrorShort]

declare @TotalErrs int
set @TotalErrs = (Select sum(Errors) from #temp2)

if @TotalErrs>0
BEGIN

	UPDATE #temp2 set [Percent of Total Errors] = Convert(float,Errors)/Convert(float,@TotalErrs)
END

select * from #temp2
GO
GRANT EXECUTE ON  [dbo].[SSRS_TotalMailErrorsSum_sp] TO [public]
GO
