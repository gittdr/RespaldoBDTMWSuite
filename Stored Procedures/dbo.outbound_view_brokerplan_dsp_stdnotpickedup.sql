...
CREATE PROCEDURE [dbo].[outbound_view_brokerplan_dsp_stdnotpickedup](
	...
	@lgh_oustatuslist	varchar(255), --had to allow nulls starting w/66176 or form blows up.
	@retrievemode		varchar(8),
	@billto			VARCHAR(8) = null,
	@revtype2		VARCHAR(6) = null)
-- 57372 MTC 20110608 Added NOLOCK to orderheader and legheader_active. Added set nocount ON.
-- MTC 66176 major overhaul to dynamic SQL for performance reasons. Added an index also.

AS
BEGIN

set nocount on --57372
...
select @SQL = @SQL + N' from legheader_active l With (NoLock) left outer join orderheader o With (NoLock) on l.mov_number = o.mov_number '  
...
select @SQL = @SQL + N'and lgh_outstatus = ''STD'' AND l.lgh_carrier <> ''' + @newcarrier + ''' '	
select @SQL = @SQL + N'and o.ord_status in (''PLN'',''DSP'',''AVL'') '
...
END
GO
GRANT EXECUTE ON  [dbo].[outbound_view_brokerplan_dsp_stdnotpickedup] TO [public]
GO
