...
CREATE procedure [dbo].[invoice_template161_rollin](@p_invoice_nbr int,@p_copies  int)  
as

/**
 * 
 * NAME:
 * dbo.invoice_template161 (AUTOPAY)
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * 02/11/2009 JSwindell  Created new proc - it's the SAME as  but omits Linehaul and instead shows 'auto pay' on invoice.
 *
 * 05/08/2009 JSwindell - PTS 47380
 *						- 1) for AutoPay we DON'T want the inv to produce output If there are no accessorial or line item charges.
 *								- to get this to work - return another column with a FLAG to 
 *								- set lb_nooutput in nvo_invprintsrvc.of_produceinvoice
 *						- 2) show $$ for MINIMUM charges
 * 08/10/2009 PTS48333  fix to print MIN when it's the only line item  -- look for   --PTS48333 Min Fix
 * 08/17/09   PTS48668  fix GST tax being calculated on accessorial charges that are tax exempt -- consolidated w/48333 
 *						Add  cht_taxtable1 to the proc.
 **/
...
(ivh_invoicenumber varchar(12) not null ,     
 ...
 cht_taxtable1  			varchar (1)  NULL,		-- PTS48668	
...
--****************
-- PTS 45519 AutoPay: <<start>>  If there are no accessorial or line item charges - they do NOT want to print the invoice.

--* PTS 47380 If @ll_nooutput = 1 THEN PRINT(default).  If @ll_nooutput = 0 THEN DO NOT PRINT.
DECLARE @ll_nooutput int		-- PTS 47380
select @ll_nooutput = 1			-- PTS 47380

--PTS48333 8-10-09 Min fix

---if (select count(*) from #invtemp_tbl where cht_primary = 'N' AND cht_itemcode <> 'GST' ) = 0
if (select count(*) from #invtemp_tbl where ( cht_primary = 'N' AND cht_itemcode <> 'GST' ) OR  cht_itemcode = 'MIN' ) = 0
	begin
				--select @v_ret_value = 0  
				--return @v_ret_value
	-- PTS 47380 do not print THIS invoice - handle with a data column SETTING.
	select @ll_nooutput = 0
	end
-- PTS 45519 AutoPay: <<end>> 
--****************
...
-- 1)Invoice header notes 2)Orderheader notes  3)company (3a -ivh_billto 3b-ivh_shipper 3c-ivh_consignee) 4) carrier 5) manpowerprofile (driver) 6) movement
...
--*****************************
-- PTS 45519 AutoPay:  The only change is to change the Charges to zero and use the cht_primary to flag it.
-- If there are no accessorial or line item charges - they do NOT want to print the invoice.

UPDATE #invtemp_tbl
	--Set ivd_charge = 0 where cht_primary = 'Y'		-- PTS 47380  show $$ for MINIMUM charges
	Set ivd_charge = 0 
...
------================================================================ FINAGEL WITH PHINEUS - calc NEW_TAX_AMOUNT 
		SELECT	CC_IDENTITY		, ivh_invoicenumber, 	ivd_rate, cht_primary, NEW_TAX_AMOUNT, cht_rollintoLH, cht_basis ,  ivd_charge
				into #TEMP_CALC_NEW_TAXES	
				FROM 	#invtemp_tbl

				WHERE	( cht_primary = 'N' AND cht_rollintoLH = 0 AND IVD_CHARGE <> 0	and cht_taxtable1 = 'Y') OR					
						( cht_primary = 'N' AND cht_basis = 'TAX' and cht_taxtable1 = 'Y' ) OR
						( cht_itemcode = 'MIN' and cht_taxtable1 = 'Y')	

--				-- PTS48668 Change select to consider taxible = Y or N !!
--				WHERE	( cht_primary = 'N' AND cht_rollintoLH = 0 AND IVD_CHARGE <> 0 ) OR
--						( cht_primary = 'N' AND cht_basis = 'TAX'  ) 					
--						OR  cht_itemcode = 'MIN'	--PTS48333 8-10-09 fix min:  include MIN (since they can be cht_primary=Y)	
		...
		UPDATE #TEMP_CALC_NEW_TAXES
		SET NEW_TAX_AMOUNT = ( select sum(NEW_TAX_AMOUNT) FROM #TEMP_CALC_NEW_TAXES where cht_basis <> 'TAX') 
		where cht_basis = 'TAX'

		UPDATE #invtemp_tbl
		SET NEW_TAX_AMOUNT = (SELECT NEW_TAX_AMOUNT FROM #TEMP_CALC_NEW_TAXES WHERE #TEMP_CALC_NEW_TAXES.CC_IDENTITY = #invtemp_tbl.CC_IDENTITY) 

		UPDATE #invtemp_tbl 
		Set ivd_charge  = (select NEW_TAX_AMOUNT FROM #TEMP_CALC_NEW_TAXES WHERE #TEMP_CALC_NEW_TAXES.CC_IDENTITY = #invtemp_tbl.CC_IDENTITY AND cht_basis = 'TAX') 
		where cht_basis = 'TAX'


-- PTS 48668 If have made any GST items zero because of tax exempt, then remove that line.
--  need to delete it to keep group 2 from printing a zero $ total line.
delete #invtemp_tbl where ivd_charge is null and cht_itemcode = 'GST' 
...
select ivh_invoicenumber,   
	 ...
	 ivh_showcons,
		...
		isnull(not_text4,'')'not_text4',	--PTS 45519
		isnull(not_text5,'')'not_text5',	--PTS 45519
		@GSTNUMBER	as 'GSTNUMBER',		--PTS 45519				-151
		@ll_nooutput as 'nooutput'				--PTS  47380	-152					
from #invtemp_tbl
...
return @v_ret_value
GO
GRANT EXECUTE ON  [dbo].[invoice_template161_rollin] TO [public]
GO
