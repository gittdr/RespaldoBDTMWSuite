...
CREATE PROC [dbo].[d_settlement_sheet_summary_80](
 ...
 * 
 * NAME:
 * d_settlement_sheet_summary_80
 *
 * TYPE:
 * [StoredProcedure)
 *
 * DESCRIPTION:
 * This procedure returns data for settlement sheet d_stlmnt_det_report_general80 
 *
 * RESULT SETS:  
 * See selection list 
 * 
 ...
 * 
 * PARAMETERS:
 * 001 - @report_type, varchar(5), input, null;
 *		 This parameter indicates the report type 'final' or 'trial'. 
 *		 The value can be empty, but not null.	
 * 002 - @payperiodstart, datetime, input, null;
 *		 This parameter indicates the report period starting date. 
 *		 The value must be non-null and non-empty.
 * 003 - @payperiodend, datetime, input, null;
 *		 This parameter indicates the report period ending date. 
 *		 The value must be non-null and non-empty.	
 * 004 - @drv_yes, varchar(3), input, null;
 *		 This parameter indicates if the report is by driver. 
 *		 The value can be XXX and must be non-null and non-empty.	
 * 005 - @trc_yes, varchar(3), input, null;
 *		 This parameter indicates if the report is by tractor. 
 *		 The value can be XXX and must be non-null and non-empty.
 * 006 - @trl_yes, varchar(3), input, null;
 *		 This parameter indicates if the report is by trailer. 
 *		 The value can be XXX and must be non-null and non-empty.
 * 007 - @drv_id, varchar(8), input, null;
 *		 This parameter indicates the id for the payable asset. 
 *		 The value will be the id of the payable asset.	
 * 008 - @trc_id, varchar(8), input, null;
 *		This parameter indicates the id for the payable asset. 
 *		 The value will be the id of the payable asset.		
 * 009 - @trl_id, varchar(13), input, null;
 *		 This parameter indicates the id for the payable asset. 
 *		 The value will be the id of the payable asset.	
 * 010 - @drv_type1, varchar(6), input, null;
 *		 This parameter indicates drv_type1 from manpowerprofile for the driver. 
 *		 The value can be null or empty.	
 * 011 - @trc_type1, varchar(6), input, null;
 *		 This parameter indicates trc_type1 from tractorprofile for the tractor. 
 *		 The value can be null or empty.	
 * 012 - @trl_type1, varchar(6), input, null;
 *		 This parameter indicates trl_type1 from trailerprofile for the trailer. 
 *		 The value can be null or empty.	
 * 013 - @terminal, varchar(8), input, null;
 *		 This parameter indicates the terminal for the payable asset. 
 *		 The value can be null or empty.	
 * 014 - @name, varchar(64), input, null;
 *		 This parameter indicates the Name of the payable asset. 
 *		 The value must be non-null and non-empty.	
 * 015 - @car_yes, varchar(3), input, null;
 *		 This parameter indicates if the report is by carrier. 
 *		 The value can be XXX and must be non-null and non-empty.
 * 016 - @car_id, varchar(8), input, null;
 *		 This parameter indicates the id for the payable asset. 
 *		 The value will be the id of the payable asset.	
 * 017 - @car_type1, varchar(6), input, null;
 *		 This parameter indicates car_type1 from carrier for the carrier. 
 *		 The value can be null or empty.	
 * 018 - @hld_yes, varchar(3),	input, null;
 *		 This parameter indicates the if the pay is on 'HOLD' or 'RELEASED'. 
 *		 The value must be non-null and non-empty.	
 * 019 - @pyhnumber, int, input, null;
 *		 This parameter indicates the Pay Header number of the report. 
 *		 The value can be 0.	
 * 020 - @relcol, varchar(3), input, null;
 *		 This parameter indicates if the pay header has been release and collected. 
 *		 The value must be non-null and non-empty.
 * 021 - @relncol, varchar(3), input, null;
 *		 This parameter indicates if the pay header has been release and not collected. 
 *		 The value must be non-null and non-empty.	
 * 022 - @workperiodstart, datetime, input, null;
 *		 This parameter indicates the report work period starting date. 
 *		 The value must be non-null and non-empty.	
 * 023 - @workperiodend, datetime, input, null;
 *		  This parameter indicates the report wor period ending date. 
 *		 The value must be non-null and non-empty.
 *
 * REFERENCES: 
 * 
 * REVISION HISTORY:
 * 02/08/08.01 ? PTS 38192- Michalynn Kelly ? Created Sea Star Lines custom settlement sheet format 80
 * 01/xx/2009  - PTS 43677- JSwindell: Multiple Changes:
 *				 1) Users want the from and to date to reflect start date of Sunday and end date of Saturday.
 *				 2) Change to getting the data returned for Terminal
 *				 3) Change to getting the data for trailer numbers.
 * 08/18/2010	PTS 53648:  Change select to handle existance of multiple rows found.
 **/

-- PTS 43677: DATE CHANGES <<START>>--------------------------------------
--	Users want the from and to date to reflect start date of Sunday and end date of Saturday.
DECLARE @Original_payperiodstart datetime
DECLARE @Original_payperiodend datetime
DECLARE @workdate datetime
DECLARE @daynumber int
set @Original_payperiodstart = @payperiodstart
set @Original_payperiodend = @payperiodend

-- find previous sunday. Sunday is weekday 1. 
IF DATEPART ( weekday , @payperiodstart ) <> 1 
BEGIN
	SET @workdate = @payperiodstart
	SET @daynumber	= DATEPART (weekday, @workdate)
	...
	BEGIN
		...
		SET @daynumber	= DATEPART (weekday, @workdate)
	END
	IF DATEPART ( weekday , @workdate) = 1 
		BEGIN
			-- if = 1 then set the new sunday date - else(error) so leave the input date alone.
			SET @payperiodstart = @workdate
		END
END

-- find following saturday.  Saturday is weekday 7.
IF DATEPART ( weekday , @payperiodend ) <> 7
BEGIN
	SET @workdate = @payperiodend
	SET @daynumber	= DATEPART (weekday, @workdate)
	...
	BEGIN
		...
		SET @daynumber	= DATEPART (weekday, @workdate)
	END
	IF DATEPART ( weekday , @workdate) = 7
		BEGIN
			-- if = 7 then set the new Satruday date - else(error) so leave the input date alone.
			SET @payperiodend = @workdate
		END
END
-- PTS 43677: DATE CHANGES <<END>>--------------------------------------


-- PTS 38192 02-14-2008 JSwindell (add trc_type1)
Declare @name_trc_type1 varchar(20)

-- PTS 43677: Change how we do terminal. (code moved to bottom & changed)
--select @name_trc_type1 = name 
--from labelfile
--where labeldefinition= 'TrcType1'
--and abbr = ( select trc_type1 from tractorprofile
...
CREATE TABLE #temp_pd(
	...
	--ord_number		char(12) null,  -- PTS 43677 fix the truncation messages
	ord_number		varchar(20) null,	-- PTS 43677 fix the truncation messages	
	...
	-- otherid			varchar(8) null,	-- PTS 43677 fix the truncation messages
	otherid			varchar(25) null,  -- PTS 43677 fix the truncation messages
	pyt_fee1		money null,
	pyt_fee2		money null,
	...
	lgh_trailer varchar(13),
	ls_compute_terminal varchar(20) null,   -- PTS 43677: Need to change how we determine terminal.
	ll_real_stp_number int null,			-- PTS 43677
	evt_trailer1 varchar(13) null)			-- PTS 43677	
...
SELECT	pd.pyd_number,
		...
		(select pyh_issuedate from payheader where pyh_pyhnumber = pd.pyh_number) pyh_issuedate,
	 	null ,
		(select lgh_primary_trailer from legheader where lgh_number = pd.lgh_number) lgh_trailer,  -- pyt_basis
		null,			-- PTS 43677: terminal.
		pd.stp_number,  -- PTS 43677:  real stop number !!!!
		null			-- PTS 43677 evt_trailer1 			
...
-- PTS 43677 fix the truncation messages
UPDATE #temp_pd
   --SET shipper_name = co.cmp_name  -- PTS 43677 fix the truncation messages (cmp_name is x(100) )
   SET shipper_name = cast(co.cmp_name as char(30)) 
...
-- PTS 43677 fix the truncation messages
UPDATE #temp_pd
   --SET consignee_name = co.cmp_name  -- PTS 43677 fix the truncation messages (cmp_name is x(100) )
 SET consignee_name = cast(co.cmp_name as char(30)) 
  FROM #temp_pd tp, company co,orderheader oh
 WHERE tp.ord_hdrnumber = oh.ord_hdrnumber
...
-- PTS 43677:  Changes <<start>>  ----------------------------------------

Update #temp_pd
SET ls_compute_terminal = ( select name		
							from labelfile
							where labeldefinition= 'TrcType1'
							and abbr = ( select trc_type1 from tractorprofile
							where trc_number = @trc_id )) 
where #temp_pd.asgn_type = 'TRC'

Update #temp_pd
SET ls_compute_terminal = ( select  name 
							from labelfile
							where labeldefinition= 'TrlType1'
							and abbr = ( select Trl_Type1 from trailerprofile
							where trl_number = @trl_id )) 
where #temp_pd.asgn_type = 'TRL'

Update #temp_pd
SET ls_compute_terminal = ( select name		
							from labelfile
							where labeldefinition= 'RevType1'
							and abbr = ( select ord_revtype1 from orderheader							
							where ord_hdrnumber = #temp_pd.ord_hdrnumber  ) )
where #temp_pd.asgn_type <> 'TRC' and #temp_pd.asgn_type <> 'TRL'

set @name_trc_type1 = ( select max(ls_compute_terminal) 
						from #temp_pd 
						where ls_compute_terminal is not null 
						AND ls_compute_terminal <> 'UNKNOWN'
						AND ls_compute_terminal <> 'UNK' ) 

-- set the various trailer numbers...
-- pts 53648; if stp_number is also null don't do this.
update #temp_pd 
set ll_real_stp_number = stp_number
where ll_real_stp_number is null
and   stp_number > 0
and	  stp_number is not null 

-- pts 53648; if multiple evt_trailer1's for a single stop# - return min#
update #temp_pd 
set evt_trailer1  = (select min(evt_trailer1) from event 
					 where event.stp_number = #temp_pd.ll_real_stp_number 
					 and #temp_pd.ll_real_stp_number > 0
					 and #temp_pd.ll_real_stp_number  is not null )	

-- PTS 43677:  Changes <<end>> ----------------------------------------
...
SELECT	pyd_number, 
		...
		pyd_pretax, 
		pyd_status, 		
		--tp.pyh_payperiod,		-- PTS 43677: return payWEEK instead of date.
		DatePart(WEEK, tp.pyh_payperiod) 'pyh_payperiod',
		lgh_startcity,
		lgh_endcity, 
		pyd_minus,
		pyd_workperiod,
		...
		end_city,
		end_state, 
		--lgh_trailer,		--PTS 43677 (use evt_trailer instead)
		evt_trailer1,		--PTS 43677 		
	    @name_trc_type1 as 'trc_type1',  -- PTS 38192 02-14-2008 JSwindell
		pyt_basis			-- PTS 43677 

		  --FROM #temp_pd tp, -- PTS 38192 02-14-2008 JSwindell
		  --payheader ph -- PTS 38192 02-14-2008 JSwindell
		  --WHERE tp.pyh_number *= ph.pyh_pyhnumber  -- PTS 38192 02-14-2008 JSwindell
 FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber   -- PTS 38192 02-14-2008 JSwindell
-----where ord_hdrnumber = 17746




GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_80] TO [public]
GO
