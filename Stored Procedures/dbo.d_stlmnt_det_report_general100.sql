...
CREATE   PROC [dbo].[d_stlmnt_det_report_general100](	@report_type varchar(5),
 ...
 *
 * REVISION HISTORY:
 * 01/30/2009 - PTS 45861
 * 07/22/2008.01 ? PTS43119 - MRK ? CREATED FOR SETTLEMENT SHEET FORMAT 100
 *
 **/
...
CREATE TABLE #temp_pay (	pyd_number int not null,
...
CREATE TABLE #temp_pd(	pyd_number				int not null,
						pyh_number				int not null,
...
INSERT INTO #temp_pd
			SELECT		pd.pyd_number,
						...
						null, 
						(select lgh_primary_trailer from legheader where lgh_number = pd.lgh_number) trl_id
...
UPDATE #temp_pd
 ...
 WHERE r.ref_table = 'orderheader' 
	   AND r.ref_tablekey = tp.ord_hdrnumber 
	   AND l.labeldefinition = 'ReferenceNumbers' 
	   AND l.abbr = r.ref_type 
	   AND r.ref_type = 'TRIP' 
	   AND o.ord_hdrnumber = tp.ord_hdrnumber 
...
UPDATE #temp_pd
   SET pyd_quantity = 0
  FROM #temp_pd tp
 WHERE tp.pyt_itemcode in ('FSCP','FSCF','FSCM','FUEL')	
...
SELECT	pyd_number, 
		...
		pyd_refnumtype,
		pyd_refnum,
		IsNULL(pyt_basis,'') pyt_basis,
		IsNULL(trl_id,'') trl_id
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general100] TO [public]
GO
