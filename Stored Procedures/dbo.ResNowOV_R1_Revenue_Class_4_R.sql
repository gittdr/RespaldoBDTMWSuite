...
CREATE PROCEDURE [dbo].[ResNowOV_R1_Revenue_Class_4_R]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
		SELECT 	ord_RevType4 AS [RevClass Abbr], 
			(SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'RevType4' and abbr = ord_RevType4) AS [Revenue Class],
		...
		WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND ord_RevType4 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_Revenue_Class_4_R] TO [public]
GO
