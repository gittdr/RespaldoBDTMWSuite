...
CREATE PROCEDURE [dbo].[outbound_view_cems_1]
...
SELECT
	...
	lgh_priority
FROM @ttbl1
WHERE  (npup_arrivaldate <= dateadd(hh,9,getdate()) or ndrp_arrivaldate <= dateadd(hh,9,getdate()) )
and ord_billto not in ('ALMLIVER','BARDAHL','LIVERPOL','HOMEDEP','WERNER','WALMART')
Order by 8 desc
