...
CREATE   PROC [dbo].[d_settlement_sheet_summary_93](
	...
	@begin_date_msum_dt datetime,
	@end_date_msum_dt datetime)
AS
/**
 * DESCRIPTION:
 *
 ...
 *
 * REVISION HISTORY:
 * 07/31/2008 PTS43521 JSwindell - Client custom proc (where clause at bottom of proc):  Created based on d_settlement_sheet_summary_13.
 *
 **/
...
SELECT pyd_number,	--1 
	...
	pyd_offsetpay_number,
	pyd_credit_pay_flag,
	'PAYDETAIL',		--vjh new columns for activity union
	null,
	null,
	null,
	null,
...
l.mov_number, l.ord_hdrnumber, s.stp_number, c.cty_nmstct, s.stp_arrivaldate, stp_lgh_mileage
from legheader l, stops s, city c
where lgh_enddate  between @begin_date_msum_dt  and @end_date_msum_dt 
and (lgh_driver1=@drv_id or lgh_driver2=@drv_id)
and s.lgh_number = l.lgh_number
and s.stp_city=c.cty_code
--following two lines added by AH 4/25/08
--Cowen wants summary to include only trips paid in that pay period
--Also, would prefer it only work for local drivers
and (select min(pyh_payperiod) from paydetail where paydetail.lgh_number = l.lgh_number) = @payperiodstart
and (select mpp_type2 from manpowerprofile where mpp_id = l.lgh_driver1) = 'LOCAL'
order by stp_arrivaldate



GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_93] TO [public]
GO
