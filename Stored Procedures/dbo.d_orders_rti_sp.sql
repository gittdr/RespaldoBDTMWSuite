...
CREATE PROCEDURE [dbo].[d_orders_rti_sp] ( @ps_date                varchar(10),  
         @ps_status              varchar(6),  
 ...
 * 07/07/2011 DPETE PTS 56313 need to be able to make mulitple selections in various fields  
 * 06/25/2012 NQIAO PTS 62654 - add 4 new inputs (@ps_othertype1 ~ @ps_othertype4)
 * 09/25/2012 NLOKE PTS 62725 - Add logic to filter inv type
 ...
 * 01/18/2017 PTS 105981 - exclude LTL order
 **/  
...
-- PTS 105981
DECLARE @ltltypes TABLE (ltlapp VARCHAR(20))
INSERT @ltltypes(ltlapp)
  SELECT items
  FROM dbo.fn_SplitString((SELECT ISNULL(gi_string1, '') FROM generalinfo WHERE gi_name = 'ExcludeLTLOrderSource'), ',')
--end 105981
...
declare  @results table (lagtime    int    NULL,  
      ...
      ord_refnumber  varchar(30)  NULL,  
      --PTS 37241 2007-05-09 JJF  
      ord_bookedby  char(20)  NULL,  
      ord_order_source varchar(6)  NULL  
      --END PTS 37241 2007-05-09 JJF  
...
if exists (select 1 from company where cmp_billto = 'Y' and cmp_invoiceby = 'MOV')  
  BEGIN  -- build of @movs tale of moves to be invoice by bill to   
	...
	LEFT JOIN @ltltypes ON orderheader.ord_order_source = ltlapp		--PTS 105981
    /*  
...
SELECT lagtime,  
  ord_shipper,  
  shipper_name as 'cmp_name',  
  ord_consignee,  
  consignee_name as 'cmp_name',  
  ord_billto,  
  ord_company,  
  billto_name as 'cmp_name',  
  ord_startdate,  
  ord_completiondate,  
  ord_number,  
  mfh_hdrnumber,  
  ...
  ord_revtype4,  
  mov_number,  
  ord_revtype1_t as 'revtype1',  
  ord_revtype2_t as 'revtype2',  
  ord_revtype3_t as 'revtype3',  
  ord_revtype4_t as 'revtype4',  
  ord_totalcharge,  
  origin,  
  destination,  
  stp_number,  
 ...
 ,notes_flag -- PTS 53511 SGB  
  FROM @results r
  join @companies c on  r.ord_billto = c.cmp_id   --62725
 where paperwork = @ps_paperworkfilter   
    or @ps_paperworkfilter = 'N/A'  
order by ord_billto, mov_number  
                        

GO
GRANT EXECUTE ON  [dbo].[d_orders_rti_sp] TO [public]
GO
