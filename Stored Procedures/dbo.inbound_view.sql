...
CREATE   PROCEDURE [dbo].[inbound_view]
	...
	--PTS 55244 JJF 20110310
	@trc_teamleader 		VARCHAR(254),
	--END PTS 55244 JJF 20110310
 ...
 * 11/12/2007.01 ? PTS40187 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 ...
 -- SGB 45016 via PTS 42994 added mpp_terminal as 241 output column
 -- SGB 42437 changed subquery to tie to main query to avoid problem Subquery returned more than 1 value
 -- vjh 50281 removed nested block comments
 -- vjh 48613 add User Defined Fields
...
--JJF/DM PTS 57840 20110706
DECLARE	@PlnWrkshtPTADisplay varchar(1)
--END JJF/DM PTS 57840 20110706
...
DECLARE @TT TABLE(
	...
	--PTS92864
	dest_cmp_lat 			decimal(14,6) null,                                                                         -- 236
	dest_cmp_long 			decimal (14,6) null,                                                                        -- 237
	dest_cty_lat 			decimal (14,6) null,                                                                        -- 238
	dest_cty_long 			decimal (14,6) null,                                                                        -- 239
	...
	--PTS 61180 JJF 20120327
	mpp2_exp1_date 			DATETIME 	NULL,
	mpp2_exp2_date 			DATETIME 	NULL,
	--END PTS 61180 JJF 20120327
	mpp_fourteenhrest		decimal (5,2) null
)
...
DECLARE @TT1 TABLE(
	...
	dest_cmp_lat 			decimal (14,6) null,                                                                        -- 245
	dest_cmp_long 			decimal (14,6) null,                                                                        -- 246
	dest_cty_lat 			decimal (14,6) null,                                                                        -- 247
	dest_cty_long 			decimal (14,6) null,                                                                        -- 248
	...
	ma_transaction_id		bigint			null,	-- RE - PTS #52017													-- 284
	ma_tour_number			int				null,	-- RE - PTS #52017													-- 285
	ma_lgh_number			int				null,	-- RE - PTS #52017													-- 286
	ma_tour_max_sequence	tinyint			null,	-- RE - PTS #52017													-- 287
	udf_trc_1				varchar(255)	null,   -- vjh 48613														-- 288
	udf_trc_2				varchar(255)	null,																		-- 289
	udf_trc_3				varchar(255)	null,																		-- 290
	udf_trc_4				varchar(255)	null,																		-- 291
	udf_trc_5				varchar(255)	null,																		-- 292
	udf_trc_6				varchar(255)	null,																		-- 293
	udf_trc_7				varchar(255)	null,																		-- 294
	udf_trc_8				varchar(255)	null,																		-- 295
	udf_trc_9				varchar(255)	null,																		-- 296
	udf_trc_10				varchar(255)	null,																		-- 297
	udf_trc_1_t				varchar(255)	null,																		-- 298
	udf_trc_2_t				varchar(255)	null,																		-- 299
	udf_trc_3_t				varchar(255)	null,																		-- 300
	udf_trc_4_t				varchar(255)	null,																		-- 301
	udf_trc_5_t				varchar(255)	null,																		-- 302
	udf_trc_6_t				varchar(255)	null,																		-- 303
	udf_trc_7_t				varchar(255)	null,																		-- 304
	udf_trc_8_t				varchar(255)	null,																		-- 305
	udf_trc_9_t				varchar(255)	null,																		-- 306
	udf_trc_10_t			varchar(255)	null,																		-- 307
	lgh_chassis				varchar(13)		null,
	lgh_chassis2			varchar(13)		null,
	lgh_dolly				varchar(13)		null,
	lgh_dolly2				varchar(13)		null,
...
--PTS 61180 JJF 20120327
DECLARE @use_priexp_include varchar(1)
DECLARE @priexp_include varchar(60)
--END PTS 61180 JJF 20120327
...
--JJF/DM PTS 57840 20110706
SELECT @PlnWrkshtPTADisplay = gi_string1 FROM generalinfo WHERE gi_name = 'PlnWrkshtPTADisplay'
SET @PlnWrkshtPTADisplay = COALESCE(@PlnWrkshtPTADisplay, '0')
--END JJF/DM PTS 57840 20110706
...
--PTS 55244 JJF 20110310
SELECT @trc_teamleader = ',' + LTRIM(RTRIM(ISNULL(@trc_teamleader, ''))) + ','
--END PTS 55244 JJF 20110310
...
BEGIN
	...
	SELECT	legheader.lgh_number,                     							-- 001
			...
			--PTS32875 MBR 05/17/06
			CASE WHEN @UseShowAsShipperConsignee = 'Y' AND ord_originpoint <> ord_showshipper AND ord_showshipper <> 'UNKNOWN' AND ord_showshipper IS NOT NULL THEN
				--PTS xxxxx JJF/MC 20111116 nolock
			     (SELECT cmp_city from company WITH(NOLOCK) where cmp_id = ord_showshipper)
			...
			CASE WHEN @UseShowAsShipperConsignee = 'Y' AND ord_destpoint <> ord_showcons AND ord_showcons <> 'UNKNOWN' AND ord_showcons IS NOT NULL THEN
				--PTS xxxxx JJF/MC 20111116 nolock
			     (SELECT cmp_city FROM company WITH(NOLOCK) WHERE cmp_id = ord_showcons)
			...
			ROUND(isnull(company_b.cmp_latseconds,0.0000)/3600.000, 6) 'dest_cmp_lat',
			ROUND(isnull(company_b.cmp_longseconds,0.0000)/3600.000, 6) 'dest_cmp_long',
			0.0000 'dest_cty_lat',
			...
			lgh_trc_comment,
			--PTS# 29623 ILB 11/14/2005
			--JJF PTS 57840 20110706
			--m.mpp_pta_date, --DPH PTS 32698
			--CASE m.mpp_id
			--	WHEN 'UNKNOWN' THEN 
			--		CASE trcp.trc_number
			--			WHEN 'UNKNOWN' THEN legheader.lgh_enddate
			--			ELSE trcp.trc_avl_date
			--		END
			--	ELSE m.mpp_pta_date
			--END as mpp_pta_date, 
			CASE	@PlnWrkshtPTADisplay
				WHEN '1' THEN
					CASE 
						WHEN lgh_outstatus in ('AVL','PLN','DSP') THEN
							legheader.lgh_enddate
						ELSE
							m.mpp_pta_date
					END 
				--PTS71466 JJF 20131205
				WHEN '2' THEN
					CASE 
						WHEN ISNULL(pow.EstimatedPTA, '1950-01-01') > ISNULL(m.mpp_pta_date, '1950-01-01') THEN
							pow.EstimatedPTA 
						ELSE
							m.mpp_pta_date
					END
				--END PTS71466 JJF 20131205
				ELSE
					m.mpp_pta_date
			END as mpp_pta_date, 
			--END JJF PTS 57840 20110706
...
--	  FROM	@terminal_table tr inner join leheader_active legheader on legheader.trc_terminal = tr.trc_terminal
	  ...
	  --FROM	@terminal_table tr inner join legheader_active legheader on legheader.trc_terminal = tr.trc_terminal
		...
		--END PTS 61180 JJF 20120327
		m.mpp_fourteenhrest
	  FROM	@terminal_table tr
			inner join legheader_active legheader  WITH(NOLOCK) on legheader.trc_terminal = tr.trc_terminal 
	  --END PTS xxxxx JJF/MC 20111116 nolock
-- PTS 32011 -- BL (end)
			--PTS xxxxx JJF/MC 20111116 nolock
			inner join company company_a WITH(NOLOCK) on legheader.cmp_id_start = company_a.cmp_id
			inner join company company_b WITH(NOLOCK) on legheader.cmp_id_end = company_b.cmp_id
			left join orderheader WITH(NOLOCK) on legheader.ord_hdrnumber = orderheader.ord_hdrnumber
			inner join manpowerprofile m WITH(NOLOCK) on legheader.lgh_driver1 = m.mpp_id
			inner join tractorprofile trcp WITH(NOLOCK) on legheader.lgh_tractor = trcp.trc_number
			inner join trailerprofile trlp WITH(NOLOCK) on legheader.lgh_primary_trailer = trlp.trl_id
			--PTS 58161 JJF 20111017
			left outer join company company_billto WITH(NOLOCK) on orderheader.ord_billto = company_billto.cmp_id
			--END PTS 58161 JJF 20111017
			--END PTS xxxxx JJF/MC 20111116 nolock
			--PTS71466 JJF 20131205
			left outer join PowerState pow on legheader.lgh_tractor = pow.PowerId
			--END PTS71466 JJF 20131205
	 ...
	 WHERE	--legheader.trc_terminal = tr.trc_terminal AND
			...
			(@reg4 = 'UNK' OR lgh_endregion4 = @reg4) AND */
	  		...
	  		--PTS 58161 JJF 20111017
	  		(@cmp_othertype1_billto = ',UNK,' OR CHARINDEX(',' + company_billto.cmp_othertype1 + ',', @cmp_othertype1_billto) > 0) AND
	  		(@cmp_othertype2_billto = ',UNK,' OR CHARINDEX(',' + company_billto.cmp_othertype2 + ',', @cmp_othertype2_billto) > 0) AND
	  		--END PTS 58161 JJF 20111017
			...
			--PTS 55244 JJF 20110310
			(@trc_teamleader = ',,' OR CHARINDEX(',' + legheader.trc_teamleader + ',', @trc_teamleader) > 0) AND
			--END PTS 55244 JJF 20110310
			...
			(@lgh_booked_revtype1 = ',,' OR CHARINDEX(',' + ISNULL(lgh_booked_revtype1, 'UNKNOWN') + ',', @lgh_booked_revtype1) > 0) AND 
			(@ord_booked_revtype1 = ',,' OR CHARINDEX(',' + ISNULL(ord_booked_revtype1, 'UNKNOWN') + ',', @ord_booked_revtype1) > 0)
		--vmj3+
		and	(@carrier = 'UNKNOWN'
...
BEGIN
	...
	SELECT	legheader.lgh_number,
	 ...
	 --PTS xxxxx JJF/MC 20111116 nolock
		...
		--END PTS 61180 JJF 20120327
		m.mpp_fourteenhrest
	  FROM	legheader_active legheader WITH(NOLOCK) inner join company company_a on legheader.cmp_id_start = company_a.cmp_id
			inner join company company_b WITH(NOLOCK) on legheader.cmp_id_end = company_b.cmp_id
			left join orderheader WITH(NOLOCK) on legheader.ord_hdrnumber = orderheader.ord_hdrnumber
			inner join manpowerprofile m WITH(NOLOCK) on legheader.lgh_driver1 = m.mpp_id
			inner join tractorprofile trcp WITH(NOLOCK) on legheader.lgh_tractor = trcp.trc_number
...
INSERT INTO	@TT1
	SELECT	TT.lgh_number,
			...
			CASE											-- RE - PTS #52017
				WHEN @ma_transaction_id IS NULL THEN NULL	-- RE - PTS #52017
				ELSE @ma_transaction_id						-- RE - PTS #52017
			END,											-- RE - PTS #52017
			NULL,											-- RE - PTS #52017
			NULL,											-- RE - PTS #52017
			NULL,											-- RE - PTS #52017
			NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,		-- 288 through 297
			NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,		-- 298 through 307
			TT.lgh_chassis,TT.lgh_chassis2,TT.lgh_dolly,TT.lgh_dolly2,TT.lgh_trailer3,TT.lgh_trailer4,
			--PTS 51918 JJF 20110209
			NULL,
			NULL,
			NULL,
			...
			/* 03/27/2012 MDH PTS 59481: <<BEGIN>> */
			tt.mpp_hosstatus,
			tt.mpp_hosstatusdate,
			tt.mpp_hosactivityupdateon,
			/* 03/27/2012 MDH PTS 59481: <<END>> */
			null,
			null,
			null,
			...
			mpp_fourteenhrest
	  --PTS xxxxx JJF/MC 20111116 nolock
	  FROM	@TT TT inner join tractorprofile WITH(NOLOCK) on TT.evt_tractor = tractorprofile.trc_number
...
IF @rowsecurity = 'Y' BEGIN
	...
	DELETE @TT1
	--PTS xxxxx JJF/MC 20111116 nolock
	from @TT1 tp inner join orderheader oh WITH(NOLOCK) on tp.mov_number = oh.mov_number
	...
	DELETE @TT1
	--PTS xxxxx JJF/MC 20111116 nolock
	from @TT1 tp inner join tractorprofile trc WITH(NOLOCK) on tp.evt_tractor = trc.trc_number
	WHERE	NOT EXISTS	(	SELECT	*  
...
UPDATE	TT1
   SET	TT1.mpp_last_home = manpowerprofile.mpp_last_home,
		...
		mpp_exp2_date = manpowerprofile.mpp_exp2_date,
 		--PTS 61180 JJF 20120327
		mpp2_exp1_date = mpp2.mpp_exp1_date,
		mpp2_exp2_date = mpp2.mpp_exp2_date,
		--END PTS 61180 JJF 20120327
		trl_exp1_date = trailerprofile.trl_exp1_date,
		trl_exp2_date = trailerprofile.trl_exp2_date,

		--vmj4+	Go back to the beginning of yesterday, because log entries are always
		--saved with a time of 00:00:00..
		loghours =
			case when manpowerprofile.mpp_last_log_date >= @ldt_yesterday
...
--		loghours = manpowerprofile.mpp_hours1,
  ...
  --PTS xxxxx JJF/MC 20111116 nolock
  FROM	@TT1 TT1  LEFT OUTER JOIN  trailerprofile WITH(NOLOCK) ON  TT1.lgh_primary_trailer  = trailerprofile.trl_id ,
		manpowerprofile WITH(NOLOCK),
		--PTS 61180 JJF 20120327
		manpowerprofile mpp2 WITH(NOLOCK),
		--END PTS 61180 JJF 20120327
 ...
 WHERE	TT1.evt_driver1 = manpowerprofile.mpp_id AND
		--PTS 61180 JJF 20120327
		TT1.evt_driver2 = mpp2.mpp_id AND
		--END PTS 61180 JJF 20120327
		TT1.evt_tractor = tractorprofile.trc_number AND
		--change
		--TT1.lgh_primary_trailer *= trailerprofile.trl_id and		-- tractorprofile.trc_trailer1 *= trailerprofile.trl_id and  --pts40187 outer join conversion
		( l_trc.abbr = trc_status ) AND
		( l_mpp.abbr = manpowerprofile.mpp_status )
...
UPDATE	TT1
  ...
  --PTS xxxxx JJF/MC 20111116 nolock	 
  FROM  @TT1 TT1  LEFT OUTER JOIN  company company_c WITH(NOLOCK) ON  TT1.f_cmpid  = company_c.cmp_id
		LEFT OUTER JOIN  company company_d WITH(NOLOCK) ON  TT1.l_cmpid  = company_d.cmp_id
		LEFT OUTER JOIN  company company_p WITH(NOLOCK) ON  TT1.p_cmpid  = company_p.cmp_id
		LEFT OUTER JOIN  city city_c WITH(NOLOCK) ON  TT1.f_ctycode  = city_c.cty_code
		LEFT OUTER JOIN  city city_p WITH(NOLOCK) ON  TT1.p_ctycode  = city_p.cty_code
		LEFT OUTER JOIN  city city_d WITH(NOLOCK) ON  TT1.l_ctycode  = city_d.cty_code
		LEFT OUTER JOIN  company company_pr WITH(NOLOCK) ON  TT1.trc_prior_cmp_id  = company_pr.cmp_id
		LEFT OUTER JOIN  city city_pr WITH(NOLOCK) ON  TT1.trc_prior_city  = city_pr.cty_code
		LEFT OUTER JOIN  company company_n WITH(NOLOCK) ON  TT1.trc_next_cmp_id  = company_n.cmp_id
		LEFT OUTER JOIN  city city_n WITH(NOLOCK) ON  TT1.trc_next_city  = city_n.cty_code
...
-- PTS 42437 BEGIN SGB updated subquery to tie temp table @tt1 lgh_number in where clause
If @v_GILocalTImeOption = 'LOCAL'  
   BEGIN  
     update @tt1 
       ...
       Case  
         ...
         when evt_latedate is null then   -- dw uses lgh_startdate if htis is true  
    		...
    		(select ((isnull(cty_GMTDelta,5)  
       		...
       		+ isnull(cty_TZMins,0)  
       		--PTS xxxxx JJF/MC 20111116 nolock
       		From city WITH(NOLOCK) where cty_code = o_city)  
         else  
              @v_LocalCityTZAdjFactor -  
             (select ((isnull(cty_GMTDelta,5)  
       		+ (@InDSTFactor * (case cty_DSTApplies when 'Y' then 0 else +1 end))) * 60)  
       		+ isnull(cty_TZMins,0)  
       		--PTS xxxxx JJF/MC 20111116 nolock
       		 From city  WITH(NOLOCK) where cty_code = (select s1.stp_city from stops s1 WITH(NOLOCK) 
...
--PTS 61180 JJF 20120327
SELECT @use_priexp_include = 'N'
SELECT @priexp_include = 'DRV1,TRC,TRL1'

SELECT	@use_priexp_include = ISNULL(gi_string1, 'N'),
		@priexp_include = ISNULL(gi_string2, 'DRV1,TRC,TRL1')
FROM	generalinfo
WHERE	gi_name = 'TractorViewMainPriorityInclude'

IF @use_priexp_include = 'Y' BEGIN
	SELECT	@priexp_include = ',' + @priexp_include + ','

	UPDATE	@tt1
	SET		pri1exp =		CASE
								WHEN (trc_exp1_date <= GetDate() AND CHARINDEX(',TRC,', @priexp_include) > 0)
									OR (mpp_exp1_date <= GetDate() AND CHARINDEX(',DRV1,', @priexp_include) > 0)
									OR (mpp2_exp1_date <= GetDate() AND CHARINDEX(',DRV2,', @priexp_include) > 0)
									OR (trl_exp1_date <= GetDate() AND CHARINDEX(',TRL1,', @priexp_include) > 0) THEN 1
								ELSE 0
							END,
			pri1expsoon =	CASE
								WHEN (trc_exp1_date <= @neardate AND CHARINDEX(',TRC,', @priexp_include) > 0) 
								OR (mpp_exp1_date <= @neardate AND CHARINDEX(',DRV1,', @priexp_include) > 0)
								OR (mpp2_exp1_date <= @neardate AND CHARINDEX(',DRV2,', @priexp_include) > 0)
								OR (trl_exp1_date <= @neardate AND CHARINDEX(',TRL1,', @priexp_include) > 0) THEN 1
								ELSE 0

							END,
			pri2exp =		CASE
								WHEN (trc_exp2_date <= GetDate() AND CHARINDEX(',TRC,', @priexp_include) > 0)
								OR (mpp_exp2_date <= GetDate() AND CHARINDEX(',DRV1,', @priexp_include) > 0)
								OR (mpp2_exp2_date <= GetDate() AND CHARINDEX(',DRV2,', @priexp_include) > 0)
								OR (trl_exp2_date <= GetDate() AND CHARINDEX(',TRL1,', @priexp_include) > 0) THEN 1
								ELSE 0

							END,
			pri2expsoon =	CASE
								WHEN (trc_exp2_date <= @neardate AND CHARINDEX(',TRC,', @priexp_include) > 0)
								OR (mpp_exp2_date <= @neardate AND CHARINDEX(',DRV1,', @priexp_include) > 0)
								OR (mpp2_exp2_date <= @neardate AND CHARINDEX(',DRV2,', @priexp_include) > 0)
								OR (trl_exp2_date <= @neardate  AND CHARINDEX(',TRL1,', @priexp_include) > 0) THEN 1
								ELSE 0
							END
...
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'FirstOrdRefNumDispinPlanner' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	...
	--PTS xxxxx JJF/MC 20111116 nolock	
	FROM orderheader ord WITH(NOLOCK) CROSS JOIN @TT1 TT1
	WHERE ord.ord_hdrnumber = (SELECT TOP 1 stp.ord_hdrnumber
							FROM stops stp WITH(NOLOCK)
...
FROM @TT1 TT1
	...
	BEGIN
		...
		--PTS xxxxx JJF/MC 20111116 nolock		
		from	@TT1 TT1, expiration b WITH(NOLOCK)
		...
		if @AVLDTTM_ONLYSETBY_OPENEXPS = 'Y'
				...
				where	b.exp_idtype = 'DRV' and
						b.exp_id = TT1.evt_driver1 and
						b.exp_compldate = (select max(exp_compldate) from expiration d WITH(NOLOCK) where
...
UPDATE TT1
  ...
  --PTS xxxxx JJF/MC 20111116 nolock
  FROM @TT1 TT1, stops WITH(NOLOCK)
...
/*	UPDATE #TT1
	...
	mpp_lastfirst_1 = 'Available'
	--PTS xxxxx JJF/MC 20111116 nolock
	FROM @TT1 TT1, tractorprofile WITH(NOLOCK), manpowerprofile WITH(NOLOCK)
	WHERE TT1.evt_tractor = tractorprofile.trc_number
...
if @citylatlongunits = 'S' 
	update TT1
	   set dest_cty_lat = isnull(cty_latitude, 0.0000) / 3600.0,
	       dest_cty_long = isnull(cty_longitude, 0.0000) / 3600.0
	  --PTS xxxxx JJF/MC 20111116 nolock
	  from @TT1 TT1, city WITH(NOLOCK)
...
--PTS xxxxx JJF/MC 20111116 nolock
from stops WITH(NOLOCK), @TT1 t1, city WITH(NOLOCK)
		 ...
		 WHERE	evt_driver1 NOT IN
					(SELECT	t.evt_driver1
					   FROM	(SELECT DISTINCT evt_driver1 FROM @TT1) t
								--PTS xxxxx JJF/MC 20111116 nolock
								inner join driverqualifications ta WITH(NOLOCK) on t.evt_driver1 = ta.drq_driver and ta.drq_expire_date >= getdate() and isnull(ta.drq_expire_flag, 'N') <> 'Y' and drq_source = 'DRV'
		 ...
		 WHERE	evt_tractor NOT IN
					(SELECT	t.evt_tractor
					   FROM	(SELECT DISTINCT evt_tractor from @TT1) t
								--PTS xxxxx JJF/MC 20111116 nolock
								inner join tractoraccesories ta WITH(NOLOCK) on t.evt_tractor = ta.tca_tractor and ta.tca_expire_date >= getdate() and isnull(ta.tca_expire_flag, 'N') <> 'Y' and tca_source = 'TRC'
		 ...
		 WHERE	lgh_primary_trailer NOT IN
					(SELECT	t.lgh_primary_trailer
					   FROM	(SELECT DISTINCT lgh_primary_trailer FROM @TT1) t
								--PTS xxxxx JJF/MC 20111116 nolock
								inner join trlaccessories ta  WITH(NOLOCK) on t.lgh_primary_trailer = ta.ta_trailer and ta.ta_expire_date >= getdate() and isnull(ta.ta_expire_flag, 'N') <> 'Y' and ta_source = 'TRL'
...
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_1' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_1_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_1 = dbo.udf_trc_1_fn(tt1.evt_tractor, 'D'),	udf_trc_1_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_2' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_2_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_2 = dbo.udf_trc_2_fn(tt1.evt_tractor, 'D'),	udf_trc_2_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_3' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_3_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_3 = dbo.udf_trc_3_fn(tt1.evt_tractor, 'D'),	udf_trc_3_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_4' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_4_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_4 = dbo.udf_trc_4_fn(tt1.evt_tractor, 'D'),	udf_trc_4_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_5' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_5_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_5 = dbo.udf_trc_5_fn(tt1.evt_tractor, 'D'),	udf_trc_5_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_6' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_6_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_6 = dbo.udf_trc_6_fn(tt1.evt_tractor, 'D'),	udf_trc_6_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_7' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_7_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_7 = dbo.udf_trc_7_fn(tt1.evt_tractor, 'D'),	udf_trc_7_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_8' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_8_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_8 = dbo.udf_trc_8_fn(tt1.evt_tractor, 'D'),	udf_trc_8_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_9' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_9_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_9 = dbo.udf_trc_9_fn(tt1.evt_tractor, 'D'),	udf_trc_9_t = @udflabel
	FROM	@TT1 TT1
END
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'UDF_TRC_10' and substring(gi_string1,1,1) = 'Y' ) BEGIN
	SELECT	@udflabel = dbo.udf_trc_10_fn('', 'L')
	UPDATE	@TT1
	SET		udf_trc_10 = dbo.udf_trc_10_fn(tt1.evt_tractor, 'D'),	udf_trc_10_t = @udflabel
	FROM	@TT1 TT1
END
...
FROM	generalinfo
WHERE gi_name = 'QualListTractorPlan'

IF @DisplayQualifications = 'Y' BEGIN
	IF @AssetsToInclude = ',,' BEGIN
		SET @AssetsToInclude = ',DRV1,DRV2,TRC,TRL1,TRL2,CAR,'
	END

	UPDATE @TT1
	SET qualification_list_drv = dbo.QualificationsToCSV_fn	(	CASE CHARINDEX(',DRV1,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_driver1 END, 
																CASE CHARINDEX(',DRV2,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_driver2 END, 
																NULL, 
																NULL, 
																NULL, 
																CASE CHARINDEX(',CAR,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_carrier END, 
																CASE CHARINDEX(',CAR,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_carrier END, 
																NULL, 
																NULL, 
																lgh_startdate, 
		...
		qualification_list_trc = dbo.QualificationsToCSV_fn	(	NULL, 
																NULL, 
																CASE CHARINDEX(',TRC,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_tractor END, 
																NULL, 
																NULL, 
																NULL, 
																NULL, 
		...
		qualification_list_trl = dbo.QualificationsToCSV_fn	(	NULL, 
																NULL, 
																NULL, 
																CASE CHARINDEX(',TRL1,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE lgh_primary_trailer END, 
																CASE CHARINDEX(',TRL2,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE lgh_primary_pup END, 
																NULL, 
																NULL, 
																NULL, 
																CASE CHARINDEX(',CAR,', @AssetsToInclude) WHEN 0 THEN 'UNKNOWN' ELSE evt_carrier END, 
...
--PTS 58199 JJF 20111018
IF EXISTS (SELECT * FROM generalinfo WHERE gi_name = 'TractorViewExtraInfo' and substring(gi_string1, 1, 1) = 'Y' ) BEGIN
	UPDATE	@TT1
	
	SET		extrainfo1 = lgh_extrainfo1,
			extrainfo2 = lgh_extrainfo2,
			extrainfo3 = lgh_extrainfo3,
			extrainfo4 = lgh_extrainfo4,
			extrainfo5 = lgh_extrainfo5,
			extrainfo6 = lgh_extrainfo6,
			extrainfo7 = lgh_extrainfo7,
			extrainfo8 = lgh_extrainfo8,
			extrainfo9 = lgh_extrainfo9,
			extrainfo10 = lgh_extrainfo10,
			extrainfo11 = lgh_extrainfo11,
			extrainfo12 = lgh_extrainfo12,
			extrainfo13 = lgh_extrainfo13,
			extrainfo14 = lgh_extrainfo14,
			extrainfo15 = lgh_extrainfo15
	FROM	@TT1 TT1
			INNER JOIN legheader_active lgh on TT1.lgh_number = lgh.lgh_number
END
--END PTS 58199 JJF 20111018
...
SELECT	lgh_number,                        -- 001
		...
		mpp_terminal,					-- 241 PTS 45016 via PTS 42994 SGB
		ma_transaction_id,														-- RE - PTS #52017
		CASE																	-- RE - PTS #52017
			WHEN ma_transaction_id IS NULL THEN @null_int						-- RE - PTS #52017
			ELSE dbo.Power_MATourNumber_fn(@DefaultCompanyID, ma_transaction_id, evt_tractor)		-- RE - PTS #52017
		END ma_tour_number,														-- RE - PTS #52017
		@null_int,																-- RE - PTS #52017
		CASE																	-- RE - PTS #52017
			WHEN ma_transaction_id IS NULL THEN @null_varchar100				-- RE - PTS #52017
			ELSE dbo.Power_MAReccomendation_fn(@DefaultCompanyID, ma_transaction_id, evt_tractor)	-- RE - PTS #52017
		...
		lgh_trailer4,                                                   -- 271
		--PTS 51918 JJF 20110209                                              
		qualification_list_drv,                                         -- 272
		qualification_list_trc,                                         -- 273
		qualification_list_trl,                                         -- 274
		--END PTS 51918 JJF 20110209                                          
		--PTS 55760 JJF 20110609                                              
		--ISNULL(mpp_rtw_date, '12/31/2049 23:59') mpp_rtw_date               
		mpp_rtw_date,                                                   -- 275
		--END PTS 55760 JJF 20110609                                          
		--PTS 58199 JJF 20111018                                              
		/* 03/27/2012 MDH PTS 59481: <<BEGIN>> */                             
		mpp_hosstatus,                                                  -- 276
		mpp_hosstatusdate,                                              -- 277
		mpp_hosactivityupdateon,                                        -- 278
		/* 03/27/2012 MDH PTS 59481: <<END>> */                               
		extrainfo1,                                                     -- 279
		extrainfo2,                                                     -- 280
		extrainfo3,                                                     -- 281
		...
		extrainfo15                                                     -- 293
		--END PTS 58199 JJF 20111018                                          
		,ud_column1			 -- PTS 51911 SGB User Defined column           -- 294
		,ud_column1_t		 --	PTS 51911 SGB User Defined column header    -- 295
		,ud_column2			 -- PTS 51911 SGB User Defined column           -- 296
		,ud_column2_t 		 --	PTS 51911 SGB User Defined column header	-- 297
		,ud_column3			 -- PTS 51911 SGB User Defined column           -- 298
		,ud_column3_t		 --	PTS 51911 SGB User Defined column header    -- 299
		,ud_column4			 -- PTS 51911 SGB User Defined column           -- 300
		,ud_column4_t 		 --	PTS 51911 SGB User Defined column header  -- 301
		,mpp2_exp1_date		--PTS 61086                                   -- 302
		,mpp2_exp2_date 		--PTS 61086	                                -- 303
		,mpp_fourteenhrest                                               -- 304
	FROM	@TT1
...
GO
GRANT EXECUTE ON  [dbo].[inbound_view] TO [public]
GO
