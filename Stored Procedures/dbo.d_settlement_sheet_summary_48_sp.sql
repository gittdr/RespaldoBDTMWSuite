...
CREATE PROC [dbo].[d_settlement_sheet_summary_48_sp](
 ...
 * 
 * NAME:
 * dbo.d_settlement_sheet_summary_48_sp
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * Proc for dw d_stlmnt_det_report_general48
 *
 * RETURNS:
 * dw result set
 *
...
insert into #paid_miles
SELECT DISTINCT asgn_type, asgn_id, sum(pyd_quantity), pyd_rate, min(pyd_number), min(pyh_number), sum(pyd_amount)
FROM #temp_pd t
where pyd_unit = 'MIL' and pyd_status not in ('HLD') and pyt_basis = 'LGH'
group by t.asgn_type, t.asgn_id, t.pyd_rate 
order by pyd_rate
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_48_sp] TO [public]
GO
