...
CREATE PROCEDURE [dbo].[ResNowOV_IVH_OrderedBy_R]
/* PARAMETER LIST. ONLY CHANGE NAME OF STORED PROCEDURE */
(
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
			SELECT 	ivh_order_by AS [OrderedByID]
		...
		BEGIN
			...
			FROM invoiceheader (NOLOCK) 
			WHERE ivh_deliverydate BETWEEN @DateStart and @DateEnd
			AND ivh_order_by NOT IN (SELECT ItemID FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			ORDER BY ivh_order_by 
		END
...
GO
GRANT EXECUTE ON  [dbo].[ResNowOV_IVH_OrderedBy_R] TO [public]
GO
