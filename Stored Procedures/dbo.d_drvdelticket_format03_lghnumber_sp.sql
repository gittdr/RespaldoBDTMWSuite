...
CREATE PROCEDURE [dbo].[d_drvdelticket_format03_lghnumber_sp] @p_lgh_number int
AS
/**
 * 
 * NAME:
 * dbo.d_drvdelticket_format03_lghnumber_sp
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * 41158 1/31/08 DPETE city for delivery location is coming from the last stop (consignee) instead of the delivery stop
 **/
...
SELECT @v_mov_number = 0

SELECT @v_mov_number = mov_number
  FROM legheader
 WHERE lgh_number = @p_lgh_number

--print cast(@p_lgh_number as varchar(20))
--print cast(@v_mov_number as varchar(20))
...
INSERT  INTO #trips
   ...
   from --orderheader ord , company shipper, company consignee, company billto,
        --city shipper_cty, city consignee_cty, city billto_cty, stops stp, 
	...
	company consignee, 
	company billto,
-- 41158	city shipper_cty, 
-- 41158	city consignee_cty, 
-- 41158	city billto_cty,
        legheader lgh
  where ord.mov_number = @v_mov_number and
	stp.lgh_number = @p_lgh_number and	
	...
	stp.lgh_number = lgh.lgh_number and	
        ...
        ord.ord_billto = billto.cmp_id -- 41158 and
-- 41158        shipper_cty.cty_code = ord.ord_origincity and 
-- 41158         consignee_cty.cty_code = ord.ord_destcity and
-- 41158         billto_cty.cty_code = billto.cmp_city            
/*
select @v_pupcnt = 0
select @v_pupcnt = count(*) 
  from stops 
 where stp_event = 'LLD'and
       mov_number = @v_mov_number

--print cast(@v_pupcnt as varchar(20))

IF @v_pupcnt = 1 
  BEGIN   
    SELECT @v_pupstp_number = 0
    SELECT @v_pupstp_number = stp_number
      FROM stops
     WHERE stp_event = 'LLD'and
           mov_number = @v_mov_number

    --print cast(@v_pupstp_number as varchar(20))
  END
*/
						...
						--print 'reference number '+ isnull(@v_ref_num,'no refnumber for this fgt number')
						 ...
						 IF @v_cnt = 4
						   BEGIN
							...
							update #trips
                                                           set fgt_cmdcode_4 = cmd_code,
                                                               fgt_weight_4 = fgt_weight,
							       fgt_volume_4 = fgt_volume,
                                                               fgt_description_4 = fgt_description
                                                          from freightdetail fgt
                                                         where fgt.fgt_sequence = @v_cnt 
                                                           and fgt.fgt_number = @v_fgt_num
                                                           and fgt.stp_number = @v_stp_num
	...
	WHILE (SELECT COUNT(*) 
		...
		BEGIN	
			...
			UPDATE #TRIPS
			  ...
			  FROM COMPANY CONSIGNEE  -- 41158, city consignee_cty
			...
			UPDATE #TRIPS
			  ...
			  FROM COMPANY SHIPPER -- 41158  , city SHIPPER_cty
...
UPDATE #TRIPS
  ...
  FROM COMPANY CONSIGNEE, event evt  -- 41158 , city consignee_cty
 WHERE CONSIGNEE.CMP_ID = @v_CMP_ID AND
  ...
  BEGIN
    UPDATE #TRIPS
     ...
     WHERE #TRIPS.pup_drp = 'D' 
       AND evt.stp_number = @v_pupstp_number
  END
...
GO
GRANT EXECUTE ON  [dbo].[d_drvdelticket_format03_lghnumber_sp] TO [public]
GO
