...
CREATE PROC [dbo].[d_settlement_sheet_summary_79](
 ...
 * 02/12/2008.01 - PTS40368 - SLM  - 'Deducts' section was not showing up in certain cases.  Modified
 *                                   code to join with paytypes table where pyt_description like '%drop delivery%'
 * 08/25/2010    - PTS53671 - SPN  - get the lgh_startdate from settlementdriverdaycustom.sdd_day
 *
 **/
...
CREATE TABLE #temp_pd(
    reportsort varchar (20) null, -- PTS 38289
	pyd_number		int not null,
	pyh_number		int not null,
...
--SLM CHANGED crd_cardnumber TO STRING 10 ON DATAWINDOW
...
-- PTS 29515 -- BL (end)
    mpp_domicile varchar(20) null)-- PTS 38289 
...
--38289 PUT EVERYTHING INTO 2 ITEMSECTIONS EARNING and expense reimbursement
UPDATE #temp_pd
   SET itemsection = 1, reportsort = '1Earnings'

-- PTS 38289 EXPENSE REIMBURSEMENT
UPDATE #temp_pd
   SET itemsection = 2, reportsort = '2Expenses'
 WHERE pyd_pretax = 'N'
       AND pyd_minus = 1-- and itemsection <> 1

--38289 PUT EVERYTHING INTO 2 ITEMSECTIONS EARNING
--   SET reportsort = '1Deducts' where pyd_description = 'Drop Delivery'
-- PTS 40368 2/12/2008 'Deducts' section was not showing up in certain cases
UPDATE #temp_pd
   SET reportsort = '1Deducts' 
...
AND LOWER(p.pyt_description) LIKE '%drop delivery%'

-- PTS 38289 COMMENTED OUT
--UPDATE #temp_pd
--   SET itemsection = 3
-- WHERE pyd_pretax = 'N'
--       AND pyd_minus = -1
...
-- PTS 38289
update #temp_pd
set #temp_pd.mpp_domicile = l.name
from #temp_pd tp, manpowerprofile m, labelfile l
where tp.asgn_id = m.mpp_id
and   m.mpp_type2 = l.abbr
and   l.labeldefinition = 'DrvType2'

--BEGIN PTS 53671 SPN
UPDATE #temp_pd
   SET lgh_startdate = 
      (SELECT MAX(settlementdriverdaycustom.sdd_day)
     	   FROM settlementdriverdaycustom
         JOIN dbo.eventcustomstl ON settlementdriverdaycustom.sdd_id = eventcustomstl.sdd_id
         JOIN event ON eventcustomstl.evt_number = event.evt_number
...
SELECT reportsort,
	...
	consignee_state,
	cmd_name,
--	pyd_billedweight, SLM COMMENTED OUT
	...
	label_name,
	otherid,
--	pyt_fee1,SLM COMMENTED OUT
--	pyt_fee2,SLM COMMENTED OUT
	start_city,
	start_state,
	end_city,
	end_state, 
       ph.pyh_paystatus,
--	ref_number_tds,SLM COMMENTED OUT
--	pyd_offsetpay_number,SLM COMMENTED OUT
--	pyd_credit_pay_flag,SLM COMMENTED OUT
--	pyd_refnumtype,SLM COMMENTED OUT
--	pyd_refnum,SLM COMMENTED OUT
---- PTS 29515 -- BL (start)SLM COMMENTED OUT
--	pyt_basis,SLM COMMENTED OUT
-- PTS 29515 -- BL (end)SLM COMMENTED OUT
  mpp_domicile-- PTS 38289
  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_79] TO [public]
GO
