...
CREATE PROCEDURE [dbo].[outbound_view_matrix]
...
SELECT
    ...
    Proyecto = (SELECT name FROM labelfile WHERE  labeldefinition = 'RevType3' and abbr = (select ord_revtype3 from orderheader where orderheader.ord_hdrnumber = (A.ord_hdrnumber))),
    Region =  (select rgh_name from regionheader where  rgh_id = (select cmp_region1 from company where cmp_id = o_cmpid )),
    Compania =  o_cmpid,
	Fecha= a.lgh_startdate,
    Operador = replace(a.evt_driver1_id ,'UNKNOWN', cast(ord_hdrnumber as varchar)),
    Status = a.lgh_outstatus
