...
CREATE PROCEDURE [dbo].[ResNowOV_R1_OtherType1]
...
*/
	...
	BEGIN
		IF IsNull(@ItemID, '') > ''
		BEGIN
		SELECT 	cmp_othertype1 AS [OtherType1 Abbr], 
			(SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'OtherTypes1' and abbr = cmp_othertype1) AS [Other Type 1],
		...
		WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND cmp_othertype1 NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
		ORDER BY cmp_othertype1 
		END
	END
	ELSE IF @Refresh = 1
	BEGIN
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[DateEnd] = @DateEnd,
				[ItemCategory] = @Mode,
				[ItemID] = cmp_othertype1, 
				[ItemDescription] = (SELECT name FROM labelfile (NOLOCK) where labeldefinition = 'OtherTypes1' and abbr = cmp_othertype1),
				[Count] = COUNT(*),
		...
		FROM orderheader (NOLOCK) Join Company (NOLOCK) on cmp_id = ord_billto
			...
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			GROUP BY cmp_othertype1
			ORDER BY COUNT(*) DESC
		SET ROWCOUNT 0
	END

GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_OtherType1] TO [public]
GO
