...
CREATE         PROC [dbo].[d_masterbill33_sp] (@reprintflag varchar(10),@mbnumber int,@billto varchar(8), 
 ...
 * 2/13/08 BDH PTS40516.  Added ability to only print invoices assigned to a user.
 * 7/12/12 TMEZE PTS62766.  Added ord_remark to be returned in query.
 *
 **/

DECLARE @int0  int,
	@PrintMBbyUser char(1),
	@tmwuser varchar(20)
...
-- BDH 2/13/08 40516
set @PrintMBbyUser = (select left(ltrim(gi_string1), 1) from generalinfo where gi_name = 'PrintMasterBillsByUser')
if isnull(@PrintMBbyUser, 'N') = 'Y'
begin	
...
-- dpete for LOR pts4785 provide for maitlto override of billto
   ...
   WHERE		 ( invoiceheader.ivh_mbnumber = @mbnumber )
		...
		--and ivd_charge <> 0
		-- BDH 40516 start
		and ((isnull(@PrintMBbyUser, 'N') = 'Y' and ivh_user_id2 = @tmwuser) or isnull(@PrintMBbyUser, 'N') = 'N')
		-- 40516 end
  END
...
----  ORDER BY	ivh_revtype1, cmd_code, ord_hdrnumber, ivd_sequence
--  ORDER BY	ivh_shipdate, cmd_code, ord_hdrnumber, ivd_sequence

  SELECT mbt.*,
		 ord.ord_remark 
  FROM		#masterbill_temp mbt
			left outer join orderheader ord ON ord.ord_hdrnumber = mbt.ord_hdrnumber and mbt.ord_hdrnumber > 0
  ORDER BY	mbt.ivh_shipdate, mbt.cmd_code, mbt.ord_hdrnumber, mbt.ivd_sequence
-- PTS 62766 end

  DROP TABLE 	#masterbill_temp
GO
GRANT EXECUTE ON  [dbo].[d_masterbill33_sp] TO [public]
GO
