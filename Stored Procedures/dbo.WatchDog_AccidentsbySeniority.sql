...
CREATE Proc [dbo].[WatchDog_AccidentsbySeniority] 
(
	@MinThreshold float = 1,
	@MinsBack int=-20,
	@TempTableName varchar(255)='##WatchDogGlobalAccidentsbySeniority',
	@WatchName varchar(255)='WatchAccidentsbySeniority',
...
As
	...
	/*
	Procedure Name:    WatchDog_AccidentsbySeniority
	Author/CreateDate: Lori Brickley / 5-6-2005
	Purpose: 	    Returns accidents which were entered in the last
					x minutes which involved a driver within
					last y years of seniority
	Revision History:
	*/
	
	/*
	if not exists (select WatchName from WatchDogItem where WatchName = 'AccidentsbySeniority')
	INSERT INTO watchdogitem (WatchName, BeginDate, EndDate, SqlStatement, Operator, EmailAddress, BeginDateMinusDays, EndDatePlusDays, DateField, QueryType, ProcName, NumericOrText, MinsBackToRun, HTMLTemplateFlag, ActiveFlag, DefaultCurrency, CurrencyDateType, Description)
	 VALUES ('AccidentsbySeniority','12/30/1899','12/30/1899','WatchDog_AccidentsbySeniority','','',0,0,'','','','','',1,0,'','','')
	*/
	...
	WHERE dra_accidentdate >= DateAdd(mi,@MinsBack,GetDate())
		AND dra_accidentdate >= mpp_senioritydate 
		--AND mpp_senioritydate
		AND dra_accidentdate <= dateadd(year, @MinThreshold, mpp_senioritydate)
