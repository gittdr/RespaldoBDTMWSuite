...
-- =============================================
CREATE PROCEDURE [dbo].[sp_auditanotasin]
...
--seleccionamos los valores recien insertados su columna tar_number es NULL 
  ...
  SELECT  @hdrorden = ord_hdrnumber, @nota = 'Ent a ' + replace(replace(replace(substring(update_note,1,len(update_note)-3),substring(substring(update_note,1,len(update_note)-3),charindex(':',substring(update_note,1,len(update_note)-3))+3, (len(substring(update_note,1,len(update_note)-3))+3) - charindex('>',substring(update_note,1,len(update_note)-3))),''),cast(year(getdate()) as varchar)+'-',''),'):',')') +'->' +updated_by 
    ,@mov = key_value  
    FROM    tmwsuite.dbo.expedite_audit
    WHERE   (activity = 'Arrival Date Changed') 
...
--ponemos todos los valores de tar_number en 0 en donde correspondan
         ...
         set tar_number = 0
         WHERE     (tar_number IS NULL) AND (activity = 'Arrival Date Changed') 
         and join_to_table_name = 'stops' and updated_By <> 'NT AUTHORITY\SYSTEM'
