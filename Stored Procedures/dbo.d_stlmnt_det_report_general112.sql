...
CREATE PROC [dbo].[d_stlmnt_det_report_general112](
 ...
 * 8-12-2008 PTS 44056 JSwindell - CREATED: This is a copy of "d_settlement_sheet_summary_new" and changed to include thirdparty.  
 * 4-12-2010 PTS 51430 - TMEZE - CREATED: This is a copy of "d_stlmnt_det_report_general94" and added loadedmiles, emptymiles.
 *
 **/
...
-- PTS 29303 -- BL (end)	
	...
	userlabelname		varchar(60) null,
	label_name		varchar(60) null,
...
-- PTS 29515 -- BL (end)
-- PTS 51430 start
	case when (pyd_description = 'Hourly Route Pay' or (pyt_basis = 'LGH' and pyd_rateunit <> 'MIL')) then
		(select isnull(sum(stp_lgh_mileage),0) from stops where stops.lgh_number = tp.lgh_number and stops.stp_loadstatus = 'LD')
	else 
		0
	end as 'LoadedMiles',
	case when (pyd_description = 'Hourly Route Pay' or (pyt_basis = 'LGH' and pyd_rateunit <> 'MIL')) then
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general112] TO [public]
GO
