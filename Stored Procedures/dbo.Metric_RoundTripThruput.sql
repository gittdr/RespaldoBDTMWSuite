...
CREATE PROCEDURE [dbo].[Metric_RoundTripThruput]
(
	...
	@Mode varchar(20) = 'RTThruputPerDay',	-- RTThruputPerDay, RTThruputPerRTEstDays, RTThruputPerTruck,
											-- RTThruputPerRT, RTThruputPerAllMiles, RTTVCPerAllMile
...
--		AND rt_EndDate between @DateStart AND @DateEnd
	...
	If @Mode = 'RTThruputPerDay'
		begin
			SELECT @ThisCount = Sum(ThruputRT) 
			FROM #ResultsTable 
	...
	Else If @Mode = 'RTThruputPerRTEstDays'	-- this is typically Thruput / Unit Time on Constraint Resource
		begin
			SELECT @ThisCount = Sum(ThruputRT) 
			FROM #ResultsTable 
	...
	Else If @Mode = 'RTThruputPerTruck'
		begin
			SELECT @ThisCount = Sum(ThruputRT) 
			FROM #ResultsTable 
	...
	Else If @Mode = 'RTThruputPerAllMiles'
		begin
			SELECT @ThisCount = Sum(ThruputRT) 
			FROM #ResultsTable 
	...
	Else If @Mode = 'RTTVCPerAllMile'
		begin
			SELECT @ThisCount = Sum(TVCRT) 
			FROM #ResultsTable 
	...
	/*
		...
		EXEC MetricInitializeItem
			...
			@sCaptionFull = 'Round Trip Thruput Metrics',
			@sPROCEDUREName = 'dbo.Metric_RoundTripThruput',
			@sCachedDetailYN = '',
	...
	*/
GO
GRANT EXECUTE ON  [dbo].[Metric_RoundTripThruput] TO [public]
GO
