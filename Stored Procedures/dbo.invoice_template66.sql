...
create procedure [dbo].[invoice_template66](@invoice_nbr int,@copies int)
as
/*	PROCEDURE RETURNS 0 - IF NO DATA WAS FOUND
	1 - IF SUCCESFULLY EXECUTED
	@@ERROR - db GLOBAL VARIABLE VALUE IF AN ERROR OCCURS

02/09/05 DPM - PTS26512 - Made Changes to Accomodate New Invoice Format 66
...
select @route_desc = labelfile.name from orderheader, labelfile where ord_revtype2 = abbr and ord_hdrnumber = @ord_hdrnumber and labeldefinition = 'RevType2'
select @ref_number = ref_number from referencenumber where ord_hdrnumber = @ord_hdrnumber and ref_type = 'GMT'
select @ref_number = ISNULL(@ref_number,'UNKNOWN')
...
/* CREATE TEMP TABLE AND SELECT INITIAL DATA SET 
 ...
 SELECT  invoiceheader.ivh_invoicenumber,   
	...
	IsNull(ivd_fsc,0.0) ivd_fsc,
	@route_desc route_desc,
	@ord_number ord_number,
	@ref_number cust_track_nbr
    into #invtemp_tbl
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template66] TO [public]
GO
