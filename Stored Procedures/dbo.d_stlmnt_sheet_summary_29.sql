...
CREATE      PROC [dbo].[d_stlmnt_sheet_summary_29](   
 ...
 * 
 * NAME:
 * dbo.D_STLMNT_SHEET_SUMMARY_29
 *
 * TYPE:
 * [StoredProcedure]
 *
 * DESCRIPTION:
 * This procedure returns data needed to print d_stlmnt_det_report_general_29
 *
 * RETURNS:
 * NONE
 *
 ...
 * 
 * REVISION HISTORY:
 * Gibson DEVELOPER 39498 created as modifcation to proc D_STLMNT_SHEET_SUMMARY_26 10/2/07
...
-- consignee_name  varchar(30) null,    
 ...
 consignee_geoloc varchar(50) NULL,    ord_totalmiles int NULL,  
...
SET ord_startdate = oh.ord_startdate,    
 ...
 ord_totalmiles = oh.ord_totalmiles,  
 ord_revtype2 = oh.ord_revtype2 ,  
 ord_completiondate = oh.ord_completiondate ,  
 cmd_code = oh.cmd_code,  
-- PTS 17477 -- BL    
 ord_revtype3 = oh.ord_revtype3
FROM  orderheader oh    
...
SET shipper_name = co.cmp_name,    
 ...
 shipper_geoloc = IsNull(co.cmp_geoloc  ,'')  
...
SET consignee_name = co.cmp_name,    
 ...
 consignee_geoloc = IsNull(co.cmp_geoloc  ,'')  
FROM  company co, city ct, orderheader oh    
WHERE #temp_pd.ord_hdrnumber = oh.ord_hdrnumber    
...
--Update the temp pay details arrivaldate    
UPDATE #temp_pd    
...
--  AND rn.ref_type = 'SID'    
-- PTS 18049 -- BL (start)
--  AND rn.ref_type = 'VLBL'    
  AND rn.ref_type IN ('VLBL#', 'HRBL#')
  AND rn.ref_type = 
     (SELECT max(ref_type)
      FROM referencenumber ref
      WHERE ref.ref_type IN ('VLBL#', 'HRBL#')
      AND ref.ref_tablekey = #temp_pd.ord_hdrnumber)
-- PTS 18049 -- BL (end)
...
-- PTS 18049 -- BAL (start)
--    (make section for tax reimbursements)
UPDATE #temp_pd    
SET itemsection = 5
WHERE pyt_itemcode = 'GST'
-- PTS 18049 -- BAL (end)

-- PTS 18049 -- BL (start)
--    (put '2% Admin Fee' into the 'Settlement Deductions' area)
UPDATE #temp_pd    
SET itemsection = 3
WHERE pyt_itemcode = 'ADMIN'
-- PTS 18049 -- BAL (end)
...
--Get the appropriate Company Name associated with the orders
/* if there is a problem with misc pay, this would be better than what follows
   Gibson says they  never have misc pay
declare @cmpname varchar(60)
select @cmpname = label_extrastring2 from labelfile
where labeldefinition = 'Company' and abbr = 
(select ord_subcompany 
  from orderheader 
  where ord_hdrnumber =
     (select max(ord_hdrnumber) from #temp_pd))


update #temp_pd
set company_name = @cmpname
*/
 update #temp_pd
    set company_name = l.label_extrastring2
   from labelfile l,
        orderheader o
  where o.ord_hdrnumber = #temp_pd.ord_hdrnumber
    and l.labeldefinition = 'company'
    and l.abbr = o.ord_subcompany

/* PAUL BOUREGEALT addition 10/18/07 */
 update #temp_pd
    set company_name = (select max(t2.company_name)
                          from #temp_pd t2)
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_sheet_summary_29] TO [public]
GO
