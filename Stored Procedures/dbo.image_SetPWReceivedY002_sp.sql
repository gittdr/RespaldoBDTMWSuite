...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[image_SetPWReceivedY002_sp] (@ordnum varchar(12),@doctype varchar(6),@p_lghnumber int = 0,@p_drv varchar(8) = 'UNKNOWN')    
As
/**
 * 
 * NAME:
...
BEGIN
   If exists (select 1 from labelfile where labeldefinition = 'PaperWork' and abbr = @doctype and isnull(retired,'N') = 'N')
    ...
    INSERT INTO paperwork (abbr,pw_received,ord_hdrnumber,pw_dt,last_updatedby,last_updateddatetime,lgh_number,pw_imaged,Mov_Number)
	      VALUES( @doctype,'Y',@v_ordhdrnumber,getdate(),'PWReceivedY002_sp',getdate(),@v_lghnumber,'Y',@v_movnumber)
...
BEGIN
	...
	pw_Received = 'Y', 
	pw_dt = GETDATE(),
	last_updatedby = 'PWReceivedY002_sp',
...
END
GO
GRANT EXECUTE ON  [dbo].[image_SetPWReceivedY002_sp] TO [public]
GO
DECLARE @xp float
SELECT @xp=1.32
EXEC sp_addextendedproperty N'Version', @xp, 'SCHEMA', N'dbo', 'PROCEDURE', N'image_SetPWReceivedY002_sp', NULL, NULL
GO
