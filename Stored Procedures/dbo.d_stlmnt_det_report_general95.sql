...
CREATE  PROC [dbo].[d_stlmnt_det_report_general95](
 ...
 * REVISION HISTORY:
 --
 --	5/2/03  18243  Expanded WHERE clause on select from invoicedetail to prevent duplicate
 --		       rows from being returned on orders with credit memos/rebills
 --      
 * 11/07/2007.01 ? PTS40186 - JGUO ? convert old style outer join syntax to ansi outer join syntax.
 * 8-19-2008 PTS 43953 JSwindell - CREATED: This is a copy of "d_stlmnt_sheet_summary_27" 
 * 9-18-2008 PTS 43953 JSwindell - Also need the manpowerprofile accounting type 9-18-2008 for subtotals.
 * 12-19-2008 PTS 45462 JSwindell - (Rudy Fix) Client wants some changes.
 * 03-11-2009 PTS 46477 JSwindell - (Rudy fix) Client wants Standard Deductions to show Transaction date.
 **/
...
CREATE TABLE #temp_pd(
	...
	--pyd_description		varchar(30) null, -- PTS 45462 (Rudy Fix)
	pyd_description		varchar(300) null,   -- PTS 45462 (Rudy Fix)	
	...
	--ord_number		char(8) null,	-- PTS 45462 (Rudy Fix)
	ord_number		varchar(30) null,	-- PTS 45462 (Rudy Fix)
	...
	--pyt_description		varchar(30) null,  -- PTS 45462 (Rudy Fix)
	pyt_description		varchar(300) null,  -- PTS 45462 (Rudy Fix)
	...
	end_state		char(2) null,
        ...
        ivd_rate                money null,
	lgh_count		int null,
	ivh_deliverydate datetime null,	-- PTS 43953:  Users want the BILLING DELIVERY DATE.
	mpp_actg_type varchar(1) null )	-- PTS 43953:  Also need the manpowerprofile accounting type 9-18-2008
...
SELECT pd.pyd_number,
	...
	ivd.ivd_quantity,
        ivd.ivd_rate,
	0,
	null,     -- PTS 43953:  Users want the BILLING DELIVERY DATE.   
	null      -- PTS 43953:  Also need the manpowerprofile accounting type 9-18-2008
--pts40186 jguo outer join conversion
FROM paydetail pd LEFT OUTER JOIN invoicedetail ivd ON (
		pd.ord_hdrnumber = ivd.ord_hdrnumber and
...
-- PTS 43953:  Users want the BILLING DELIVERY DATE.
UPDATE #temp_pd
SET #temp_pd.ivh_deliverydate = (select min(ivh_deliverydate)
						from invoiceheader
						where ord_hdrnumber = #temp_pd.ord_hdrnumber
						and ord_hdrnumber <> 0) -- PTS 46477 

-- PTS 43953:  Also need the manpowerprofile accounting type 9-18-2008
UPDATE #temp_pd
set mpp_actg_type = (select mpp_actg_type from manpowerprofile
					 where mpp_id = asgn_id ) 

-- PTS 46477 <<start>> Users want pyd_transdate date for standard deductions.
UPDATE #temp_pd
set ivh_deliverydate = pyd_transdate
WHERE itemsection = 3
-- PTS 46477 <<end>>

-- PTS 45462 (Rudy Fix) << start >>
-- If we have standard deductions they still want it
-- to be grouped by pyt_itemcode.  SO fake it - the ord_number for these is null anyway so
-- make the ord_number =  pyt_itemcode if itemsection = 3 (standard deductions)

UPDATE #temp_pd
set ord_number = pyt_itemcode
where itemsection = 3 and ord_number is null
-- PTS 45462 (Rudy Fix) << end >>
...
SELECT pyd_number, 
	...
	pyd_payto,
	pyt_itemcode,
	--pyd_description, -- PTS 45462 (Rudy Fix)
	cast(pyd_description as char(30))'pyd_description',   -- PTS 45462 (Rudy Fix)
	pyr_ratecode, 
	pyd_quantity, 
	pyd_rateunit, 
	pyd_unit, 
	...
	itemsection,
	ord_startdate,
	-- ord_number, -- PTS 45462 (Rudy Fix)
	cast(ord_number as char(8))'ord_number',  -- PTS 45462 (Rudy Fix)
	ref_number,
	stp_arrivaldate,
	shipper_name,
	shipper_city,
	...
	pyd_payrevenue,
	cht_basisunit,
	--pyt_description,  -- PTS 45462 (Rudy Fix)
	cast(pyt_description as char(30))'pyt_description', -- PTS 45462 (Rudy Fix)
	userlabelname,
	label_name,
	otherid,
	trc_drv,
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general95] TO [public]
GO
