...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Metric_Plan12Pct] (
...
/* NOTE: This SQL is used by MetricProcessing to automatically generate an new metric item in a category called NewItems.
	...
	EXEC MetricInitializeItem
		...
		@nPlusDeltaIsGood = 1,
		@nCumulative = 0,
		@sCaption = '% Planned (12 hr)',
		@sCaptionFull = 'Percent of orders that are planned 12 hours out',
		@sProcedureName = 'Metric_Plan12Pct',
		@sCachedDetailYN = '',
...
*/
	...
	/*
	-- DISPATCH #3: Percent Planned (12 hours):
	SET NOCOUNT ON
	...
	-- TOTAL ORDERS started today booked over 12 hours ago.
	SELECT @ThisCount = COUNT(*) FROM #orderheader WHERE DATEDIFF(hour, ord_bookdate, ord_completiondate) > 12
	-- PERCENTAGE of orders booked over 12 hours ago.
...
BEGIN
	-- Step 1: get count of Legheaders for next 12 hours
	-- Step 2: Get count of Legheaders where assingment has been made
	Declare @LegheaderCountNext12Hours Float
	Declare @LegheaderCountNext12HoursWithAssignments Float
	Set @LegheaderCountNext12Hours =
		(select count(*) 
		from 	legheader (NOLOCK)
		where 	lgh_startdate between Getdate() and dateAdd(hh,12,Getdate())
	...
	Set @LegheaderCountNext12HoursWithAssignments =
		(select count(*) 
		from 	legheader (NOLOCK)
		where 	lgh_startdate between Getdate() and dateAdd(hh,12,Getdate())
	...
	BEGIN
		...
		SELECT @ThisTotal =@LegheaderCountNext12Hours
		SELECT @ThisCount = @LegheaderCountNext12HoursWithAssignments
	END 
...
GO
GRANT EXECUTE ON  [dbo].[Metric_Plan12Pct] TO [public]
GO
