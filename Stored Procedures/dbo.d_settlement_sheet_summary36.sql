...
CREATE   PROC [dbo].[d_settlement_sheet_summary36](
...
CREATE TABLE #temp_pd(
	...
	not_sequenced           int null,
	not_sequenced2		int null)
...
--DPH PTS 22675
DECLARE @Minpyd_num INT, @Minpyd_num2 INT, @MinOrd INT, @CurOrd INT, @Break_Loop INT
SET @Minpyd_num = 0
SET @MinOrd = 0
SET @CurOrd = 0
SET @Minpyd_num2 = 0
...
WHILE (SELECT COUNT(*) FROM #temp_pd WHERE not_sequenced = 1) > 0--loop through all Pay-Details
 BEGIN

	SELECT @Minpyd_num2 = MIN(pyd_number) from #temp_pd where not_sequenced = 1 --pyd_number > @Minpyd_num2
	SET @Break_Loop = 0
	

	WHILE @Break_Loop <> 1
	 BEGIN
        	SELECT @CurOrd = ord_hdrnumber from #temp_pd where pyd_number = @Minpyd_num2
	
		--Set sort_sequence column for each pyd_number for this order
		IF exists (Select * from #temp_pd where (pyt_itemcode = 'STOPS' or pyt_itemcode = 'STOPLL')
						  	and pyd_number = @Minpyd_num2)
		 BEGIN
			Update #temp_pd set sort_sequence = '3' where pyd_number = @Minpyd_num2
			Update #temp_pd set not_sequenced = '0' where pyd_number = @Minpyd_num2
		 END
		ELSE IF exists (Select * from #temp_pd where (pyt_itemcode = 'STOPSU' or pyt_itemcode = 'STOPLU')
						  	and pyd_number = @Minpyd_num2)
		 BEGIN
			Update #temp_pd set sort_sequence = '5' where pyd_number = @Minpyd_num2
			Update #temp_pd set not_sequenced = '0' where pyd_number = @Minpyd_num2
		 END
		ELSE IF exists (Select * from #temp_pd where pyd_loadstate = 'UNLD' and summary_code = 'MIL'
						  	and pyd_number = @Minpyd_num2)
		 BEGIN
			Update #temp_pd set sort_sequence = '6' where pyd_number = @Minpyd_num2
			Update #temp_pd set not_sequenced = '0' where pyd_number = @Minpyd_num2
		 END
		ELSE IF exists (Select * from #temp_pd where pyd_loadstate = 'LD' and summary_code = 'MIL'
						  	and pyd_number = @Minpyd_num2)
		 BEGIN
			Update #temp_pd set sort_sequence = '2' where pyd_number = @Minpyd_num2
			Update #temp_pd set not_sequenced = '0' where pyd_number = @Minpyd_num2
		 END
		ELSE
		 BEGIN
			Update #temp_pd set sort_sequence = '4' where pyd_number = @Minpyd_num2
			Update #temp_pd set not_sequenced = '0' where pyd_number = @Minpyd_num2
		 END
		--Set sort_sequence column for each pyd_number for this order

		Select @Minpyd_num = MIN(pyd_number) from #temp_pd
				     WHERE pyd_number > @Minpyd_num2
				     AND ord_hdrnumber = @CurOrd
	
	 	If Isnull(@Minpyd_num, -99999) = -99999 Set @Break_Loop = 1
		Else Set @Minpyd_num2 = @Minpyd_num


	 END
 END                   

--Loop Through to sort empty mileage (sort_sequence = 6)
DECLARE @temp_pyd_number1 INT, @temp_pyd_number2 INT, @temp_pyd_number3 INT,
	@pyd_sequence1 INT, @pyd_sequence2 INT, @pyd_sequence3 INT
SET @Minpyd_num = 0
SET @MinOrd = 0
SET @CurOrd = 0
SET @Minpyd_num2 = 0
SET @Break_Loop = 0
SET @temp_pyd_number1 = '-99999'
SET @temp_pyd_number2 = '-99999'
SET @pyd_sequence1 = '99999'
SET @pyd_sequence2 = '99999'
SET @pyd_sequence3 = '99999'

WHILE (SELECT COUNT(*) FROM #temp_pd WHERE not_sequenced2 = 1) > 0--loop through all Pay-Details
 BEGIN
	SELECT @Minpyd_num2 = MIN(pyd_number) from #temp_pd where not_sequenced2 = 1 --pyd_number > @Minpyd_num2
 	SET @Break_Loop = 0
 	...
 	WHILE @Break_Loop <> 1
 	 BEGIN
 		...
 		--Set sort_sequence column for empty_mileage
 		If exists (Select * from #temp_pd where sort_sequence = 6 and pyd_number = @Minpyd_num2)
		 BEGIN
			IF @temp_pyd_number1 = '-99999' 
			 BEGIN
				Select @temp_pyd_number1 = @Minpyd_num2
				Select @pyd_sequence1 = pyd_sequence from #temp_pd where pyd_number = @temp_pyd_number1
			 END
			ELSE IF @temp_pyd_number2 = '-99999'
			 BEGIN
				Select @temp_pyd_number2 = @Minpyd_num2
				Select @pyd_sequence2 = pyd_sequence from #temp_pd where pyd_number = @temp_pyd_number2
			 END
			ELSE
			 BEGIN
				Select @temp_pyd_number3 = @Minpyd_num2
				Select @pyd_sequence3 = pyd_sequence from #temp_pd where pyd_number = @temp_pyd_number3
			 END
		 END
 		--Set sort_sequence column for empty_mileage
		Update #temp_pd set not_sequenced2 = '0' where pyd_number = @Minpyd_num2 		
	...
	--Set sort_sequence column for empty_mileage
	If @pyd_sequence1 < @pyd_sequence2 and @pyd_sequence1 < @pyd_sequence3
	 BEGIN
		Update #temp_pd set sort_sequence = '1' where pyd_number = @temp_pyd_number1
	 END
	Else If @pyd_sequence2 < @pyd_sequence1 and @pyd_sequence2 < @pyd_sequence3
	 BEGIN
		Update #temp_pd set sort_sequence = '1' where pyd_number = @temp_pyd_number2
	 END
	Else If @pyd_sequence3 < @pyd_sequence1 and @pyd_sequence3 < @pyd_sequence2
	 BEGIN
		Update #temp_pd set sort_sequence = '1' where pyd_number = @temp_pyd_number3
	 END
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary36] TO [public]
GO
