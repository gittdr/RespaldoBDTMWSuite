...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Metric_Plan48Pct] (
...
/* NOTE: This SQL is used by MetricProcessing to automatically generate an new metric item in a category called NewItems.
	...
	EXEC MetricInitializeItem
		@sMetricCode = 'Plan48Pct',
		@nActive = 0,	-- 1=active, 0=inactive.
		@nSort = 104, 	-- Used to determine the sort order that updates should be run.
		@sFormatText = 'PCT',	-- Typically 'PCT' or blank ('').
		@nNumDigitsAfterDecimal = 0,
		@nPlusDeltaIsGood = 1,
		@nCumulative = 0,
		@sCaption = '% Planned (48 hr)',
		@sCaptionFull = 'Percent of orders that are planned 48 hours out',
		@sProcedureName = 'Metric_Plan48Pct',
		@sCachedDetailYN = '',
...
*/
	...
	-- OLD
	/*
	-- DISPATCH #4: Percent Planned (48 hours): 
	SET NOCOUNT ON
	...
	SELECT @ThisCount = COUNT(*) FROM #orderheader WHERE DATEDIFF(hour, ord_bookdate, ord_completiondate) > 48
	...
	*/
	...
	-- Step 1: get count of Legheaders for next 48 hours
	-- Step 2: Get count of Legheaders where assingment has been made
	Declare @LegheaderCountNext48Hours Float
	Declare @LegheaderCountNext48HoursWithAssignments Float
	Set @LegheaderCountNext48Hours =
		(select count(*) 
		from 	legheader (NOLOCK)
		where 	lgh_startdate between Getdate() and dateAdd(d,2,Getdate())
	...
	Set @LegheaderCountNext48HoursWithAssignments =
		(select count(*) 
		from 	legheader (NOLOCK)
		where 	lgh_startdate between Getdate() and dateAdd(d,2,Getdate())
	...
	BEGIN
		Set @ThisTotal= 
			(Select DailyTotal from MetricDetail where MetricCode='Plan48Pct' and PlainDate =@DateStart)
		Set @ThisCount = 
			(Select DailyCount from MetricDetail where MetricCode='Plan48Pct' and PlainDate =@DateStart)
	END

	IF ( abs( DateDiff(d, Getdate(),@DateStart))<=1 )
	BEGIN

		SELECT @ThisTotal =@LegheaderCountNext48Hours
		SELECT @ThisCount = @LegheaderCountNext48HoursWithAssignments
	END 

	SELECT @Result = CASE ISNULL(@ThisTotal, 0) WHEN 0 THEN NULL ELSE @ThisCount / @ThisTotal END

GO
GRANT EXECUTE ON  [dbo].[Metric_Plan48Pct] TO [public]
GO
