...
CREATE PROC [dbo].[credit_pay_special_paydetail_list_sp]	(@ivh_hdrnumber int, 
														@ivh_definition varchar(6), 
														@user_id varchar(20), 
														@ivh_credit_hdrnumber int, 
														@ord_hdrnumber int,
...
If @ivh_definition = 'SUPL' OR
	(select count(*) 
		from invoicedetail i, chargetype c 
		where i.ivh_hdrnumber = @ivh_hdrnumber 
		and c.cht_itemcode = i.cht_itemcode 
		and cht_basis = 'SHP') <= 0
...
-- End OF Populate pay data for list of paydetails	
	...
	-- Update assetassignment /Conditional				
	...
	END 
	-- end of Update assetassignment /Conditional	
...
GO
GRANT EXECUTE ON  [dbo].[credit_pay_special_paydetail_list_sp] TO [public]
GO
