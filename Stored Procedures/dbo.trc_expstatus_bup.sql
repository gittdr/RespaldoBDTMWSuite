 ...
 **Sentencia de prueba:
 ...
 exec trc_expstatus_bup '1497'
 
 *******************************************************************************************************************************************************/
CREATE PROCEDURE [dbo].[trc_expstatus_bup] 
(
  @trcid  VARCHAR(8),
  @debug  INTEGER = 0
)
...
if @expcode =0
begin
 print 'No se encontro expiracion Activa'
end
else
begin

 print 'Se encontro el exp code: ' + cast(@expcode as varchar(20)) + ' con el status ' + @expstat + ' con la fecha ' + cast(@expdate  as varchar(100))
end
------------------
...
BEGIN
  ...
  ELSE
  --debug emolvera-------------------------------------------
   print 'la expiracione es 900 por lo que es un OUT'
   ----------------------------------------------------------
    SELECT  trc_number,
...
SELECT  @plnlgh = Planned.lgh_number,
  ...
  FROM  TrcExpstatus_GetActivity_fn(@trcid, @PlnDspCodes) Planned
   ...
   print 'EXPCODE es: ' + cast(@expcode as varchar(10))


   	 --Variable alternar para contener el valor de la exp y no lo cambie la exp logic
		 declare @expstatalt varchar(10)  = @expstat
	...
	begin
	     ...
	     ---debug emolvera---------------------------------
		 print 'Como no se encontro expiracion activa busca la logica de exp del tractor en general info la cual esta en ' + @NewTrcExpLogic + '  trayendo la ultima expiracion completada...'  
...
--Emolvera DEBUG-----------------------------------------------------
print '****************************************'
print 'LEG STD AVL: ' +  isnull(cast( @lgh as varchar(20)),'--')  + ' Status ' +  isnull(@avlstat,'--')
print 'LEG PLN: ' + isnull(cast( @plnlgh as varchar(20)),'--') + ' Compania: ' + isnull( @plncmp,'--')
print 'EXP: ' + cast( @expcode as varchar(20))
print 'EXP STAT: '   + cast( @expstat as varchar(20))
print 'EXP STAT ALT: ' + cast( @expstatalt as varchar(20))
print 'AVL STAT:'    + cast( @avlstat as varchar(20))
print 'EVT :' + isnull(@evt,'')
print '****************************************'
---------------------------------------------------------------------
...
BEGIN
  ...
  print 'Entrando a Modo Leg STD,AVL existe y expiracion existe'


  IF @avlstat <> 'USE' 
  BEGIN

   print '   Tiene un AVL estado diferente a USE  y tiene un leg en STD,AVL'
...
/****************************MOD 001 by EMOLVERA 9 agosto 2019*******************
In order to give prority to INSHOP over USE
************************************************************************************/
  else if @avlstat in ('USE') and @expstatalt  ='INSHOP'
   begin
     print '  Entrando a Modo Tiene AVL stat y USE y EXPSTATALT INSHOP'
	SELECT  @avlstat = @expstatalt,
		      @avldate = @expavldate,
		      @avlcmp = @expavlcmp,
		      @avlcity = @expavlcity
   end
...
BEGIN
  print 'Entrando a Modo Tiene un in service'
  SET @UseProfileAssets = 'Y'
END
-- Have active expiration only
ELSE IF @expcode > 0
BEGIN
  print 'Entrando a Modo Tiene solo expiracion'
	SELECT  @avlstat = @expstat,
		      @avldate = @expavldate,
		      @avlcmp = @expavlcmp,
		      @avlcity = @expavlcity
...
BEGIN
 print 'Entrando a Modo No tiene expiracion ni leg activo'
  SELECT  @avlstat = 'AVL',
		      @avldate = '19500101',
		      @avlcmp = 'UNKNOWN',
		      @avlcity = 0,
...
---emolvera debug----------------------------------------------------------------------------------------------
print 'El estatus AVL es : ' + @avlstat +' y el leg planeado es ' + isnull(cast( @plnlgh  as varchar(20)),'--')
---------------------------------------------------------------------------------------------------------------
...
BEGIN
 ---emolvera debug------------------
 print 'Existe un leg PLN por lo cual el AVL STATUS se va a *PLN*'
 ------------------------------------
  SELECT  @avlstat = 'PLN'
END
...
BEGIN
  ---emolvera debug------------------
  print 'Haciendo el update al tractorprofile........'
  ------------------------------------
