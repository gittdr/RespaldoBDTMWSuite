...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[d_tar_viewtariffs_stl_sp] 
	@Primary char(1) , 
	@Date datetime , 
	@BillTo char(8) , 
	@CmpType1 char(6) , 
 ...
 * 
 * NAME:
 * dbo.proc for dw d_tar_viewtariffs_stl
 *
 * TYPE:
 * [StoredProcedure|
 *
 * DESCRIPTION:
 * proc for d_tar_viewtariffs_stl_sp
 ...
 * REVISION HISTORY:
 * Author unknown
 * DPETE PTS 29757 return company name and geoloc
 * 12/22/05 MRH 30829 Add 3rd party & third party type.
 * LOR	PTS# 31777	add backouts
 * vjh	PTS38858	Add asset terminal restrictions
 * vjh	PTS47730	handle multiple belongsto 
 * SPN	PTS53449	terminal restrictions issue fixed
 *
 **/
...
If @Primary  = 'B' 
	...
	WHERE 	( tariffkey.trk_primary = @Primary ) AND 
			...
			( @thirdpartytype in (tariffkey.trk_thirdpartytype, 'UNKNOWN')) AND
			-- BEGIN PTS 53449 SPN
			--( @mpp_terminal in (tariffkey.mpp_terminal, 'UNKNOWN')) AND
			--( @trc_terminal in (tariffkey.trc_terminal, 'UNKNOWN')) AND
			--( @trl_terminal in (tariffkey.trl_terminal, 'UNKNOWN'))
			( @mpp_terminal in (IsNull(tariffkey.mpp_terminal,'UNK'), 'UNK', 'UNKNOWN')) AND
			( @trc_terminal in (IsNull(tariffkey.trc_terminal,'UNK'), 'UNK', 'UNKNOWN')) AND
			( @trl_terminal in (IsNull(tariffkey.trl_terminal,'UNK'), 'UNK', 'UNKNOWN'))
			-- END PTS 53449 SPN
...
Else
	...
	WHERE 	( tariffkey.trk_primary = @Primary ) AND 
			...
			( @thirdpartytype in (tariffkey.trk_thirdpartytype, 'UNKNOWN')) AND
			-- BEGIN PTS 53449 SPN
			--( @mpp_terminal in (tariffkey.mpp_terminal, 'UNK')) AND
			--( @trc_terminal in (tariffkey.trc_terminal, 'UNK')) AND
			--( @trl_terminal in (tariffkey.trl_terminal, 'UNK'))
			( @mpp_terminal in (IsNull(tariffkey.mpp_terminal, 'UNK'), 'UNK')) AND
			( @trc_terminal in (IsNull(tariffkey.trc_terminal, 'UNK'), 'UNK')) AND
			( @trl_terminal in (IsNull(tariffkey.trl_terminal, 'UNK'), 'UNK'))
			-- END PTS 53449 SPN
			--PTS 51570 JJF 20100510
			--AND dbo.RowRestrictByUserMultiple(tar_belongsto, '', '', '') = 1
			AND EXISTS (	SELECT	*
...
GO
GRANT EXECUTE ON  [dbo].[d_tar_viewtariffs_stl_sp] TO [public]
GO
