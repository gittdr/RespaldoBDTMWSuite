...
CREATE  PROC [dbo].[d_stlmnt_det_report_general97](
 ...
 *
 * REVISION HISTORY:
 * 09/11/2008 - PTS 43376 JSwindell - Created for dwo: d_stlmnt_det_report_general97 (( BASED on "d_settlement_sheet_summary_new" ))
 * used for 2 dwo's:  d_stlmnt_det_report_general97 and d_stlmnt_det_report_general97_embedded
 * 09/30/2008 - PTS 43376 change to proc:  don't delete the line items, make their qty zero. (near bottom of proc)
 * 12/31/2008 - PTS 45176 JSwindell - Additional cols & new sorting mechanism. 1/20/2009 fix added @ bottom.
 * 
 **/
...
-- PTS 29303 -- BL (end)	
	...
	--ord_number		char(12) null,		-- PTS 43376 
	ord_number			varchar(20) NULL,	-- PTS 43376 
...
-- PTS 29515 -- BL (end)
    shift_ss_id int null,	-- PTS 43376
	Origin_id Varchar(8) null,	-- PTS 45176
	Dest_id Varchar(8) null,	-- PTS 45176	
	stp_event varchar(6) null, -- PTS 45176
	true_stp_number		int null, -- PTS 45176
	stp_event_sort_sequence int null, -- PTS 45176
	true_stp_mfh_sequence int null, -- PTS 45176 
	lgh_startpoint Varchar(8) null,	-- PTS 45176 
	lgh_endpoint Varchar(8) null	-- PTS 45176
)
...
SELECT pd.pyd_number,
	...
	null, -- pyt_basis -- PTS 29515
	null, -- shift_ss_id -- PTS 43376 
	null,  -- Origin_id							-- PTS 45176
	null,   -- Dest_id 							-- PTS 45176	
	null,   -- stp_event varchar(6) null		-- PTS 45176
	pd.stp_number, -- true_stp_number			-- PTS 45176
	null, -- stp_event_sort_sequence			-- PTS 45176
	isnull(pd.stp_mfh_sequence, 99)'stp_mfh_sequence', --true_stp_mfh_sequence	-- PTS 45176
	pd.lgh_startpoint,							-- PTS 45176 
	pd.lgh_endpoint 							-- PTS 45176	
...
-- PTS 43376 <<start>>
UPDATE #temp_pd
	SET shift_ss_id = (Select shift_ss_id 
					   FROM legheader lh
                       WHERE lh.lgh_number = #temp_pd.lgh_number)
-- PTS 43376 <<end>>
...
UPDATE #temp_pd
   SET ord_number = Rtrim(Ltrim(ord_number)) + '/S'
...
-- PTS 43376 9-30-2008:  Don't delete the line items, make their qty null.
--PTS 43376 remove any mileage rows if the paytype NOT in ('TSM', 'EMTYMI')
--delete from #temp_pd
--where pyd_unit = 'MIL' 
--and pyt_itemcode NOT in ('TSM', 'EMTYMI')

--PTS 43376 remove weekend paytypes
--delete from #temp_pd
--where pyt_itemcode in ('LDWK', 'UNLDWK')


-- BEGIN PTS 49813 SGB 11/13/2009 
-- Remove hard coded list of paytypes to GI setting.
/*
-- PTS 43376 9-30-2008:  Don't delete the line items, make their qty null.
update #temp_pd
set pyd_quantity = NULL
where pyd_unit = 'MIL' 
and pyt_itemcode NOT in ('TSM', 'EMTYMI')
...
Update #temp_pd
	set stp_event_sort_sequence = (CASE stp_event
		WHEN 'BMT' 	THEN 1
		WHEN 'LLD' 	THEN 2
		WHEN 'LUL' 	THEN 3
		WHEN 'EMT' 	THEN 4
...
where pyt_itemcode <> 'LOAD' AND  pyt_itemcode <> 'UNLOAD'

Update #temp_pd	
	set stp_event_sort_sequence = 99
	where stp_event_sort_sequence  is null 	

-- PTS 45176 <<1-20-2009 start fix>>	
Update #temp_pd
	SET lgh_startpoint = NULL 
	where ( lgh_startpoint = 'UNKNOWN' 	or lgh_startpoint = 'UNK' )

Update #temp_pd
	SET lgh_endpoint = NULL 
	where ( lgh_endpoint = 'UNKNOWN' OR lgh_endpoint = 'UNK' ) 
-- PTS 45176 <<1-20-2009 end fix>>	
--================================== PTS 45176 <<end>>
...
SELECT pyd_number, 
	...
	itemsection,
	ord_startdate,
	--ord_number,	-- pts 43376
	CAST(ord_number AS varchar(12))'ord_number',	-- pts 43376	
	ref_number,
	stp_arrivaldate,
	shipper_name,
	shipper_city,
	...
	shift_ss_id,	-- PTS 43376
	Origin_id,			-- PTS 45176
	Dest_id, 				-- PTS 45176
	stp_event,				-- PTS 45176
	true_stp_number,		-- PTS 45176
	stp_event_sort_sequence, -- PTS 45176
	true_stp_mfh_sequence,	-- PTS 45176
	lgh_startpoint,			-- PTS 45176 
	lgh_endpoint 			-- PTS 45176	

  FROM #temp_pd tp LEFT OUTER JOIN payheader ph ON tp.pyh_number = ph.pyh_pyhnumber

GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general97] TO [public]
GO
