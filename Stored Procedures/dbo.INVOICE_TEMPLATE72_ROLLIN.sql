...
GO
  ...
  CREATE PROCEDURE [dbo].[INVOICE_TEMPLATE72_ROLLIN](@invoice_nbr   int,@copies int)  
AS  
  
/**
 * NAME:
 * dbo.INVOICE_TEMPLATE72_ROLLIN
 *
 * TYPE:
 * StoredProcedure
 *
 ...
 * This procedure provides RollINTOLH details for Format 72
 *
 * RETURNS:
 ...
 *
 * REVISION HISTORY:
 * 11/14/2005 ? Dan Hudec - Created Procedure
...
SELECT ivh.ivh_invoicenumber,   
       ...
       ivd.ivd_description,   
       ivd.cht_itemcode,     
       --MRUTH 2/28/05 - Added for Mario's 
       ivd_quantity = case when ivd.cht_itemcode = 'LHF' 
                            then (Select max(ivd2.ivd_quantity) 
                                  from   invoicedetail ivd2 
                                  where  ivd2.ivh_hdrnumber = ivh.ivh_hdrnumber 
                                         and ivd2.cht_itemcode = 'DEL') 
                            else ivd.ivd_quantity 
                            end, 
 	...
 	--invoicedetail.ivd_taxable3,     
   	...
   	ivd_taxable4 = IsNull(cht.cht_taxtable4,ivd.ivd_taxable4),  
        --MRUTH 2/28/05 - Added for Mario's  
        ivd_unit = case when ivd.cht_itemcode = 'LHF' 
                            then (Select max(ivd2.ivd_unit) 
                                  from   invoicedetail ivd2 
                                  where  ivd2.ivh_hdrnumber = ivh.ivh_hdrnumber 
                                         and ivd2.cht_itemcode = 'DEL') 
                            else ivd.ivd_unit 
...
carrier.car_type4,
@temp_labelfilename car_type4_name
  INTO #invtemp_tbl   
FROM invoiceheader ivh  LEFT OUTER JOIN  carrier  ON  ivh.ivh_carrier  = carrier.car_id ,
...
update #invtemp_tbl  
set car_type4_name = la.name  
from labelfile la  
where la.labeldefinition = 'CarType4' and  
     la.abbr = #invtemp_tbl.Car_type4
...
GO
GRANT EXECUTE ON  [dbo].[INVOICE_TEMPLATE72_ROLLIN] TO [public]
GO
