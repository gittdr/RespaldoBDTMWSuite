...
CREATE PROC [dbo].[d_settlement_sheet_summary56](
 ...
 *   
 * NAME:  
 * dbo.d_settlement_sheet_summary56  
 *  
 * TYPE:  
 * StoredProcedure  
 *  
 * DESCRIPTION:  
 * This procedure retrieves pay data for settlement sheet format 56  
 * for GA Foss, this format sequences the pay items according to GA Foss  
 * rules to approximate the sequence of events on a trip  
 *  
 * RETURNS:  
 * None  
 *  
 ...
 *  
 * PARAMETERS:  
* 001 - @p_report_type varchar(5),  
* 002 - @p_payperiodstart datetime retrieve records where pyh_payperiod between this and next (end)  
* 003 - @p_payperiodend datetime,  
* 004 - @p_drv_yes varchar(3) 'XXX' exludes drivers from return set  
* 005 - @p_trc_yes varchar(3) 'XXX' excludes tractors from return set  
* 006 - @p_trl_yes varchar(3) 'XXX' excludes trailers from return set  
* 007 - @p_drv_id varchar(8)    
* 008 - @p_trc_id varchar(8),  
* 009 - @p_trl_id varchar(13),  
* 010 - @p_drv_type1 varchar(6)  pass through to return set  
* 011 - @p_trc_type1 varchar(6)  pass through to return set  
* 012 - @p_trl_type1 varchar(6)  pass through to return set  
* 013 - @p_terminal varchar(8),  
* 014 - @p_name varchar(64)  passed through to return set  
* 015 - @p_car_yes varchar(3)  'XXX' to exclude carriers  
* 016 - @p_car_id varchar(8),  
* 017 - @p_car_type1 varchar(6)  pass through to return set  
* 018 - @p_hld_yes varchar(3),   
* 019 - @p_pyhnumber int,  
* 020 - @p_relcol varchar(3),  
* 021 - @p_relncol varchar(3),  
* 022 - @p_workperiodstart datetime,  
* 023 - @p_workperiodend datetime)  
 *  
 * REFERENCES: (called by and calling references only, don't   
 *              include table/view/object references)  
 *   
 * REVISION HISTORY:  
 * 02/24/06.01 ? PTS31104 - DPETE - created for settlement shett format 56 for GA Foss  
       copy of d_settlement_sheet_summary_new modified to add a sequence number to  
 *     sequence pay items within an order to approximete the sequence on the trip (see SR) 
 *
 * 08/18/06.02 - PTS34147 DPETE sequence not correct for evetns on report 
...
-- PTS 29303 -- BL (end)   
 ...
 cht_rateunit  varchar(6) null,  
 std_number  int null,  
 stp_number_111  int null,  
 unc_factor  float null,  
 stp_mfh_sequence_111 int null,  
 pyt_description  varchar(30) null,  
 cht_itemcode  varchar(6) null,   userlabelname  varchar(20) null,  
 label_name  varchar(20) null,  
 otherid   varchar(8) null,  
 pyt_fee1  money null,  
 pyt_fee2  money null,  
...
BEGIN  
  ...
  IF @p_drv_yes <> 'XXX'  
    INSERT INTO #temp_pay  
      SELECT pyd_number,  
        ...
        pyd_status,  
        @p_drv_type1  
    /* control sequence of items within ordet by customer rules see SR 31104 */  
 ,sequencenbr = Case pyt_itemcode  
           when 'LOADPY' then 3  
           when 'EXTMPU' then 4  
           when 'DRVMI'  then  
             case  
              when pyd_loadstate = 'LD' then 5  -- DRVMI and loaded  
              else   
               case isnull((select stp_event from stops   
                           where stops.lgh_number = paydetail.lgh_number and stp_lgh_mileage = pyd_quantity
                            and stops.stp_mfh_sequence = (select max(s2.stp_mfh_sequence) from stops s2 where s2.lgh_number = paydetail.lgh_number
                            and s2.stp_lgh_mileage = pyd_quantity)),'??')  
                when 'EMT'  then  10  -- DRVMI and empty at end of trip  
                when 'IEMT' then 10  
                else  1   -- DRVMI and empty at begining of trip  
                end  
               end  
            when 'MILEOO'  then  
             case  
              when pyd_loadstate = 'LD' then 6  
              else   
               case isnull((select stp_event from stops   
                           where stops.lgh_number = paydetail.lgh_number and stp_lgh_mileage = pyd_quantity
                            and stops.stp_mfh_sequence = (select max(s2.stp_mfh_sequence) from stops s2 where s2.lgh_number = paydetail.lgh_number
             ...
             when 'CROBDR' then 13  
              ...
              case isnull(std_number,0)  -- stand deduct has std_number > 0  
               when 0 then 14  
               else 99  
               end  
             end ,
 ...
 ,sequencenbr = Case pyt_itemcode  
           ...
           when 'DRVMI'  then  
             case  
              when pyd_loadstate = 'LD' then 50  -- DRVMI and loaded  
              else   
               case isnull((select stp_event from stops   
                           where stops.lgh_number = paydetail.lgh_number and stp_lgh_mileage = pyd_quantity
                            and stops.stp_mfh_sequence = (select max(s2.stp_mfh_sequence) from stops s2 where s2.lgh_number = paydetail.lgh_number
                            and s2.stp_lgh_mileage = pyd_quantity)),'??')  
                when 'EMT'  then  100  -- DRVMI and empty at end of trip  
                when 'IEMT' then 100  
                else  10   -- DRVMI and empty at begining of trip  
                end  
               end  
    when 'MILEOO'  then  
             case  
              when pyd_loadstate = 'LD' then 60  
              else   
               case isnull((select stp_event from stops   
                           where stops.lgh_number = paydetail.lgh_number and stp_lgh_mileage = pyd_quantity
                            and stops.stp_mfh_sequence = (select max(s2.stp_mfh_sequence) from stops s2 where s2.lgh_number = paydetail.lgh_number
...
BEGIN  
 ...
 IF @p_drv_yes <> 'XXX'  
                  ...
                  when 'DRVMI'  then  
                    case  
                      when pyd_loadstate = 'LD' then 5  -- DRVMI and loaded  
                        ...
                        case isnull((select stp_event from stops   
                           where stops.lgh_number = pd.lgh_number and stp_lgh_mileage = pyd_quantity
                            and stops.stp_mfh_sequence = (select max(s2.stp_mfh_sequence) from stops s2 where s2.lgh_number = pd.lgh_number
                            and s2.stp_lgh_mileage = pyd_quantity)),'??')  
                          when 'EMT'  then  10  -- DRVMI and empty at end of trip  
                          when 'IEMT' then 10  
                          else  1   -- DRVMI and empty at begining of trip  
  ...
  WHERE ph.asgn_type = 'DRV'  
     ...
     AND @p_drv_id = ph.asgn_id     -- LOR select paydetails for the given payheader only  
...
SELECT pd.pyd_number,  
 ...
 null -- pyt_basis  
-- PTS 29515 -- BL (end)  
    ,sequencenbr = isnull(sequencenbr,999) ,
tp.stp_number ,
tp.stp_mfh_sequence
FROM paydetail pd  
...
UPDATE #temp_pd  
   SET stp_mfh_sequence_111 = (SELECT MIN(st.stp_mfh_sequence)  
                             FROM stops st  
...
UPDATE #temp_pd  
   SET stp_number_111 = (SELECT MAX(st.stp_number)   
                       FROM stops st   
                      WHERE st.mov_number = #temp_pd.mov_number  
                            AND st.stp_mfh_sequence = #temp_pd.stp_mfh_sequence_111)  
...
UPDATE #temp_pd  
   SET stp_arrivaldate = (SELECT stp_arrivaldate  
                            FROM stops st  
                           WHERE st.stp_number = #temp_pd.stp_number_111)  
...
UPDATE #temp_pd  
   SET cmd_name = (SELECT MIN(cmd_name)   
                     FROM freightdetail fd,   
                          commodity cd  
                    WHERE fd.stp_number = #temp_pd.stp_number_111   
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary56] TO [public]
GO
