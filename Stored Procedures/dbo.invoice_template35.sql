...
GO
  ...
  create procedure [dbo].[invoice_template35](@invoice_nbr   int,   @copies  int)  
as  
...
----------- --------------- ------- ------- --------------------------------------------------------------------------  
06/13/2002 Vern Jewett  14623 (none) Original, copied from invoice_template2, then added return column carrier.  
07/24/2002 Vern Jewett  14991 vmj1 Fixes some errors discovered at Turner.  
...
declare   
  --vmj1+ Causing a "string data may be truncated" error..  
...
SELECT invoiceheader.ivh_invoicenumber,     
  ...
  from chargetype  RIGHT OUTER JOIN  invoicedetail  ON  chargetype.cht_itemcode  = invoicedetail.cht_itemcode   
	   ...
	   labelfile l  RIGHT OUTER JOIN  invoiceheader  ON  (l.abbr  = invoiceheader.ivh_revtype1   and l.labeldefinition = 'RevType1')
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template35] TO [public]
GO
