...
CREATE PROCEDURE [dbo].[ResNowOV_R1_OrderedBy]
...
*/
	...
	BEGIN
		...
		WHERE ord_completiondate BETWEEN @DateStart and @DateEnd
		AND ord_status in ('AVL','PLN','DSP','STD','CMP')
		AND ord_company NOT IN (SELECT ItemID  FROM RNTrial_Cache_TopValues (NOLOCK) WHERE ItemCategory = @Mode)
		AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
		ORDER BY ord_company 
		END
	...
	BEGIN
		...
		SELECT 		[LastUpdate] = GETDATE(),
				...
				[ItemCategory] = @Mode,
				[ItemID] = ord_company, 
				[ItemDescription] = (SELECT cmp_name + ' - ' + cty_nmstct FROM company (NOLOCK) where cmp_id = ord_company),
				[Count] = COUNT(*),
			...
			AND ord_status in ('AVL','PLN','DSP','STD','CMP')
			AND (@Parameters =',,' or CHARINDEX(',' + orderheader.ord_revtype1 + ',', @Parameters) > 0)
			GROUP BY ord_company 
			ORDER BY COUNT(*) DESC		
		SET ROWCOUNT 0
	END

GO
GRANT EXECUTE ON  [dbo].[ResNowOV_R1_OrderedBy] TO [public]
GO
