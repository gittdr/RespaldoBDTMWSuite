...
-- exec sp_crea_poliza_AQRO_JR 


-- Procedimiento para generar poliza de diesel a el proveedo

--DROP PROCEDURE sp_crea_poliza_APUE_JR 
--GO


CREATE PROCEDURE [dbo].[sp_crea_poliza_AQRO_JR]  
AS
...
select @li_numperiodo = min(psd_id) from paySchedulesdetail where psh_id = 106 and psd_status = 'OPN'

-- Revisar si ese periodo de pago no contiene detalle de pagos ya procesados.

select @li_yaprocesados = count(*) from paydetail where psd_id = @li_numperiodo and asgn_id = 'AQRO'
...
select @FechaPeriodo = psd_date from paySchedulesdetail where psh_id = 106 and psd_status = 'OPN' and psd_id = @li_numperiodo

-- si no hay paydetail ya en ese periodo se lee los paydetails con la fecha del periodo activo...
execute dbo.d_stlmnt_det_final_sp_JR @phnum=-1, @type = 'TPR', @id = 'AQRO', @paydate = @FechaPeriodo,@numperiodo = @li_numperiodo



-- verifica que haya al menos 1 paydetail

IF (select count(*) from paydetail where psd_id = @li_numperiodo and asgn_id = 'AQRO') <= 0 Return 2
		...
		VALUES ( 'REL', 'A', @FechaPeriodo, 'UNKNOWN', @return_number, 'TPR', 'AQRO', 
		...
		WHERE psd_id = @li_numperiodo and  asgn_type = 'TPR'and asgn_id = 'AQRO' 
