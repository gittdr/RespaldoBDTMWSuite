...
-- y pasarlos a la tabla checkcall y que actualize las ordenes
--DROP PROCEDURE sp_lee_geopos_act_ordenes
--GO

-- exec sp_lee_geopos_act_ordenes

CREATE   PROCEDURE [dbo].[sp_lee_geopos_act_ordenes]
AS
...
WHERE  cuenta = 2 and idTransaccion > 1140000 and economico = '1044'
order by 1


--WHERE GEO.ProximidadPI Is null AND	      GEO.antena <> GEO.economico 
...
/*
		...
		execute @V_CONSECCKC = Tmwdes..getsystemnumber_gateway N'CKCNUM' , NULL , 1 
		-- Inserta el nuevo checkcall
		
		Insert Tmwdes..checkcall(
...
-------------------------------------------------- Codigo para inserta mensaje en TotalMail
					...
					-- Toma la configuración de la Unidad
						...
						FROM Tmwdes..tblTrucks Trucks, tmwSuite..tblCabUnits Cab 
					...
					-- Toma el nombre de la flota
						...
						FROM Tmwdes..tblDispatchGroup flota 
						WHERE SN = @li_flota
						...
						INSERT INTO Tmwdes..tblMessages (Type, Status, Priority, FromType, DelivertoType, DTSent, DTReceived, FOLDER,
