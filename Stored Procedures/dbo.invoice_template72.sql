...
GO
  ...
  create procedure [dbo].[invoice_template72](@invoice_nbr   int,@copies  int)  
as  
...
26523  copied from invoice_template2 to add carrier type 4
...
invoiceheader.ivh_trailer,     
        ...
        ivd_quantity = case when invoicedetail.cht_itemcode = 'LHF' 
                            then (Select max(ivd2.ivd_quantity) 
                                  from   invoicedetail ivd2 
                                  where  ivd2.ivh_hdrnumber = invoiceheader.ivh_hdrnumber 
                                         and ivd2.cht_itemcode = 'DEL') 
                            else invoicedetail.ivd_quantity 
                            end, 
 ...
 -- invoicedetail.ivd_taxable3,     
   ...
   ivd_taxable4 =IsNull(chargetype.cht_taxtable4,invoicedetail.ivd_taxable4),  
       --MRUTH 2/28/05 - Added for Mario's  
        ...
        ivd_unit = case when invoicedetail.cht_itemcode = 'LHF' 
                            then (Select max(ivd2.ivd_unit)
                                  from   invoicedetail ivd2 
                                  where  ivd2.ivh_hdrnumber = invoiceheader.ivh_hdrnumber 
                                         and ivd2.cht_itemcode = 'DEL') 
                            else invoicedetail.ivd_unit 
                            end, 
...
@temp_labelfilename car_type4_name
    ...
    FROM chargetype  RIGHT OUTER JOIN  invoicedetail  ON  chargetype.cht_itemcode  = invoicedetail.cht_itemcode   
		...
		invoiceheader  LEFT OUTER JOIN  carrier  ON  invoiceheader.ivh_carrier  = carrier.car_id  
...
GO
GRANT EXECUTE ON  [dbo].[invoice_template72] TO [public]
GO
