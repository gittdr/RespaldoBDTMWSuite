...
CREATE    PROC [dbo].[d_paydetail_report_carrier_sp] 
...
/**
 * DESCRIPTION:
 *   Created to get carrier paydetail for settlement detail report  
 * PARAMETERS:
 *
 * RETURNS:
 *	
...
CREATE TABLE #carrier_paydetail_temp  (
...
-- Get paydetail info
INSERT INTO #carrier_paydetail_temp  
...
sc.cty_nmstct, 
ec.cty_nmstct,
paydetail.pyt_itemcode paydetail_pyt_itemcode, 
-- PTS 19822 -- BL (start) 
--ivh_billdate,
--ivh_invoicenumber
-- PTS 31363 -- BL (start)
...
--where paydetail.ord_hdrnumber *= invoiceheader.ord_hdrnumber
-- PTS 19822 -- BL (end) 
WHERE 
--sc.cty_code =* paydetail.lgh_startcity and
--ec.cty_code =* paydetail.lgh_endcity and 
(paydetail.asgn_type = 'CAR' and 
-- PTS 32226 -- BL (start)   (31363)
--@carrier_id in ( 'UNKNOWN', paydetail.asgn_id)) and 
(@carrier_id = 'UNKNOWN' or @carrier_id = paydetail.asgn_id)) and 
...
-- PTS 32226 -- BL (end)   (31363)
	...
	and paydetail.pyh_number not in 
		...
		from #carrier_paydetail_temp) 
-- PTS 25416 -- BL (end)

--BEGIN PTS 53314 SPN
UPDATE #carrier_paydetail_temp
   SET ivh_billdate      = (SELECT MAX(ivh_billdate)
                              FROM invoiceheader
                             WHERE #carrier_paydetail_temp.ord_hdrnumber IS NOT NULL
                               AND invoiceheader.ord_hdrnumber IS NOT NULL
                               AND #carrier_paydetail_temp.ord_hdrnumber = invoiceheader.ord_hdrnumber
                               AND #carrier_paydetail_temp.ord_hdrnumber <> 0
                           )
...
UPDATE #carrier_paydetail_temp
   SET ivh_billdate      = (SELECT MAX(ivh_billdate)
                              FROM invoiceheader
                             WHERE #carrier_paydetail_temp.mov_number IS NOT NULL
                               AND invoiceheader.mov_number IS NOT NULL
                               AND #carrier_paydetail_temp.mov_number = invoiceheader.mov_number
                               AND #carrier_paydetail_temp.mov_number <> 0
                           )
     , ivh_invoicenumber = (SELECT MAX(ivh_invoicenumber)
                              FROM invoiceheader
                             WHERE #carrier_paydetail_temp.mov_number IS NOT NULL
...
--set 	ivh_billdate = (SELECT 	max(ivh_billdate)
--						from 	invoiceheader
--						where	#carrier_paydetail_temp.ord_hdrnumber = invoiceheader.ord_hdrnumber)
--where	#carrier_paydetail_temp.ord_hdrnumber > 0
--
--update 	#carrier_paydetail_temp
--set		ivh_invoicenumber = (select max(ivh_invoicenumber) 
--							from 	invoiceheader 
--							where 	ivh_billdate = #carrier_paydetail_temp.ivh_billdate)
--where 	#carrier_paydetail_temp.ord_hdrnumber > 0
---- PTS 31363 -- BL (end)
--END PTS 53314 SPN
...
Begin
	...
	Delete from #carrier_paydetail_temp  
...
if @sch_date1 > convert(datetime, '1950-01-01 00:00') OR 
	...
	Delete from #carrier_paydetail_temp
	where #carrier_paydetail_temp.ord_hdrnumber > 0 and 
		#carrier_paydetail_temp.ord_hdrnumber in (select ord_hdrnumber 
						from stops
...
-- PTS 35274 11/2008 JSwindell RECODE<<start>>
	...
	IF isNull(@revtype1,'UNK') <> 'UNK'
		Begin
			If @excl_revtype1 = 'Y'
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype1,'UNK') = @revtype1
			Else
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype1,'UNK') <> @revtype1
		End
	IF isNull(@revtype2,'UNK') <> 'UNK'
		Begin
			If @excl_revtype2 = 'Y'
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype2,'UNK') = @revtype2
			Else
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype2,'UNK') <> @revtype2
		End
	IF isNull(@revtype3,'UNK') <> 'UNK'
		Begin
			If @excl_revtype3 = 'Y'
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype3,'UNK') = @revtype3
			Else
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype3,'UNK') <> @revtype3
		End
	IF isNull(@revtype4,'UNK') <> 'UNK'
		Begin
			If @excl_revtype4 = 'Y'
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype4,'UNK') = @revtype4
			Else
				DELETE FROM #carrier_paydetail_temp WHERE isNull(#carrier_paydetail_temp.ord_revtype4,'UNK') <> @revtype4
		End
	--	LOR
...
--SELECT * from #carrier_paydetail_temp  
...
DROP TABLE #carrier_paydetail_temp  
  
GO
GRANT EXECUTE ON  [dbo].[d_paydetail_report_carrier_sp] TO [public]
GO
