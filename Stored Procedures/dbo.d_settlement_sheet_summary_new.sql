...
CREATE PROC [dbo].[d_settlement_sheet_summary_new](
 ...
 * 02.27.2014 MTC PTS70751 Changed Temp Tables to Table Variables to reduce/eliminate stats based recompilations
 *
 **/
set nocount on
-- jyang pts13004
...
DECLARE @temp_pay TABLE  (
	pyd_number int primary key,
...
-- PTS 29303 -- BL (end)	
	...
	ord_number		varchar(15) null, --added length to end to append /S below and prevent truncation error.
...
UPDATE @temp_pd
SET lgh_startdate = lh.lgh_startdate
from @temp_pd tp inner join legheader lh on lh.lgh_number = tp.lgh_number;


-- Update the temp with number of legheaders for the move
-- actually, just find if there was another legheader on the move
WITH MOVQTY (mov_number, lhqty) AS
	(
	SELECT tp2.mov_number, COUNT(distinct lh.lgh_number) as lhqty
	FROM legheader lh inner join @temp_pd tp2 on lh.mov_number = tp2.mov_number
	group by tp2.mov_number
	)
UPDATE @temp_pd
SET lgh_count = m.lhqty
from @temp_pd tp inner join MOVQTY m on tp.mov_number = m.mov_number;
...
from @temp_pd tp inner join standingdeduction sd 
ON sd.std_number = tp.std_number
...
FROM @temp_pd tp INNER JOIN payheader ph
ON ph.pyh_pyhnumber = tp.pyh_number
...
UPDATE @temp_pd
SET stp_mfh_sequence = a.minmfhseq
from @temp_pd tp inner join
(
	SELECT tp2.mov_number, MIN(st.stp_mfh_sequence) minmfhseq
	FROM stops st inner join @temp_pd tp2 ON st.mov_number = tp2.mov_number 
	AND st.stp_event IN ('DRL', 'LUL', 'DUL', 'PUL')
	group by tp2.mov_number
) a on tp.mov_number = a.mov_number


UPDATE @temp_pd
SET stp_number = a.maxstp
from @temp_pd tp inner join
(
	SELECT tp2.mov_number, tp2.stp_mfh_sequence, MAX(st.stp_number) as maxstp
	FROM stops st inner join @temp_pd tp2 on 
		st.mov_number = tp2.mov_number
	AND st.stp_mfh_sequence = tp2.stp_mfh_sequence
	group by tp2.mov_number, tp2.stp_mfh_sequence
) a on tp.mov_number = a.mov_number and tp.stp_mfh_sequence = a.stp_mfh_sequence


-- Update for stop arrivaldate
UPDATE @temp_pd
SET stp_arrivaldate = st.stp_arrivaldate
from @temp_pd tp inner join stops st
ON st.stp_number = tp.stp_number

--Update the temp pay details with commodity data
UPDATE @temp_pd
SET cmd_name = a.mincmdname
from @temp_pd tp inner join
(
	SELECT tp2.stp_number, MIN(cd.cmd_name) as mincmdname
	FROM @temp_pd tp2 inner join freightdetail fd on tp2.stp_number = fd.stp_number
	inner join commodity cd on fd.cmd_code = cd.cmd_code
	group by tp2.stp_number
) a on tp.stp_number = a.stp_number


--Need to get the bill-of-lading from the reference number table
--Update the temp pay details with reference number data
UPDATE @temp_pd
SET ref_number = a.minrefnum
from @temp_pd tp inner join
(
SELECT tp2.ord_hdrnumber, MIN(r.ref_number) as minrefnum
FROM @temp_pd tp2 inner join referencenumber r on tp2.ord_hdrnumber = r.ref_tablekey
WHERE r.ref_table = 'orderheader' and r.ref_type = 'SID'
group by tp2.ord_hdrnumber
) a on tp.ord_hdrnumber = a.ord_hdrnumber
...
DECLARE  @ytdbal TABLE (asgn_type	varchar (6) not null,
	asgn_id			varchar (13) not null,
	ytdcomp			money null,
...
FROM @temp_pd tp 
	inner join referencenumber r on tp.ord_hdrnumber = r.ref_tablekey 
	inner join orderheader o on r.ref_type = o.ord_reftype and o.ord_hdrnumber = tp.ord_hdrnumber 
	inner join labelfile l on r.ref_type = l.abbr 
WHERE 
...
if exists (select * from generalinfo where gi_name = 'StlFindNextMTLeg' and gi_string1 = 'Y')
	delete @temp_pd from @temp_pd tp inner join paydetail ON tp.pyd_number = paydetail.pyd_number and paydetail.tar_tarriffnumber = '-1'
...
GO
GRANT EXECUTE ON  [dbo].[d_settlement_sheet_summary_new] TO [public]
GO
