...
CREATE PROC [dbo].[d_tar_gettarkeys_li_stl]
   @tarnum int,
   @billdate datetime,
   @billto char(8),
   @ordby char(8),
...
PTS 26793 (recode 20297) - DJM - Add the Localization setting of Origin/Desitnation service_center and service_region to the
   tariffkey parameters. Values are not used in Settlements but were added to remain consistent with Invoicing.

PTS# 37918  added missing parms  @thirdparty, @thirdpartytype, @mpp_terminal, @trc_terminal, @trl_terminal, @primary_driver
vjh PTS# 37595 add stop_othertype1/2
* 11/19/2007 PTS 38811 JDS:  mod 4 cols from INT  to Dec(19,4)
*  LOR   PTS# 40764 - added @time
...
create table #temp (trk_number int null,
   ...
   stop_othertype2   varchar(6)  null,
   ...
   select t.trk_number,
      ...
      t.trk_stp_event ,
         trk_minvariance,
         trk_maxvariance,
      isnull(t.trk_thirdparty,'UNKNOWN'), --t.trk_thirdparty,
      isnull(t.trk_thirdpartytype, 'UNKNOWN'), -- t.trk_thirdpartytype
      ...
      isnull(t.trk_trl_company, 'UNK') trl_company,
      isnull(t.trk_trl_fleet, 'UNK') trl_fleet,
      isnull(t.trk_trl_division, 'UNK') trl_division,
      isnull(t.trk_trc_company, 'UNK') trc_company,
      isnull(t.trk_trc_fleet, 'UNK') trc_fleet,
      isnull(t.trk_trc_division, 'UNK') trc_division,
      isnull(t.trk_mpp_company, 'UNK') mpp_company,
      isnull(t.trk_mpp_fleet, 'UNK') mpp_fleet,
      isnull(t.trk_mpp_division, 'UNK') mpp_division,
      isnull(t.trk_mpp_domicile, 'UNK') mpp_domicile,
      isnull(t.trk_mpp_teamleader, 'UNK') mpp_teamleader,
      isnull(trk_ratemode, 'UNK') trk_ratemode,       -- 11/18/2011 NQIAO PTS 58978
      isnull(trk_servicelevel, 'UNK') trk_servicelevel   -- 11/18/2011 NQIAO PTS 58978
   ...
   IsNull(t.trk_thirdparty, 'UNKNOWN') in (@thirdparty, 'UNKNOWN') AND
   IsNUll(t.trk_thirdpartytype, 'UNKNOWN') in (@thirdpartytype, 'UNKNOWN') AND
   ...
   and IsNull(t.trk_mintime, 0) <= @time
   ...
   AND IsNull(t.trk_mpp_branch, 'UNKNOWN') in (@mpp_branch, 'UNKNOWN')                 -- vjh 71091
   AND IsNull(t.trk_trc_branch, 'UNKNOWN') in (@trc_branch, 'UNKNOWN')                 -- vjh 71091
   AND IsNull(t.trk_trl_branch, 'UNKNOWN') in (@trl_branch, 'UNKNOWN')                 -- vjh 71091
   AND IsNull(t.trk_car_branch, 'UNKNOWN') in (@car_branch, 'UNKNOWN')                 -- vjh 71091
...
GO
GRANT EXECUTE ON  [dbo].[d_tar_gettarkeys_li_stl] TO [public]
GO
