...
CREATE PROC [dbo].[d_stlmnt_det_report_general102](
 ...
 * 4-1-2009  PTS45746 JSwindell CREATED (based on old format 32) 
 * 4-9-2009  PTS45746 JSwindell:  Made correction to the supress repeating values logic.
 *
 **/
...
CREATE TABLE #temp_pd(
	...
	----ord_number		char(12) null,  -- pts 45746  FIX TRUNCATION
	ord_number		char(30) null,
	...
	ivh_quantity float null, --JD 19603
	ivh_unit varchar(6) null, --JD 192603
	ivh_rate money null,					-- PTS 45746
	ivh_totalweight  float null,			-- PTS 45746
	rate_by_label_name  varchar(20) null,	-- PTS 45746
	ord_rate money null,					-- PTS 45746
	ord_totalweight float null,				-- PTS 45746
	ord_rateunit varchar(6) null,			-- PTS 45746	
	flag_pyd_sequence	int null			-- PTS 45746	
)
...
SELECT pd.pyd_number,
	...
	0,
	null,
	0,		--	ivh_rate				-- PTS 45746
	0,		--	ivh_totalweight  		-- PTS 45746
	null,   --	rate_by_label_name		-- PTS 45746
	0,		--	ord_rate				-- PTS 45746
	0,		--	ord_totalweight			-- PTS 45746
	null,	--	ord_rateunit			-- PTS 45746	
	0		-- flag_pyd_sequence		-- PTS 45746	
...
UPDATE #temp_pd
   SET ord_startdate	= oh.ord_startdate,
       ord_number		= oh.ord_number,
		ord_rate 		= oh.ord_rate,			-- PTS 45746
		ord_totalweight	= oh.ord_totalweight,	-- PTS 45746
		ord_rateunit	= oh.ord_rateunit		-- PTS 45746	
  FROM #temp_pd tp, 
       orderheader oh
...
UPDATE #temp_pd
	--SET ord_number = ord_number + '/S'	-- PTS 45746 FIX TRUNCATION	
...
-- PTS 45746 cmp_name is > 30 so truncate it.
UPDATE #temp_pd
   SET shipper_name = cast(co.cmp_name as varchar(30))
...
-- PTS 45746 cmp_name is > 30 so truncate it.
UPDATE #temp_pd
     SET consignee_name = cast(co.cmp_name as varchar(30))
  FROM #temp_pd tp, company co,orderheader oh
 WHERE tp.ord_hdrnumber = oh.ord_hdrnumber
...
--JD 19603 9/4/03
Update #temp_pd 
set ivh_quantity = invoiceheader.ivh_quantity,
		ivh_unit = invoiceheader.ivh_unit,
		ivh_rate = invoiceheader.ivh_rate,			-- PTS 45746
		ivh_totalweight = invoiceheader.ivh_totalweight -- PTS 45746
from invoiceheader 
where #temp_pd.ord_hdrnumber = invoiceheader.ord_hdrnumber and
      invoiceheader.ivh_hdrnumber = (select max(ivh_hdrnumber) from invoiceheader b where b.ord_hdrnumber = #temp_pd.ord_hdrnumber)
...
-- PTS 45746 <<start>> 
-- Per the SR - if the invoice data is missing, populate from the orderheader.

UPDATE #temp_pd
	SET ivh_rate	 = ord_rate,	 				
	ivh_totalweight  = ord_totalweight
where ivh_rate = 0

 --Update the rate_by_label_name
UPDATE #temp_pd
	SET rate_by_label_name = (select name from labelfile 
								where labeldefinition = 'RateBy'
								and abbr = ( select min(ivd_rateunit) from invoicedetail
								where ivd_rateunit is not null and
								ivh_hdrnumber = ( select min(pyd_ivh_hdrnumber) from paydetail where ord_hdrnumber = #temp_pd.ord_hdrnumber )))
								  
-- if the rate unit from invoice is null - try the orderheader rate unit.
UPDATE #temp_pd
	SET rate_by_label_name = (select name from labelfile 
							  where labeldefinition = 'RateBy'
							  and abbr = (select ord_rateunit from orderheader where ord_hdrnumber = #temp_pd.ord_hdrnumber ))
WHERE rate_by_label_name is null
-- PTS 45746 <<end>>

-----  logic to supress repeating values ---------- PTS 45746 << start >>
Create table #temp_supress
(	sid int identity not null,
	pyd_number int null,
	ord_hdrnumber		int null, 
	pyd_sequence		int null )

Insert into #temp_supress (pyd_number, ord_hdrnumber, pyd_sequence) 
select pyd_number, ord_hdrnumber, pyd_sequence 
from #temp_pd
where itemsection < 2 and ord_hdrnumber > 0
order by ord_hdrnumber, pyd_sequence

select min(a.sid) 'sid', a.ord_hdrnumber 
into  #temp_supress1 
from  #temp_supress a
where  a.ord_hdrnumber in (select distinct( b.ord_hdrnumber ) from #temp_supress b) 
group by a.ord_hdrnumber 

UPDATE #temp_pd
set flag_pyd_sequence = 1 
where pyd_number in ( select pyd_number from #temp_supress 
						where sid in (select sid from #temp_supress1 )  ) 

UPDATE #temp_pd
	set ivh_rate = null,
	ivh_totalweight = null,
	rate_by_label_name	= null
WHERE flag_pyd_sequence <> 1 

-----  logic to supress repeating values ---------- PTS 45746 << end >>
...
SELECT pyd_number, 
	...
	itemsection,
	ord_startdate,
	CAST(ord_number AS VARCHAR(12))'ORD_NUMBER',  -- pts 45746 FIX TRUNCATION
	ref_number,
	stp_arrivaldate,
	shipper_name,
	shipper_city,
	...
	ivh_unit,
	ivh_rate, -- PTS 45746
	ivh_totalweight, -- PTS 45746
	rate_by_label_name	-- PTS45746	
...
GO
GRANT EXECUTE ON  [dbo].[d_stlmnt_det_report_general102] TO [public]
GO
