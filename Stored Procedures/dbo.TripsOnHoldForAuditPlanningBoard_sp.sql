...
CREATE PROC [dbo].[TripsOnHoldForAuditPlanningBoard_sp] (
 @status varchar(6),
 @drvyes varchar(3),  
 @trcyes varchar(3), 
 @trlyes varchar(3), 
 ...
 @p_pyd_workcycle_status varchar(30),
 ...
 *
 * NAME:
 * dbo.TripsOnHoldForAuditPlanningBoard_sp
 *
 * TYPE:
 * StoredProcedure
 *
 * DESCRIPTION:
 * Proc for dw TripsOnHoldForAuditPlanningBoard_sp
 *
 * RETURNS:
 * dw result set
 *
...
-- PTS 90502 | 2015/06/17 | KARAW - Create Audited Planning Board
-- PTS 93267 | 2015/07/30 | KARAW - Create On Hold for Audit Planning Board
-- PTS 99077 | 2016/03/03 | AVANEK - Fix population of asgn_id and asgn_payto columns in #trips table for MPP, TRC, TRL, CAR resource types
**/
...
BEGIN
    ...
    SELECT DISTINCT pd.mov_number,
			...
			asgn_controlling = aa.asgn_controlling,
            ...
            -- PTS 47740 <<end>>
      CASE 
        WHEN ISNULL(pd.pyd_payto, 'UNKNOWN') <> 'UNKNOWN' THEN pd.pyd_payto 
        WHEN ISNULL(aa.asgn_payto, 'UNKNOWN') <> 'UNKNOWN' THEN aa.asgn_payto 
        ELSE ISNULL(mpp.mpp_payto, 'UNKNOWN') 
      END as asgn_payto
  FROM paydetail pd
		Left Outer Join orderheader oh on pd.mov_number = oh.mov_number
		Join legheader lh on pd.lgh_number = lh.lgh_number
    JOIN assetassignment aa (NOLOCK) ON aa.asgn_number = pd.asgn_number
    JOIN manpowerprofile mpp (NOLOCK) ON mpp.mpp_id = pd.asgn_id
...
BEGIN
  ...
  SELECT DISTINCT pd.mov_number,
        ...
        CASE 
          WHEN ISNULL(pd.pyd_payto, 'UNKNOWN') <> 'UNKNOWN' THEN pd.pyd_payto 
          WHEN ISNULL(aa.asgn_payto, 'UNKNOWN') <> 'UNKNOWN' THEN aa.asgn_payto 
          ELSE ISNULL(trc.trc_owner, 'UNKNOWN') 
        END as asgn_payto
  FROM paydetail pd
		Left Outer Join orderheader oh on pd.mov_number = oh.mov_number
		Join legheader lh on pd.lgh_number = lh.lgh_number
    JOIN assetassignment aa (NOLOCK) ON aa.asgn_number = pd.asgn_number
    JOIN tractorprofile trc (NOLOCK) ON trc.trc_number = pd.asgn_id
...
BEGIN
  ...
  SELECT DISTINCT pd.mov_number,
        ...
        CASE 
          WHEN ISNULL(pd.pyd_payto, 'UNKNOWN') <> 'UNKNOWN' THEN pd.pyd_payto 
          WHEN ISNULL(aa.asgn_payto, 'UNKNOWN') <> 'UNKNOWN' THEN aa.asgn_payto 
          ELSE ISNULL(trl.trl_owner, 'UNKNOWN') 
        END as asgn_payto
  FROM paydetail pd
		Left Outer Join orderheader oh on pd.mov_number = oh.mov_number
		Join legheader lh on pd.lgh_number = lh.lgh_number
    JOIN assetassignment aa (NOLOCK) ON aa.asgn_number = pd.asgn_number
    JOIN trailerprofile trl (NOLOCK) ON trl.trl_id = pd.asgn_id
	...
	BEGIN
	  ...
	  SELECT DISTINCT pd.mov_number,
          ...
          CASE 
            WHEN ISNULL(pd.pyd_payto, 'UNKNOWN') <> 'UNKNOWN' THEN pd.pyd_payto 
            WHEN ISNULL(aa.asgn_payto, 'UNKNOWN') <> 'UNKNOWN' THEN aa.asgn_payto 
            ELSE ISNULL(car.pto_id, 'UNKNOWN') 
          END as asgn_payto
		FROM paydetail pd
			Left Outer Join orderheader oh on pd.mov_number = oh.mov_number
			Join legheader lh on pd.lgh_number = lh.lgh_number
      ...
      JOIN assetassignment aa (NOLOCK) ON aa.asgn_number = pd.asgn_number
      JOIN carrier car (NOLOCK) ON car.car_id = pd.asgn_id
...
delete from #trips 
where #trips.ord_hdrnumber in (select tpa.ord_hdrnumber from Thirdpartyassignment tpa
...
DROP TABLE #trips
GO
GRANT EXECUTE ON  [dbo].[TripsOnHoldForAuditPlanningBoard_sp] TO [public]
GO
