...
CREATE PROC [dbo].[d_flarock_payxface_detintact]
@pyh_number		int
AS
/**
 * 
...
CREATE TABLE #payview_detintact 
			(pyd_number int,
...
WHERE	 (pd.asgn_type  = 'DRV')
 AND	(pd.pyh_number  = @pyh_number)

-- Update the order header number from the order table
UPDATE #payview_detintact
set ord_number =       (SELECT Min(IsNull(ord_number, ''))
			FROM orderheader oh
			WHERE 	( #payview_detintact.ord_hdrnumber = oh.ord_hdrnumber )
			)
...
set lgh_tractor =      (SELECT Min(IsNull(lh.lgh_tractor, ''))
			FROM legheader lh
			WHERE ( #payview_detintact.lgh_number = lh.lgh_number )
			)
...
set ivh_invoice_number = 	(SELECT MIN(ivh_invoicenumber)
				FROM invoiceheader
				WHERE ( #payview_detintact.ord_hdrnumber = invoiceheader.ord_hdrnumber )
				)

SELECT * from #payview_detintact
GO
GRANT EXECUTE ON  [dbo].[d_flarock_payxface_detintact] TO [public]
GO
