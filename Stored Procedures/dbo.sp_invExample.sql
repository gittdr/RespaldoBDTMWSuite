...
--use tmwSuite
--drop procedure sp_invExample
--TTSTMWrevanalysis_invheader
create procEDURE [dbo].[sp_invExample]
		(
...
--SELECT @mpp_type3 = ',' + LTRIM(RTRIM(ISNULL(@invoicestatuslist, ''))) + ',' 
...
print 'Revenue3' + cast(@revtype3 as nvarchar(30))
print 'Revenue3' + cast(@revtype4 as nvarchar(30))
...
Where 
	...
	(@revtype3 = ',,' OR mov_number in (select mov_number from legheader where CHARINDEX(',' + mpp_type3 + ',' , ',EUC,') > 0 ) )
	--(@revtype3 = ',,' OR mov_number in (select mov_number from legheader where mpp_type3 like %@revtype2% ) )
	And
