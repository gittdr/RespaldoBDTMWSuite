...
create proc [dbo].[WORD_InterfaceEXAMPLE_23805]
		@lgh_number int
as
/*************************

exec WORD_InterfaceEXAMPLE_23805 21917
...
-- PTS 26036 -- BL (start)
--Declare @MinordNumber int
--Declare @ThisordNumber int
Declare @MinordNumber varchar(12)
Declare @ThisordNumber varchar(12)
-- PTS 26036 -- BL (end)
Declare @ThisRefNumber varchar(20)
Declare @ordNumberList varchar(254)
Declare @RefNumberList varchar(254)
...
select distinct orderheader.ord_number,
	...
	where stops.ord_hdrnumber=orderheader.ord_hdrnumber
		and stops.lgh_number = @lgh_number

select @MinordNumber = -1, @ordNumberList = '', @RefNumberList = ''
select @ThisordNumber = min(ord_number)
	from #Tconcat
	where ord_number > @MinordNumber
while @ThisordNumber is not null begin
	if @ordNumberList <> '' select @ordNumberList = @ordNumberList + ', '
	select @ordNumberList = @ordNumberList + ltrim(@ThisordNumber)
	select @ThisRefNumber = ltrim(isnull(ord_refnum,''))
		from #Tconcat
		where ord_number = @ThisordNumber

	if @ThisRefNumber <> '' begin
		if @RefNumberList <> '' select @RefNumberList = @RefNumberList + ', '
		select @RefNumberList = @RefNumberList + ltrim(@ThisRefNumber)
	end --if

	select @MinordNumber = @ThisordNumber
	select @ThisordNumber = min(ord_number)
		from #Tconcat
		where ord_number > @MinordNumber
end --while
...
SELECT @TotalWeight =
    ( Select sum(isnull(stp_weight,0))
        FROM
         stops
        WHERE stops.ord_hdrnumber = @ordhdrNumber
            and stp_type='DRP')

SELECT @Pup_stp_comment = (Select stp_comment FROM stops where stp_number = (select min(stp_number) from stops
WHERE stops.ord_hdrnumber = @ordhdrNumber 
		and stp_mfh_sequence = (select min(stp_mfh_sequence) from stops where stops.ord_hdrnumber = @ordhdrNumber and stp_type='PUP')))

SELECT @Drp_stp_comment = (Select stp_comment FROM stops where stp_number = (select min(stp_number) from stops
WHERE stops.ord_hdrnumber = @ordhdrNumber 
...
begin	--NON split trip
	...
	Upper(C1.cmp_address1)     shipper_cmp_address1,
	Upper(C1.cmp_address2)     shipper_cmp_address2,
	Upper(cty1.cty_nmstct)     shipper_cty_nmstct,
	Upper(rtrim(cty1.cty_name)+', '+ cty1.cty_state)    shipper_cty_nmstctFULL,	
	C1.cmp_zip          shipper_cmp_zip,
	C1.cmp_primaryphone     shipper_cmp_primaryphone,

	C2.CMP_NAME         Consignee_CMP_NAME ,
	Upper(C2.cmp_address1)     Consignee_cmp_address1,
	Upper(C2.cmp_address2)     Consignee_cmp_address2,
	Upper(cty2.cty_nmstct)     Consignee_cty_nmstct,
	Upper(rtrim(cty2.cty_name)+', '+ cty2.cty_state)    Consignee_cty_nmstctFULL,	
...
begin 	--- Split trip support
	...
	Upper(company.cmp_address1)     shipper_cmp_address1,
	Upper(company.cmp_address2)     shipper_cmp_address2,
	Upper(city.cty_nmstct)     shipper_cty_nmstct,
	Upper(rtrim(city.cty_name)+', '+ city.cty_state)    shipper_cty_nmstctFULL,	
...
--    AND
    ...
    and orderheader.ord_hdrnumber = @ordhdrNumber  --vjh 23805 - removing this line will give multiple rows for legs with multiple orders
...
ende:
GO
GRANT EXECUTE ON  [dbo].[WORD_InterfaceEXAMPLE_23805] TO [public]
GO
