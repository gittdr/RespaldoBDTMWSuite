...
CREATE PROCEDURE [dbo].[tm_create_async_message] @MessageID varchar(12),
 ...
 * 
 * NAME:
 * dbo.[tm_create_async_message]
 *
 * TYPE:
 * Stored Procedure
 *
 * DESCRIPTION:
 *  populate tblSQLMessage and tblSQLMessageData
 *
 * RETURNS:
 *  MessageID Int
 * 
 ...
 * 
 * Change Log: 
 * 2013-07-28 - APC - increase size of tblSQLMessageData.msd_fieldvalue to varchar(500)
 *
 **/
...
IF @lMessageID > 0 
	BEGIN
		IF NOT EXISTS(SELECT NULL FROM tblSqlMessage (NOLOCK) WHERE msg_id = @lMessageID)
		BEGIN
			RAISERROR('tm_create_async_message: Message ID %d not found in Asynchronous table', 16, 1, @lMessageID)
			RETURN
		END
	...
	BEGIN
		...
		if isdate(@MessageDate) = 0
			BEGIN
				...
				ELSE
					BEGIN
						RAISERROR('tm_create_async_message: Invalid message date: %s', 16, 1, @MessageDate)
						RETURN
					END
			END
		else
		...
		if isnumeric(@FormID) = 0 
			BEGIN
				RAISERROR('tm_create_async_message: Form ID must be numeric: %s', 16, 1, @FormID)
				RETURN
			END

		if ISNULL(@MessageTo, '') = ''
			BEGIN
				RAISERROR('tm_create_async_message: Message To not set', 16, 1)
				RETURN
			END
	END
...
IF @lMessageID = 0
	BEGIN
		INSERT INTO tblSQLMessage (msg_date, 
								  msg_FormID, 
								  msg_To, 
								  msg_ToType, 
								  msg_FilterData, 
	...
	BEGIN
		...
		UPDATE tblSQLMessage SET msg_date = ISNULL(@dteMessageDate, msg_date), 
...
BEGIN
	...
	INSERT INTO tblSQLMessageData (msg_ID, 
								  msd_Seq, 
								  msd_FieldName, 
								  msd_FieldValue) VALUES
								 (@lMessageID, 
								  CASE WHEN (@Flags & 1) = 0 THEN @lFieldCount ELSE 1 END,
								  @sFieldName,
								  @sFieldValue)
...
GO
GRANT EXECUTE ON  [dbo].[tm_create_async_message] TO [public]
GO
