...
CREATE PROC [dbo].[d_loadcarid_active_sp] @comp varchar(8) , @number int AS
...
if exists	(	SELECT	car_id 
				...
				WHERE	car_id LIKE @comp + '%'
						AND (IsNull(car_terminationdt, '12-31-49 23:59:59') >= @date)
						AND car_status = 'ACT'
						--PTS 64250 JJF 20120808
						AND	(	(@rowsecurity <> 'Y')
...
GO
GRANT EXECUTE ON  [dbo].[d_loadcarid_active_sp] TO [public]
GO
