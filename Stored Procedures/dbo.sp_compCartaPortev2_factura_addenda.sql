...
--SECION RECEPTOR (1:1)
    ...
    --isnull((select replace(replace(replace(cmp_name,'|',''),'"','' ),'''','') from company where cmp_id = orderheader.ord_billto),'')                   --4 Nombre Receptor (R)
 ...
 ''
   ...
   isnull(replace(TT2_UnidadSat,'|',''),'')                                        
	...
	isnull(replace(TT2_cht_description,'|',''),'')                                        
																		        +'|'+ 
	casT(isnull(cast(TT2_rate as decimal(10,4)),'0')  as varchar(20))	          
																				+'|'+ 
...
--jrcase (invoiceheader.ivh_currency) when 'MX$' 	then 
...
--SECCION ADDENDA
	...
	'ColgateReference'		+
																					+'|'+ 
	isnull(REPLACE(LTRIM(RTRIM(invoiceheader.ivh_ref_number)),'|',''),'')	  --Referencia de addenda
																					+ '\n' +
	'08'
																				      +'|'+ 
	'ColgateDoc'
