...
CREATE VIEW [dbo].[vSSRSRB_SafetyReport]
As

/*************************************************************************
 *
 * NAME:
 * dbo.[vSSRSRB_SafetyReport]
 *
 * TYPE:
 * View
 *
 * DESCRIPTION:
 * View based on the old vSSRSRB_SafetyReport
 *
**************************************************************************

Sample call

SELECT * FROM [vSSRSRB_SafetyReport]
...
(Cast(Floor(Cast(srp_EventDate as float))as smalldatetime)) AS 'Rpt Event Date Only',
srp_EventLocIs  [Rpt Event Location Is],
Case When srp_EventLocIs = 'A' Then
	(select acd_dotRecordable from Accident WITH (NOLOCK) where Accident.srp_id = SafetyReport.srp_id)
     Else
	'NA'
...
IsNull((select cty_name from city WITH (NOLOCK) where cty_code=srp_EventCity),'') as [Rpt Event City],
...
(Cast(Floor(Cast(srp_ReportedDate as float))as smalldatetime)) AS 'Rpt Reported Date Only',
...
[Rpt Actual Cost] = IsNull((select sum(IsNull(sc_recoveredcost,0)) from safetycost WITH (NOLOCK) where safetycost.srp_id = safetyreport.srp_id),0),
...
[Rpt DriverOrEmployee Name] = IsNull(IsNull((select mpp_lastfirst from manpowerprofile WITH (NOLOCK) where srp_driver1 = manpowerprofile.mpp_id and manpowerprofile.mpp_id <> 'UNKNOWN'),(select IsNull(ee_lastname,'') + ',' + IsNull(ee_firstname,'') from employeeprofile WITH (NOLOCK) where srp_mpporeeid = employeeprofile.ee_ID and ee_ID <> 'UNKNOWN')),(select mpp_lastfirst from manpowerprofile WITH (NOLOCK) where srp_mpporeeid = manpowerprofile.mpp_id)),
...
IsNull((select cty_name from city WITH (NOLOCK) where cty_code=srp_inscocity),'') as [Rpt InsuranceCo City],
...
lgh_number [Rpt Leg Number],
[Bill To ID] = IsNull((select top 1 ivh_billto from invoiceheader WITH (NOLOCK) where invoiceheader.ord_number = SafetyReport.ord_number), (select ord_billto from orderheader WITH (NOLOCK) where orderheader.ord_number = SafetyReport.ord_number)),
RevType1=(select ord_revtype1 from orderheader WITH (NOLOCK) where orderheader.ord_number = safetyreport.ord_number),
RevType2=(select ord_revtype2 from orderheader WITH (NOLOCK) where orderheader.ord_number = safetyreport.ord_number),
RevType3=(select ord_revtype3 from orderheader WITH (NOLOCK) where orderheader.ord_number = safetyreport.ord_number),
RevType4=(select ord_revtype4 from orderheader WITH (NOLOCK) where orderheader.ord_number = safetyreport.ord_number),
[Rpt Picture Count] = IsNull((Select count(*) from ps_blob_data WITH (NOLOCK) where blob_table = 'safetyreport' and blob_key = SafetyReport.srp_number),0)
From SafetyReport WITH (NOLOCK)

GO
GRANT SELECT ON  [dbo].[vSSRSRB_SafetyReport] TO [public]
GO
