...
Create VIEW [dbo].[OperationsTripViewDetails_TimeZone]
as     
...
select a.OrderNumber, 
	...
	a.mov_number, a.EtaStatus, a.EtaComment, a.[Priority],
 	 ...
 	 (select ((((select isnull(usr_GMTDelta,5)   
from ttsusers where usr_userid = ISNULL(
(SELECT usr_alias from spid_tracking where spid = @@spid and rdbms_login = suser_sname()) ,
(SELECT Max (usr_userid) FROM ttsusers WHERE usr_windows_userid = suser_sname()))) + ((case dbo.InDst(getdate(),0) when 'Y' then 1 else 0 end ) * 0)) * 60) +   0  ) - ((isnull(cty_GMTDelta,5)  
            + (case dbo.InDst(getdate(),0) when 'Y' then 1 else 0 end   * (case ISNULL(cty_DSTApplies, 'Y') when 'Y' then 0 else +1 end))) * 60)  
            + isnull(cty_TZMins,0)  
             From city where cty_code = (select s1.stp_city from stops  s1 JOIN legheader l1 
               ON s1.lgh_number = l1.lgh_number  
               and s1.mov_number = l1.mov_number  
               and s1.stp_mfh_sequence = (select min(s2.stp_mfh_sequence)  
                     ...
                     where l1.lgh_number = a.lgh_number
                      )) As TimeZoneTripAdjustMinutes  
from	(
...
GO
GRANT DELETE ON  [dbo].[OperationsTripViewDetails_TimeZone] TO [public]
GO
GRANT INSERT ON  [dbo].[OperationsTripViewDetails_TimeZone] TO [public]
GO
GRANT SELECT ON  [dbo].[OperationsTripViewDetails_TimeZone] TO [public]
GO
GRANT UPDATE ON  [dbo].[OperationsTripViewDetails_TimeZone] TO [public]
GO
