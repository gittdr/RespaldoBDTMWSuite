...
CREATE view [dbo].[MobileOrdersView_TDR_DASH]
AS
...
select * from [dbo].[MobileOrdersView_TDR_DASH] where billtoid = 'PILGRIMS'
...
WHERE	ord.ord_status not in ( 'CAN', 'PND', 'ICO', 'MST')
         ...
         and datediff(day,ord_startdate,getdate()) = 0  or  datediff(day,ord_completiondate,getdate()) = 0
		 and ord_status not in ( 'CAN', 'PND', 'ICO', 'MST')
