...
CREATE view [dbo].[EstatHistoricalOrdersViewTDR] as 
select
...
Estatus =
	 ...
	 case when ord_billto='pilgrims' then
	 '<a href="https://69.20.92.116:8090/BitacoraPilgrims.aspx?lgh_header=' +cast ((select min(lgh_number) from legheader (nolock) where legheader.ord_hdrnumber = ord.ord_hdrnumber)as varchar(20))+'"  target="_blank">'+
	 
	 cast( (select min(lgh_number) from legheader (nolock) where legheader.ord_hdrnumber = ord.ord_hdrnumber) as varchar(20))+'   </a>' 
	 else cast((select min(lgh_number) from legheader (nolock) where legheader.ord_hdrnumber = ord.ord_hdrnumber) as varchar(20)) end as leg,
