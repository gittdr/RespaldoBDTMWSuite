...
CREATE View [dbo].[vSSRSRB_DriverComplaints]

AS
/**
 *
 * NAME:
 * dbo.[vSSRSRB_DriverComplaints]
 *
 * TYPE:
 * View
 *
 * DESCRIPTION:
 * Driver Complaints view
...
select * from [vSSRSRB_DriverComplaints]
...
SELECT     drivercomplaint.mpp_id as 'Driver ID', 
	   ...
	   mpp_type3 as 'DrvType3', 
           ...
           Case When (Select mpp_terminationdt from manpowerprofile WITH (NOLOCK) where drivercomplaint.mpp_id = manpowerprofile.mpp_id) > GetDate() Then 
                 'Y' 
           Else 
                 'N' 
           End As 'ActiveYN',
           --**Received Date**
       	   drc_datereceived as 'Received Date',  
           --Day
           (Cast(Floor(Cast([drc_datereceived] as float))as smalldatetime)) as [Received Date Only], 
           Cast(DatePart(yyyy,[drc_datereceived]) as varchar(4)) +  '-' + Cast(DatePart(mm,[drc_datereceived]) as varchar(2)) + '-' + Cast(DatePart(dd,[drc_datereceived]) as varchar(2)) as [Received Day],
           --Month
           Cast(DatePart(mm,[drc_datereceived]) as varchar(2)) + '/' + Cast(DatePart(yyyy,[drc_datereceived]) as varchar(4)) as [Received Month],
           DatePart(mm,[drc_datereceived]) as [Received Month Only],
           --Year
           DatePart(yyyy,[drc_datereceived]) as [Received Year],
	    --**Occured Date**
       	   drc_dateoccured as 'Occured Date',  
           --Day
           (Cast(Floor(Cast([drc_dateoccured] as float))as smalldatetime)) as [Occured Date Only], 
           Cast(DatePart(yyyy,[drc_dateoccured]) as varchar(4)) +  '-' + Cast(DatePart(mm,[drc_dateoccured]) as varchar(2)) + '-' + Cast(DatePart(dd,[drc_dateoccured]) as varchar(2)) as [Occured Day],
           --Month
           Cast(DatePart(mm,[drc_dateoccured]) as varchar(2)) + '/' + Cast(DatePart(yyyy,[drc_dateoccured]) as varchar(4)) as [Occured Month],
           DatePart(mm,[drc_dateoccured]) as [Occured Month Only],
           --Year
           DatePart(yyyy,[drc_dateoccured]) as [Occured Year],
	   drc_company as 'Company ID', 
           drc_city as 'City', 
           drc_location as 'Location', 
           drc_state as 'State', 
           drc_description as 'Description', 
           drc_drivercomments as 'Comments', 
           drc_receivedby as 'Received By',  
           drc_handledby as 'Handled By',
           drc_code as 'Driver Complaint Code',
          ...
          (Cast(Floor(Cast(mpp_senioritydate as float))as smalldatetime)) AS [Seniority Date Only],
        ...
        mpp_alternatephone as 'Alternate Phone Number',  
           ...
           (Cast(Floor(Cast(mpp_pln_date as float))as smalldatetime)) AS [Planned Date Only],
           ...
           (Cast(Floor(Cast(mpp_gps_date  as float))as smalldatetime)) AS [GPS Date Only],
           ...
           (Cast(Floor(Cast(mpp_last_log_date   as float))as smalldatetime)) AS [Last Log Date Only], 
           mpp_hours1 as 'Hours1', 
           mpp_hours2 as 'Hours2',  
           mpp_hours3 as 'Hours3' , 
           IsNull(mpp_home_city,'') as 'Home City',   
           mpp_exp1_date as 'Exp1 Date', 
           (Cast(Floor(Cast(mpp_exp1_date as float))as smalldatetime)) AS [Exp1 Date Only], 
           mpp_exp2_date as 'Exp2 Date', 
           (Cast(Floor(Cast(mpp_exp2_date as float))as smalldatetime)) AS [Exp2 Date Only], 
	   ...
	   mpp_lastlog_estdate as 'Last Log Estimate Date' , 
	   (Cast(Floor(Cast(mpp_lastlog_estdate as float))as smalldatetime)) AS [Last Log Estimate Date Only], 
           mpp_lastlog_cmp_name as 'Last Log Company Name', 
           mpp_estlog_datetime as 'Estimate Log Date Time'
           	
FROM       dbo.drivercomplaint WITH (NOLOCK) 
Inner Join manpowerprofile WITH (NOLOCK) On manpowerprofile.mpp_id = drivercomplaint.mpp_id


GO
GRANT SELECT ON  [dbo].[vSSRSRB_DriverComplaints] TO [public]
GO
