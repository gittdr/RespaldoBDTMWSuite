...
--select * from [TMWScrollTractorView_ubicacion]



CREATE view [dbo].[TMWScrollTractorView_ubicacion] AS
SELECT 
...
trc_owner,
trc_fleet,
trc_fleetn = (select name from labelfile where labeldefinition = 'fleet' and abbr = trc_fleet),
...
case 
		 ...
		 isnull(cast( min(stp_rpt_miles)  as varchar(10)),'')  + ' Kms        ----------------'  
		 +   (case when  isnull(cast(min(stp_est_drv_time) as varchar(10)),'') < 60 then    isnull(cast(min(stp_est_drv_time) as varchar(10)),'')  + ' Minutos de Manejo'
		 else isnull(cast(min(stp_est_drv_time)/60 as varchar(10)),'') + ' Hora(s) de Manejo'   end) + '---------------' + 'ETA: ' + isnull(cast(min(stp_eta) as varchar(20)),'')  
		...
		where stops.mov_number
		 ...
		 isnull(cast( min(stp_rpt_miles)  as varchar(20)),'ND.............')  + ' Kms        ----------------'  
		 +   (case when  isnull(cast(min(stp_est_drv_time) as varchar(10)),'') < 60 then    isnull(cast(min(stp_est_drv_time) as varchar(10)),'')  + ' Minutos de Manejo'
		 else isnull(cast(min(stp_est_drv_time)/60 as varchar(10)),'') + ' Hora(s) de Manejo'   end) + '---------------' + 'ETA: ' + isnull(cast(min(stp_eta) as varchar(20)),'')  
...
trc_gps_heading as trc_DEFLevel 
...
case 
          when trc_status = 'USE' 
          then   (select trl_Gps_desc from trailerprofile nolock where trl_number = (select lgh_primary_trailer  from legheader_active (nolock)  where 
		  legheader_active.lgh_number   = ((select max(lgh_number)  from legheader_active where legheader_active.lgh_tractor = dbo.tractorprofile.trc_number and legheader_active.lgh_outstatus in ('STD','DSP')) )))
...
case 
		  ...
		  case when datediff(dd,trl_gps_date,getdate()) = 0  then  substring(convert(varchar(24),trl_gps_date,114),1,5)
           else +'.'+substring(convert(varchar(24),trc_gps_date,1),0,6)  +' '  +  substring(convert(varchar(24),trl_gps_date,114),1,5)
           end
		     from trailerprofile nolock where trl_number =(select lgh_primary_trailer  from legheader_active (nolock)  where 
		  legheader_active.lgh_number   = ((select max(lgh_number)  from legheader_active where legheader_active.lgh_tractor = dbo.tractorprofile.trc_number and legheader_active.lgh_outstatus in ('STD','DSP')) )))
...
GO
GRANT SELECT ON  [dbo].[TMWScrollTractorView_ubicacion] TO [public]
GO
