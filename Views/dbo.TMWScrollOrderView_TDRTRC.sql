...
CREATE view [dbo].[TMWScrollOrderView_TDRTRC] AS
...
substring(convert(varchar(24),(ord.ord_bookdate),1),0,6)  +' '  +  substring(convert(varchar(24),(ord.ord_bookdate) ,114),1,5)  as 'ord_bookdate',
 ...
 (select name from labelfile where labeldefinition = 'fleet' and abbr = t.trc_fleet) as 'ord_billto',
...
Elogist =  case when nse.ste_miles_out is null then 'No disponible' else
...
FROM 
   dbo.tractorprofile t WITH (NOLOCK)
	...
	and ord.ord_status not in ('CMP','CAN','AVL')
