...
CREATE VIEW [dbo].[OperationsTripViewStopsLrdReq]       
as         
...
select OperationsTripViewDetails.*, stops.stp_number, stops.stp_mfh_sequence, stops.cmp_id, stops.cmp_name, city.cty_nmstct, city.cty_state, stops.stp_zipcode,        
  ...
  stops.stp_detstatus,  
  (select ISNULL((select top 1 stp_schdtearliest from stops (nolock) where mov_number = OperationsTripViewDetails.mov_number and stp_type = 'PUP' order by stp_mfh_sequence), '1950-01-01 00:00:00')) as PickupEarliest,  
  (select ISNULL((select top 1 stp_schdtlatest from stops (nolock) where mov_number = OperationsTripViewDetails.mov_number and stp_type = 'PUP' order by stp_mfh_sequence), '2049-12-31 23:59:59')) as PickupLatest,
  dbo.BuildLoadRequirementString(OperationsTripViewDetails.mov_number, stops.stp_number) AS 'Load Req.'          
...
GO
GRANT SELECT ON  [dbo].[OperationsTripViewStopsLrdReq] TO [public]
GO
