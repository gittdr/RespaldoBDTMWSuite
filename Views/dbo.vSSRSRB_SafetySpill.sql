...
CREATE VIEW [dbo].[vSSRSRB_SafetySpill]
AS

/*****************************************************************
 *
 * NAME:
 * dbo.[vSSRSRB_SafetySpill]
 *
 * TYPE:
 * View
 *
 * DESCRIPTION:
 * View based on the old vTTSTMW_SafetySpill
 *
******************************************************************
...
Select
	...
	IsNull((select cty_name from city WITH (NOLOCK) where cty_code=spl_LawEnfDeptCity),'') as [LawEnf Dept City],
	...
	IsNull((select cty_name from city WITH (NOLOCK) where cty_code=spl_OwnerCity),'') as [Owner City],
	...
	srp_ID as [Report ID]
From Spill WITH (NOLOCK)
JOIN vSSRSRB_SafetyReport WITH (NOLOCK)
	ON Spill.srp_ID = vSSRSRB_SafetyReport.[Rpt Report ID]
WHERE vSSRSRB_SafetyReport.[Rpt Classification] = 'SPILL'
GO
GRANT SELECT ON  [dbo].[vSSRSRB_SafetySpill] TO [public]
GO
