...
CREATE VIEW [dbo].[vDRVMgmt_SSRSRB_PayDetails]
AS

/*******************************************************************************************************************  
  Object Description:
  This view is a copy of vSSRSRB_PayDetails. It is needed for the Driver Management tab on Driver Profile in TMW
  Operations. Since there is no guarantee that all clients will have vSSRSRB_PayDetails, this copy was made so that
  all clients will have this view.

  Revision History:
  Date         Name             Label/PTS      Description
  -----------  ---------------  -------------  ----------------------------------------
  05/09/2017   Cory Sellers     NSUITE-201261  Initial Release
...
SELECT 
	...
	[Bill To ID] = (select Max(ivh_billto) from invoiceheader  with(NOLOCK) where pyd.ord_hdrnumber = invoiceheader.ord_hdrnumber),
...
GO
GRANT SELECT ON  [dbo].[vDRVMgmt_SSRSRB_PayDetails] TO [public]
GO
