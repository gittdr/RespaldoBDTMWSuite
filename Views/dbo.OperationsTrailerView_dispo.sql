...
CREATE View [dbo].[OperationsTrailerView_dispo] AS  
 SELECT  trailerprofile.trl_id,  
  ...
  trailerprofile.trl_type2 trl_type2,   
   ...
   case when trailerprofile.trl_type3 = 'BAJ ' and (select cty_region1 from city  where cty_code = trailerprofile.trl_avail_city) = 'GD' then 'BGDA'
        when trailerprofile.trl_type3 = 'BAJ ' and (select cty_region1 from city  where cty_code = trailerprofile.trl_avail_city) = 'MX' then 'BMEX'
        when trailerprofile.trl_type3 = 'BAJ ' and (select cty_region1 from city  where cty_code = trailerprofile.trl_avail_city) = 'MT' then 'BMTY'
		when trailerprofile.trl_type3 = 'BAJ ' and (select cty_region1 from city  where cty_code = trailerprofile.trl_avail_city) = 'NV' then 'BNVL'
		when trailerprofile.trl_type3 = 'BAJ ' and (select cty_region1 from city  where cty_code = trailerprofile.trl_avail_city) = 'QR' then 'BQRO'
		else trailerprofile.trl_type3  end as trl_type3,
...
ISNULL(trl_exp2_date,'12/31/49') as trl_exp2_date  
  FROM trailerprofile JOIN company AS company_a ON ISNULL(trailerprofile.trl_avail_cmp_id, 'UNKNOWN') = company_a.cmp_id   
   ...
   WHERE trailerprofile.trl_id <> 'UNKNOWN' AND trailerprofile.trl_status <> 'OUT'  
   


GO
GRANT SELECT ON  [dbo].[OperationsTrailerView_dispo] TO [public]
GO
