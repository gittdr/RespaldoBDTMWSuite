...
CREATE VIEW [dbo].[OperationsDispatchWorksheet]
AS
SELECT	lh.lgh_number, 
		...
		(select cmp_name 
		   ...
		   from dbo.company with (nolock) --, index(pk_id))  PTS 57430 removed index hint!
		...
		ISNULL((select top 1 st2.stp_event from dbo.stops st2 with (nolock)--, index(dk_leghdrnum)) PTS 79441 removed index hint
		...
		ISNULL((select count(distinct(fd.cmd_code)) 
				  ...
				  from dbo.freightdetail fd WITH (nolock) 
                                    join dbo.stops st4 WITH (nolock)--, index(sk_stp_ordnum)) PTS 79441 removed index hint
		...
		END next_stop_name,
				CASE 
			WHEN lblDispStatus.code < 300 THEN (SELECT cty_nmstct FROM stops with (nolock) join company with (nolock)--, index(pk_id)) PTS 57430 removed index hint!
		on stops.cmp_id = company.cmp_id where stops.lgh_number = lh.lgh_number AND stops.stp_mfh_sequence = 1)
			ELSE (SELECT TOP 1 cty_nmstct FROM stops with (nolock) join company with (nolock)--, index(pk_id)) PTS 57430 removed index hint!
 ...
 FROM	dbo.legheader lh /* 08/05/2011 MDH PTS 58306: Commented out index that was changed: with (index(date)) */ 
                         LEFT OUTER JOIN dbo.orderheader oh ON lh.ord_hdrnumber = oh.ord_hdrnumber 
					     LEFT OUTER JOIN dbo.stops st ON lh.stp_number_end = st.stp_number 
					     LEFT OUTER JOIN dbo.stops st_begin ON lh.stp_number_start = st_begin.stp_number 
					     LEFT OUTER JOIN dbo.carrier ON lh.lgh_carrier = carrier.car_id 
					     LEFT OUTER JOIN dbo.trailerprofile t1 ON lh.lgh_primary_trailer = t1.trl_id 
					     LEFT OUTER JOIN dbo.trailerprofile t2 ON lh.lgh_primary_pup = t2.trl_id 
					     LEFT OUTER JOIN dbo.event e ON e.stp_number = st.stp_number and e.evt_sequence = 1
					     LEFT OUTER JOIN dbo.event e_begin ON e_begin.stp_number = st_begin.stp_number and e_begin.evt_sequence = 1
					     LEFT OUTER JOIN dbo.checkcall ck ON (ck.ckc_number = (select max(ck2.ckc_number) 
																		         from dbo.checkcall ck2 with (nolock) 
						...
						--END PTS 63598 JJF 20121011
						 ...
						 LEFT OUTER JOIN labelfile lblDispStatus on (lh.lgh_outstatus = lblDispStatus.abbr AND lblDispStatus.labeldefinition='DispStatus'), 
        dbo.dispatchview dv 
  WHERE	dv.dv_type = 'DW' AND ISNULL(dv.dv_view_type, '0') = '0'
GO
GRANT REFERENCES ON  [dbo].[OperationsDispatchWorksheet] TO [public]
GO
GRANT SELECT ON  [dbo].[OperationsDispatchWorksheet] TO [public]
GO
