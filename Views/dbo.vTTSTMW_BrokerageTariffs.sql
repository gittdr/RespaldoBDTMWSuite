...
--select top 100 * from vTTSTMW_BrokerageTariffs

CREATE   View [dbo].[vTTSTMW_BrokerageTariffs]

As



select		trk_number as [Tariff Key Number], 
		...
		[Currency Unit] = cht_currunit,
                [Carrier City]=(select cty_name from city (NOLOCK),carrier (NOLOCK) where trk_carrier=car_id and city.cty_code = carrier.cty_code),
		[Carrier State]=(select cty_state from city (NOLOCK),carrier (NOLOCK) where trk_carrier=car_id and city.cty_code = carrier.cty_code),
		[Carrier Zip]=(select cty_zip from city (NOLOCK),carrier (NOLOCK) where trk_carrier=car_id and city.cty_code = carrier.cty_code)
 -- blm	11.12.03
	from (
...
Union
	...
	'' as [Currency Unit],
	[Carrier City]=(select cty_name from city (NOLOCK),carrier (NOLOCK) where CH.Crh_Carrier=car_id and city.cty_code = carrier.cty_code),
	[Carrier State]=(select cty_state from city (NOLOCK),carrier (NOLOCK) where CH.Crh_Carrier=car_id and city.cty_code = carrier.cty_code),
	[Carrier Zip]=(select cty_zip from city (NOLOCK),carrier (NOLOCK) where CH.Crh_Carrier=car_id and city.cty_code = carrier.cty_code)
	From CarrierHistory CH (NOLOCK) 




GO
GRANT SELECT ON  [dbo].[vTTSTMW_BrokerageTariffs] TO [public]
GO
