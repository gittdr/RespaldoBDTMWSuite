...
CREATE VIEW [dbo].[TMWScrollInvoicesOnHoldForNoTouchBillingView] AS
/*******************************************************************************************************************  
  Object Description:
  This query retrieves invoice records that are on hold, to be processed by the No Touch Processor Service
...
AS
  (
    ...
    WHERE NoTouchBillingConfiguration.ntbTypeId = 1 AND labelfile.code >= 50
  )
SELECT
  ivh.ivh_hdrnumber,
...
WHERE ivh.ivh_invoicestatus = 'HLD'
  AND EXISTS (SELECT 1 FROM applicableNoTouchConfigBillTos WHERE applicableNoTouchConfigBillTos.billToId IN ('UNKNOWN', ivh.ivh_billto))
GO
GRANT DELETE ON  [dbo].[TMWScrollInvoicesOnHoldForNoTouchBillingView] TO [public]
GO
GRANT INSERT ON  [dbo].[TMWScrollInvoicesOnHoldForNoTouchBillingView] TO [public]
GO
GRANT SELECT ON  [dbo].[TMWScrollInvoicesOnHoldForNoTouchBillingView] TO [public]
GO
GRANT UPDATE ON  [dbo].[TMWScrollInvoicesOnHoldForNoTouchBillingView] TO [public]
GO
