...
CREATE  View [dbo].[vSSRSRB_StandingDeduction] 
As

/**
 *
 * NAME:
 * dbo.vSSRSRB_StandingDeduction
 *
 * TYPE:
 * View
 *
 * DESCRIPTION:
 * Retrieve Data for StandingDeduction
 *
 *
 * REVISION HISTORY:
 *
...
Select
	...
	CASE 
    		WHEN asgn_type = 'DRV'  THEN IsNull((Select mpp_terminal from manpowerprofile WITH (NOLOCK) where asgn_id = manpowerprofile.mpp_id),'') 
      	End as [Driver Terminal],
	CASE 
    		WHEN asgn_type = 'TRC'  THEN IsNull((Select trc_terminal from tractorprofile WITH (NOLOCK) where asgn_id = tractorprofile.trc_number),'') 
      	End as [Tractor Terminal],

	CASE 
    		WHEN asgn_type = 'DRV'  THEN IsNull((Select mpp_division from manpowerprofile WITH (NOLOCK) where asgn_id = manpowerprofile.mpp_id),'') 
      	End as [Driver Division],
	CASE 
    		WHEN asgn_type = 'TRC'  THEN IsNull((Select trc_division from tractorprofile WITH (NOLOCK) where asgn_id = tractorprofile.trc_number),'') 
      	End as [Tractor Division]

From    standingdeduction WITH (NOLOCK)

GO
GRANT SELECT ON  [dbo].[vSSRSRB_StandingDeduction] TO [public]
GO
