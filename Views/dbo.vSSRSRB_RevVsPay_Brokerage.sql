...
CREATE VIEW [dbo].[vSSRSRB_RevVsPay_Brokerage]

AS
/**
 *
 * NAME:
 * dbo.vSSRSRB_RevVsPay_Brokerage
 *
 * TYPE:
 * View
 *
 ...
 * select * from [vSSRSRB_RevVsPay_Brokerage]
 * REVISION HISTORY:
 *
 ...
 * 10/9/2014 JR - put in case statement to allow for clients without a shiftschedules table and removed
  ...
  * 10/27/2014 created revvspay for brokwerage specific items
 **/
SELECT 
...
SELECT	
	...
	IsNull(dbo.TMWSSRS_fnc_allocatedTotFuelRevByMiles(lgh.lgh_number),0.00) as 'Fuel Surcharge', 
	[Last GPS Date] = 
		(SELECT MAX(ckc_date) 
			FROM checkcall WITH (NOLOCK) 
	...
	IsNull(dbo.TMWSSRS_fnc_allocatedTotOrdLineHaulRevByMiles(lgh.lgh_number),0.00) AS 'LineHaul Revenue',
	...
	Revenue = convert(money,IsNull(dbo.TMWSSRS_fnc_allocatedTotOrdRevByMiles(lgh.lgh_number),0.00)),
	'Revenue Date' = 
   ...
   ISNULL(lgh.trc_terminal,trc.trc_terminal) AS 'Tractor Terminal', --'Tractor Terminal' 
	...
	case(SELECT count(*) 
                 ...
                 AND  TABLE_NAME = 'shiftschedules')
	when 1 then (select top 1 s.ss_shift 
		...
		case(SELECT count(*) 
                 ...
                 WHERE TABLE_SCHEMA = 'dbo' 
                 AND  TABLE_NAME = 'shiftschedules')
	when 1 then (select top 1 s.ss_date 
			from ShiftSchedules s with(nolock)
		where lgh.shift_ss_id = s.ss_id)
	else null
	end
	'Shift Date' ,
		case(SELECT count(*) 
                 ...
                 WHERE TABLE_SCHEMA = 'dbo' 
                 AND  TABLE_NAME = 'shiftschedules')
	when 1 then (select top 1 s.ss_shiftstatus 
			from ShiftSchedules s with(nolock)
		where lgh.shift_ss_id = s.ss_id)
	else null end 'Shift Status',
		case(SELECT count(*) 
                 FROM INFORMATION_SCHEMA.TABLES 
                 WHERE TABLE_SCHEMA = 'dbo' 
                 AND  TABLE_NAME = 'shiftschedules')
	when 1 then (select top 1 s.ss_terminal
			from ShiftSchedules s with(nolock)
		where lgh.shift_ss_id = s.ss_id)
	else null end 'Shift Terminal',
	case(SELECT count(*) 
                 FROM INFORMATION_SCHEMA.TABLES 
                 WHERE TABLE_SCHEMA = 'dbo' 
                 AND  TABLE_NAME = 'shiftschedules')
	when 1 then (select top 1(CAST(FLOOR(CAST(s.ss_date  AS float))AS smalldatetime))
	from ShiftSchedules s with(nolock)
		where lgh.shift_ss_id = s.ss_id)
	else null end [Shift Date Only],	
	lbg.lgh_booked_Revtype1 as 'Executing Terminal',	
	lbg.ord_booked_revtype1 as 'Booking Terminal'



FROM legheader lgh	
left join legheader_brokered lbg with (nolock) on lbg.lgh_number = lgh.lgh_number
...
GO
GRANT SELECT ON  [dbo].[vSSRSRB_RevVsPay_Brokerage] TO [public]
GO
