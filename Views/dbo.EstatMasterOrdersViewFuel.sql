...
SET ANSI_NULLS ON
GO
CREATE view [dbo].[EstatMasterOrdersViewFuel]
as select 
...
from orderheader as ord
		...
		left outer join stops on ccompany.cmp_id = stops.cmp_id and stops.stp_type ='DRP'
		left outer join freightdetail fgt1 with (nolock) on fgt1.stp_number = stops.stp_number and fgt1.fgt_sequence = 1
		left outer join freightdetail fgt2 with (nolock) on fgt2.stp_number = stops.stp_number and fgt2.fgt_sequence = 2 
		left outer join freightdetail fgt3 with (nolock) on fgt3.stp_number = stops.stp_number and fgt3.fgt_sequence = 3
		left outer join freightdetail fgt4 with (nolock) on fgt4.stp_number = stops.stp_number and fgt4.fgt_sequence = 4
where ord.ord_status = 'MST' and ord.ord_order_source in ('ESTAT', 'ESFUEL')
GO
GRANT INSERT ON  [dbo].[EstatMasterOrdersViewFuel] TO [public]
GO
GRANT SELECT ON  [dbo].[EstatMasterOrdersViewFuel] TO [public]
GO
GRANT UPDATE ON  [dbo].[EstatMasterOrdersViewFuel] TO [public]
GO
