...
--select MinutesInBetweenStops,HoursInBetweenStops,DaysInBetweenStops from vTTSTMW_StopDetail 
--select top 1000 [Move Number],[Hub Miles],[Hub Mile Reading],Event from vTTSTMW_StopDetail where [Order Header Number] = 1208

CREATE                                     View [dbo].[vTTSTMW_StopDetail]
As
...
Select TempStopDetail.*,
	   ...
	   0
       End As Tolerance,
       DateDiff(mi,[Arrival Date],[Departure date]) as ArrivalToDepartureLagm,
...
select orderheader.ord_number as 'Order Number',
      ...
      'Hub Miles' = IsNull(event.evt_hubmiles,0) - IsNull((select Max(b.evt_hubmiles) from event b (NOLOCK),stops c (NOLOCK) where c.mov_number = stops.mov_number and c.stp_mfh_sequence = stops.stp_mfh_sequence - 1 and b.stp_number = c.stp_number),0),
       MinutesInBetweenStops = DateDiff(mi,(select Min(c.stp_arrivaldate) from stops c (NOLOCK) where c.mov_number = stops.mov_number and c.stp_mfh_sequence = stops.stp_mfh_sequence - 1),stp_arrivaldate),
   ...
   --Day
     ...
     --stp_reasonlate_text as [Reason Late Arrival],
...
from stops (NOLOCK) Left Join  orderheader (NOLOCK) On stops.ord_hdrnumber = orderheader.ord_hdrnumber
	            Inner Join legheader (NOLOCK) On legheader.lgh_number = stops.lgh_number
...
GO
GRANT SELECT ON  [dbo].[vTTSTMW_StopDetail] TO [public]
GO
