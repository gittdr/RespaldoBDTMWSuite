...
CREATE              view [dbo].[vTTSTMW_DriverAccidents]
...
--   Ver 5.4 LBK
--2. Forced SSN to be 10 characters Ver 5.4 LBK

SELECT     driveraccident.mpp_id as 'Driver ID', 
           'DrvType1 Name' = IsNull((select labelfile.name from labelfile (NOLOCK) where labelfile.abbr = manpowerprofile.mpp_type1 and labelfile.labeldefinition = 'DrvType1'),''),
	   manpowerprofile.mpp_type1 as 'DrvType1', 
           'DrvType2 Name' = IsNull((select labelfile.name from labelfile (NOLOCK) where labelfile.abbr = manpowerprofile.mpp_type2 and labelfile.labeldefinition = 'DrvType2'),''),
	   manpowerprofile.mpp_type2 as 'DrvType2', 
           'DrvType3 Name' = IsNull((select labelfile.name from labelfile (NOLOCK) where labelfile.abbr = manpowerprofile.mpp_type3 and labelfile.labeldefinition = 'DrvType3'),''),
	   manpowerprofile.mpp_type3 as 'DrvType3', 
           'DrvType4 Name' = IsNull((select labelfile.name from labelfile (NOLOCK) where labelfile.abbr = manpowerprofile.mpp_type4 and labelfile.labeldefinition = 'DrvType4'),''),
           manpowerprofile.mpp_type4 as 'DrvType4', 
           Case When (Select manpowerprofile.mpp_terminationdt from manpowerprofile (NOLOCK) where driveraccident.mpp_id = manpowerprofile.mpp_id) > GetDate() Then 
                 'Y' 
           Else 
                 'N' 
           End As 'ActiveYN',
	   ...
	   driveraccident.dra_filenumber as 'File Number', 
           ...
           IsNull((select city.cty_name from city (NOLOCK) where manpowerprofile.mpp_city = city.cty_code),'') as 'City', 
           ...
           IsNull((select city.cty_name from city (NOLOCK) where  manpowerprofile.mpp_avl_city = city.cty_code),'') as 'Available City', 
           ...
           IsNull((select city.cty_name from city (NOLOCK) where manpowerprofile.mpp_pln_city  = city.cty_code),'') as 'Planned City',  
           ...
           IsNull((select city.cty_name from city (NOLOCK) where manpowerprofile.mpp_prior_city = city.cty_code),'') as 'Prior City',   
	   ...
	   --<TTS!*!TMW><End><FeaturePack=Other> 
	   --<TTS!*!TMW><Begin><FeaturePack=Euro> 
	   --manpowerprofile.mpp_country as [Country],
	   --<TTS!*!TMW><End><FeaturePack=Euro> 
	   ...
	   --<TTS!*!TMW><End><FeaturePack=Other> 
	   --<TTS!*!TMW><Begin><FeaturePack=Euro> 
	   --manpowerprofile.mpp_gp_class as [GP Class], 
	   --<TTS!*!TMW><End><FeaturePack=Euro> 
	  ...
	  --<TTS!*!TMW><End><FeaturePack=Other> 
	  --<TTS!*!TMW><Begin><FeaturePack=Euro> 
	  --manpowerprofile.mpp_qualificationlist as [Qualification List]
	  --<TTS!*!TMW><End><FeaturePack=Euro> 

FROM         dbo.driveraccident (NOLOCK) Inner Join manpowerprofile (NOLOCK) On manpowerprofile.mpp_id = driveraccident.mpp_id
...
GO
GRANT SELECT ON  [dbo].[vTTSTMW_DriverAccidents] TO [public]
GO
