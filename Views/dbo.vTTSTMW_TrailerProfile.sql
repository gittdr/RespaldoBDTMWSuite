...
CREATE          view [dbo].[vTTSTMW_TrailerProfile]
...
SELECT     trl_number as 'Trailer ID',
	   ...
	   End as 'ActiveYN',  
           ...
           'TrlType1 Name' = IsNull((select name from labelfile (NOLOCK) where labelfile.abbr = trl_type1 and labeldefinition = 'TrlType1'),''),
           trl_type2 as 'TrlType2', 
           'TrlType2 Name' = IsNull((select name from labelfile (NOLOCK) where labelfile.abbr = trl_type2 and labeldefinition = 'TrlType2'),''),
           trl_type3 as 'TrlType3', 
           'TrlType3 Name' = IsNull((select name from labelfile (NOLOCK) where labelfile.abbr = trl_type3 and labeldefinition = 'TrlType3'),''),
           trl_type4 as 'TrlType4', 
           'TrlType4 Name' = IsNull((select name from labelfile (NOLOCK) where labelfile.abbr = trl_type4 and labeldefinition = 'TrlType4'),''),       
           trl_year as 'Year', 
           trl_startdate as 'Start Date', 
           trl_retiredate as 'Retire Date', 
           trl_mpg as 'Miles Per Gallon', 
           trl_company as 'Trailer Company', 
           'Trailer Company Name' = IsNull((select min(name) from labelfile (NOLOCK) where labelfile.abbr = trl_company and labeldefinition = 'Company'),''), 
	   trl_fleet as 'Trailer Fleet',
	   'Trailer Fleet Name' = IsNull((select min(name) from labelfile (NOLOCK) where labelfile.abbr = trl_division and labeldefinition = 'Fleet'),''), 
           trl_division as 'Trailer Division', 
           'Trailer Division Name' = IsNull((select min(name) from labelfile (NOLOCK) where labelfile.abbr = trl_division and labeldefinition = 'Division'),''),
	   trl_terminal as 'Trailer Terminal', 
           'Trailer Terminal Name' = IsNull((select min(name) from labelfile (NOLOCK) where labelfile.abbr = trl_terminal and labeldefinition = 'Terminal'),''),
	   cmp_id as 'Company ID', 
           (select cty_name from city (NOLOCK) where city.cty_code = trailerprofile.cty_code) as 'City',
           trl_ilt as 'Ilt', 
	   ...
	   trl_status as 'Trailer Status', 
           ...
           IsNull((select cty_name from city (NOLOCK) where cty_code = trl_sch_city),'') as 'Scheduled City',
           trl_sch_status as 'Scheduled Status', 
           trl_avail_date as 'Available Date', 
           trl_avail_cmp_id as 'Available Company ID', 
           IsNull((select cty_name from city (NOLOCK) where cty_code = trl_avail_city),'') as 'Available City',
           ...
           IsNull((select cty_name from city (NOLOCK) where cty_code = trl_next_city),'') as 'Next City',
           ...
           IsNull((select cty_name from city (NOLOCK) where cty_code = trl_prior_city),'') as 'Prior City',
	   ...
	   --<TTS!*!TMW><Begin><FeaturePack=Other>
	   '' as [Conversion Weight],
	   --<TTS!*!TMW><End><FeaturePack=Other>
	   --<TTS!*!TMW><Begin><FeaturePack=Euro>
	   --trl_conversion_weight as [Conversion Weight],
	   --<TTS!*!TMW><End><FeaturePack=Euro>
...
GO
GRANT SELECT ON  [dbo].[vTTSTMW_TrailerProfile] TO [public]
GO
