...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[Vista_Roger_DECP1_JR]
AS
...
FROM            dbo.tractorprofile LEFT OUTER JOIN
                         dbo.tractoraccesories ON dbo.tractorprofile.trc_number = dbo.tractoraccesories.tca_tractor AND dbo.tractoraccesories.tca_type = 'DECP1' INNER JOIN
                         dbo.labelfile AS B ON dbo.tractorprofile.trc_type3 = B.abbr CROSS JOIN
                         dbo.labelfile AS A
WHERE        (dbo.tractorprofile.trc_status <> 'OUT') AND (dbo.tractorprofile.trc_number <> 'UNKNOWN') AND (B.labeldefinition = 'TrcType3') AND (A.abbr = 'DECP1') AND (A.labeldefinition = 'trcacc')
GO
...
Begin DesignProperties = 
   ...
   Begin CriteriaPane = 
      Begin ColumnWidths = 11
         ...
         Or = 1350
         Or = 1350
     ', 'SCHEMA', N'dbo', 'VIEW', N'Vista_Roger_DECP1_JR', NULL, NULL
GO
...
DECLARE @xp int
SELECT @xp=2
EXEC sp_addextendedproperty N'MS_DiagramPaneCount', @xp, 'SCHEMA', N'dbo', 'VIEW', N'Vista_Roger_DECP1_JR', NULL, NULL
GO
