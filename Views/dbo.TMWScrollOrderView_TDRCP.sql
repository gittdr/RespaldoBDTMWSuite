...
CREATE view [dbo].[TMWScrollOrderView_TDRCP] AS
...
substring(convert(varchar(24),(ord.ord_bookdate),1),0,6)  +' '  +  substring(convert(varchar(24),(ord.ord_bookdate) ,114),1,5)  as 'ord_bookdate',
 ...
 ord.ord_exitport, 
 ord_driver1 = 
 mpp.mpp_ID + '     |   ' +  mpp.mpp_firstname+' '+mpp.mpp_lastname + '   |    Movil:' + isnull(mpp.mpp_currentphone,'')+ ' / Casa: '+ isnull(mpp.mpp_homephone,'') + ' /Licencia: '  + isnull(mpp_licensenumber,'') + ' /RFC: ' + isnull(mpp_misc3,'') + ' /SSN: ' + isnull(mpp_misc1,'')  ,
...
Escuderia = (select name from labelfile where labeldefinition  ='TeamLeader' and abbr  = mpp.mpp_teamleader),
...
Elogist =  case when (nse.ste_miles_out is null  and  ns.cmp_id is null) then 'Ultimo evento' 
                when (nse.ste_miles_out is null  and  ns.cmp_id is not null) then 'No disponible' else
		 ...
		 else isnull(cast((nse.ste_seconds_out/3600) as varchar(10)),'') + 'H'   end),'K - M','')  end,


		  Etadif = 
		  case when nse.ste_seconds_out is null then 0 else
		  round(cast(datediff(MINUTE,ns.stp_schdtlatest,dateadd(n,(nse.ste_seconds_out/60),getdate())) as float) /60 ,1) end ,
...
FROM 
   ...
   -- and t.trc_number <> 'UNKNOWN'
	...
	--and round(cast(datediff(MINUTE,ns.stp_schdtlatest,dateadd(n,(nse.ste_seconds_out/60),getdate())) as float) /60 ,1)  is not null
