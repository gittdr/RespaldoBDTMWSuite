...
--select top 100 * from vTTSTMW_DriverProfile

CREATE                               View [dbo].[vTTSTMW_DriverProfile]
...
TempMan2.*,
[Last Trip Tractor] = (select min(asgn_id) from AssetAssignment where AssetAssignment.lgh_number = TempMan2.lgh_number and asgn_type = 'TRC'),
[Last Trip Final Destination] = (select min(lgh_endcty_nmstct) from legheader where TempMan2.lgh_number = LegHeader.lgh_number ),
[Last Trip Final State] = (select min(lgh_endstate) from legheader where TempMan2.lgh_number = LegHeader.lgh_number )
...
TempMan1.*,
[lgh_number] = (select min(lgh_number) from AssetAssignment where asgn_number = MaxAssignmentNumber),
[Last Trip Move Number] = (select min(mov_number) from AssetAssignment where asgn_number = MaxAssignmentNumber),
[Last Trip Status] = (select min(asgn_status) from AssetAssignment where asgn_number = MaxAssignmentNumber),
[Last Trip Assignment Date] = (select min(asgn_date) from AssetAssignment where asgn_number = MaxAssignmentNumber),
[Last Accident Description] = (select max(cast(dra_description as varchar(255))) from driveraccident da (NOLOCK) where [Last Accident Date] = da.dra_accidentdate and [Driver ID] = da.mpp_id),
[Last Accident Preventable] = (select max(dra_preventable) from driveraccident da (NOLOCK) where [Last Accident Date] = da.dra_accidentdate and [Driver ID] = da.mpp_id),
[Last Accident Cost] = (select max(dra_cost) from driveraccident da (NOLOCK) where [Last Accident Date] = da.dra_accidentdate and [Driver ID] = da.mpp_id),
[Last Accident Reference Number] = (select max(dra_filenumber) from driveraccident da (NOLOCK) where [Last Accident Date] = da.dra_accidentdate and [Driver ID] = da.mpp_id),
--[Last Accident Status] = (select max(dra_status) from driveraccident da (NOLOCK) where [Last Accident Date] = da.dra_accidentdate and [Driver ID] = da.mpp_id),
...
SELECT     mpp_id as 'Driver ID',
	   ...
	   Else
	        'N'
	   End as 'ActiveYN', 
	   [Last Accident Date] = (select max(dra_accidentdate) from driveraccident da (NOLOCK) where manpowerprofile.mpp_id = da.mpp_id),
	...
	Cast(DatePart(yyyy,[mpp_hiredate]) as varchar(4)) +  '-' + Cast(DatePart(mm,[mpp_hiredate]) as varchar(2)) + '-' + Cast(DatePart(dd,[mpp_hiredate]) as varchar(2)) as [Hire Day],
       ...
       DatePart(yyyy,[mpp_hiredate]) as [Hire Year], 
	     ...
	     (Cast(Floor(Cast([mpp_dateofbirth] as float))as smalldatetime)) as [Date Of Birth Date Only], 
	...
	--Month
	   ...
	   [Team Leader Name] =  (select labelfile.name from labelfile (NOLOCK) where labelfile.abbr = mpp_teamleader  and labelfile.labeldefinition = 'TeamLeader'),
           mpp_fleet as 'Fleet', 
          ...
          (select sum(b.driving_hrs + b.on_duty_hrs) from log_driverlogs b where manpowerprofile.mpp_id = b.mpp_id and b.log_date >= (mpp_last_log_date - ((convert(int,(Left([mpp_servicerule],charindex('/',[mpp_servicerule],1)-1))))-1)) and b.log_date <= mpp_last_log_date)) as [Hours1],
	...
	[Region1] = (select min(cty_region1) from city (NOLOCK) where cty_code = mpp_avl_city),
	[Region2] = (select min(cty_region2) from city (NOLOCK) where cty_code = mpp_avl_city),
	[Region3] = (select min(cty_region3) from city (NOLOCK) where cty_code = mpp_avl_city),
	[Region4] = (select min(cty_region4) from city (NOLOCK) where cty_code = mpp_avl_city),
	[Company Name] = (select name from labelfile (NOLOCK) where abbr = mpp_company and labeldefinition = 'Company'),
        [Division Name] = (select name from labelfile (NOLOCK) where abbr = mpp_division and labeldefinition = 'Division'), 
        [Terminal Name] = (select name from labelfile (NOLOCK) where abbr = mpp_terminal and labeldefinition = 'Terminal'),
        [Fleet Name] = (select name from labelfile (NOLOCK) where abbr = mpp_fleet and labeldefinition = 'Fleet'),
	1 as DriverCount
...
GO
GRANT SELECT ON  [dbo].[vTTSTMW_DriverProfile] TO [public]
GO
