...
SET ANSI_NULLS ON
GO
CREATE view [dbo].[FreightAgentFindOrderView] 
as 
select oh.ord_number as 'Order', lgh_outstatus as 'Status' , oh.ord_hdrnumber, 
       convert(varchar, lgh_startdate, 110) + '\n' + convert(varchar, lgh_startdate, 108) as 'Start Date', ISNULL(fc.cmp_name, '') as 'Origin', fcy.cty_nmstct as 'Origin City/State',
       convert(varchar, lgh_enddate, 110) + '\n' + convert(varchar, lgh_enddate, 108) as 'End Date', ISNULL(lc.cmp_name, '') as 'Final', lcy.cty_nmstct as 'Final City/State',
        (select cmp_name from company where cmp_id = oh.ord_company) 'OrderBy',
    	oh.ord_company 'OrderByID',
...
GO
GRANT SELECT ON  [dbo].[FreightAgentFindOrderView] TO [public]
GO
