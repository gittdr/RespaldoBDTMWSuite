...
/*******************************************************************************************************************  
  Object Description:
  This function is a copy of fnc_allocatedTotOrdLineHaulRevByMiles. It is needed for the Driver Management tab on 
  Driver Profile in TMW Operations. Since there is no guarantee that all clients will have 
  fnc_allocatedTotOrdLineHaulRevByMiles, this copy was made so that all clients will have this function.
...
CREATE FUNCTION [dbo].[udf_DRVMgmt_allocatedTotOrdLineHaulRevByMiles]
	(@lgh_number int)
...
Set @OrdRev = (select convert(money,sum(IsNull(dbo.udf_DRVMgmt_convertcharge(ivh_charge,ivh_currency,'Revenue',ivh_hdrnumber,ivh_currencydate,ivh_shipdate,ivh_deliverydate,ivh_billdate,ivh_revenue_date,ivh_xferdate,default,ivh_printdate,default,default,default),0))) from InvoiceHeader WITH (NOLOCK) where mov_number=@MoveNumber and ivh_invoicestatus <> 'CAN')
...
BEGIN
	...
	Set @OrdRev =(select convert(money,sum(IsNull(dbo.udf_DRVMgmt_convertcharge(ord_charge,ord_currency,'Revenue',ord_hdrnumber,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0))) from Orderheader WITH (NOLOCK) where mov_number=@MoveNumber and 
				(
...
GO
GRANT EXECUTE ON  [dbo].[udf_DRVMgmt_allocatedTotOrdLineHaulRevByMiles] TO [public]
GO
