...
CREATE FUNCTION [dbo].[TrcExpstatus_GetActivity_fn]
(
  @asgnId       VARCHAR(13),
  @statusCodes  TMWTable_char6 READONLY
)
RETURNS @ExpstatusGetActivity TABLE 
...
BEGIN
  INSERT @ExpstatusGetActivity
    ...
    UPDATE  ES
       SET  ES.Trailer1 = trailers.trailer1,
            ES.Trailer2 = trailers.trailer2
      FROM  @ExpstatusGetActivity ES
              INNER JOIN  (SELECT TOP 1
                                  lgh_number,
                                  CASE
                                    WHEN ect.ect_trlend = 'Y' and ect.ect_bt_start = 'Y' THEN 'UNKNOWN'
                                    ELSE e.evt_trailer1
                                  END trailer1,
                                  CASE
                                    WHEN ect.ect_trlend = 'Y' and ect.ect_bt_start = 'Y' THEN 'UNKNOWN'
                            ...
                            FROM  stops s
	                         ...
	                         WHERE  s.lgh_number = (SELECT  lgh_number
                                                    FROM  @ExpstatusGetActivity)
                             AND  e.evt_sequence = 1
                             AND  s.stp_status = 'DNE'
                          ORDER BY s.stp_mfh_sequence DESC) trailers ON trailers.lgh_number = ES.lgh_number

  RETURN
END  
GO
GRANT SELECT ON  [dbo].[TrcExpstatus_GetActivity_fn] TO [public]
GO
