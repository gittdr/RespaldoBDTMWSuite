...
CREATE Function [dbo].[fnc_TMWRN_RevenueMilesSum2]
(
...
-- end added 4/3/2008
	...
	SELECT 	@ExcludeRevenue = IsNull(convert(money,sum(IsNull(dbo.fnc_convertcharge(ivd_charge,ord_currency,'Revenue',ivd_number,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0.00))),0.00)
	FROM  invoicedetail (NOLOCK) Inner Join orderheader (NOLOCK)On orderheader.ord_hdrnumber = invoicedetail.ord_hdrnumber   
	WHERE	orderheader.mov_number = @MoveNumber
	...
	--Allocate Revenue back to the segment or a portion of a segment
	
	If @MilesToAllocate Is Null 
	Begin 
		--We are adding 25 miles to all
		--zero mile legheaders (that way at least some revenue can be allocated back)                           
		Set @MilesToAllocate = IsNull(dbo.fnc_TMWRN_LegHeaderMiles (@LegHeaderNumber,25,default,default,default,default),0)
	End
		
	If @MoveMiles Is Null 
	Begin
		Set @MoveMiles = IsNull(dbo.fnc_TMWRN_MoveMiles(@MoveNumber,default,25,default,default,default,default),0)
	End
		
	Set @PercentToAllocate = 0
			
	IF (@MoveMiles>0)
	BEGIN
		Set @PercentToAllocate = (@MilesToAllocate/@MoveMiles)
	END 
-- changed 4/3/2008
--	Set @Revenue = IsNull(@PercentToAllocate * @Revenue,0)
	Set @Revenue = Round(IsNull(@PercentToAllocate * @Revenue,0),2)
-- end changed 4/3/2008
...
GO
GRANT EXECUTE ON  [dbo].[fnc_TMWRN_RevenueMilesSum2] TO [public]
GO
