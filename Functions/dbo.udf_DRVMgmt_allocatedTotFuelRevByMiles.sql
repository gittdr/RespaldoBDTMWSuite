...
/*******************************************************************************************************************  
  Object Description:
  This function is a copy of fnc_allocatedTotFuelRevByMiles. It is needed for the Driver Management tab on Driver 
  Profile in TMW Operations. Since there is no guarantee that all clients will have fnc_allocatedTotFuelRevByMiles, 
...
CREATE FUNCTION [dbo].[udf_DRVMgmt_allocatedTotFuelRevByMiles]
	(@lgh_number int)
RETURNS Money
AS
BEGIN
...
Set @OrdRev = (
			...
			SELECT 	convert(money,sum(IsNull(dbo.udf_DRVMgmt_convertcharge(ivd_charge,ivh_currency,'Revenue',ivd_number,ivh_currencydate,ivh_shipdate,ivh_deliverydate,ivh_billdate,ivh_revenue_date,ivh_xferdate,default,ivh_printdate,default,default,default),0.00)))	
...
BEGIN
	...
	(	SELECT  --BASE LEVEL SQL 
		        ...
		        --SQL 2000 or higher SQL
			convert(money,sum(IsNull(dbo.udf_DRVMgmt_convertcharge(ivd_charge,ord_currency,'Revenue',ivd_number,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0.00)))	
...
END
GO
GRANT EXECUTE ON  [dbo].[udf_DRVMgmt_allocatedTotFuelRevByMiles] TO [public]
GO
