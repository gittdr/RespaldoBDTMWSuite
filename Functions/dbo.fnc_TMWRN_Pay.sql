...
CREATE FUNCTION [dbo].[fnc_TMWRN_Pay] 
(
...
BEGIN
	...
	Set @Pay = IsNull((
				select sum(IsNull(dbo.fnc_convertcharge(pyd_amount,pyd_currency,'Pay',pyd_number,pyd_currencydate,default,default,default,default,default,default,default,pyd_transdate,pyd_workperiod,pyh_payperiod),0)) 
				...
				where ((@Mode = 'Segment' or @Mode = 'LegHeader') And lgh_number=@LegHeaderNumber)
                   ...
                   --And	(
                    ...
                    And pyt_itemcode not in ('VIATIC','ANTOP','ANTER','ANTMAN')
					And	(
...
GO
GRANT EXECUTE ON  [dbo].[fnc_TMWRN_Pay] TO [public]
GO
