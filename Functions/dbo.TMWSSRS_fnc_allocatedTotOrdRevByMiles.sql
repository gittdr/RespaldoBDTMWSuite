...
CREATE                      FUNCTION [dbo].[TMWSSRS_fnc_allocatedTotOrdRevByMiles]
	(@lgh_number int)

--Revision History
--1. Added Currency Converting Logic Ver 5.4 LBK
 -- Changed @IncludeAllOrderStatuses = 'True' instead of false.  SSRS should filter the orders that you don't want to see the revenue of. - MREED
...
BEGIN
	...
	Else
	Begin
		Set @OrdRev =(select convert(money,sum(IsNull(dbo.TMWSSRS_fnc_convertcharge(IsNull(ord_totalcharge,0),ord_currency,'Revenue',ord_hdrnumber,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0))) from Orderheader WITH (NOLOCK) where mov_number=@MoveNumber and 
				(
				  (@IncludeAllOrderStatuses = 'True' And ord_status <> 'CAN')
				  OR
				  (@IncludeAllOrderStatuses = 'False' And (ord_status = 'STD' or ord_status = 'CMP' or ord_status = 'PKD' or orderheader.ord_status = 'ICO'))
...
GO
GRANT EXECUTE ON  [dbo].[TMWSSRS_fnc_allocatedTotOrdRevByMiles] TO [public]
GO
GRANT REFERENCES ON  [dbo].[TMWSSRS_fnc_allocatedTotOrdRevByMiles] TO [public]
GO
