...
SET ANSI_NULLS ON
GO
CREATE  FUNCTION [dbo].[RowRestrictValidRecord_Orderheader_fn](@mov_number int, @TmwUser varchar(255))
RETURNS  int
...
BEGIN
 ...
 FROM stops stp
   ...
   INNER JOIN RowRestrictValidAssignments_for_tmwuser_fn('orderheader', @TmwUser) rsva on ( oh.rowsec_rsrv_id = rsva.rowsec_rsrv_id
 ...
 ELSE BEGIN
  ...
  FROM stops stp
    ...
    INNER JOIN RowRestrictValidAssignments_for_tmwuser_fn('tractorprofile', @TmwUser) rsva on ( trc.rowsec_rsrv_id = rsva.rowsec_rsrv_id
                            OR rsva.rowsec_rsrv_id = 0
                           )
  WHERE stp.mov_number = @mov_number
...
END
GO
GRANT EXECUTE ON  [dbo].[RowRestrictValidRecord_Orderheader_fn] TO [public]
GO
GRANT REFERENCES ON  [dbo].[RowRestrictValidRecord_Orderheader_fn] TO [public]
GO
