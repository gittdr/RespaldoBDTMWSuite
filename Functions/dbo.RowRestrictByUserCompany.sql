...
CREATE FUNCTION [dbo].[RowRestrictByUserCompany]
(
 ...
 * Intended to be used for company lookups.  If the  company being tested is a bill to, then an additional return of 2 read only indicates the user does not have rights to modify, but can read.
 *
 * RETURNS:
 * 0 restrict
 * 1 ok
...
BEGIN
	...
	ELSE BEGIN
		...
		IF @rowsecurity = 'Y' BEGIN
			IF EXISTS	(	SELECT	* 
							FROM	RowRestrictValidAssignments_company_fn() r
...
END
GO
GRANT EXECUTE ON  [dbo].[RowRestrictByUserCompany] TO [public]
GO
GRANT REFERENCES ON  [dbo].[RowRestrictByUserCompany] TO [public]
GO
