...
CREATE Function [dbo].[fnc_TMWRN_RevenueSegmentXD2a]
(
	@MoveNumber int = Null,
							...
							Select AvailableRev
								,AllocPct = 
									Case When IsNull(TreeMiles,0) = 0 Then
										0
									Else
										Cast(LegMiles as Float) / Cast(TreeMiles as Float)
									End
							from	(
										...
										From	(
													select distinct S0.ord_hdrnumber
														,L0.lgh_number
														,S0.mov_number
														,LegMiles = IsNull(dbo.fnc_TMWRN_LegHeaderMiles (L0.lgh_number,@MinMilesToAllocate,@LoadStatus,@StopStatusList,@BilledMilesOnlyYN,@NonBilledMilesOnlyYN),0)
										...
										group by T1.ord_hdrnumber,lgh_number,LegMiles
									) as FunctionTable0 
						) as FunctionTable1 
			) as FunctionTable2
...
GO
GRANT EXECUTE ON  [dbo].[fnc_TMWRN_RevenueSegmentXD2a] TO [public]
GO
