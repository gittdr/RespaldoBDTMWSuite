...
/*******************************************************************************************************************  
  Object Description:
  This function is a copy of fnc_convertcharge. It is needed for the Driver Management tab on Driver Profile in TMW
  Operations. Since there is no guarantee that all clients will have fnc_convertcharge, this copy was made so that
  all clients will have this function.
...
CREATE Function [dbo].[udf_DRVMgmt_convertcharge]  
(  
 @charge money,  
 @currency varchar(25),  
 @typeofcharge varchar(255),  
 @referencnumber int,  
 ...
 @workperioddate datetime='',  
 @payperioddate datetime=''  
)    
RETURNS Money  
AS  
BEGIN  
 ...
 IF (SELECT COUNT(*) FROM currency_exchange) = 0  
 BEGIN  
 ...
 BEGIN  
  ...
  Begin  
   ...
   Set @multiplyordivide = 'Multiply'--(select gi_value from MR_GeneralInfo where RTrim(gi_key) = 'CurrencyMultiplyOrDivide')  
   ...
   Set @currencydatetype = 'Currency Date'--(select ses_value from MR_SessionID where ses_key = @typeofcharge and ses_SPID = @@SPID )  
...
GO
GRANT EXECUTE ON  [dbo].[udf_DRVMgmt_convertcharge] TO [public]
GO
