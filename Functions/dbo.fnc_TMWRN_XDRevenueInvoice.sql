...
Create Function [dbo].[fnc_TMWRN_XDRevenueInvoice] 
(
	...
	Else If @BaseRevenueCategoryTLAFN = 'L'		-- if base revenue is Linehaul, get Linehaul revenue
		Begin
			...
			WHERE invoiceheader.ivh_hdrnumber = @InvoiceHeaderNumber 
					...
					AND invoicedetail.cht_itemcode IN (SELECT chargetype.cht_itemcode FROM chargetype (NOLOCK) 
													WHERE chargetype.cht_primary = 'Y' OR
															(chargetype.cht_primary = 'N' AND IsNull(chargetype.cht_LH_Rpt,'N') = 'Y')
												)
	...
	Else If @BaseRevenueCategoryTLAFN = 'A'		-- if base revenue is Accessorial, get Accessorial revenue INCLUDING FUEL
		Begin
			...
			WHERE invoiceheader.ivh_hdrnumber = @InvoiceHeaderNumber 
				And ((@InvoiceStatusList = ',,' And ivh_invoicestatus <> 'CAN') OR (CHARINDEX(',' + RTRIM( ivh_invoicestatus ) + ',', @InvoiceStatusList) > 0)) 
				AND invoicedetail.cht_itemcode IN (SELECT chargetype.cht_itemcode FROM chargetype (NOLOCK) 
													WHERE chargetype.cht_primary = 'N' AND IsNull(chargetype.cht_LH_Rpt,'N') = 'N'
												)
	...
	If (@SubtractFuelSurchargeYN = 'Y') OR (@BaseRevenueCategoryTLAFN = 'F')
		begin
			...
			WHERE invoiceheader.ivh_hdrnumber = @InvoiceHeaderNumber 
				And ((@InvoiceStatusList = ',,' And ivh_invoicestatus <> 'CAN') OR (CHARINDEX(',' + RTRIM( ivh_invoicestatus ) + ',', @InvoiceStatusList) > 0)) 
				AND invoicedetail.cht_itemcode IN (SELECT chargetype.cht_itemcode FROM chargetype (NOLOCK) 
													INNER JOIN FuelChargeTypes (NOLOCK) on chargetype.cht_itemcode = FuelChargeTypes.cht_itemcode
															WHERE chargetype.cht_primary = 'N' AND IsNull(chargetype.cht_LH_Rpt,'N') = 'N'
												)
...
GO
GRANT EXECUTE ON  [dbo].[fnc_TMWRN_XDRevenueInvoice] TO [public]
GO
