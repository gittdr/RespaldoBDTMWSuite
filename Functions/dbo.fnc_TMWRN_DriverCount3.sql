...
CREATE FUNCTION [dbo].[fnc_TMWRN_DriverCount3] 
	( 
		@Mode varchar(25) = 'Current', -- Current, Total, OOS, Historical
...
Begin 
  ...
  IF datediff(dd,@DriverCountDate,getdate()) = 0
    BEGIN
	  Insert into @DriverExpirations (Driver)
		...
		AND exp_priority = '1' 
        AND exp_code not in  ('OUT','LIC') AND exp_completed = 'N'
		--AND (exp_compldate >= @DriverCountDate And exp_expirationdate <= @DriverCountDate)
     END
  else if datediff(dd,@DriverCountDate,getdate()) <> 0
       BEGIN
	  Insert into @DriverExpirations (Driver)
		Select exp_id
		FROM expiration WITH (NOLOCK) 
        ...
        AND exp_code not in  ('OUT','LIC') --AND exp_completed = 'N'
	    --AND (exp_compldate >= @DriverCountDate And exp_expirationdate <= @DriverCountDate)
        AND  @DriverCountDate between  exp_expirationdate and exp_compldate 
     END
---------------------------------------------------------------------------------------------------------------------
	...
	Else If @Mode = 'Current'	-- consider expirations
		BEGIN
			INSERT @DriverList (Driver)
				...
				AND driver_id not in  ('UNKNOWN','TDRTD') 
                and driver_id not in (Select exp_id FROM expiration WITH (NOLOCK) WHERE exp_idtype = 'DRV'  AND exp_priority = '1' AND exp_code = 'OUT' AND exp_completed = 'N')
				And Not Exists (select Driver from @DriverExpirations DE where RNDCF.driver_id = DE.Driver)
				-- include filters
        		...
        		--AND (@OnlyDrvFleetList =',,' OR CHARINDEX(',' + driver_fleet + ',', @OnlyDrvFleetList) > 0) 
               	AND (@OnlyDrvFleetList =',,' OR CHARINDEX(',' + (select mpp_fleet from manpowerprofile where mpp_id = driver_id) + ',', @OnlyDrvFleetList) > 0) 
	...
	Else If @Mode = 'Inactivos'
	 ...
	 INSERT INTO @DriverList (Driver)
	 		...
	 		Select driver_id 
				...
				AND driver_id not in  ('UNKNOWN','TDRTD') 
                and driver_id  in  
	            (select exp_id  from expiration  where exp_idtype = 'DRV' and exp_completed = 'N' and exp_code not in ('OUT','BAJAA','LIC')
	             and exp_id not in (select mpp_id from manpowerprofile where mpp_status = 'OUT'))
	...
	Else If @Mode = 'Total'	-- do NOT consider expirations
		BEGIN
			...
			INSERT @DriverList (Driver)
				...
				Where (driver_terminationdate > @DriverCountDate AND driver_hiredate <= @DriverCountDate) 
				AND (@DriverCountDate >= Driver_DateStart AND @DriverCountDate < Driver_DateEnd) 
					AND driver_id not in  ('UNKNOWN','TDRTD')  and driver_id not in (Select exp_id FROM expiration WITH (NOLOCK) WHERE exp_idtype = 'DRV' 
		             AND exp_priority = '1' AND exp_code = 'OUT' AND exp_completed = 'N')
				-- include filters
				AND (@OnlyDrvType1List =',,' OR CHARINDEX(',' + driver_Type1 + ',', @OnlyDrvType1List) > 0) 
        		AND (@OnlyDrvType2List =',,' OR CHARINDEX(',' + driver_Type2 + ',', @OnlyDrvType2List) > 0) 
        		AND (@OnlyDrvType3List =',,' OR CHARINDEX(',' + driver_Type3 + ',', @OnlyDrvType3List) > 0) 
	...
	Else If @Mode = 'Historical'	-- ALL drivers from whenever
		BEGIN
			INSERT @DriverList (Driver)
				...
				FROM   ResNow_DriverCache_Final RNDCF (NOLOCK) 
				where  driver_id not in  ('UNKNOWN','TDRTD')
				-- include filters
				AND (@OnlyDrvType1List =',,' OR CHARINDEX(',' + driver_Type1 + ',', @OnlyDrvType1List) > 0) 
        		AND (@OnlyDrvType2List =',,' OR CHARINDEX(',' + driver_Type2 + ',', @OnlyDrvType2List) > 0) 
        		AND (@OnlyDrvType3List =',,' OR CHARINDEX(',' + driver_Type3 + ',', @OnlyDrvType3List) > 0) 
...
GO
GRANT SELECT ON  [dbo].[fnc_TMWRN_DriverCount3] TO [public]
GO
