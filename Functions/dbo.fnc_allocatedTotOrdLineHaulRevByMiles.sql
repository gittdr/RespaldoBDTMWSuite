...
CREATE                   FUNCTION [dbo].[fnc_allocatedTotOrdLineHaulRevByMiles]
	(@lgh_number int)
...
Set @MoveNumber =(Select mov_number from legheader (NOLOCK) where lgh_number=@lgh_number)		
--Base Level Code
--Set @OrdRev =(select sum(ivh_charge) from Invoiceheader(NOLOCK) where mov_number=@MoveNumber and ivh_invoicestatus <> 'CAN')

--2000 and higher Currency Convert Code
Set @OrdRev = (select convert(money,sum(IsNull(dbo.fnc_convertcharge(ivh_charge,ivh_currency,'Revenue',ivh_hdrnumber,ivh_currencydate,ivh_shipdate,ivh_deliverydate,ivh_billdate,ivh_revenue_date,ivh_xferdate,default,ivh_printdate,default,default,default),0))) from InvoiceHeader(NOLOCK) where mov_number=@MoveNumber and ivh_invoicestatus <> 'CAN')
...
BEGIN
	...
	Set @OrdRev =(select convert(money,sum(IsNull(dbo.fnc_convertcharge(ord_charge,ord_currency,'Revenue',ord_hdrnumber,ord_currencydate,ord_startdate,ord_completiondate,default,default,default,default,default,default,default,default),0))) from Orderheader(NOLOCK) where mov_number=@MoveNumber and 
				(
