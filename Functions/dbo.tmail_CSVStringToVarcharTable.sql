...
CREATE FUNCTION [dbo].[tmail_CSVStringToVarcharTable] (@List varchar(4000))
/*******************************************************************************************************************  
  Object Description:
...
RETURNS TABLE WITH SCHEMABINDING AS
 RETURN
  ...
  --===== Do the actual split. The ISNULL/NULLIF combo handles the length for the final element when no delimiter is found.
   SELECT [Value]       = LTRIM(SUBSTRING(@List, l.N1, l.L1))
     FROM cteLen l
  ;
GO
GRANT SELECT ON  [dbo].[tmail_CSVStringToVarcharTable] TO [public]
GO
