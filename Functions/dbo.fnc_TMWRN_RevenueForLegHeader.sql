...
CREATE           FUNCTION [dbo].[fnc_TMWRN_RevenueForLegHeader] (@LegHeaderNumber int,@MoveNumber int,@NonAllocatedRevenueForOrder float)
...
BEGIN
  ...
  Where     mov_number IN (select b.mov_number from stops b WITH (NOLOCK) where b.lgh_number = @LegHeaderNumber)
   ...
   Set @MoveMiles = IsNull((Select sum(IsNull(stp_lgh_mileage,0)) from @Stops stops where stops.lgh_number = @LegHeaderNumber),0)
   --Set @AllocatedRevenue = (select min(mov_number) from @OrderMilesOnMoves b where )

   Set @AllocatedRevenueForOrderOnMove = IsNull((Select sum(AllocatedRevenueForOrderOnMove) from @OrderMilesOnMoves Where mov_number = @MoveNumber),@NonAllocatedRevenueForOrder)
